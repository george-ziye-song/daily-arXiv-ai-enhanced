...
TL;DR: AtmoFuseNet 提出一种融合多视角天空相机图像、毫米波云雷达和云高仪观测数据的框架，通过交叉模态分层聚合、条件变分细化与基于相关的运动估计三个模块，实现对云微物理场和风场的四维（3D+时间）稠密重建，在半干旱站点达到 LWC MAE 0.026 g/m³ 和风速 MAE 1.18 m/s。


<details>
  <summary>Details</summary>
Motivation: 地面稀疏仪器对云微物理场的稠密体素重建仍是一个开放问题，主要原因是可用测量在模态和空间覆盖上异构性很强。

Method: 三阶段架构：(1) 跨模态分层聚合模块，通过逐层交叉注意力融合图像特征金字塔与仪器导出的垂直廓线；(2) 条件变分细化模块，在可微分的雷达和图像前向模型约束下将体积映射到物理一致的微物理场；(3) 基于相关的运动估计器，从连续体素重建中恢复每个体素的3D风矢量。

Result: 在半干旱站点的联合观测数据上，液态水含量 MAE 达 0.026 g/m³，风速 MAE 达 1.18 m/s，优于现有检索基线；消融实验验证了各模块的贡献。

Conclusion: AtmoFuseNet 有效融合了异构地面观测数据，实现了云微物理场和风场的四维稠密重建，性能优于现有方法。
...
TL;DR: 本文提出一种流式高斯编码器（Streaming Gaussian Encoder, SGE），通过维护持久化的体积场景表示（固定大小的潜高斯查询集），结合自运动补偿和置信度引导的预算约束刷新机制，实现相机-based 4D全景占据跟踪（4D-POT）。该方法利用深度监督塑造高斯不透明度作为可见性代理，使置信度随时间累积为持久场景支持的临时聚合度量，配合预热的多帧训练策略，在表示层面实现时间一致性。在Occ3D扩展的nuScenes和Waymo数据集上达到了新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于掩码的4D-POT pipeline在每个时间步重新计算体积表示，限制了几何时间一致性，尤其在遮挡和静态场景元素下表现不佳。为解决这一问题，需要一种能跨时间维持持久化场景表示的方法。

Method: 1) 将场景建模为固定大小的潜高斯查询集；2) 通过自运动补偿传播高斯查询，并在置信度引导的预算约束下刷新；3) 利用深度监督塑造高斯不透明度作为可见性代理，使置信度累积为持久场景支持的临时聚合度量；4) 采用预热的多帧训练策略。

Result: 在Occ3D扩展的nuScenes和Waymo数据集上，相机-based 4D-POT达到新SOTA，跟踪一致性显著提升，计算开销可忽略，且与现有基于掩码的pipeline完全兼容。

Conclusion: 流式高斯编码器通过维护持久化的体积场景表示，在表示层面而非仅解码器层面实现时间一致性，有效提升了4D-POT的跟踪性能，为未来4D场景理解提供了高效且兼容的范式。
...
TL;DR: 提出一种量化轨迹数据集间相似性的框架，通过对24个主要数据集的分布度量分析，证明迁移性分数与跨数据集模型性能强相关，为运动预测中的数据集选择、预训练和基础模型构建提供指导。


<details>
  <summary>Details</summary>
Motivation: 现有运动预测模型在跨数据集泛化时性能显著下降，缺乏系统量化数据集间相似性的方法，导致难以选择有效的预训练数据和评估模型泛化能力。

Method: 学习数据集的隐空间表征，使用分布度量指标量化数据集间相似性（转移性分数），在24个主流运动预测数据集上进行大规模跨数据集迁移实验，验证分数与模型性能的相关性。

Result: 提出的转移性分数与跨数据集模型性能之间存在强相关性，有效预测了不同数据集间的迁移效果。

Conclusion: 该框架为运动预测领域的数据集选择、预训练策略以及构建通用基础模型提供了实用指导，有助于开发更泛化、更鲁棒的预测系统。
...
TL;DR: 本文提出RTSM方法，通过将少量标注的目标域样本直接加入监督检测损失，显著提升无源域自适应目标检测（SFDA-OD）性能，并发现简单的监督信号难以被更复杂的伪标签反馈策略超越。


<details>
  <summary>Details</summary>
Motivation: 现有SFDA-OD方法通常假设目标域完全无标注，但实际中获取少量标注是可行的。作者希望探索少量（1%-10%）均匀采样标注能否系统性地提升SFDA-OD性能，并检验更复杂的反馈策略相比简单监督是否有额外收益。

Method: 提出Random-Target Supervised Mixing (RTSM)，在保留原始无监督自适应分支不变的基础上，将少量标注目标域样本通过标准监督检测损失加入训练。此外评估了十种稀疏标签反馈插件，涵盖伪标签选择、目标补全和优化控制三类策略。

Result: 在四种SFDA方法、两种检测器、多个自适应任务上，RTSM在1%-10%标注预算下提升1.7-18.3 AP50。十种复杂反馈插件相比RTSM收益有限且依赖方法，未超越简单监督基线。

Conclusion: 简单监督信号在稀疏标签SFDA-OD中非常有效且难以被超越，RTSM可作为该设定下的强基线锚点。
...
TL;DR: GaussLite提出一种任务驱动的3DGS建图系统，根据自然语言任务描述动态分配表示密度，在有限高斯预算下显著提升感兴趣区域的PSNR，并支持多智能体地图融合。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS系统均匀分布表示能力，忽略了机器人下游任务只关注部分重建场景这一事实，导致将宝贵的机载计算资源分配给优化不相关的场景部分，限制了在线容量或导致最相关部分优化不足。

Method: 利用单次LLM解析器从自然语言任务中提取目标和锚定对象，通过开放词汇检测器逐帧定位并生成逐像素相关性掩码；建图器根据任务相关性分配种子密度、梯度流和缩放。

Result: 在匹配高斯预算和资源受限硬件实时建图（4Hz）条件下，GaussLite在Replica数据集上ROI PSNR平均提升+2.72dB，真实硬件室内外场景提升+2.23dB。多智能体地图融合通过逐体素投票实现，较直接拼接提升+3.42dB，仅共享平均7.08%的地图。

Conclusion: 任务驱动的表示分配能有效提升关键区域重建质量，降低计算浪费，且支持多智能体高效地图融合，适用于资源受限的机器人实时建图场景。
...
TL;DR: 本文提出AVTok，一种用于音频-视频联合生成的统一分词器，采用双流Transformer架构，包含共享编码器-解码器和模态特定的可学习查询，通过统一码本将音视频对编码为紧凑的一维潜在表示，并设计分层训练策略应对异构信息不平衡问题。实验表明其在重建和下游生成任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有音视频生成方法多采用双分支设计，分别对音频和视频进行分词和生成，忽略了模态间的表征鸿沟，且需要大量计算资源进行训练。作者旨在设计一个统一的音视频分词器，以紧凑的联合表示解决模态鸿沟问题，提高生成质量和效率。

Method: 提出AVTok统一分词器，采用双流Transformer架构，包含共享编码器-解码器和模态特定的可学习查询，将音视频对编码为紧凑的一维潜在表示，并使用统一码本。为应对异构信息不平衡问题，设计分层训练策略，逐步实现各模态的重建能力。

Result: AVTok在音视频重建任务中表现优异，并在下游任务（音频到视频、视频到音频、类别条件联合音视频生成）中均取得出色效果。

Conclusion: AVTok为联合音视频分词这一挑战性任务开辟了新路径，为构建用于音视频生成的统一大规模多模态模型提供了潜在方向。
...
TL;DR: SyncCache是一种针对DiT音频驱动肖像动画的无训练缓存加速方法，通过空间不对称探测和模态解耦缓存，分别优先处理动态人脸区域的高频动态和稳定残差复用，结合离线动态规划实现缓存比控制，在HunyuanVideo-Avatar和Wan-S2V上分别达到4.12x和3.75x加速，保持近无损视觉保真度和精确音频对齐。


<details>
  <summary>Details</summary>
Motivation: 现有无训练扩散缓存加速方法主要针对文本条件生成，忽视了音频驱动肖像动画中空间和模态的不平衡性：音频条件驱动的高频动态集中在人脸区域，比低频视觉背景更难缓存和重用。

Method: 提出Spatially-Asymmetric Probing优先关注动态人脸区域的误差敏感性；Modality-Decoupled Caching通过复用稳定的块间残差绕过繁重DiT块，同时连续重新计算轻量音频块以保持唇形同步；引入缓存比控制容量，将内存自适应缓存选择建模为离线动态规划问题。

Result: 在HunyuanVideo-Avatar上实现4.12x加速，在Wan-S2V上实现3.75x加速，同时保持近无损视觉质量和精确音频对齐。

Conclusion: SyncCache显著优于现有方法，是首个针对DiT音频驱动肖像动画不对称动态特性的无训练缓存加速方案。
...
TL;DR: 提出标签模仿游戏(LIG)框架，通过训练图灵测试网络(TTN)作为任务无关的"裁判"，在数据集全局上下文中评估伪标签质量，有效消除基础模型零样本推理中的幻觉错误。TTN展现出零样本任务迁移能力，在目标检测任务上取得最高44%的F1分数提升，并能实现"类别复兴"。


<details>
  <summary>Details</summary>
Motivation: 基础模型通过零样本推理进行伪标注可大规模扩展，但幻觉错误会绕过标准阈值筛选，严重影响性能。现有基于孤立阈值的过滤方法无法有效消除这些错误。

Method: 受图灵测试启发，将伪标签剪枝形式化为对抗性审问游戏(LIG)，训练一个任务无关的图灵测试网络(TTN)作为"裁判"，在数据集全局上下文中评估候选伪标签质量，而非依赖孤立阈值。

Result: 在四个数据集上，TTN一致提升了三种最先进视觉语言模型的标签准确性。实现零样本任务迁移，在目标检测伪标签剪枝中，F1分数提升28%（最差基线）至44%（任务特定微调）。观察到"类别复兴"现象，TTN剪枝"解毒"了训练信号，使下游模型在易迁移类别上从零召回率恢复。

Conclusion: 学习到的语义-上下文逻辑是空间-几何验证的鲁棒替代方案，可实现独特的零样本任务迁移能力。TTN无需昂贵监督或重新训练，即可有效提升伪标签质量。
...
TL;DR: 本文提出了一种从单目图像中通过分解物体为多个结构元素并整合外部知识（设计规则、几何关系、常规尺寸）来估计物体实际尺度的方法，并生成近似真实环境的3D资产，用于提升车载摄像头在虚拟空间中的验证精度。


<details>
  <summary>Details</summary>
Motivation: 在车载摄像头验证中，虚拟与真实环境之间被验证物体的尺度差异会导致摄像头识别性能下降。尤其是高空安装的交通标志，难以通过LiDAR或立体相机测距，需要从单目图像进行尺寸估计。

Method: 将物体分解为多个结构元素，从单目图像检测各组件，通过考虑结构关系和与周围元素的尺寸一致性来估计各组件的尺寸，最后重建生成3D资产。

Result: 能够生成尺度近似真实环境的3D资产，并将其放置于数字孪生空间中。
...
TL;DR: GRAPE是一个统一架构，通过图注意力头建模共现关系、概念不匹配安全检测机制、以及开放词汇原型锚定，解决了原型医学图像分类器在临床中的三大局限性，仅增加极小延迟即可显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有原型医学图像分类器存在三个临床局限性：将病灶视为独立不相关的（忽略共现关系）、默默放大不安全的医生反馈（无安全检测机制）、以及添加新病灶类别时需要完整重新训练。

Method: 1) 图注意力任务头（Graph Attention Task Head）建模解剖概念共现关系；2) 概念不匹配安全检查（Concept-Mismatch Safety Check）检测医生标注区域内的主导发现与声称标签是否冲突；3) 开放词汇原型锚定（Open-Vocabulary Prototype Anchoring）将视觉原型与临床文本对齐，实现从单张标注图像添加新病灶。

Result: TBX11K上宏F1提升+13.8pp；安全检查捕获85%错误标注（MC-Dropout仅51%），无额外推理代价；NIH ChestX-ray14上单张Effusion示例恢复全监督定位精度；TBX11K上原型图实现2.6倍更好的病灶定位；三项能力仅增加+1ms延迟。

Conclusion: GRAPE为原型医学图像分类器提供了统一解决方案，同时解决了共现建模、安全检测和增量学习三个临床实用性问题，且计算开销极小。
...
TL;DR: 提出了一种几何约束的测试时提示调整框架，通过感知可靠性重加权、轻量级提示适配器和时间平滑原型对齐，实现了无需额外标注的LiDAR语义分割自适应。


<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割在真实部署中因传感条件变化而性能下降，重新收集标注不现实；现有测试时自适应(TTA)策略直接应用于LiDAR数据时，由于伪标签可靠性存在空间异方差性（随距离变化的稀疏性和遮挡），对全局共享参数的统一更新会引入不稳定梯度，导致自适应不稳定。

Method: 1) 从深度一致的波束终止和邻域支持中估计每个位置的感知可靠性，用于重加权空间监督；2) 将自适应限制在插入冻结骨干网络的轻量级提示适配器中，并引入空间门控机制防止不可靠区域扰动全局共享表示；3) 利用时间平滑的原型对齐策略，随时间累积可靠语义证据以稳定在线更新。

Result: 在标准LiDAR基准上的实验表明，所提方法在无需额外标注的情况下，在部署变化下实现了更优的自适应稳定性和分割性能。

Conclusion: 几何约束的测试时提示调整框架有效解决了LiDAR语义分割在部署变化下的自适应不稳定问题，为无需重新标注的实用化部署提供了可行方案。
...
TL;DR: 本文提出Prost-RL，一种将强化学习策略与基础模型编码器-解码器相结合的微超声前列腺癌检测方法，通过学习"看哪里"生成可解释空间注意力图，在6607个活检核心数据集上实现了79.0 AUROC和64.6%敏感性，超越现有基线方法。


<details>
  <summary>Details</summary>
Motivation: 微超声（μUS）是一种新兴的前列腺癌检测成像模态，但其准确性高度依赖临床经验，存在显著观察者间差异。机器学习辅助可以降低这种差异，但训练可靠的深度模型面临两大挑战：监督信号稀疏且充满噪声（仅有核心级别组织病理结果，缺少像素级标注），以及严重的类别不平衡问题。

Method: Prost-RL将微超声前列腺癌检测重构为一个空间感知的策略驱动推理问题，核心创新包括：(1) 将轻量级强化学习策略集成到基础模型编码器-解码器中，生成可解释空间注意力图作为软提示；(2) 提出自适应策略优化（APO）以稳定混合监督-RL训练；(3) 结合对称交叉熵与负熵正则化的噪声鲁棒目标函数，缓解弱标签噪声并鼓励锐利定位。

Result: 在来自5个临床中心、693名患者、6607个活检核心的数据集上，Prost-RL在核心级别检测上达到79.0±3.5 AUROC和64.6±6.3%敏感性（80%特异性下），分别比最强基线高出2.1 AUROC和4.5个百分点的敏感性；在临床显著性癌症分类上达到79.3±5.8 AUROC。

Conclusion: Prost-RL成功将强化学习策略与基础模型结合，为微超声前列腺癌检测提供了可解释的、空间有依据的检测框架，在提供定量风险预测的同时，能够生成与活检区域对齐的透明空间证据，代码已开源。
...
TL;DR: PhotoQuilt是一种无需训练的框架，通过引导式分块去噪过程生成任意分辨率的光镶嵌图像，解决了高分辨率生成中局部细节与全局结构难以兼顾的问题。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在高分辨率下生成光镶嵌时，要么直接生成高分辨率图像成本高昂且倾向于平滑图像而非马赛克效果，要么使用分块拼接保持局部细节但丢失全局结构，无法同时满足两种尺度要求。

Method: 提出PhotoQuilt框架：先以低分辨率生成全局构图固定布局，然后在潜空间上采样并重新注入噪声恢复生成能力，最后在固定分块内进行去噪，使每个分块形成独立图像，同时共享全局结构保持统一布局。

Result: PhotoQuilt在全局结构和局部真实性上均优于当前基线方法，且由于分块生成独立处理，可扩展到大画布而无需二次注意力成本。

Conclusion: PhotoQuilt有效解决了高分辨率光镶嵌生成中局部与全局的权衡问题，实现了任意分辨率的、局部细节丰富且全局结构连贯的生成，且无额外训练开销。
...
TL;DR: 本文指出现有AI生成视频检测基准（如GenVidBench、AIGVDBench）存在评估协议漏洞（未控制混杂因素），导致简单分类器即可刷榜。作者提出六项标准审计协议，揭示多个高AUC方法实际失效，并发布VidAudit工具包（含14个检测器、统一API及排行榜），推动评估从排名转向可信度量。


<details>
  <summary>Details</summary>
Motivation: 现有AI生成视频检测基准的评估协议存在未控制的混杂因素，导致报告的性能被虚假膨胀。一篇20篇论文的调查发现，没有任何一篇应用了全部六项标准控制措施。作者希望建立审计协议来揭示这一问题，并推动更严谨的评估。

Method: (1) 提出六项标准控制措施组成的审计协议；(2) 构建一个仅使用三特征剪辑长度的平凡分类器作为存在性证明；(3) 对六个代表性特征源（三个已发布检测器+三个改造信号源）进行跨数据集审计；(4) 引入白盒正控制TemporalSpec（基于编码运动向量）；(5) 提出跨底物特征融合（XSFF）方法验证互补性；(6) 发布VidAudit工具包（14个检测器、统一API、排行榜及Croissant元数据）。

Result: 平凡分类器在未审计下AUC达0.998，审计后跌至接近随机（0.529）；CLIP基线被发现携带数据集身份信息；WaveRep在审计后仍保持OOD LOGO AUC 0.996且实-实一致性为随机水平；在0.1% FPR下，多个高AUC方法召回率跌至个位数，排行榜顺序发生变化；XSFF融合后互补性通过审计存活。

Conclusion: 现有基准评估存在严重混杂问题，需要审计协议来确保度量真实能力。推荐使用审计元组（AUC、高于基线裕度、工作点召回率、校准度）替代单一指标。VidAudit工具包为社区提供了标准化评估基础设施。
...
TL;DR: 本文提出利用视觉基础模型SAM 3作为结构先验，通过零样本点提示生成稀疏掩码，在无需手动像素级标注的情况下实现手术视频中动作感知的器械关键点定位（尖端和锚点）。通过多帧网络预测提示点生成非Oracle掩码，并与视觉和热力图特征融合来细化预测。实验在7,867个剪辑上验证，尖端和锚点定位F1分别达72.4%和58.0%，表明预测派生先验和辅助监督提供了有效的中介结构指导。


<details>
  <summary>Details</summary>
Motivation: 手术视频中器械的功能性关键点（如尖端、锚点）与分割输出的目标级结构不同，需要动作条件化的语义信息。现有方法依赖手动像素级标注，成本高且泛化性差。探索如何利用视觉基础模型的结构先验，在无手动掩码标注条件下提升稀疏动作感知关键点定位的精度。

Method: 提出轻量级细化框架：1）使用SAM 3作为结构先验；2）粗多帧网络预测尖端和锚点提示；3）生成非Oracle掩码并与视觉、热力图特征融合以细化功能关键点预测；4）对比直接掩码增强监督、预测派生掩码先验细化和辅助掩码监督三种策略。

Result: 在来自YouTube、Cholec80、HeiChole、SurgVU、CRCD的60个手术视频、7,867个剪辑上评估，达到尖端定位F1 72.4%，锚点定位F1 58.0%。直接施加掩码到热力图目标会偏向宽泛工具区域，而预测派生先验和辅助监督提供有效中介结构指导。

Conclusion: 视觉基础模型的结构先验（如SAM 3掩码）可有效辅助手术器械功能关键点定位，但需要以预测派生先验或辅助监督等间接方式引入，而非直接施加到热力图目标。该方法在无需手动像素级掩码标注下取得有竞争力性能。
...
TL;DR: 本文提出了一种名为DSAFormer的新型双稀疏聚合Transformer，用于多光谱目标检测。该方法通过双稀疏Transformer（DSFormer）中的空间稀疏交叉注意力（SSMHCA）和通道稀疏交叉注意力（CSMHCA）分别从空间和通道层面筛选高匹配度的查询-键对，消除无关区域的信息交互；结合多尺度特征细化层（MSFRL）抑制冗余信息，并利用可学习加法融合模块（LAFB）通过特征重加权有效融合模态内和模态间特征。在四个公开数据集上取得了优于现有方法的检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于Transformer的多光谱目标检测方法在相似度计算时使用所有可用token，导致来自无关区域的冗余信息交互，从而降低检测性能。本文旨在解决这一信息冗余问题，提升检测精度。

Method: 提出DSAFormer框架，包含两个核心模块：1) DSFormer，内含空间稀疏多头交叉注意力（SSMHCA）和通道稀疏多头交叉注意力（CSMHCA）分别从空间和通道维度筛选高相似度查询-键对，以及多尺度特征细化层（MSFRL）聚合层次特征并抑制冗余；2) 可学习加法融合模块（LAFB），通过特征重加权聚合模态内和模态间特征。

Result: 在MFAD、FLIR、M^3FD和LLVIP四个公开数据集上的实验结果表明，所提DSAFormer在检测性能上优于当前最先进的方法。

Conclusion: 通过稀疏化注意力计算和可学习融合策略，DSAFormer有效解决了多光谱目标检测中的信息冗余问题，显著提升了检测性能。
...
TL;DR: 提出了一种基于显著性引导的扭曲-反扭曲框架（WarpI2I），通过在编码前将空间表示重分配到显著区域，在不增加潜空间分辨率的前提下提升图像到图像翻译中的结构细节保留能力；同时提出了基于外推的合成数据生成管线用于图像重光照任务。


<details>
  <summary>Details</summary>
Motivation: 紧凑型潜扩散模型（LDMs）在编码时会压缩高分辨率输入到空间下采样的潜空间，导致细粒度结构信息难以保留。

Method: 1) 显著性引导的扭曲-反扭曲框架：在编码前对图像进行扭曲，将空间表示集中于显著区域，经扩散模型处理后通过逆扭曲映射回原空间；2) 基于外推的合成数据生成管线，用于产生高质量配对重光照数据。

Result: 在人体重光照、驾驶场景重光照及翻译任务上，实现了更好的结构保留、光照忠实度和图像质量；可自然地扩展到视频，帧间时间稳定性良好。

Conclusion: 该方法模型无关、无需修改架构、计算开销可忽略，有效解决了紧凑LDMs在I2I任务中的结构保留问题。
...
TL;DR: TerraDiT-Ω提出统一空间控制框架，支持从任意原生地理空间基元（多边形、折线、边界框、点）直接生成卫星图像，通过几何感知局部注意力机制实现可控布局，在多种条件格式下均优于现有方法，并能用于可控数据增强提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有生成模型应用于卫星图像时面临挑战：卫星场景具有空间复杂且语义分明的几何结构，而传统方法要么使用密集栅格（代价高），要么使用稀疏提示（保真度低），且与地理信息常用矢量基元不兼容，难以在标注成本与生成质量间取得平衡。

Method: 提出TerraDiT-Ω统一空间控制框架，可直接接受任意原生地理空间基元（多边形、折线、边界框、点）作为条件输入；并提出几何感知局部注意力（Geometry-Aware Local Attention）机制，将显式几何线索注入注意力空间，以有效利用不同基元的几何信息指导图像生成。

Result: 在所有条件格式下，该方法均一致优于密集控制和稀疏控制基线；使用单一生成模型进行可控合成数据增强，可在土地覆盖分割、目标检测、道路图提取和场景分类等下游任务中提升性能。

Conclusion: TerraDiT-Ω实现了从原生地理基元到卫星图像的统一可控生成，兼顾标注成本与生成质量，与端到端GeoAI工作流自然兼容，并具备提升下游任务性能的实用价值。
...
TL;DR: 本文提出预测性可微渲染（PDR），一种基于2D高斯表示的端到端视频预测新范式，通过轻量级适配器PredGS和CUDA加速渲染器predgsplat，结合L1和SSIM损失优化，显著提升了视频预测中的细节保留与视觉保真度，在多个基准数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有确定性视频预测模型在离散像素空间中运行，主要使用像素级MSE损失优化，导致预测结果过度平滑、缺乏精细视觉细节，而真实世界本质上是连续的。为弥合离散与连续表示之间的差距，本文提出PDR方法。

Method: 提出Predictive Differentiable Rendering (PDR)框架，包含两个核心组件：(1) PredGS，一种基于2D高斯表示的轻量级即插即用适配器，可无缝集成到现有像素空间预测器中；(2) predgsplat，支持任意通道的CUDA加速可微2D高斯渲染器，每个高斯由5+C个可学习参数定义（位置、尺度、旋转及C个通道振幅），渲染速度比基线快10倍。优化目标采用L1和SSIM联合损失替代MSE损失。

Result: 在TaxiBJ、WeatherBench、KTH和Human3.6M等多个真实世界基准数据集上的实验表明，PDR在细节保留、视觉保真度和预测准确性上持续优于现有方法。

Conclusion: PDR通过引入连续2D高斯表示替代离散像素表示，结合可微渲染和感知损失优化，有效克服了传统MSE导致的过度平滑问题，为视频预测提供了一种新的高质量范式。
...
TL;DR: 本文提出ADAPT框架，通过直接干预文本到图像的交叉注意力动态来减少多模态大语言模型中的幻觉现象，包含视觉锚点、注意力监督推理和视觉注意力引导DPO三个核心组件，在多个基准上将幻觉率降低40%-60%。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型存在严重的幻觉问题，即生成与输入图像不一致的内容。现有缓解策略大多是结果驱动的，没有直接针对模型内部注意力失败模式（如交叉注意力逐渐退化导致关注不聚焦或偏斜）进行干预。

Method: 提出ADAPT框架，包含三个关键组件：(1) 从早期解码阶段精炼的交叉注意力视觉锚点，提供稳定的空间依据；(2) 注意力监督推理机制，在线检测并纠正注意力漂移；(3) 视觉注意力引导DPO（偏好优化），使偏好朝向视觉上基于的响应进行对齐。

Result: 每个组件都对减少幻觉有贡献，完整框架在多个幻觉基准测试上取得最佳结果，跨主流骨干网络将幻觉率降低40%-60%，同时保持通用多模态能力。

Conclusion: 通过探索模型内部的文本到图像交叉注意力行为，提供了缓解幻觉的注意力视角，验证了直接干预注意力动态是减少多模态幻觉的有效途径。
...
TL;DR: 本文提出了一种将物理感知逆渲染与数据驱动扩散模型相结合的方法，通过将扩散模型的预测作为相似性核（而非直接目标值）来正则化优化过程，从而联合重建几何、材质和光照，生成可用于标准渲染管线的高质量可重光照3D资产。


<details>
  <summary>Details</summary>
Motivation: 物理感知逆渲染虽然提供了精确的图像形成模型，但严重欠约束——缺乏强先验时，光照会被烘焙到材质中，导致泛化能力差。而数据驱动的扩散模型虽能预测视觉合理的材质，但其预测不满足渲染方程，无法直接用于物理渲染。现有两大范式各自存在局限性，亟需一种桥梁方法结合两者的优势。

Method: 核心思想是将扩散模型的预测作为相似性核（similarity kernel）而非目标值：设计一种正则化损失，在扩散预测近似恒定的表面区域惩罚优化材质的偏差，同时保留优化过程自由匹配输入图像的能力。基于该正则化器构建端到端管线，联合重建几何、材质和光照。

Result: 在Synthetic4Relight、Stanford-ORB和DTC-Synthetic三个数据集上，该方法在重建精度和重光照质量方面均显著优于当前最先进的基线方法。

Conclusion: 成功弥合了物理感知逆渲染与数据驱动扩散模型之间的鸿沟，生成的资产可直接用于标准渲染管线并实现忠实重光照。
...
TL;DR: 本文提出一种混合UNet-Transformer架构，直接从复合材料微观结构几何图像预测十种应力/应变场，在多种二维两相复合材料构型上取得优异性能（R²最高达0.9991），并通过Grad-CAM分析验证模型学习到了物理有意义的内部表征。


<details>
  <summary>Details</summary>
Motivation: 传统有限元方法在层次化复合材料微观结构的应力应变场预测中计算成本极高（分钟至天级），亟需高效的代理模型以支持物理信息驱动的材料设计。

Method: 提出混合UNet-Transformer架构，以复合材料微观结构几何图像为输入，直接预测十种不同的应力/应变场分布，作为FEM的高效替代。在正方形、六边形、三角形拼贴等多种二维两相构型及不同边界条件下进行训练和评估。

Result: 在周期性拼贴几何子数据集上达到R²=0.9991、SSIM=0.9936、MAE=0.0050的最优性能；六个子数据集的MAE低于0.05（归一化[0,1]像素尺度）。在不规则正方形网格稀疏软相夹杂的S11正应力子数据集上性能下降（R²=0.7735，SSIM=0.7126）。

Conclusion: 模型在大多数子数据集上表现优异，Grad-CAM分析证实模型无需显式结构监督即可定位相边界、韧带连接和压头接触区等力学关键区域。在存在尖锐应力不连续性的不规则几何上性能受限，符合平滑损失图像翻译模型的已知局限。
...
TL;DR: 提出了一种零样本语义导航框架，通过在线构建层次化三维场景图（HSG）作为全局状态抽象，并引入基于层次化信念的规划方法，在HSG模拟器上执行有限范围 rollout 来估计长期回报，从而在长距离导航中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法缺乏结构化的全局表示，决策常依赖局部观察和贪婪策略，导致长距离导航中探索效率低下和短视行为。

Method: 在线维护层次化三维场景图（HSG），构建多粒度语义拓扑（物体-区域-空间）；在HSG上引入层次化信念规划，融合语义先验与探索证据；在HSG模拟器上进行有限范围 rollout 以显式估计候选宏动作的长期期望回报。

Result: 在多个高保真仿真环境和数据集上，优于现有SOTA方法，在长距离场景中成功率和SPL分别平均提升9.4%和5.0%。

Conclusion: 层次化场景图结合信念驱动的有限范围规划能有效实现全局一致的决策，减少冗余回溯，提升长距离语义导航性能。
...
TL;DR: 针对多模态图像匹配训练数据稀缺的问题，提出AnyMatch框架，利用单视图图像合成大规模多模态训练数据，通过显式3D重投影保证几何一致性，构建的Any-syn数据集微调后的匹配网络在多模态基准上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 多模态图像匹配在视觉定位和多传感器融合中至关重要，但大规模且几何标注精确的训练数据极为稀缺。真实世界数据集成本高昂、场景多样性有限且存在SfM-MVS管道累积误差；现有合成方法难以同时保证3D几何一致性和逼真外观。

Method: AnyMatch框架整合了单目深度估计、3D重投影、基于扩散的图像修复和跨模态图像翻译，利用易于获取的单视图图像合成多视角、多模态图像对。通过显式3D重投影提供严格遵循3D几何一致性的标注，避免SfM-MVS误差累积；通过可调输入和相机参数实现可控的场景多样性和标注难度。

Result: 构建了大规模合成多模态数据集Any-syn，将LoFTR、EDM、RoMa等匹配网络在其上微调后，在多模态基准上取得显著性能提升，展现出比现有数据训练模型更优的泛化能力和鲁棒性。

Conclusion: AnyMatch有效解决了多模态训练数据稀缺问题，其合成的数据具有3D几何一致性、场景多样性和可扩展性，能显著提升匹配网络的性能。
...
TL;DR: 本文提出从静态泛化转向动态适应的新范式，通过Fleet框架实现连续少样本演化，在AIGI检测中从20.4%恢复到73.1%性能。


<details>
  <summary>Details</summary>
Motivation: 现有AIGI检测方法依赖静态特征空间假设，认为可从历史数据学到不变伪影实现零样本泛化，但在快速演化的生成器（如SD3、Nano Banana Pro）上性能严重下降。

Method: 提出Fleet框架，采用动态连续少样本演化范式，用约束路由修正替代无约束特征更新，通过回避路由将AI样本重定向到解耦子空间中非AI主导的路径。

Result: 在Treasure基准（64个模型、36万张图像，含20个闭源商业引擎）上，静态SOTA方法在现代生成器上灾难性失败，而Fleet仅用10-shot适应就将性能从20.4%提升至73.1%（Doubao Seedream 4.0）。

Conclusion: AIGI检测应从静态泛化范式转向动态适应范式，Fleet框架通过约束路由修正的少样本演化可有效应对快速演化的生成式威胁。
...
TL;DR: CasaMaestro是一种前馈式模型，仅需20-50张稀疏的多视图室内全景图作为输入，即可直接预测度量深度和相机位姿，实现对整个住宅的快速点云重建，无需数千张图像和增量对齐，有效解决了传统针孔相机3D重建在大型室内空间中的效率问题。


<details>
  <summary>Details</summary>
Motivation: 家庭部署的具身AI系统对快速、度量的住宅空间3D重建需求日益增长，但传统针孔相机3D重建管线因视场有限，需要数千张图像且存在长链增量对齐带来的漂移问题，难以高效建模大型室内住宅空间。

Method: 提出CasaMaestro前馈模型，以20-50张稀疏多视图室内全景图为输入，直接预测度量深度和相机位姿，实现全屋覆盖的快速点云重建。

Result: 在真实场景和合成场景中均能鲁棒地提供高质量重建结果，可作为获取住宅尺度3D室内资产并应用于闭环仿真的强基础。

Conclusion: CasaMaestro是首个支持多视图全景图进行住宅尺度重建的模型，为家庭具身AI系统的导航、交互和长程任务执行提供了高效的3D重建解决方案。
...
TL;DR: InterTalk是一个基于运动架构的高效交互式对话人脸生成框架，支持多轮多人实时对话，通过显式建模多轮对话动态、运动反馈迭代生成策略以及面部组件解耦，实现了30 FPS的实时生成，在交互质量上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有对话人脸生成方法无法同时满足灵活性（任意数量参与者的多轮对话）、自然性（连贯动作与非语言反馈）和效率（实时生成与低计算开销）三大核心需求。

Method: 1) 基于运动架构，支持实时对话合成；2) 显式建模每位参与者的多轮对话动态，无参与者数量限制；3) 引入多参与者运动反馈和迭代生成策略；4) 将运动解耦为多个面部组件（如唇部同步、眨眼），实现针对性优化；5) 构建多人对话数据集并采用3D人脸数据增强。

Result: InterTalk在交互质量上优于现有方法，同时实现了30 FPS的实时性能。

Conclusion: InterTalk有效平衡了灵活性、自然性和效率三大挑战，为高交互性对话人脸生成提供了可行的解决方案。
...
TL;DR: 本文提出ART（Anchoring on Reality Makeup Transfer）两阶段框架，通过真实锚定循环解决伪训练数据导致的伪影和身份漂移问题，并发布首个2K分辨率化妆人脸数据集MF2K，在复杂妆容迁移任务中实现了更优的妆容保真度、背景稳定性和身份保持能力。


<details>
  <summary>Details</summary>
Motivation: 现有妆容迁移方法受限于缺乏真实配对训练数据，依赖弱先验或大规模编辑模型生成的合成伪目标，导致细粒度细节退化、合成伪影和身份漂移等问题。

Method: 提出两阶段ART框架：第一阶段利用伪目标初始化模型，建立语义对齐和全局妆容布局；第二阶段将监督信号从伪目标切换为真实参考图像，通过可微分循环从素颜版本重建参考图，惩罚遗漏细节并覆盖合成伪影。

Result: 在MF2K数据集及复杂妆容风格上，方法在妆容保真度、背景稳定性和身份保留方面均优于现有方法。

Conclusion: 基于真实锚定的循环精细化策略有效克服了伪目标监督的局限性，结合高分辨率数据集，为妆容迁移任务提供了更可靠的解决方案。
...
TL;DR: 提出结构保持平均流（SPMF）框架，通过拓扑不变传输过程实现血管图像跨模态翻译，在保持结构完整性的同时完成外观迁移，在NIRII-to-2PF和眼底数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有跨模态翻译方法主要关注像素级保真度或视觉真实性，将结构保持视为最终输出的属性而非生成过程的不变量，导致结构断裂和伪影，影响解剖学连贯性和临床可靠性。

Method: 1) 基于结构不变性原理推导出流速度场上的正交约束，将外观传输与拓扑变形分离；2) 在布朗桥扩散模型中实现为时间加权替代目标，在每个扩散步骤保持拓扑；3) 提出原型引导结构精修（PGSR）模块，对齐退化的推理时结构与可靠的训练时结构。

Result: 在NIRII-to-2PF数据集上PSNR达24.96 dB，眼底数据集上PSNR达24.83 dB，一致优于现有最先进方法。

Conclusion: SPMF框架通过将结构保持作为生成过程的不变量，有效解决了血管图像跨模态翻译中的拓扑失真问题，提升了临床可靠性。
...
TL;DR: Horizon3D是一种结合高斯原语与稀疏BEV特征的雷达-相机融合框架，用于长距离3D目标检测。它通过关键点引导的高斯初始化、目标中心的稀疏融合、BEV投影以及双路径时序融合（BEV路径和Gaussian路径），在TruckScenes数据集上实现了SOTA性能（NDS +3.0, mAP +1.6），同时保持有竞争力的推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有雷达-相机融合方法在长距离检测中表现有限：BEV方法计算量快速增长且丢失物体细节，query方法缺乏场景级上下文。时序融合需要同时处理多帧累积和运动建模。

Method: 1) Keypoint-Guided Gaussian Initialization：在雷达和相机估计的目标关键点处初始化高斯原语；2) Object-Centric Sparse Fusion：以目标为中心对高斯原语进行稀疏融合和细化；3) 将高斯原语splat到BEV平面，融合目标级细节与稀疏雷达BEV上下文；4) Dual-Path Temporal Fusion：BEV路径进行场景级累积，Gaussian路径进行目标级运动传播。

Result: 在TruckScenes验证集上，相比之前最佳方法提升+3.0 NDS和+1.6 mAP，同时保持有竞争力的推理速度。

Conclusion: Horizon3D通过高斯原语与稀疏BEV特征的结合以及双路径时序融合，有效解决了长距离3D检测中场景上下文与目标细节的平衡问题，以及时序融合中的累积与运动建模问题。
...
TL;DR: PiLoT v2提出了一种轻量级的无人机地理定位方法，将像素到3D网格的注册范式转变为像素到正交地图（TDOM/DSM）的注册，通过CPU友好的地图裁剪操作替代GPU密集型3D渲染，并利用跨视角特征配准网络和传感器先验（重力方向+激光测距）提升鲁棒性，在保持甚至超越原版精度的同时大幅降低了存储和计算开销。


<details>
  <summary>Details</summary>
Motivation: 原版PiLoT依赖厚重的3D网格，存在存储开销大、地图获取复杂、计算渲染成本高的问题，严重阻碍了在嵌入式平台上的部署。

Method: 1) 将像素到3D注册转变为像素到正交地图（TDOM+DSM）注册；2) 用CPU友好的地图裁剪操作替代GPU密集型3D渲染；3) 训练跨视角特征配准网络，并构建大规模几何标注数据集；4) 将重力方向和单点激光测距传感器先验融入位姿优化流形。

Result: PiLoT v2在性能上与甚至超越原版PiLoT的同时，大幅降低了存储和计算成本。

Conclusion: 通过从3D网格到2.5D正交地图的范式转变，结合传感器先验和跨视角配准网络，实现了轻量级、鲁棒的无人机自由视角地理定位，更适合嵌入式平台部署。
...
TL;DR: 提出View-PNDF框架，通过从神经元层面检测和微调特定视角的模式，解决多视角X光报告生成中视角不一致的问题，仅更新视角特定神经元即可实现一致诊断并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有放射学报告生成（RRG）方法在多视角X光图像处理中采用直接特征融合，忽略了不同视角间潜在的临床不一致性和不准确性，影响性能和临床可靠性。

Method: View-PNDF包含三个模块：(i) 视角特定神经元检测模块，识别对特定视角响应的神经元；(ii) 验证模块，量化这些神经元的存在；(iii) 选择性微调策略，增强检测到的神经元同时保留视角无关表示。仅更新视角特定神经元，最后利用LLM将各视角报告整合为完整报告。

Result: 在两个医学RRG基准上的实验表明，View-PNDF显著提升了视角特定的胸部X光报告生成质量，同时保持稳健的通用视角性能。

Conclusion: 从神经元层面出发的参数高效框架能有效解决多视角报告生成中的一致性问题，兼顾性能与计算效率。
...
TL;DR: TaxoMIL提出了一种基于分类学约束的多粒度文本生成框架，将WSI诊断从平面分类重构为层次化文本生成任务，通过双头Transformer解码器和分类学引导目标，在三个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统MIL将WSI诊断视为离散标签上的平面分类任务，忽略了临床推理中固有的层次化、从粗到细的特性。现有层次分类器和VLM要么无法捕捉相关诊断间的语义连续性，要么产生不受约束的文本生成导致分类学幻觉和父子标签违背。

Method: TaxoMIL采用双头Transformer解码器分别生成粗粒度和细粒度诊断文本，引入分类学引导的目标函数来显式结构化标签嵌入空间，并将幻灯片级视觉表示严格约束在临床分类学框架内。

Result: 在三个不同的WSI数据集上的大量实验表明，TaxoMIL一致优于最先进的MIL分类器和基于VLM的生成方法，产生了准确且具有层次感知的诊断预测。

Conclusion: TaxoMIL通过分类学约束的文本生成有效弥合了传统平面分类与临床层次化推理之间的结构鸿沟，为WSI诊断提供了一种更符合临床实践的范式。
...
TL;DR: InfiniVerse是一个统一框架，能从单帧图像生成长距离、2D-3D对齐、可控的动态城市场景视频，通过3D占据重建、自回归场景扩展和视频扩散模型实现，在Waymo和nuScenes上达到SOTA（FID 6.4, FVD 67.97）。


<details>
  <summary>Details</summary>
Motivation: 生成真实、可控且时序连贯的动态城市场景是自动驾驶领域的关键挑战，现有方法在长距离生成、2D-3D对齐和时空一致性方面存在不足。

Method: 1) 从输入多视角帧重建3D占据表示；2) 基于该表示沿任意轨迹进行自回归场景扩展；3) 视频扩散模型将粗粒度占据网格转化为真实、时空一致的视频序列；4) 分层草图-精炼范式，将生成视频反投影为图像条件反馈以增强3D占据表示，实现跨模态对齐与相互增强。

Result: 在Waymo Open Dataset和nuScenes上达到SOTA，FID为6.4，FVD为67.97，在生成时长和稳定性上显著优于现有基准。

Conclusion: InfiniVerse通过2D-3D联合建模与跨模态反馈机制，有效解决了长距离动态城市场景生成中的可控性和时序一致性问题。
...
TL;DR: 本文提出基于神经雅可比场（NJF）的方法，通过自监督学习从单目视频中生成具有复杂运动（如服装动态）的逼真人脸/身体化身，引入约束泊松求解器、符号距离雅可比正则化和变形引导残差光流损失，有效处理自遮挡和不可见表面问题，在时序稳定性和几何一致性上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 单目设定下生成具有复杂运动（如服装动态）的逼真人体化身面临全局与局部变形建模的挑战，尤其是自遮挡区域和不可见表面的处理困难。

Method: 基于神经雅可比场（NJF）表征半刚性变形，通过自监督神经网络预测姿态相关的雅可比矩阵并求解泊松方程；引入约束泊松求解器、符号距离雅可比正则化和变形引导残差光流损失三个关键组件。

Result: 在基准测试和野外视频上的实验表明，该方法生成时序稳定且几何一致的化身，优于现有最先进方法。

Conclusion: 提出的约束泊松求解器、符号距离正则化和残差光流损失能有效抑制边界伪影、恢复频繁遮挡区域（如腋下、大腿）并增强时序一致性，为单目人体动态建模提供了有效方案。
...
TL;DR: WildProp是一个无需训练的框架，利用大规模无约束图像库通过检索驱动对应关系估计野生动物身体比例分布，在鸟类和两栖动物数据集上中位相对误差为10-20%，适用于任意物种和用户定义部位。


<details>
  <summary>Details</summary>
Motivation: 传统形态测量方法需要受控成像或物理标本处理，难以扩展至大规模研究，因此需要一种能从无约束网络图像中可扩展地估计野生动物身体比例的方法。

Method: 使用单张用户标注的规范图像，通过基础模型特征进行姿态感知检索，利用密集块级匹配传递部位端点，通过几何一致性过滤预测，并聚合跨检索图像的测量结果以估计群体水平比例分布。

Result: 在三个大型形态测量数据集（鸟类和两栖动物）上实现10-20%的中位相对误差；通过跨多个分类群的案例研究（鸟类、青蛙、昆虫、花朵）证明了广泛适用性；消融实验表明姿态感知检索对稳定估计至关重要。

Conclusion: 精心策划的2D对应关系可以从网络规模图像中提供可扩展的形态测量代理，用于跨分类群、地理和季节性的比较和子群分析。
...
TL;DR: 本文提出SkillSpotter，一种面向增强现实眼镜和固定摄像头场景的感知多视角架构，能够同时检测和分级技能动作，在Ego-Exo4D基准上类特定mAP提升76%，平衡准确率提升至60.40%。


<details>
  <summary>Details</summary>
Motivation: 现有系统无法在自我-外部视频（ego-exo）设定下同时检测个体技能动作并判断其执行质量（正确或需改进），现有时序动作检测架构在分级任务上近乎随机表现。

Method: SkillSpotter包含三个任务特定模块：(1) 自适应时序抑制模块处理不同技能动作密度变化；(2) 门控3D身体姿态融合模块利用身体运动学作为视觉特征的互补信号；(3) 双向跨视角注意力模块有效结合自我视角和外部视角信息。

Result: 类特定mAP从12.40提升至21.82（+76%），平衡准确率从55.99%提升至60.40%，优于所有基线方法；各模块可迁移至其他时序动作检测模型；方法泛化至HoloAssist数据集。

Conclusion: SkillSpotter证明了在自我-外部视频设定中联合检测和分级技能动作的可行性，且其模块化设计具有良好的可迁移性和泛化能力，为个性化实时教练系统提供了有效方案。
...
TL;DR: LINet是一种多流神经网络，通过线性积分卷积(LIConv2d)算子在每一层实现RGB-D特征的连续跨模态融合，解决了传统融合方式的结构性缺陷。针对连续融合中的梯度崩溃问题提出1/N常数初始化，并采用渐进式模态丢弃防止通路坍塌。在SUN RGB-D 19类场景分类上达到45.2%均值精度（从头训练），49.6%（ScanNet预训练）。


<details>
  <summary>Details</summary>
Motivation: 现有RGB-D多模态架构将特征融合视为离散的临时事件：早期融合过早纠缠表征，晚期融合隔离到最后一层，混合或注意力方法需要架构试探来放置中间融合模块。作者旨在解决这种结构性妥协，实现每层连续的跨模态学习。

Method: 提出LINet，包含三个并行流（RGB、深度和积分流）。核心是LIConv2d算子，在非线性激活前对来自两个模态流的滤波信号进行线性积分。针对连续融合导致的梯度崩溃问题，提出1/N常数初始化替代Kaiming初始化。采用渐进式模态丢弃（从0开始递增丢弃概率）防止通路坍塌。

Result: 在SUN RGB-D 19类场景分类上，ResNet18规模的LINet从头训练达到45.2%平均类精度，超越此前从头训练结果；使用ScanNet域内预训练后提升至49.6%。

Conclusion: LINet通过连续的跨模态融合机制有效提升了RGB-D场景分类性能，验证了在每一层进行积分式融合的优越性，并为连续融合训练提供了关键的技术解决方案（初始化策略和渐进式丢弃）。
...
TL;DR: FROST是一种无需训练的少样本分割方法，通过将参考前景和背景视为DINOv3特征单位球面上的两个点云，利用非参数密度比进行查询令牌分类，在17个遥感基准上超越现有方法，且模型规模最小。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中标签稀缺，且与自然图像分布差异大；现有方法要么绑定训练分布，要么用有损摘要（如单一原型、聚类原型）表示类别，无法保持多模态类别的内部结构。作者认为类别更适合用分布而非点来描述，且冻结的自监督特征已包含足够结构来直接估计该分布。

Method: 将支持集的前景和背景特征视为DINOv3特征单位球面上的两个点云；对每个查询令牌，通过非参数密度比（核密度估计）计算其属于前景的概率；贝叶斯规则在等先验下将阈值固定为零；核带宽和空间门控等所有参数均从支持集读取而非调优；决策随支持样本增多而锐化（密度估计方差随样本量增加而减小）。

Result: 在17个遥感基准上，FROST超越所有无需训练和有监督学习方法；单样本标注时领先5.6 mIoU，随着支持集增大优势扩大；同时保持最小模型规模之一。

Conclusion: 基于密度估计的类别表示能有效捕捉遥感图像中多实例、多模态类别的内部结构；无需训练的方法在领域偏移下比有监督方法更具泛化优势。
...
TL;DR: WaterGen提出了一种将场景生成与水体介质效果解耦的潜在扩散框架，用于生成大规模、逼真且多样化的水下图像，并独立控制场景内容与水体条件。该方法先微调U-Net生成无退化的水下场景潜编码，再通过物理精确的介质退化解码过程合成水下外观，从而在保证多样性和真实感的同时实现可控性。实验表明，基于WaterGen构建的合成数据集能有效提升水下恢复和语义分割等下游任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有水下计算机视觉任务受限于大规模、多样化训练数据的稀缺性。已有的水下图像生成方法要么提供可控性但缺乏真实感和多样性，要么生成真实场景但无法准确独立地建模水体介质效应，无法同时满足真实感、多样性和可控性的需求。

Method: 将水下图像合成解耦为两个阶段：(1) 使用无退化水下图像微调潜在扩散U-Net，学习生成多样且真实的无退化水下场景潜编码；(2) 将物理精确的介质退化合成建模为条件解码过程，作用于上述潜编码生成最终水下图像。这种解耦设计使模型能独立控制场景内容和水体外观。

Result: WaterGen能生成大规模、场景结构多样、水体效果精确且带有伪标签的合成水下数据集。在下游任务（水下恢复和语义分割）中，使用该合成数据训练能持续提升模型性能。

Conclusion: 将场景生成与介质建模解耦的潜在扩散框架是解决水下图像生成中真实感、多样性与可控性矛盾的有效途径，生成的高质量合成数据可有效缓解水下视觉任务训练数据匮乏的问题。
...
TL;DR: 提出PruneGround框架，通过语言引导的空间剪枝、多视图描述重构和LLM定位器三大组件，在3D视觉定位任务中实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有3DVG方法在全场景推理导致歧义预测和高计算成本，而指代表达往往依赖局部空间上下文，因此需要更高效的局部化推理方法。

Method: 提出PruneGround框架，包含三个关键组件：1) 语言引导空间剪枝(LGSP)，利用冻结VLM识别语言相关区域；2) 多视图条件描述重构(MCDR)，将复杂表达分解为简化的目标-锚点关系；3) LLM-Grounder，将检测预训练的空间LLM改造为语言条件定位模型。

Result: 在三个点云基准数据集上，所有ScanRefer设置和9/10的Nr3D/Sr3D设置达到SOTA结果。

Conclusion: PruneGround作为即插即用框架，通过局部空间剪枝有效降低了计算成本并提升定位精度，验证了利用局部空间上下文进行3D视觉定位的有效性。
...
TL;DR: 提出FAT框架，通过任务分解实现基础模型与专用模型的协作：专用模型生成结构化假设，基础模型通过代理任务（如选择/验证）进行上下文推理，在2D/3D检测、轨迹预测、语义分割等任务中一致提升性能。


<details>
  <summary>Details</summary>
Motivation: 基础模型在具身智能中表现优异，但在结构化预测任务（需要精确几何和数值估计）上不如专用模型，存在能力不匹配问题。需要探索基础模型与专用模型的最佳协作方式。

Method: 提出FAT框架，将结构化预测分解为三阶段：专用模型在原生输出空间生成几何和物理上有效的假设；将假设重构为多模态候选表示；基础模型执行有边界的代理任务（如选择或验证）来优化候选。具体实例化为ProxySelect，使用视觉-语言模型。

Result: 在2D目标检测、3D目标检测、轨迹预测和语义分割四个任务上，ProxySelect一致提升专用基线性能，且显著优于直接用基础模型进行回归的方法，同时计算成本更低。

Conclusion: 专用模型保留任务特定结构，基础模型通过上下文代理推理精化假设，这种任务分解式的协作是一种通用的协作原则。
...
TL;DR: 针对自动驾驶VLA规划器中自回归推理延迟过高的问题，本文提出双推理器架构：例行推理器（专用轨迹历史+FlatRoPE注意力重定向）和审慎推理器（保留原VLA处理视觉证据），采用投机解码协作，结合Action-aware RL（AARL）训练，实现推理速度约4倍提升。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶VLA规划器的因果链推理是自回归的，主导推理延迟，而轨迹生成是并行的且廉价。延迟是自动驾驶的运行约束，因此加速推理步骤是核心问题。

Method: 1) 将推理拆分为两个专门路径：例行推理器（处理可预测延续，关注轨迹历史）和审慎推理器（处理新情况，关注当前视觉证据），以投机解码为架构模板协作。2) FlatRoPE：在草稿模型中引入1D旋转位置编码，打破目标模型3D M-RoPE的旋转对称性，将注意力从视觉令牌重定向到轨迹历史令牌。3) Action-aware RL (AARL)：使用动作质量奖励+静态参考KL锚点的后训练阶段。

Result: 双推理器系统相对于原始Alpamayo规划器，推理步骤运行时间减少约4倍。

Conclusion: 将CoC推理中常规延续与需要新视觉证据的提交进行分离，通过专门化的双推理器设计（而非标准投机解码的小模型复制），结合FlatRoPE注意力重定向和AARL训练，能显著加速自动驾驶VLA规划器的推理过程。
...
TL;DR: 本文揭示了场景坐标回归（SCR）方法在视觉定位中的隐私泄露风险：通过查询攻击，可以从SCR模型中重建训练环境的3D几何结构，并恢复近似颜色外观，直接反驳了SCR表示天然具有隐私保护性的论断。


<details>
  <summary>Details</summary>
Motivation: SCR方法因仅将场景隐式编码于网络参数中而不显式存储图像或地图，常被认为具有隐私保护性。本文旨在验证这一假设在实际中是否成立。

Method: 提出基于查询的攻击方法：使用与目标场景无关的代理图像批量查询SCR模型，获取密集像素级3D坐标；通过小输入扰动下的稳定性筛选可靠点（白盒设定下可进一步优化）；累积稳定点重建场景几何；进一步从恢复的3D表示中反转网络特征合成任意视角图像。

Result: 在室内和室外数据集上的实验表明，训练环境的大部分区域能够以高几何保真度重建，并可恢复近似的颜色外观，暴露了可识别的布局和潜在敏感场景元素。

Conclusion: SCR表示并非天然具有隐私保护性，在私有或安全关键场景中部署此类系统存在真实风险。
...
TL;DR: S-OBI是一个用于评估多模态大语言模型在句子级别甲骨文理解能力的新型基准，通过替换清晰字形构建标准化句子级实例，实验表明当前模型仍局限于字符级识别，难以实现结构化文句理解。


<details>
  <summary>Details</summary>
Motivation: 现有甲骨文识别研究集中于字符级别，忽略了完整占卜文句中包含的长文本连贯性和上下文依赖关系。

Method: 基于95片经专家验证的原始拓片，通过字形替换与组合将原拓片中的字符替换为清晰字形样本，保留整体铭文结构和语义组织，合成清晰标准的句子级甲骨文实例；并设计了语义匹配、语义槽提取、上下文推理三类任务，构建695个问答对。

Result: 当代多模态大语言模型在句子级甲骨文理解上表现不佳，特别是未掩码区域的视觉感知错误会沿推理链传播，导致对掩码字符的错误预测，表明模型对句子级理解仍强烈依赖于字符级识别。

Conclusion: S-OBI提供了一个诊断基准，用于评估MLLMs是否能超越孤立字符识别，迈向结构化的铭文级别理解。
...
TL;DR: 本文提出HSDF-Lane方法，通过隐式建模高度对齐的有符号距离场（HSDF）和可微渲染，联合生成精确的高度图和表面对齐特征，并引入车道感知语义位置编码（LSPE）将几何结构与语义引导耦合，在OpenLane基准上实现3D车道检测和高度图估计的SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有单目3D车道检测方法要么基于平坦地面假设导致几何失真，要么依赖稀疏锚点回归且仅将恢复的几何用于空间变换而非语义理解，缺乏有效的几何-语义耦合机制。

Method: 提出HSDF-Lane，包含两个核心模块：(1) 高度对齐的有符号距离场（HSDF），在密集采样的3D特征体上隐式建模路面，通过可微渲染联合输出高度图和表面对齐特征；(2) 车道感知语义位置编码（LSPE），从表面对齐特征中提取车道存在先验注入Transformer查询，实现几何结构与语义引导的耦合。

Result: 在OpenLane公开基准上，HSDF-Lane在3D车道检测和高度图估计两项任务上均达到最先进（SOTA）性能。

Conclusion: 隐式神经表示（如HSDF）结合可微渲染能有效解决单目3D车道检测中的几何-语义耦合问题，为自动驾驶中的3D感知提供了新的范式。
...
TL;DR: 本文提出 GaussianMap，一种在线高精地图构建框架，通过可学习的高斯场景表示替代传统均匀密集网格的BEV表示，利用高斯原语灵活编码局部区域，结合前馈高斯编码器与多传感器融合，在nuScenes和Argoverse 2数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有在线高精地图构建方法采用固定分辨率的密集网格BEV特征表示周围空间，但地图元素在空间上是稀疏的且需要精细的几何定位，均匀分配的BEV表示存在冗余且对矢量地图预测效果不佳。

Method: GaussianMap 的核心创新在于：1）提出可学习的高斯场景表示，在BEV平面上使用一组高斯原语，每个原语编码灵活局部区域的几何属性与特征向量，将表示能力分配给地图相关区域；2）设计前馈高斯编码器，通过高斯交互建模和多传感器特征聚合逐步细化这些原语；3）将细化后的高斯表示通过splatting操作投影为BEV特征图，再解码为矢量地图预测。

Result: 在nuScenes和Argoverse 2数据集上，GaussianMap在纯相机和相机-激光雷达融合设置下均达到最先进的性能。

Conclusion: 自适应高斯表示相比固定分辨率密集网格更能有效建模稀疏的地图元素，提升了在线高精地图构建的精度与效率。
...
TL;DR: 本文提出UCF101-AD基准数据集，用于评估多模态大语言模型对动作否定（action denial）的识别能力。实验发现20个SOTA MLLM在动作存在任务上准确率超85%，但在动作否定任务上暴跌至50%以下。作者提出CausalAct因果图模型，通过自然语言提示引入场景结构，显著降低误报率。


<details>
  <summary>Details</summary>
Motivation: 现有视频理解研究多聚焦于动作识别（动作存在），但忽略了模型识别"动作未发生"的能力。尽管MLLM在标准基准上表现优异，但它们倾向于根据上下文线索"想当然"地确认动作发生，而非真正验证动作是否真实存在。这暴露了模型在因果推理方面的盲点。

Method: 1) 构建UCF101-AD数据集，为每个正样本视频（动作存在）生成对应的负样本视频（动作否定），保留相同的人、物体、位置和运动线索，仅缺失定义性动作。2) 提出CausalAct因果图框架，通过自然语言提示显式建模场景结构（上下文、交互、运动）的因果关系，引导模型进行因果推理。

Result: 20个SOTA MLLM在UCF101-AD上：正样本动作分类准确率>85%，但负样本动作否定准确率<50%（接近随机）。引入CausalAct因果提示后，假阳性率显著降低，表明否定推理是可学习的。

Conclusion: 当前MLLM存在严重的因果推理缺陷：倾向于基于强上下文线索"确认"动作而非真正验证。UCF101-AD为诊断和改进多模态模型的因果推理能力提供了新视角。否定推理是可通过因果建模来学习和提升的能力。
...
TL;DR: 提出了一种基于时间一致性学习框架的实时经直肠超声前列腺视频分割方法，通过置信度加权的时间一致性约束和双尺度原型对齐模块，在保持2D网络单帧推理效率的同时实现时间连贯性，在SUN-SEG和TRUS-V基准上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 经直肠超声前列腺实时视频分割中，2D方法忽略时间上下文导致帧间不一致，3D方法推理延迟高，现有时间约束方法会从前列腺周围不稳定声学环境传播错误梯度。

Method: 1) 置信度加权时间一致性目标：基于光流扭曲残差选择性衰减不可靠区域贡献；2) 双尺度原型对齐模块：局部边界和全局语义特征的对比学习；3) 几何等变伪标签+知识蒸馏，避免逐帧视频标注。

Result: 在SUN-SEG和新引入的TRUS-V基准（2,679帧）上实现了最先进的精度和时间一致性，且达到实时速度。

Conclusion: 通过将时间连贯性蒸馏到2D网络中，解决了实时性与时序一致性的矛盾，为TRUS引导的前列腺介入提供高效解决方案。
...
TL;DR: 提出ExPLoRe方法，利用Soft MoE的调度权重作为逐块损失系数，通过损失耦合机制实现多目标掩码图像建模中不同图像块的自适应损失加权，在ImageNet-1K和ADE20K上取得优于非MoE基线的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MIM方法使用全局标量加权多个学习目标（token蒸馏、CLS对齐、像素重建），忽略了不同图像块之间的空间异质性，无法为不同patch分配差异化的损失权重。

Method: 将Soft MoE的调度权重重新用作逐块损失系数，允许损失梯度通过调度权重反向传播到路由器，实现内容依赖的专家专业化（损失耦合机制）。针对下游迁移，提出Freeze Routing、Expert Dropout和Freeze Attention三种微调策略。

Result: ImageNet-1K上ViT-Base：Token+CLS组合k-NN提升0.5%、线性探测提升4.4%；Token+Pixel组合k-NN提升2.2%，线性探测达80.6%，微调准确率85.3%。ADE20K上缩小2.5-2.9 mIoU差距，MoE模型匹配或超越非MoE基线。消融实验证实损失耦合是关键机制，阻断梯度导致性能下降1.6%。

Conclusion: 逐块自适应的损失加权能有效利用MIM中多目标的互补信息，Soft MoE的调度权重可被重新用于差异化损失分配，损失耦合是实现内容依赖专业化的核心机制。
...
TL;DR: 本文提出AC3S框架，通过自适应条件调制和基于多智能体视觉语言模型的3D感知提示生成，在保持照片真实感的同时实现精确3D结构对齐，解决了扩散模型生成中过条件化导致的伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的合成数据生成方法虽然能产生逼真图像，但在精确控制3D结构和姿态一致性方面存在困难。基于边缘图等视觉提示的引导方法常因过条件化产生伪影，降低了图像真实感和数据集质量。

Method: 1) 提出自监督视觉提示调制器（Adaptive Conditioning），动态调整ControlNet条件强度，防止过条件化；2) 开发多智能体视觉语言模型框架，生成与几何结构对齐的3D感知提示，增强多样性和语义一致性。

Result: 实验表明，该方法在图像质量和下游任务实用性上均有显著提升，能够规模化生成带有精确2D和3D标注的高质量合成数据集。

Conclusion: AC3S通过自适应条件机制在结构对齐和生成表现力之间取得了有效平衡，为3D感知的合成数据生成提供了一种可扩展的高质量解决方案。
...
TL;DR: AA是一个多视图多模态的屏幕注视估计数据集，包含8个屏幕内置摄像头和2个侧面摄像头的同步面部观察数据，以及精确的屏幕空间注视目标，支持基于全局和局部视觉线索的多模态学习。


<details>
  <summary>Details</summary>
Motivation: 现有单视图注视数据集在视角变化和遮挡情况下的鲁棒性不足，缺乏多视角覆盖的数据支持。

Method: 构建包含8个屏幕内置摄像头和2个侧面摄像头的多视图同步采集系统，在受控注视条件下收集精确的屏幕空间注视目标，并提供结构化面部区域裁剪和标准化数据处理流程。

Result: 生成了包含多视角面部观察、结构化面部区域裁剪的多模态数据集，提供了独立于主体的评估划分。

Conclusion: AA数据集通过多视角覆盖和标准化流程，为注视估计领域提供了更鲁棒、可复现的研究基准。
...
TL;DR: CooperScene是一个高保真协作自动驾驶数据集，包含真实C-V2X通信特性，涵盖3辆联网自动驾驶车辆和1个路侧单元，共59K帧、344K个标注对象，支持多智能体扩展和实际部署场景的评估。


<details>
  <summary>Details</summary>
Motivation: 现有C-V2X协作数据集忽略了真实部署中的复杂性，如有限且动态的通信带宽、异构感知模态以及多智能体协作的可扩展性。

Method: 构建了包含交叉口、高速公路匝道和停车场等多场景数据集，涉及3辆CAV和1个RSU，均配备多模态传感器和商用C-V2X无线通信设备。实现了严格的传感器与智能体同步、厘米级定位与空间对齐、精确跨模态标定以及3GPP标准兼容的C-V2X通信，以10Hz频率提供全局一致的3D标注。

Result: 数据集包含59K帧、344K个标注对象，支持多智能体扩展和实际部署性能的严格基准评估。

Conclusion: CooperScene为评估协作自动驾驶在真实部署环境中的多智能体扩展和实际性能提供了可靠的基准。
...
TL;DR: ForgeDrive提出了"先行动后想象"（act-then-imagine）的统一自回归扩散框架，通过视觉-动作交叉条件化级联，将未来分解为逐时间步的帧-动作对，在训练中解耦两种模态的扩散时间步并使用UniDiffuser风格噪声调度器，在推理时先生成动作再用其提升未来帧生成质量，从而避免传统"先想象后行动"范式中视觉生成错误级联到规划的问题，在NAVSIM上统一了驾驶模拟、规划和视觉里程计并超越了现有规划器。


<details>
  <summary>Details</summary>
Motivation: 现有基于世界模型的自动驾驶方法采用"先想象后行动"（imagine-then-act）范式，导致更具挑战性的视觉生成阶段产生的错误会级联传播到动作规划中，影响最终驾驶性能。

Method: 1) 统一自回归扩散框架，将未来分解为逐时间步的帧-动作对序列；2) 训练时解耦图像和动作两种模态的扩散时间步，引入UniDiffuser风格噪声调度器实现跨模态条件推断；3) 推理时采用act-then-imagine范式，先生成动作再用于提升未来帧生成质量；4) 增加未来自我状态预测来增强规划能力。

Result: 在NAVSIM数据集上，ForgeDrive将驾驶模拟、规划和视觉里程计统一到单一模型中，且无需任何后训练策略即超越了现有的强规划器。

Conclusion: 通过反转传统范式的因果顺序（先行动后想象），可以避免视觉生成误差向规划阶段的级联传播，从而在统一多任务的同时提升规划性能。
...
TL;DR: 本文针对超高清多焦图像融合面临的数据、模型和部署三大障碍，提出了首个大规模UHD-MFF数据集和UMF-LUT框架（包含粗区域查找表C-LUT和细节边缘查找表D-LUT），在低分辨率下利用多梯度与语义线索进行区域级决策，在高分辨率下利用拉普拉斯线索进行边缘级决策，实现了4K实时融合并具备智能手机部署潜力，在视觉质量和量化指标上均超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有大多数多焦图像融合方法局限于低分辨率图像，在超高清（UHD）场景下面临三大障碍：数据可用性（缺乏大规模超高清数据集）、模型适应性（现有模型难以处理超高清图像）和部署可行性（计算开销大，难以实时运行）。

Method: 提出了UMF-LUT框架，包含两个查找表：C-LUT在低分辨率尺度下联合查询多种梯度线索和语义线索进行粗粒度区域级决策；D-LUT在高分辨率尺度下利用高效的拉普拉斯线索提供互补的边缘级决策信息。同时构建了首个大规模超高清多焦融合数据集UHD-MFF。

Result: 在视觉保真度和量化指标上均超越现有最先进方法，实现4K实时融合，具备智能手机部署可行性，计算开销极低。

Conclusion: 该方法有效突破了超高清多焦图像融合的三大障碍，推动了多焦图像融合向超高清成像场景发展，代码已开源。
...
TL;DR: 本文提出了一种双曲手术视频-语言预训练框架，通过显式保留手术知识的分层结构（如动作→步骤→阶段），解决传统方法将多层级语义压缩到单一平面嵌入空间导致的跨层级语义包含关系丢失和长程依赖建模不足的问题，在多个手术基准上取得了零样本和少样本阶段识别的一致提升。


<details>
  <summary>Details</summary>
Motivation: 现有手术视觉-语言基础模型通常将教学材料中多维度、层次化的手术知识（细粒度动作线索、中层级关键步骤、全局过程上下文）压缩到单一平面嵌入空间，隐含假设层级间独立，忽略了跨层级的语义包含关系（如动作属于步骤、步骤构成阶段），导致长程依赖建模能力减弱。

Method: 提出双曲手术视频-语言预训练框架，利用双曲空间的几何特性（可指数级扩展以容纳层次结构）来显式保留手术知识的层级结构；通过缓解由过程上下文导致的虚假负样本（structural false negatives），并强制父阶段与子步骤之间的语义一致性来增强跨层级约束。

Result: 在多个手术基准数据集上的大量实验表明，所提方法在不同手术流程和机构间的一致实现了零样本和少样本阶段识别的性能提升。

Conclusion: 通过显式建模手术知识的多层级结构（而非扁平化嵌入），能够更有效地捕捉跨层级的语义包含关系和长程依赖，从而提升模型在手术阶段识别任务上的泛化能力。
...
TL;DR: 本文提出了一种紧凑的多模态时序建模框架，通过融合2D/3D骨骼、面部Blendshape、DINOv2/v3、X-CLIP和Gemini等多源特征，结合交叉注意力机制和自适应多实例学习池化，在第四届EI-MIGA-IJCAI挑战赛Track 3中以0.76923测试准确率获得第一名，并揭示了视觉基础模型在微动态任务中的表示坍缩现象及公共排行榜驱动的伪泛化机制。


<details>
  <summary>Details</summary>
Motivation: 长视频中隐式情绪证据微弱且稀疏，现有方法难以有效捕捉。

Method: 提出紧凑的多模态时序建模框架，集成2D/3D骨架、面部Blendshape、DINOv2/v3、X-CLIP和Gemini语义先验等多源特征；设计以静态姿态特征(Base)为Query、动态微动差分特征(Offset)为Key/Value的交叉注意力机制消除个体体型身份静态偏差；采用基于多实例学习的自适应池化方法提取瞬时情绪并抑制长序列背景噪声。

Result: 在Track 3中取得测试准确率0.76923，获得第一名。

Conclusion: 揭示了视觉基础模型在微动态任务中的表示坍缩现象，以及网络因捷径学习和死记硬背陷入公共排行榜驱动的伪泛化机制。
...
TL;DR: 本文发现，通过线性探针和转向恢复读取VLMs隐式知识的标准方法会系统性地高估模型实际对图像的利用程度。以空间推理为例，引入空白图像替换的因果控制揭示了三种不同的"接地"机制：接地（依赖视觉）、先验（不依赖视觉）和反转（可解码、可控但符号错误）。在14个VLM中，水平轴接地，垂直轴是先验，深度轴是反转的，且这种反转随模型规模扩大而出现。空白图像仲裁器能清晰分离这三种机制，应成为VLM隐式知识和转向研究的默认控制方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法（线性探针+转向恢复）可能高估VLM对图像的实际依赖程度，混淆了真正的视觉感知与模型先验知识，需要一种更精确的方法来区分模型是否真正"看到"了图像内容。

Method: 在空间推理任务中，将图像替换为灰色空白作为因果控制，对比模型在有/无图像输入时的表现，从而区分三种接地机制（接地、先验、反转）。同时使用线性探针和训练自由的转向恢复方法评估模型。

Result: 水平轴接地（依赖视觉正确）、垂直轴是先验（不依赖视觉）、深度轴反转（可解码且可因果控制，但部署时符号错误，导致得分低于随机水平）。这种反转在更大规模模型中更明显。通过训练自由的旋转或低秩编辑可以修正反转。

Conclusion: 空白图像仲裁器是一种简单有效的因果控制方法，能清晰分离VLM的视觉感知、反转感知和先验替代三种机制，应作为VLM隐式知识和转向研究的默认控制手段。
...
TL;DR: WarpHammer是一种无需训练的新型视图合成框架，通过引入显式3D物体重建（来自SAM3D等生成先验）来增强扭曲场景，解决了大轨道运动下投影条件NVS方法出现的稀疏扭曲、隐藏表面和伪影问题，并首次支持融合外部无位姿辅助视图。


<details>
  <summary>Details</summary>
Motivation: 现有投影条件NVS方法在视角变化较小时效果良好，但在大轨道运动下扭曲变得稀疏，隐藏表面占主导，产生镜面伪影，生成器丢失像素内容和隐式相机线索。作者希望解决这一失效模式，同时扩展NVS能力以融合外部辅助视图。

Method: 1) 从原生3D生成先验（如SAM3D）获取物体的显式3D重建；2) 用该重建增强扭曲场景，补充缺失的前景表面并遮挡不应再可见的背景点；3) 对参考图和辅助图，利用预训练的多视图几何基础模型联合预测统一点云，融合到3D物体重建中，无需用户提供辅助视图的相机位姿。

Result: 在五个基准数据集上，WarpHammer在基线方法崩溃的大视角偏差下仍能生成稳定的新视图，并且是首个能够自然融合外部无位姿物体视图的场景级NVS方法。

Conclusion: 通过引入显式3D物体先验增强扭曲场景，无需微调即可修复大视角变化下的退化问题，并解锁了融合外部辅助视图的新能力，显著扩展了NVS的实用边界。
...
TL;DR: 本文提出了一种失败驱动的自我提升范式，利用LLM诊断失败轨迹，生成代码补丁来改进计算机使用智能体，在OSWorld基准上将OpenCUA-72B的成功率从42.3%提升至48.9%，无需额外训练成本。


<details>
  <summary>Details</summary>
Motivation: 现有的计算机使用智能体通过成功轨迹进行自我提升，但丢弃了蕴含丰富信息的失败轨迹，导致数据利用不充分。

Method: 采用LLM诊断失败模式，提出推理时解决方案，生成经人工轻量验证的代码补丁来升级智能体。

Result: 在OSWorld基准上，OpenCUA-72B模型成功率从42.3%提升至48.9%，提升了6.6个百分点，无需额外训练成本，仅需少量推理开销。

Conclusion: 失败驱动的自我提升是成功驱动管道的有效补充，能以更低成本更高效地改进智能体。
...
TL;DR: 本文指出现有视觉-语言模型的不确定性估计方法存在视觉过自信导致输出多样性不足的问题，提出仅扰动图像而固定文本查询的视觉语义熵（VSE）方法，通过在语义原型间计算质量加权离散度来估计不确定性，在多个模型和基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言模型在视觉模糊输入上会产生过度自信的预测，现有基于语义熵（SE）的方法因过自信的视觉嵌入抑制了输出多样性而低估不确定性；近期基于输入扰动的方法虽有所改进，但其变异性主要来自文本变化而非视觉证据，导致估计的是提示敏感性而非视觉模糊性。

Method: 提出Visual Semantic Entropy (VSE)：仅对图像进行扰动以探测邻近视觉变化，同时保持文本查询固定；通过对生成的答案进行语义原型聚类，并计算各原型之间的质量加权离散度来衡量不确定性。

Result: 在五个现代视觉-语言模型和五个多样化的VQA基准上，VSE有效捕获了视觉模糊性，建立了VLM不确定性估计的新SOTA。

Conclusion: 仅扰动图像而固定文本的VSE方法能更准确地反映视觉模糊性，避免了以往方法中文本变化主导输出多样性导致的不确定性估计偏差。
...
TL;DR: CLIMB提出了一种结合层次化质心记忆库与知识蒸馏的在线持续自监督学习方法，在不依赖任务边界的情况下，通过存储有限数量的代表性图像并进行对比学习，有效缓解了表示漂移问题，在多个基准测试上超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有OCSSL方法要么仅依赖重放缓冲区利用潜在空间结构，要么仅依赖正则化，未能有效结合两者的优势来处理无标签数据流中的表示漂移问题。

Method: 提出层次化质心记忆库（Intelligent Memory Bank），以有限总数存储图像并将相似图像分组为质心；结合知识蒸馏对重放样本进行约束，限制表示漂移；利用质心提供难区分的样本用于对比学习，同时覆盖观测分布的多样性。

Result: 在Split CIFAR-100和Split ImageNet-100上，以及在标准基准和包含不规则任务分布的新协议上，CLIMB均优于现有最先进的OCSSL方法。

Conclusion: 结合层次化质心记忆与知识蒸馏的混合策略是OCSSL的有效范式，能够在不依赖任务边界和内存受限的条件下持续学习高质量表示。
...
TL;DR: MICE是一种无需训练的、基于多模态扩散Transformer的多实例图像编辑策略，通过修改联合注意力机制的加性偏置来调控实例间交互，实现高保真、高解耦的多区域并发编辑，在多项指标上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有单次前向传播的多元素图像编辑方法在面对多个指令指向不同区域时，存在语义干扰导致的属性泄漏和编辑解耦差的问题，且随着编辑数量增加这一问题愈发严重。

Method: MICE通过修改多模态扩散Transformer中联合注意力的加性偏置（additive bias），基于用户提供的分割掩码，实现实例内注意力增强、相邻区域令牌交互惩罚以及无关跨实例注意力抑制，从而调控实例间交互。

Result: 在LoMOE-Bench和新提出的MICE-Bench（平均每图8.5个并发编辑）上，MICE在视觉质量保持和编辑指令忠实度方面均优于强基线和近期竞品方法。

Conclusion: MICE作为一种无需训练的策略，能够有效实现可扩展的多实例并发图像编辑，在保证全局视觉一致性的同时强化属性绑定，解决了多编辑场景下的语义干扰问题。
...
TL;DR: ToothForge是一个深度谱生成框架，通过同步谱嵌入在低维空间中学习牙冠几何形态，在有限数据集下实现与空间方法相当或更优的重建与生成性能，并保持紧凑性和几何可解释性。


<details>
  <summary>Details</summary>
Motivation: 牙冠形态的精确建模对诊断、正畸规划和计算机辅助修复设计至关重要，但可用于训练此类模型的数据集通常规模有限，亟需一种能在小样本下有效学习的方法。

Method: 提出ToothForge深度谱生成框架，利用同步谱嵌入在谱域中建模牙冠几何，通过Laplace-Beltrami特征基的同步化处理消除不同样本间连接不一致性问题，在低维空间中高效学习潜在流形。

Result: 同步谱建模在重建和生成性能上达到或超过基于空间的方法，同时保持紧凑性和几何可解释性；该框架在有限数据集场景下表现尤其出色。

Conclusion: ToothForge的紧凑同步系数和低维学习空间使其特别适合牙科和医学领域常见的有限数据集，并在各诊所形状连接不一致的现实场景中具有实用性。
...
TL;DR: SpikeLogBERT是一种基于脉冲神经网络的日志解析框架，通过脉冲Transformer架构和BERT知识蒸馏，在HDFS数据集上实现了0.99997的解析精度，同时将理论能耗降低62.6%。


<details>
  <summary>Details</summary>
Motivation: 现有神经日志解析方法依赖密集矩阵乘法，计算成本高且能耗大，亟需一种兼顾语义表示能力与能效的日志解析方案。

Method: 提出SpikeLogBERT框架，集成脉冲Transformer架构，利用脉冲神经元稀疏激活和事件驱动特性，并通过BERT教师模型的知识蒸馏来保持语义表示能力。

Result: 在HDFS数据集上解析精度达0.99997，优于ANN神经日志解析模型；在标准45nm CMOS假设下，理论能耗降低高达62.6%。

Conclusion: 脉冲神经网络可用于日志解析任务，在保持高精度的同时显著降低能耗，为绿色日志分析提供了新的研究方向。
...
TL;DR: 本文提出了一种结合小波优化的伪3D加速扩散模型（CL-DM），用于解决计算层析成像中因视野限制导致的投影数据截断问题。该方法将重建目标扩展至数据不完全区域，通过2D扩散模型切片聚合与3D模型迭代重建保证数据一致性，并引入小波正则化、平移不变机制和低频保留策略消除切片间不连续，显著提升了成像范围和重建质量。


<details>
  <summary>Details</summary>
Motivation: 计算层析成像中视野限制导致投影数据截断，这是一个病态逆问题，会引发严重重建伪影。现有深度学习方法依赖缺乏严格数据一致性约束的2D架构，且将伪影去除局限在视野内，丢弃了视野外潜在可恢复的信息。

Method: 首先对CL视野进行综合分析，将空间划分为数据完全、数据不完全和数据自由三个区域，并将重建目标扩展至数据不完全区域。提出小波优化的伪3D加速扩散模型（CL-DM）：利用标准2D扩散模型进行切片聚合，结合3D基于模型的迭代重建保证数据一致性；引入沿z方向的小波正则化、平移不变机制和低频保留策略缓解切片间不连续；采用3D快速采样架构加速推理。

Result: 大量仿真和真实实验表明，CL-DM在有效消除截断伪影和恢复高保真连续3D结构方面表现优越。

Conclusion: 通过扩展重建目标区域并设计伪3D扩散模型，有效解决了CL中视野限制导致的截断伪影问题，显著扩大了有效成像范围并提高了扫描效率。
...
TL;DR: 提出加速似然最大化（ALM）方法，一种无需训练、直接优化未观测区域的扩散模型采样策略，在保持计算效率的同时实现全局一致的多样化内容生成，并在多项任务中超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在从部分给定输入生成多样、连贯且合理内容时存在局限：基于训练的方法成本高且泛化性差；无需训练的方法效率高但未显式优化未观测变量，导致全局不一致。

Method: ALM是一种集成到逆向扩散过程中的无需训练采样策略，通过直接优化采样过程中的未观测区域来实现全局连贯生成，并引入加速策略提升计算效率而不牺牲性能。

Result: 在多个数据领域和任务中，ALM一致优于最先进方法，证明了其作为通用内容生成范式的有效性。

Conclusion: ALM通过直接优化未观测区域并引入加速策略，克服了现有方法在效率和全局一致性上的权衡，为扩散模型在多样化生成任务中的应用建立了强大的新范式。
...
TL;DR: Vega是一个统一视频理解与生成的框架，采用共享词汇表联合建模文本与视觉表示，结合自回归预测关键帧语义令牌与扩散模型渲染高分辨率视频帧，在视频生成（VBench）和理解（VideoMME）基准上均取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 视频领域统一建模（理解与生成）尚未被充分探索，核心挑战在于视频理解需要紧凑的语义表示，而视频生成需要保留视觉细节和时间连贯性的密集信号。视频天然包含空间语义和时间动态，比静态图像更适合统一多模态建模。

Method: 提出Vega统一框架：采用共享词汇表联合建模文本与视觉表示；使用混合架构，结合自回归预测（AR）与扩散渲染；AR模型负责预测关键帧的语义视觉令牌，提供结构化表示；扩散模块基于AR输出渲染密集高分辨率视频帧。

Result: 在视频生成基准VBench和视频理解基准VideoMME上均取得强性能。

Conclusion: Vega框架成功弥合了视频理解与生成之间的表示差异，验证了统一建模在视频领域的可行性和有效性。
...
TL;DR: 在基于MURA肘部X光片的骨科异常检测任务中，研究了多种预处理管线（含轻量级DnCNN学习预处理模块）对判别力和校准能力的影响。结果表明，在患者级泄漏感知协议下，预处理策略带来的改进有限且依赖具体配置，原始输入的DenseNet121基线在所有指标上仍具竞争力。


<details>
  <summary>Details</summary>
Motivation: 探究在面向分诊的骨科异常检测任务中，学习型预处理管线（如DnCNN）相比传统预处理方法是否能带来判别力和校准性能的显著提升，尤其是在患者级泄漏感知评估协议下。

Method: 基于MURA肘部X光片数据集，在泄漏感知协议下进行患者级异常检测；以DenseNet121为基线，对比多种预处理管线（含/不含轻量级DnCNN模块）；使用AUROC、PR-AUC评估判别力，ECE、Brier score评估校准能力，并分析高特异性下的验证集选择工作点。

Result: 不同预处理策略间的差异较小且依赖具体配置，无任何策略在所有判别指标上一致优于原始输入基线；原始及多样化输入搭配DnCNN前端降低了ECE和Brier score，但CLAHE结合DnCNN未改善校准性能。

Conclusion: 在患者级评估下，预处理带来的增益有限且依赖配置；原始输入的DenseNet121基线在所有测试策略中保持竞争力，未发现任何预处理策略能产生一致的判别优势。
...
TL;DR: 本文提出RCL-Mamba，一种基于状态空间模型的测量导向双域图像复原网络，用于解决旋转扫描计算层析成像（RCL）中因连续稀疏扫描引起的投影域旋转模糊和图像域稀疏伪影问题，在保持重建质量的同时将扫描视角从512减少到64，实现约8倍的检测效率提升。


<details>
  <summary>Details</summary>
Motivation: RCL在大面积平板元件无损检测中广泛应用，但连续稀疏扫描导致的投影域旋转模糊和图像域稀疏伪影等跨域退化问题尚未得到有效解决，现有方法难以同时处理这两种退化。

Method: 提出RCL-Mamba级联联合处理框架：先在投影域校正旋转模糊，再在图像域抑制稀疏伪影；并设计Mamba-CNN双分支模块自适应平衡大尺度模糊校正与局部细节恢复。

Result: 在模拟数据集和真实PCB扫描数据上，RCL-Mamba在模糊去除、伪影抑制和结构保持方面均优于现有基线方法；线轮廓结构测量验证了该方法能更好保持通孔/焊盘边界和细长走线轮廓；将所需扫描视角从512减少至64，检测效率提升约8倍且不牺牲重建质量。

Conclusion: RCL-Mamba为高通量RCL检测提供了一种鲁棒的测量导向复原方案，有效提升了结构测量保真度。
...
TL;DR: 本文提出LA-SR框架，利用视觉-语言模型将非配对超分辨率问题映射到语言空间解决，通过从单张高质图像中提取真实退化LR patches，并设计语言引导的内容损失和质量损失，实现真实LR图像的超分辨率重建。


<details>
  <summary>Details</summary>
Motivation: 现有SR方法依赖人工合成退化数据（手工艺核或相机ISP调整），无法捕捉真实LR图像的复杂退化，导致泛化性能差。真实场景中获取配对的HR-LR数据困难。

Method: 提出LA-SR框架：(1) 从单张高质图像中利用不同深度区域分辨率差异提取真实LR patches；(2) 使用视觉-语言模型将图像映射到语义空间；(3) 设计两种语言引导损失：语言内容损失保持语义保真度，语言质量损失增强感知真实性。

Result: LA-SR能有效超分辨率真实LR输入，生成逼真的输出结果，克服了基于合成数据训练方法的局限性。

Conclusion: 通过将非配对SR问题重新定义到语言空间，利用视觉-语言模型弥合LR-HR差距，LA-SR为真实场景下的超分辨率提供了有效的解决方案。
...
TL;DR: 本文提出双流双层循环优化框架，将循环自训练扩展至目标检测领域，通过均值教师和额外归一化解决了伪标签不可靠、训练不稳定和回归损失爆炸三大挑战。


<details>
  <summary>Details</summary>
Motivation: 循环自训练(CST)在图像分类中表现优异，但将其扩展至目标检测面临三个主要挑战：1)三种不可靠伪标签（分类错误、定位错误及其组合）限制了CST在检测中的上界；2)多目标检测直接应用CST导致训练不稳定；3)回归坐标数值范围过宽导致损失爆炸。

Method: 提出双流双层循环优化框架，具体包括：1)将CST同时应用于分类和回归分支；2)在Mean Teacher上构建CST以防止训练不稳定；3)使用额外归一化将回归边界框映射到标准化空间以解决损失爆炸问题；4)提供了回归边界的理论推导。

Result: 在四个跨域标准场景上的大量实验表明，该框架取得了可观的效果。

Conclusion: 通过双流双层循环优化框架，成功将CST从图像分类推广到目标检测领域，有效解决了伪标签不可靠、训练不稳定和回归损失爆炸等核心问题。
...
TL;DR: 本文提出了一种名为MAPE的深度学习防御方法，包含SAPE（通道注意力U-Net消除扰动）和PPSA（负动量预训练模型调度算法），在CIFAR-10和Mini-ImageNet上分别达到95.1%和71.5%以上的平均防御率，有效抵御多种可迁移对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 可迁移对抗攻击具有高隐蔽性和检测难度，现有防御方法难以有效应对此类攻击，亟需设计鲁棒的防御机制来消除多种可迁移的对抗扰动。

Method: 提出多源对抗扰动消除（MAPE）框架，包含两个核心组件：(1) 单源对抗扰动消除（SAPE）机制，采用通道注意力U-Net作为防御模型，利用预训练模型生成的对抗样本进行训练；(2) 预训练模型概率调度算法（PPSA），通过模型差异量化和负动量策略调度多个预训练模型，最大化训练过程中对抗样本的差异性，增强防御鲁棒性。

Result: 在黑盒攻击场景下，以ResNet-34为目标模型，MAPE在CIFAR-10上平均防御率超过95.1%，在Mini-ImageNet上超过71.5%，达到最新最优性能。

Conclusion: MAPE能有效消除多种对抗样本中的扰动，为来自不同替代模型的攻击提供鲁棒防御，验证了通道注意力U-Net与负动量调度策略在对抗防御中的有效性。
...
TL;DR: 提出MS-Resampler，通过多范围视觉重采样框架（注入显式空间范围先验到重采样注意力中）替代传统单范围重采样器，在固定token预算下同时捕获局部细节与全局上下文，以最小计算开销提升多模态理解与推理性能。


<details>
  <summary>Details</summary>
Motivation: 传统MLLM中基于重采样（如Q-Former、Perceiver Resampler）的投影器采用单范围全局交叉注意力，将密集视觉特征压缩为紧凑token序列时，会模糊细粒度局部证据，难以在固定token预算下同时捕捉局部细节与全局上下文。

Method: 提出MS-Resampler，实例化多个范围特定的重采样器，每个分支通过注入显式空间范围先验（如位置编码或注意力掩码）到重采样注意力中，实现从局部到全局的不同粒度视觉信息聚合，最后通过自适应融合各分支输出得到最终视觉表示。

Result: 在10个公开多模态基准上，MS-Resampler相较于传统单范围重采样器，一致提升了视觉理解与多模态推理能力，且仅引入极小的计算开销。

Conclusion: 多范围视觉重采样框架能有效平衡局部细节与全局语义，为MLLM提供更丰富的视觉表示，是一种高效且即插即用的改进方案。
...
TL;DR: OVOW是一个无需训练的从单目视频重建实例级、可仿真4D网格场景的系统，通过四阶段流程（视觉语言模型实例发现、类别感知重建、迭代渲染匹配优化、物理接地组装）生成水密网格场景，在结构化视频到4D基准测试中达到最优效果，速度快1-2个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有4D重建方法（隐式场、高斯原语、点云）缺乏水密拓扑、实例分离和标准化物理接口，无法直接用于物理仿真器和具身AI。OVOW旨在填补这一空白，从单目视频生成可直接用于下游物理仿真和编辑的实例级4D网格场景。

Method: 四阶段流水线：1) 视觉语言模型发现、标注和运动分类所有实例；2) 类别感知重建，为刚体生成逐实例网格，为可变形物生成拓扑一致的网格序列；3) 迭代渲染-匹配-优化过程恢复度量尺度和6-DoF位姿轨迹；4) 物理接地组装确保地面接触和物体间支撑。所有运动（刚性和非刚性）通过直接顶点变形建模，无需类别先验或骨架绑定。

Result: 在两个合成基准（静态和4D）上，OVOW在布局和几何精度、光度误差和语义误差方面达到最优；在单目视频上运行速度比基线快1-2个数量级；下游物理仿真验证了其物理稳定性。

Conclusion: OVOW首次实现了从单目视频到实例级、可仿真4D网格场景的无训练重建，为结构化视频到4D评估建立了首个基准，同时可扩展为合成配对的视频到4D仿真数据的引擎，服务于未来4D世界模型和具身AI。
...
TL;DR: 本文提出 TemporalLens 诊断框架和 YOLO-3D 架构，用于检测视频目标检测器是否真正利用时序信息。TemporalLens 通过受控扰动揭示模型行为差异，YOLO-3D 表明保留时序深度是性能关键驱动因素（+3.7 pp mAP@50）。


<details>
  <summary>Details</summary>
Motivation: 现有单阶段视频目标检测器是否真正利用时序上下文还是仅依赖单帧信息尚不明确，标准指标（如 mAP）无法区分预测结果的达成方式，存在评估盲区。

Method: 1）TemporalLens：模型无关的诊断框架，通过受控扰动、结构化遮挡、时序打乱、冗余注入和分辨率退化等操作探测时序依赖；2）YOLO-3D：基于 YOLOv8 的模块化实时时空检测器，通过在骨干网络中保留时序深度来利用多帧信息。

Result: stacked 2D 模型在移除目标帧时性能崩溃，而时空模型能从较早帧恢复预测；YOLO-3D 在 32 帧平均下 mAP@50 提升 3.7 个百分点，骨干网络中保留时序深度是主要性能驱动因素。

Conclusion: TemporalLens 和 YOLO-3D 将"检测器是否利用时间信息"这一模糊问题转化为可测量、可操作的具体评估与设计方法。
...
TL;DR: 本文提出了一种无提示扩散图像隐写框架，通过级联仿射耦合模块建立秘密图像与潜表示的确定性双射映射，并融合风格语义先验控制隐写图像生成，同时引入预测-校正机制来优化逆向过程中的轨迹偏差，在安全性、重建精度和可控性方面达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有基于文本提示的扩散模型隐写方法受限于文本提示的灵活性不足，难以生成高质量的隐写图像并准确恢复秘密图像。

Method: 1) 级联仿射耦合模块(CACM)：建立秘密图像与其潜表示之间的双射确定性映射；2) 风格语义融合：将风格语义先验集成到扩散过程中，控制潜表示生成，确保视觉不可感知性；3) 预测-校正机制：通过当前状态和预测下一状态的反馈迭代优化生成轨迹，缓解无条件逆向过程中的轨迹偏差。

Result: 在安全性、秘密图像重建精度和可控性方面，该方法与最先进方法相比取得了有竞争力的性能表现。

Conclusion: 无提示的扩散图像隐写框架通过引入风格语义先验和预测-校正机制，有效克服了文本提示方法的局限性，实现了更鲁棒、更可靠的隐写图像生成。
...
TL;DR: 本研究探讨了动态4DCT中左心房及左心耳分割的训练集时间设计策略，发现全帧训练在早期低对比度阶段表现最佳，但生理学选择的子集在填充期后性能相当，前景归一化能部分提升缩减数据集的性能。


<details>
  <summary>Details</summary>
Motivation: 动态增强心脏CT可实现左心房和左心耳的时间分辨分析，但准确分割面临时间对比度变化大、单一标注序列等挑战，需探索训练集时间设计对分割性能的影响。

Method: 基于nnUNet框架，比较三种训练集设计：最小两帧数据集（模拟临床实践）、生理学选择的帧子集、完整27帧序列；并评估前景归一化的影响。

Result: 全帧训练在早期低对比度阶段性能最优；生理学子集在填充期后性能接近全帧；前景归一化可提升缩减数据集在低对比度帧的性能，但未能完全消除差距。
...
TL;DR: 本文针对格里高利记谱法的光学识别问题，设计了一种基于S-GABC的通用编码，将四个不同编码的数据集统一转换，并训练了一个共享的端到端基础模型，在所有四个数据集上都达到了新的最优水平。


<details>
  <summary>Details</summary>
Motivation: 现有的格里高利记谱法数据集采用不同的编码格式，缺乏统一标准，阻碍了模型泛化能力的提升和跨数据集的性能比较。

Method: 基于S-GABC提案设计通用编码方案，将四个现有数据集统一转换为此编码，并训练一个共享的端到端基础模型。

Result: 在所有四个数据集上都达到了新的最优水平。

Conclusion: 通用编码结合共享基础模型的方法能够有效统一异构数据，提升格里高利记谱法光学识别的整体性能。
...
TL;DR: SatAgent是一种无人机-卫星协同空间推理模型，通过双路径视觉机制、3D重建编码器和多视图拓扑-语义对齐模块，在复杂城市环境中实现了显著的性能提升（比通用基础模型高25.91%，比专用空间推理模型高11.69%），并构建了首个大规模无人机-卫星协同多视图空间推理数据集SatAgent-SR130K。


<details>
  <summary>Details</summary>
Motivation: 现有基于无人机的空间推理方法存在两大关键局限：单视角感知易受遮挡和透视畸变影响；大多数VLM缺乏显式几何建模，依赖语义线索，在视角和尺度变化下推理不一致。因此需要一种能融合无人机与卫星视角、具备显式几何建模能力的空间推理模型。

Method: 1) 几何感知3D重建编码器(Geometric-Aware 3D Reconstruction Encoder)，将2D无人机特征提升为显式3D空间表征；2) 多视图拓扑-语义对齐模块(Multi-view Topology-Semantic Alignment)，在统一BEV坐标系中融合跨视角特征；3) 多视图一致性损失(Multi-view Consistency Loss)，鼓励视角不变的表征；4) 构建了首个大规模无人机-卫星协同多视图空间推理数据集SatAgent-SR130K。

Result: 在多样化任务中，SatAgent比通用基础模型平均高出25.91%，比专业空间推理模型平均高出11.69%，特别是在复杂几何关系推理中取得了极高的准确率。

Conclusion: 通过模拟人类视觉系统的双路径机制，无人机与卫星视角的协同融合以及显式3D几何建模能够显著提升空间推理的鲁棒性和准确性，为航空具身智能在复杂城市环境中的应用提供了有效方案。
...
TL;DR: 提出一种异步架构的开放词汇3D场景图方法，将轻量级在线建图与重量级语义细化并行运行，支持探索过程中实时查询，在多个基准上达到或超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D场景图方法采用"先重建后增强"的两阶段顺序流程，导致探索过程中图不可查询，限制了实时应用能力。

Method: 提出异步架构：1) 基于概率体素的轻量级在线建图主干，增量式维护稳定物体身份；2) 后台VLM智能体渐进式丰富语义图；3) 语义闭环解决重复物体轨迹；4) 细粒度视觉属性附加与空间关系推导；5) 多目标帧调度器选择信息量大的关键帧，分摊VLM计算成本。

Result: 在语义分割任务上匹配或超越现有方法（ScanNet, Replica）；在视觉定位基准（Sr3D+, Nr3D, ScanRefer）上以A@0.25指标超越此前最优方法15.3到18.8个百分点。

Conclusion: 异步解耦轻量级在线建图与重量级语义细化是可行且有效的，能实现探索过程中实时查询且语义随时间增长，性能优于传统两阶段流程。
...
TL;DR: DrivingDepth提出将稀疏LiDAR作为几何提示，通过残差逐像素尺度校正来校准冻结的深度基础模型，在保留密集视觉几何结构的同时实现度量精度，在nuScenes上达到SOTA性能（AbsRel 11.19, EdgeCR 5.741）。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶的密集深度估计面临几何-尺度冲突：深度基础模型提供像素对齐的密集视觉几何但缺乏可靠度量尺度，而投影LiDAR提供的度量锚点稀疏、噪声大且与图像结构不对齐。现有方法从头生成深度，覆盖了基础模型的连贯几何结构，在视觉连续表面上产生结构伪影。

Method: 提出DrivingDepth，将稀疏LiDAR视为几何提示，通过残差逐像素尺度校正来局部校准冻结的深度基础先验，通过构造保留密集视觉几何结构。核心思想是基础模型已捕获几何连贯的相对深度，只需逐像素尺度因子将相对几何映射到度量坐标。

Result: 在nuScenes数据集上，使用4帧环视输入，DrivingDepth达到AbsRel 11.19和EdgeCR 5.741，优于MapAnything（11.99/1.914），同时实现了SOTA度量精度和几何一致性。

Conclusion: 通过利用冻结的深度基础模型和稀疏LiDAR提示的局部尺度校正，可以在不牺牲几何一致性的前提下获得精确的度量深度，解决了几何-尺度冲突。
...
TL;DR: HVPNet是一种受人类视觉过程启发的轻量级通用计算架构，通过视网膜整合模块（RIM）和皮层解码器（CD）实现多模态显著/伪装目标检测，在22个数据集上达到精度与效率的优越平衡。


<details>
  <summary>Details</summary>
Motivation: 现有多模态显著目标检测（SOD）和伪装目标检测（COD）方法依赖复杂的跨模态特征融合与解码结构，导致模型参数过大、结构冗余且检测性能不理想。受人类视觉过程高效识别目标的启发，作者探索能否通过模拟人眼视觉机制实现更简洁的建模策略。

Method: 提出HVPNet，包含两个核心模块：1）视网膜整合模块（RIM），以视网膜多层级信息整合为概念隐喻，通过层级特定的多阶段整合策略有效融合多模态特征；2）皮层解码器（CD），将解码过程分解为低层和高层视觉阶段，抽象人脑视觉皮层层次化处理机制。

Result: HVPNet可扩展至4种模态下的7个任务，在涵盖这7个任务的22个数据集上取得了优秀的精度-效率平衡，无需额外技巧即可达到优异性能。

Conclusion: 从人类视觉过程中汲取概念灵感，可以设计出结构简单、通用性强且性能优异的多模态目标检测模型，有效解决现有方法结构冗余与参数过大的问题。
...
TL;DR: 本文提出了一种全自动深度学习框架，用于在OCT图像中对GA相关的关键生物标志物（RPE丢失、EZ丢失、EZ变薄）进行像素级分割。该框架基于298个SD-OCT体积（涵盖AMD全表型谱）训练，并在独立外部数据集（n=43）上验证。结果显示高分割精度（Dice RPE丢失: 0.88, Dice EZ丢失: 0.87, Pearson's r > 0.99），总EZ厚度测量亚像素平均偏差2.15 μm，再现性ICC > 0.98。该工具为临床试验和常规眼科护理中的GA评估提供了可靠手段。


<details>
  <summary>Details</summary>
Motivation: 年龄相关性黄斑变性继发的地图样萎缩需要精确监测相关结构生物标志物，以评估疾病阶段、进展和治疗反应。现有方法在自动化和精确量化方面存在不足。

Method: 使用三个专门的语义分割模型分别描绘RPE丢失、EZ边界（包括中断）和Bruch膜；在包含AMD全表型谱的298个SD-OCT体积数据集上训练，并在独立外部数据集（n=43）上验证；还评估了重复性、读者间可靠性、B-scan密度影响及按病灶大小分层的亚组性能。

Result: Dice RPE丢失: 0.88, Dice EZ丢失: 0.87, Pearson's r > 0.99；总EZ厚度测量亚像素平均偏差2.15 μm；再现性ICC > 0.98。

Conclusion: 该全自动框架能够准确一致地量化光感受器外层退化和RPE丢失，为临床试验和常规眼科护理中的GA评估提供了高度可靠的工具。
...
TL;DR: SimpleSearch-VL 提出了一种高效、可靠且实用的多模态智能搜索框架，通过改进智能体自身的搜索与验证过程（而非扩展数据或工具），在仅用少量数据的情况下显著提升了 Qwen3-VL 基线的性能，并达到与 Gemini-3-Pro 竞争的水平。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖扩展数据、工具或辅助模型组件来提升多模态智能搜索性能，导致成本高、复杂度大，缺乏对智能体自身搜索与验证过程的优化。

Method: 1) Factorized Adaptive Rollout (FAR) 提升采样效率；2) evidence-verified reasoning 进行显式思维链验证；3) 轻量工具接口与网页自摘要机制。

Result: 使用仅5K监督工具交织轨迹和2K强化学习数据，SimpleSearch-VL 在8B和30B-A3B变体上分别比 Qwen3-VL 智能体基线提高15.8和16.0个平均点，30B-A3B模型性能与 Gemini-3-Pro 竞争。

Conclusion: 通过聚焦智能体自身的搜索-验证流程优化，而非扩展外部资源，可以高效实现多模态智能搜索的显著提升。
...
TL;DR: PRISM利用预训练VAE潜空间实现单图反射去除，通过潜空间线性分离假设、流匹配速度场和两种解耦策略（LCC和LCS），在六个基准上显著超越SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有SIRR方法在像素空间操作，sRGB形成模型的非线性导致两层纠缠，限制了泛化能力。作者发现预训练VAE潜空间中图像层间相干性远低于像素空间，提供了更有利的分解工作空间。

Method: 1) 将SIRR重新定义为潜空间线性分离问题；2) 在预训练FLUX骨干上学习流匹配速度场，单次前向恢复传输层和反射层；3) 提出潜成分一致性策略(LCC)，通过跨样本交换反射潜变量并施加循环损失实现鲁棒解耦；4) 提出层对比分离损失(LCS)，通过补丁级对比学习促进层间语义分离，无需显式反射目标。

Result: 在六个基准数据集上显著优于现有SOTA方法，对野外图像具有强泛化能力。

Conclusion: 在潜空间中建模线性分离是SIRR的有效范式，预训练VAE潜空间为图像分解任务提供了比像素空间更有利的条件。
...
TL;DR: MV-GEL是一个多视角几何实体定位框架，通过提示条件视图排序模块（GELviews）选择最优视角，结合VLM推理分割和几何感知光线投射，实现从自然语言查询中对多边形网格上的细粒度几何实体（边、面、体）进行定位，相比基线方法在面级IoU提升1.7倍，边级F1提升4.5倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在图像领域的指代分割已取得进展，但将语言驱动的定位扩展到结构化3D几何面临重大挑战：3D对象外观对视角高度敏感，单一视角可能因遮挡或透视缩短导致目标实体不可见。

Method: 提出MV-GEL框架，包含三部分：(1) GELviews——基于提示条件的视角排序模块，根据语言提示的几何CAD实体可观测性对视角排序；(2) VLM推理分割骨干处理选定视角；(3) 通过几何感知光线投射将预测掩码提升到对应网格。框架完全CAD无关，仅依赖3D网格。

Result: 面级IoU提升1.7倍，边级F1提升4.5倍以上，显著优于CLIP基线和随机视角采样方法，尤其对薄结构和视角敏感结构效果突出。

Conclusion: 通过选择使查询实体最大化可解释的视角，结合VLM分割与3D几何映射，可以有效解决细粒度几何实体定位中的视角敏感性问题，且无需依赖CAD先验知识。
...
TL;DR: 本文提出DataEvolver，一个自演化的多智能体框架，通过将数据构建视为基于反馈的演化策略，利用被拒绝样本中的失败信号指导后续数据构建。在文本丰富图像生成任务上，DataEvolver在0.75M数据规模下，OCR-F1比最强基线提升85.3%（TextScenesHQ）和35.3%（LongTextBench），且改进跨模型迁移。


<details>
  <summary>Details</summary>
Motivation: 传统文本丰富图像数据构建遵循"爬取-过滤-冻结"的静态范式，过滤后被拒绝的样本被直接丢弃，但这些样本中往往包含OCR错误、语义不匹配等有价值的失败信号，导致后续构建轮次重复相同的失败模式。

Method: DataEvolver是一个自演化多智能体框架，包含四个模块：Retriever收集候选样本，Verifier分配质量分数和拒绝原因，Critic将轮次级反馈总结为语义反馈，Generator通过目标合成补全覆盖不足的区域。更新的反馈记忆指导下一轮构建，形成闭环演化。

Result: 在PixArt-alpha上，0.75M数据规模下，DataEvolver在TextScenesHQ上OCR-F1提升85.3%，在LongTextBench上提升35.3%，均超过固定数据集基线。改进在Show-o2上也一致，表明不依赖单一下游生成器。

Conclusion: 被拒绝的样本能够为改进文本丰富图像数据构建提供可操作的反馈信号，基于反馈驱动的自演化数据构建策略优于静态固定数据集方法。
...
TL;DR: AugSplat通过利用神经辐射场合成额外视角图像来增强稀疏视图下的高斯泼溅优化，在保持实时渲染能力的同时提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 神经辐射场在稀疏视图下重建鲁棒但推理慢，高斯泼溅渲染快但对初始几何质量敏感，在稀疏视图下因点云不完整或含噪而性能下降。旨在结合二者优势，提升稀疏视图下的高斯泼溅重建质量。

Method: 先用稀疏输入视图训练神经辐射场，合成邻近新视角图像以扩展视图覆盖；再将合成视图作为辅助监督用于高斯泼溅优化。提出两种变体：Staged AugSplat（先用合成视图预训练，再切换真实图像）和Dual AugSplat（联合训练真实与合成视图，合成损失权重递减）。

Result: 在稀疏视图mip-NeRF 360数据集上，AugSplat在重建质量上优于标准高斯泼溅。Staged AugSplat平均性能最佳，Dual AugSplat表现接近且全程保持真实图像监督，两种变体均保持推理时实时渲染。

Conclusion: 提出一种简单有效的框架，通过辐射场视图增强显著提升稀疏视图下高斯泼溅的重建质量，同时保留实时渲染优势。
...
TL;DR: 针对3D点云掩码自编码中位置信息过度依赖导致的语义表示弱化问题，本文提出MPL-MAE框架，通过重新校准位置嵌入和门控位置接口模块，平衡空间先验与语义特征，在多个下游任务中取得竞争性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D MAE框架的解码器过度依赖位置信息，导致语义表示学习被弱化，特征质量次优。

Method: 1) 重新校准的位置嵌入模块：抑制度量主导的坐标信号，同时保留几何拓扑；2) 门控位置接口模块：动态调控重建过程中的位置信息注入。

Result: 在多个下游任务中一致取得竞争性能，验证了方法的有效性。
...
TL;DR: 针对偏滤器温度场实时重建需求，提出物理感知神经算子Transformer（PNOT），通过图注意力捕捉空间依赖、物理感知算子聚合相似物理条件、Sobolev正则化约束梯度一致性，实现高精度且物理一致的快速预测。


<details>
  <summary>Details</summary>
Motivation: 传统有限元等数值方法计算成本高，无法满足聚变装置偏滤器温度场实时重建与控制的需求，亟需一种快速且可泛化的方法。

Method: 提出PNOT：1）将边界热流关系建模为结构化图，用图注意力显式捕捉空间物理依赖；2）设计物理感知神经算子模块，通过切片聚合相似物理条件的查询点并建模热扩散；3）采用梯度约束的Sobolev正则化损失，强制函数值及其导数的一致性。

Result: 实验表明，所引入的物理约束在保持物理一致性的同时提升了预测精度。

Conclusion: PNOT能够高效、准确地实现偏滤器温度场的时空演化建模，适用于实时应用场景。
...
TL;DR: 本文研究了局部化共形预测在视觉-语言模型图像分类中的应用，发现直接使用余弦相似度无法优于非局部基线，并提出了一种简单的非线性变换方法，在保持边际覆盖保证的同时显著降低了平均预测集大小。


<details>
  <summary>Details</summary>
Motivation: 共形预测的完全条件保证是不可达的，现有方法通过局部化近似条件行为在回归任务中表现良好，但在图像分类中尚未充分探索。作者旨在填补这一空白，研究局部化共形预测在视觉-语言模型图像分类中的有效性。

Method: 基于开源实现的局部化共形预测算法，在自然图像分类任务上使用视觉-语言模型进行广泛基准测试。提出对余弦相似度进行简单的非线性变换，以改进局部化共形预测的性能。

Result: 直接使用视觉特征间的余弦相似度无法提升超越非局部基线；所提出的非线性变换方法在保持边际覆盖保证的前提下，实现了统计上显著的平均预测集大小缩减。

Conclusion: 简单的非线性余弦相似度变换可以有效提升局部化共形预测在视觉-语言模型图像分类中的表现，为不确定性量化提供了实用改进方案。
...
TL;DR: 本文发现基于相机参数的Transformer位置编码在视角合成任务中，由于将旋转和平移信息存储在值向量的相同维度中，导致训练后期性能停滞。为此提出DPPE方法，显式解耦旋转和平移的位置编码，实现了稳定的长期训练和更好的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于相机参数的位置编码在缩放训练时，模型性能在训练后期出现停滞瓶颈。作者旨在探究该瓶颈的根本原因并提出解决方案。

Method: 提出Decoupled Pose Positional Encoding (DPPE)，显式地将旋转和平移信息解耦到不同维度中，避免两者在值向量中混合存储导致的识别不确定性。

Result: DPPE在视角合成任务中实现了稳定的长期训练，即使在缩放训练设置下也表现良好；在泛化外推场景（如增加视角数量和zoom-in场景）中展现出优越的性能。

Conclusion: 旋转与平移信息在位置编码中的维度耦合是导致训练可扩展性瓶颈的关键因素，DPPE通过显式解耦有效解决了该问题。
...
TL;DR: 提出ViToS双流强化学习框架，通过主动视觉令牌剪枝和跨反馈序列优化，在医学多模态推理中减少77%视觉令牌，同时性能提升至108.27%（Lingshu-7B）和104.16%（HuatuoGPT-Vision-7B）。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在医学图像推理中表现不佳，因为医学图像的视觉证据极其稀疏，而目前的强化学习框架未能统一实现主动视觉令牌剪枝与医学多模态推理。

Method: 提出双流强化学习框架ViToS，包含两个任务分支：一个专注接地（grounding），另一个在视觉令牌剪枝后进行令牌稀疏推理；通过交叉反馈序列优化解决耦合策略学习问题，避免梯度冲突并促进共享策略模型收敛。

Result: 在7个医学基准上，视觉令牌减少至原始序列长度的77%，在Lingshu-7B上达到108.27%的相对性能，在HuatuoGPT-Vision-7B上达到104.16%的相对性能。

Conclusion: ViToS在医学多模态推理中实现了优越的性能和推理加速，建立了一种高效的推理范式。
...
TL;DR: 提出了一种基于不确定性引导的合成上下文增强策略，通过保留原始像素标签有效性、仅对不确定区域进行上下文重绘，有效提升了语义分割模型在稀疏、小目标及罕见类别上的性能，在Cityscapes、UAVID和BDD100K上取得了显著mIoU提升。


<details>
  <summary>Details</summary>
Motivation: 语义分割模型在处理数据稀疏、罕见或视觉多样化的区域（如密集区域、小目标）时表现不佳。直接合成增强数据会导致标签与生成像素对齐错误，现有方法依赖外部模型或使用粗粒度启发式策略（如无差别增强所有前景或背景），浪费了未信息像素的计算资源。

Method: 1) 利用基线分割器的预测熵识别不确定语义区域；2) 仅对不确定区域之外的互补上下文进行修补（inpainting）；3) 微调时仅在原始像素上计算损失，排除重绘区域，使模型专注于未修改的不确定区域在新上下文中的学习。

Result: 在Cityscapes、UAVID和BDD100K数据集上取得了显著的mIoU提升，特别是在罕见和困难类别（如公交车、火车、航空视角下的汽车）上增益最大。

Conclusion: 不确定性引导的上下文增强是一种高效利用合成数据提升分割性能的策略，无需外部护栏即可严格保持标签有效性，并最大化每个合成样本的像素信息量。
...
TL;DR: 本文提出了一种统一的高阶结构对齐框架用于多视图雷达分割，通过可学习超图捕获雷达回波间的高阶依赖关系，利用非平衡最优传输（UOT）对齐异构雷达投影视图，并采用自适应注意力机制融合互补视图。在CARRADA和RADIal基准上分别达到63.8%和83.4% mIoU，超越此前最优方法1.7和2.3个点。


<details>
  <summary>Details</summary>
Motivation: 现有雷达分割方法大多依赖基于网格的编码和成对交互，难以捕捉同一物理目标产生的多个雷达回波所构成的高阶关系结构，导致在稀疏、嘈杂、弱语义的雷达数据上进行密集语义分割效果不佳。

Method: 提出统一的高阶结构对齐框架，包含三个关键模块：(1) 可学习超图用于捕获空间相关雷达响应间的高阶依赖；(2) 非平衡最优传输（UOT）实现不同雷达投影视图（RA/RD/AD）间无需对应关系的特征对齐，适应不同测量密度和局部观测；(3) 自适应注意力机制融合互补雷达视图，在稀疏和噪声条件下强调结构信息丰富的响应。通过监督分割与跨视图一致性正则化联合训练。

Result: 在CARRADA基准上达到63.8% mIoU，在RADIal基准上达到83.4% mIoU，分别比此前最优方法提升1.7和2.3个mIoU点，在两个数据集上均取得一致改进。

Conclusion: 高阶关系建模对于鲁棒的雷达感知至关重要，所提出的结构对齐框架能够有效提升多视图雷达分割的性能。
...
TL;DR: 本文提出ATMem，一种将GUI智能体记忆从被动存储转变为主动维护执行状态的方法，并引入STR-GRPO在线强化学习方法，使智能体学会根据任务完成贡献选择性使用记忆。同时构建了挑战性的移动端基准测试，评估智能体在长序列任务中完成范围内工作并避免范围外操作的能力。


<details>
  <summary>Details</summary>
Motivation: 现有记忆方法将记忆视为被动存储，仅积累和检索历史观测，但在长序列任务中，检索到的值无法揭示其在工作流中的当前角色（是否已使用、待使用或需等待依赖），导致重复或遗漏操作。需要一种能主动维护执行状态的记忆机制。

Method: 提出ATMem（主动任务驱动记忆），将任务相关信息维护为持续更新的执行状态，每个值与其角色和当前状态关联。同时提出STR-GRPO，一种在线强化学习方法，通过对比记忆开启和关闭的轨迹来估计何时使用记忆能改善执行，并使用记忆成本感知奖励来抑制无益的记忆使用。

Result: 构建了挑战性移动端基准测试，要求智能体从近似的条目列表中，对每个满足指令的条目执行操作，同时拒绝违反约束的条目，以评估智能体在长序列执行中完成范围内工作并避免范围外操作的能力。

Conclusion: ATMem将记忆从被动存储转变为主动执行状态，结合STR-GRPO选择性使用记忆，有望提升GUI智能体在长序列任务中的可靠性，尤其在高相似度字段和干扰项存在的复杂场景中。
...
TL;DR: 本文提出一种解耦的多智能体海上无人机着陆框架，通过USV的3-RPU稳定平台与双DRL智能体（SAC波浪补偿+多模态RL进近控制）协同，在15次高保真仿真试验中实现100%着陆成功率，粗糙海况下稳定效率87.8%，96%时间维持平台水平度在1°以内。


<details>
  <summary>Details</summary>
Motivation: 海上基础设施的自主航空巡检常因随机海况而失败，存在高动能撞击、着陆后倾覆和传感器遮挡等风险，亟需可靠的无人机海上着陆方案。

Method: 采用解耦的多智能体框架：USV搭载3-RPU稳定平台，利用SAC智能体进行高频波浪补偿；同时独立的DRL智能体控制UAV最终进近，两者协同完成安全着陆。

Result: 在高保真海事仿真中，15次试验从平静到粗糙海况下均100%成功着陆；粗糙条件下平均稳定效率87.8%，96%任务时间内着陆表面水平偏差小于1°。

Conclusion: 所提解耦双DRL框架能有效补偿海况扰动，显著提升海上无人机着陆的安全性与成功率，为自主海上巡检提供可靠技术支撑。
...
TL;DR: PrISM-IQA 将智能手机图像质量评估重塑为多问题有序诊断任务，预测每个 ISP 相关问题的严重等级（无、轻微、严重、危急），并通过累积有序编码与结构化推理实现逻辑一致性，支持实际的 ISP 调优。


<details>
  <summary>Details</summary>
Motivation: 现有智能手机 IQA 方法将感知质量简化为单一分数，与 ISP 调优实践脱节——工程师需要识别具体质量问题、评估严重程度并判断是否可接受，而非仅获取一个标量分数。

Method: PrISM-IQA 采用多问题有序诊断框架：1) 对每个 ISP 相关问题预测有序严重等级（无/轻微/严重/危急）；2) 结合累积有序编码（cumulative ordinal encoding）与结构化推理（structured inference），捕获问题内的单调性及问题间的包含与互斥关系。

Result: 在重构的 SPAQ 基准（含53个 ISP 相关问题标注）和小规模专家标注真实数据集上验证了有效性，显示其在实际问题级诊断中的优势，并通过线性探测揭示了可迁移的感知质量表征，预测结果能够支持可操作且有意义的 ISP 调优。

Conclusion: PrISM-IQA 通过结构化多问题有序诊断范式，弥合了 IQA 与 ISP 调优实践之间的鸿沟，为智能手机图像质量评估提供了更具实用价值的新方向。
...
TL;DR: LiteMatch是一个轻量级立体匹配框架，通过成本体积稳定化实现强零样本泛化，无需昂贵的3D卷积。它采用两个互补编码器（CVCE和HFE）以及CVC-Loss损失函数，在参数仅3.36M-9.58M的情况下，在多个基准数据集上取得了有竞争力的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的立体匹配方法为了实现高精度，往往依赖沉重的骨干网络和计算密集的3D成本体积处理，导致大量内存和运行时开销，且跨域泛化能力差，限制了实际部署。

Method: LiteMatch包含三个关键组件：(1) 交叉视图对应编码器(CVCE)捕捉全局跨视图交互；(2) 高频编码器(HFE)通过基于FFT的频率线索增强细粒度结构细节；(3) 成本体积一致性损失(CVC-Loss)，一种应用于softmax归一化成本分布的体素级二元交叉熵目标，通过鼓励尖锐且单峰的视差概率来稳定成本分布并实现快速收敛。此外还设计了轻量级精化模块，以低迭代更新产生全分辨率视差。

Result: LiteMatch在3.36M到9.58M参数范围内，在Scene Flow、KITTI、Middlebury、ETH3D和DrivingStereo等多个基准数据集上取得了有竞争力的EPE和D1性能，展示了优异的零样本跨域泛化能力。

Conclusion: 轻量级架构能够在不牺牲精度的情况下实现跨域泛化，为立体匹配的实际部署提供了可行的方案。
...
TL;DR: 本文提出RoboSpatialBrain，基于RoboBrain2.5-8B-NV，通过两种免训练推理机制（强制<think>前缀激活+任务特定后提示、显式参考框架重定向流水线）提升视觉语言模型在具身任务中的空间推理能力，在RoboSpatial Challenge中获得第一名，成功率达80.9%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在通用感知上表现优异，但在具身任务所需的空间推理方面存在不足，特别是相机中心与物体中心的空间歧义问题亟待解决。

Method: 1) 强制<think>前缀激活策略结合任务特定后提示，促使模型对上下文和兼容性任务进行深思熟虑的推理；2) 显式参考框架重定向流水线，解决相机中心和物体中心的空间歧义问题；3) 额外探索在兼容性数据上微调RoboBrain2.5，并分析其与提示机制的交互作用。

Result: 在RoboSpatial Challenge中获得第一名，在RoboSpatial-Home数据集上总体成功率达到80.9%。

Conclusion: 所提出的免训练推理机制有效提升了视觉语言模型在具身任务中的空间推理能力，且团队开源了代码。
...
TL;DR: 本文提出Q-Margin，一种基于α-散度的损失函数，通过在参考测度（先验概率）中编码边界惩罚，在保持稀疏性的同时实现判别性嵌入，在人脸和说话人验证任务中达到或超越ArcFace/CosFace性能，尤其在低误接受率下表现优异，并支持精确且内存高效的训练。


<details>
  <summary>Details</summary>
Motivation: 现有基于边界惩罚的softmax损失（如CosFace、ArcFace）在面部和说话人验证中表现优异，但新提出的α-散度损失函数虽能诱导稀疏解（α>1时），却无法自然地扩展标准几何边界。作者希望设计一种能在α-散度概率框架下引入原则性边界的损失函数，同时保留稀疏性优势。

Method: Q-Margin将边界惩罚直接编码到参考测度（先验概率）中，而非像传统方法那样对logits施加几何惩罚。该损失函数基于α-散度框架，通过修改先验分布来实现判别性嵌入，同时保持α>1时的稀疏性特性。

Result: 在IJB-B和IJB-C人脸验证基准以及VoxCeleb说话人验证上，Q-Margin达到竞争性或更优性能。与相同训练配置下的ArcFace和CosFace基线相比，Q-Margin在低误接受率（FAR）下持续改进。此外，Q-Margin后验的极端稀疏性实现了精确且内存高效的训练。

Conclusion: Q-Margin通过将边界惩罚融入先验概率，成功将几何边界思想推广到α-散度框架，兼顾了判别性、稀疏性和训练效率，为大规模身份识别任务提供了可扩展的解决方案。
...
TL;DR: 论文提出SAMBA，一种基于散射引导双向Mamba的自监督预训练基础模型，用于SAR目标解译。其核心创新包括：线性复杂度的Mamba编码器、三级层次化散射引导掩码自动编码器（SG-MAE）策略、轻量级SpatialMix特征交互模块，以及两阶段跨域预训练流程。在多个下游数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: SAR ATR在实际部署中受限于稀缺的标注训练数据。现有自监督预训练方法中，Transformer架构计算复杂度呈二次增长，而传统通用掩码策略忽略了SAR图像独特的电磁散射特性。

Method: (i) 线性复杂度的Mamba编码器，引入序列中间类令牌以缓解计算瓶颈；(ii) 三级层次化散射引导掩码自动编码器（SG-MAE）策略，基于SAR物理先验设计掩码，使预训练任务对齐SAR成像机制；(iii) 轻量级SpatialMix特征交互模块，增强跨区域特征融合；(iv) 两阶段跨域预训练流程优化整体预训练过程。

Result: 在所有预训练配置下均表现优异，参数量远少于CNN和Transformer基线。SG-MAE策略相比标准MAE默认掩码策略进一步提升少样本迁移能力。在7个下游数据集（涵盖分类与检测任务）上，大部分指标达到SOTA。

Conclusion: SAMBA有效解决了SAR ATR中标注数据稀缺问题，其物理引导的自监督预训练方法在效率和性能上均优于现有方法，具备良好的泛化能力。
...
TL;DR: WorldRoamBench是一个评估交互式世界模型在动作、视觉、物理和记忆四个维度上长程稳定性的开放世界基准测试，包含600+测试用例，实验表明现有模型尚未能全面满足所有维度要求。


<details>
  <summary>Details</summary>
Motivation: 现有基准仅在轨迹级别评估动作执行，忽略了记忆和交互物理，缺乏对交互式世界模型在多个维度上细粒度、长程稳定性的系统评测。

Method: 提出四维度基准：(i) 动作：逐帧动作指标，规避跨模型语义尺度差异；(ii) 视觉：基于片段的漂移指标，捕获中间序列坍塌；(iii) 物理：可控性门控评估，覆盖力学、光学和3D一致性；(iv) 记忆：动作解耦协议，通过过渡局部3D点云重建和跟踪+VLM推理评估场景与主体记忆。

Result: 评估10+个开源和闭源模型，发现没有任何模型能可靠满足所有维度要求，最佳模型也仅取得中等分数。

Conclusion: WorldRoamBench上的进步是迈向稳定、物理基础扎实、记忆忠实且可部署于实际应用的交互式世界模型的关键步骤。
...
TL;DR: REDI方法利用监督TF-IDF（基于DINOv3最终块的视觉词汇）与注意力图的逐元素乘积，为每个补丁分配保留/合并/压缩角色。在ViT-B/16上减少46.8%序列长度，ImageNet-1K Top-1达84.706%，优于密集基线83.514%，表明类别级语料统计与图像级注意力提供互补的补丁排序信号。


<details>
  <summary>Details</summary>
Motivation: 探索在固定token预算下，如何通过受控的监督参考（ground truth类别信息）来优化补丁分配，实现分类精度与效率的更好权衡。

Method: 1) 将DINOv3最终块补丁表示量化成语义词表，基于TF-IDF计算类别条件语料分数；2) 对验证图像，用真实类别索引TF-IDF表，四变换视图生成对齐中心裁剪的TF-IDF图；3) 对同一裁剪进行密集前向得到注意力图；4) 两者独立min-max归一化后逐元素乘积得REDI分数；5) 根据分数排名分配补丁角色（保留/合并/压缩），根据分数幅度加权合并与压缩操作。

Result: 冻结DINOv3 ViT-B/16骨干，线性分类器，序列从201降至107 tokens（46.8%缩减）。REDI（基于输入注意力质量）达84.706% Top-1，对比密集基线83.514%、仅注意力82.634%、仅TF-IDF 81.796%。三种替代注意力公式上，加入语料项均优于纯注意力版本。

Conclusion: 类别特定的语料库统计（TF-IDF）与图像特定的注意力图提供互补信号，联合使用显著优于任一单独信号，为ViT token缩减提供了有效的监督引导策略。
...
TL;DR: ShellMaker是一种语言引导的建筑外壳补全框架，能在保持室内结构约束的前提下，为建筑生成风格一致的外立面、屋顶及PBR材质。


<details>
  <summary>Details</summary>
Motivation: 现有室内场景生成方法只能生成楼层平面和墙壁结构，缺乏风格一致的外立面和屋顶，而直接使用无约束生成模型会破坏已有的建筑足迹、墙体和开口语义等结构约束。

Method: 结合参数化屋顶生成、基于LLM的部件感知提示精炼、联合墙-屋顶材质检索以及几何感知装配四个模块，在格式无关的建筑支架表示上运行，支持多种输入格式。

Result: 在保持结构一致性的同时，比检索方法和无约束生成基线实现了更好的建筑连贯性，并能泛化到不同室内生成器、CityGML和CAD输入。

Conclusion: ShellMaker成功解决了在固定结构约束下补全建筑外壳的挑战，为连接室内生成与室外风格化提供了有效方案。
...
TL;DR: 本文提出HIG（直方图约束图像生成）框架，通过最优传输理论在扩散模型采样过程中精确约束颜色/潜变量直方图分布，实现了处于文本提示与密集条件之间的中等粒度控制。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型控制方法在控制粒度上存在两极分化：文本提示提供全局语义但缺乏局部精度，ControlNet类方法提供精确局部结构但需密集条件。中间粒度的控制机制（如分布级约束）尚缺乏有效方案。

Method: 将用户指定的分布约束（如颜色直方图、潜变量标记分布）建模为最优传输问题，在采样过程中施加显式引导变换，驱动扩散轨迹对齐目标直方图，实现精确的分布级控制。

Result: 在颜色/潜变量直方图约束生成以及直方图级信息嵌入等多种应用中验证了HIG的通用性和有效性。

Conclusion: 分布控制作为一种灵活可解释的控制方案，完全兼容现有控制机制，丰富了可控图像生成的混合策略体系。
...
TL;DR: 本文提出了一种基于单帧点云的双范围-体素表征（DRVR）方法用于3D语义占用预测，通过融合范围视图的上下文信息和体素视图的几何信息，相比多帧方法在nuScenes数据集上实现了5.4%的mIoU提升和2.1倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有基于多帧堆叠的点云语义占用预测方法存在计算负担大和位姿变换噪声等效率与鲁棒性问题，因此需要探索仅利用单帧点云实现高效且鲁棒的3D语义占用预测方法。

Method: 提出DRVR框架，包含三个关键组件：1）范围视图编码器提取场景的紧凑上下文特征；2）几何感知体素视图编码器分别提取多尺度体素视图特征并组合用于几何占用预测；3）范围-体素融合模块通过体素到范围和范围到体素的双向融合机制协同两种视图特征。

Result: 在nuScenes-Occupancy、SemanticKITTI和SemanticPOSS三个数据集上均取得优异性能，其中在nuScenes-Occupancy上，单帧DRVR相比多帧方法mIoU提升5.4%，推理速度加速2.1倍。

Conclusion: DRVR通过有效融合单帧点云的范围视图和体素视图信息，消除了对多帧输入的依赖，在提升语义占用预测精度的同时大幅降低了计算开销，为实际自动驾驶应用提供了更具实用性的方案。
...
TL;DR: 本文提出IS-SNN架构，通过拓扑感知的权重标准化和改进的残差连接消除脉冲神经网络中的批归一化层，在不牺牲精度的情况下大幅降低硬件资源消耗。


<details>
  <summary>Details</summary>
Motivation: 现有深度SNN依赖动态批归一化（BN）变体，但这些BN变体引入运行时乘法操作，削弱了SNN的硬件效率优势。作者试图解决精度与硬件效率之间的矛盾。

Method: 提出IS-SNN架构：1) 识别归一化自由SNN性能退化主因为灾难性发放率衰减；2) 通过拓扑感知的权重标准化和修改的残差连接实现信号稳态，消除激活归一化层；3) 将标准化操作离线折叠到静态权重中，消除运行时统计跟踪和乘法操作，恢复面向累加的推理数据路径。

Result: 在VGG、ResNet和Transformer模型中性能优于动态BN技术；ImageNet上达到68.05%竞争性准确率；FPGA查找表资源消耗减少96.4%。

Conclusion: IS-SNN是构建准确且硬件友好的深度神经形态系统的实用框架，克服了先前无BN方法的深度限制。
...
TL;DR: 本文系统评估了稀疏自编码器（SAEs）在扩散模型中的概念检测与干预能力，发现SAEs虽能可靠检测语义概念，但直接潜空间干预会导致分布外激活和视觉伪影。作者提出基于检测的嵌入替换方法，比潜在空间操控产生更干净的擦除效果，揭示了概念检测与干预之间的根本差距。


<details>
  <summary>Details</summary>
Motivation: 验证SAEs是否可以作为扩散模型中概念层面操作的可控制干预点，特别是在对象擦除和语义引导任务中，弥合概念检测与概念干预之间的差距。

Method: 使用SAE激活作为语义检测器识别包含目标对象的图像区域，然后将这些区域的patch嵌入替换为不含目标对象的patch嵌入，从而保留扩散模型的激活统计特性。

Result: 基于检测的嵌入替换方法相比直接潜在空间引导产生显著更干净的对象擦除结果，有效避免了视觉伪影；SAEs在概念检测方面表现出色，但直接作为控制旋钮进行干预效果不佳。

Conclusion: 单语义或稀疏特征本质上不适合作为扩散模型的控制旋钮；SAEs是强大的可解释性分析工具，但在用于直接操作（如概念擦除/遗忘）时存在重要局限性。
...
TL;DR: Phantom是一种统一的人脸换脸深度伪造防护框架，通过联合约束潜在空间和空间域中的扰动，自适应合成身份偏移但属性保留的目标来引导潜在优化，并在语义相关面部区域施加掩码扰动，有效提升防护成功率（在躲避场景中提升16.6%-27.8%）和视觉质量。


<details>
  <summary>Details</summary>
Motivation: 人脸换脸深度伪造对个人隐私构成严重威胁，现有对抗方法在换脸场景中应用不足：固定或随机目标导致潜在引导模糊，缺乏空间约束使扰动扩散到身份无关区域，身份-风格解耦会抑制对抗信号。

Method: 提出Phantom框架，在潜在空间和空间域联合约束扰动：自适应合成身份偏移但属性保留的目标以引导潜在优化，并在语义相关面部区域施加掩码扰动。

Result: 在躲避场景中，针对UniFace、INSwapper和SimSwap的防护成功率分别提升27.8%、25.6%和16.6%，同时提升视觉质量；在冒充场景中防护成功率提升最多10.2%，感知保真度也得到改善。

Conclusion: 联合利用潜在和空间约束能够有效实现鲁棒且连贯的面部隐私保护，Phantom在防护性能和视觉质量上均优于现有方法。
...
TL;DR: 本文提出了WIDER-FAIR数据集，在WIDER-FACE基础上手动标注了人脸的种族和性别属性（4个种族×2个性别，共16,256张图像），并通过实验表明黑人群体的检测性能显著低于其他群体，突显了人口统计标注数据集在理解和评估人脸检测模型偏差中的价值。


<details>
  <summary>Details</summary>
Motivation: 人脸检测模型在真实场景中可能存在跨人口群体的性能差异，但缺乏带有敏感属性标注的人脸检测数据集是研究和缓解这类偏差的主要障碍。

Method: 在WIDER-FACE基准上构建WIDER-FAIR数据集，手动标注亚洲人、黑人、印度人和白人四个种族以及男性和女性两个性别类别；使用人脸嵌入、K近邻分类器和t-SNE可视化评估标注质量；基于YOLOv5进行消融实验分析各敏感特征对检测性能的影响。

Result: 黑人群体的检测性能显著低于其他群体；从训练中排除黑人群体会比排除其他任何种族群体导致更大的公平性差异。

Conclusion: 人口统计标注数据集对于理解、评估和缓解人脸检测模型中的偏差具有重要价值。
...
TL;DR: 本文提出UniCoder，一种统一强化学习框架，通过符号属性对齐和参考引导代码优化两种机制，解决视觉到代码生成中像素级对齐的难题，8B参数模型在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 标准多模态大语言模型通过监督微调无法实现视觉到代码生成所需的像素级精确对齐，而强化学习面临奖励粗糙（语义指标无法惩罚细粒度偏差）和探索停滞（稀疏异构的代码搜索空间阻碍有效轨迹引导）两大障碍。

Method: 提出UniCoder框架，包含两个核心机制：(1)符号属性对齐，利用轻量级辅助LLM将生成代码解析为离散视觉属性（如十六进制颜色、坐标范围），实现密集的逐元素奖励计算；(2)参考引导代码优化，动态将真实轨迹注入低性能rollout组，将盲探索转化为引导式策略改进。

Result: 在ChartMimic、UniSVG、Design2Code和ScreenBench基准上，8B参数模型超越所有开源基线，达到与专有模型相当的SOTA性能。

Conclusion: UniCoder建立了通用视觉到代码合成的新范式，证明通过精心设计的RL机制可以有效弥合视觉与代码之间的精度鸿沟。
...
TL;DR: 提出MemLearner，一种基于学习的自适应上下文查询方法，通过查询令牌连接上下文与预测令牌，解决视频世界模型因缺乏记忆导致的长时间场景不一致问题，尤其擅长处理场景遮挡和动态物体场景。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在长时间生成中缺乏记忆机制，导致生成场景不一致。现有基于规则的上下文帧检索方法在场景遮挡和动态物体存在时泛化能力差。

Method: 提出MemLearner，利用视频生成模型自身进行上下文查询（无需额外模块训练），通过查询令牌桥接上下文与预测令牌；收集含场景遮挡和动态物体的长视频数据集并标注相机位姿；提出多数据集训练策略，同时利用标注的渲染视频和未标注的真实世界视频。

Result: 在场景一致性和记忆保持方面显著优于先前的视频世界模型，尤其在遮挡和动态场景下表现突出。

Conclusion: MemLearner通过自适应上下文查询有效解决了视频世界模型的长期记忆问题，为交互式视频生成提供了更一致的场景表现。
...
TL;DR: 本文提出了JL1-CC&QA多任务基准数据集，在JL1-CD数据集基础上扩展了变化描述（CC）和变化问答（QA）两个注释层，包含5,000对0.5-0.75m高分辨率吉林一号卫星影像，提供17,021条质量验证的描述和20,060个问答对，通过多模态大模型生成、视觉大模型评判和人工专家验证的三阶段流水线完成标注，旨在推动遥感变化理解从像素级分割向语义级理解发展。


<details>
  <summary>Details</summary>
Motivation: 传统遥感变化检测仅关注像素级二值分割，能识别变化位置但无法描述变化内容及其原因，存在显著的语义鸿沟。为弥补这一不足，需要构建同时包含变化掩码、变化描述和变化问答的多任务基准数据集。

Method: 在JL1-CD数据集（5,000对吉林一号卫星0.5-0.75m分辨率双时序影像）基础上，构建两个互补注释层：JL1-CC（17,021条质量验证的变化描述）和JL1-QA（20,060个覆盖8种问题类型的问答对）。采用三阶段标注流水线：多模态大语言模型生成、基于视觉的大语言模型评判、人工专家验证。

Result: 成功构建了统一的遥感变化理解多任务基准数据集，包含二值变化掩码、变化描述和变化问答三类标注，覆盖多种土地覆盖变化类型和8种问答类型。

Conclusion: JL1-CC&QA作为首个在同一影像集上统一二值变化掩码、变化描述和变化问答的基准数据集，将为遥感领域多任务变化理解研究提供有价值的资源，推动从"变化在哪里"到"变化是什么及为什么"的语义级理解发展。
...
TL;DR: 本文提出RhythmJEPA，一种面向远程光电容积描记术(rPPG)的节奏结构联合嵌入预测学习框架。不同于重建RGB帧，该方法在潜在空间中预测掩码面部视频的教师表征，并通过循环节奏状态规划器(CRSP)建模脉冲信号周期结构，配合双顺序Mamba编码器(DOM)和空间脉搏混合器(SPM)，在PURE、UBFC-rPPG和MMPD数据集上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有自监督rPPG方法主要重构掩码像素或低级视觉表征，导致模型偏向面部外观而非潜在生理动态；同时，大多数基于Mamba的方法仅按时间顺序扫描面部视频令牌，未能充分利用脉冲信号的循环结构特性。

Method: 1) 联合嵌入预测学习：预测掩码面部视频的潜在教师表征而非重构RGB帧，促进生理感知表征学习；2) 循环节奏状态规划器(CRSP)：估计逐帧潜在生理状态，通过带约束转移语法的动态规划解码最可能的循环状态路径；3) 双顺序Mamba编码器(DOM)：结合常规时间顺序扫描与状态顺序扫描，捕获局部时间连续性和长程节奏一致性依赖；4) 空间脉搏混合器(SPM)：提取紧凑的脉搏敏感面部令牌，平衡复杂度与性能。

Result: 在PURE、UBFC-rPPG和MMPD三个数据集上，RhythmJEPA的性能优于多种代表性rPPG方法。

Conclusion: 通过引入节奏结构化的潜在空间预测学习和循环状态建模，有效解决了现有rPPG方法中视觉表征偏差和脉冲周期结构利用不足的问题，展示了联合嵌入预测范式在生理信号估计中的潜力。
...
### [117] [Breaking Failure Cascades: Step-Aware Reinforcement Learning for Medical Multimodal Reasoning](https://arxiv.org/abs/2606.31825)
*Junha Jung,Minbyul Jeong,Suhyeon Lim,Sungwook Jung,Jaehoon Yun,Taeyun Roh,Mujeen Sung,Jaewoo Kang*

Main category: cs.CV

TL;DR: 本文提出MRPO，一种基于过程奖励的强化学习算法，通过给早期错误推理步骤分配指数级更大惩罚来打破医疗VQA中的级联故障链，显著提升推理质量与最终答案准确率。


<details>
  <summary>Details</summary>
Motivation: 现有后训练管道以最终答案正确性为中心，存在稀疏奖励分配问题，难以优化临床应用中至关重要的推理过程；早期推理失败导致的级联错误是医疗VQA中预测错误的主要原因。

Method: 提出Medical Reasoning-aware Policy Optimization (MRPO)，一种引入逐步过程奖励的RL算法。当最终答案错误时，MRPO对早期无效推理步骤的token分配指数级更大的惩罚，从而打破故障级联而不影响成功的推理路径。

Result: 在三个多模态LLM骨干上，MRPO一致优于标准GRPO和近期RL基线；在Qwen3-VL-8B-Instruct上甚至超越更大的医学MLLM（HuatuoGPT-Vision-34B）2.79个百分点；早期推理失败从64.0%降至13.0%。

Conclusion: 针对性缓解级联故障既能提升推理质量又能提高最终答案准确率，证明了过程奖励在医疗多模态推理中的有效性。
...
TL;DR: 本文利用卷帘快门相机对高速运动球体产生的畸变作为时序信息，通过无对应点的几何反投影方法，从单帧图像中解耦并估计球体的三维平移和旋转角速度。


<details>
  <summary>Details</summary>
Motivation: 传统上卷帘快门畸变被视为需要校正的伪影，本文反其道而行之，将其作为宝贵的时间信息源，用于估计快速运动物体的运动参数。

Method: 设计鲁棒且易检测的球面图案，提出无对应点的公式，通过反投影框架中的几何一致性约束恢复运动；利用球体几何特性将平移与旋转运动解耦，通过两阶段优化分别估计。

Result: 在合成和真实数据集上的大量实验表明，该方法能够在挑战性的高速条件下准确且鲁棒地估计运动参数。

Conclusion: 卷帘快门畸变可以被有效利用来估计高速运动物体的三维速度，尤其适用于无纹理的球体对象。
...
TL;DR: 针对自动驾驶、监控和机器人等现实场景中的域偏移问题，本文提出RT-SFOD方法，基于无NMS的双头检测器YOLOv10，通过双头伪标签融合（DHF）和多尺度自适应表示多样化损失（MARD），在不增加推理开销的前提下，实现了更优的域自适应检测性能，同时速度更快、模型更小。


<details>
  <summary>Details</summary>
Motivation: 现有源自由目标检测（SFOD）方法依赖重型架构，仅关注精度而忽略延迟和内存约束，无法满足现实场景需求。作者旨在证明精度与效率之间并非不可兼得，需在轻量级检测器上实现高效域自适应。

Method: (1) 基于YOLOv10（无NMS双头检测器）构建；(2) 提出双头伪标签融合（DHF），选择性融合O2O和O2M头的预测结果，兼顾精度与召回；(3) 提出多尺度自适应表示多样化损失（MARD），通过对多尺度特征图施加检测感知的方差和协方差约束，缓解域偏移导致的多尺度特征判别性退化。两个模块仅在训练时引入，推理阶段无额外开销。

Result: 在多个域偏移基准上，相比现有最优SFOD方法，mAP提升1.4%~3.5%，吞吐量提升1.3倍，参数量减少约2倍。主实验基于YOLOv10，并在其他YOLO系列和DETR系列双头检测器上验证了泛化性。

Conclusion: RT-SFOD在精度、速度和模型大小之间实现了更优的帕累托前沿，验证了轻量级双头检测器结合精心设计的自训练策略可以同时兼顾域自适应效果与实时性要求。
...
TL;DR: 提出Barycentric Dominance Field (BDF)连续表示，将网格拓扑连接性编码为表面连续信号，使扩散模型能够生成高质量、高可扩展性的原生3D网格，克服自回归范式在面片数、顶点分辨率和纹理支持上的局限。


<details>
  <summary>Details</summary>
Motivation: 自回归(AR)模型在原生3D网格生成中受限于其内在约束，导致生成的网格面片数有限、顶点分辨率受限、难以支持纹理。为克服这些瓶颈，需要一种能够连接离散网格拓扑与连续扩散生成建模的表示方法。

Method: 提出Barycentric Dominance Field (BDF)，一种定义在三角网格表面上的连续表示，通过将拓扑连接性转化为连续表面信号，无缝融入现有3D扩散流水线，无需修改架构。

Result: 实验证明BDF使扩散模型生成的原生网格在质量、可扩展性和鲁棒性上显著优于最先进的自回归方法。

Conclusion: BDF作为网格的内在属性，与纹理图高度相似，有效弥合了离散网格拓扑与连续扩散模型之间的根本鸿沟，为高质量3D网格生成提供了新范式。
...
TL;DR: 本文提出一种操作符级别的视觉tokens跳过框架，通过选择性绕过冗余的注意力或FFN算子来加速多模态大语言模型，在Qwen3-VL上减少33.7% TFLOPs的同时保留99.5%的性能。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM加速方法通常粗粒度地移除整个视觉token或跳过整层更新，这会丢弃细粒度证据或同时抑制有用与冗余的算子。本文从答案可观察角度发现晚期视觉token更新对答案表示影响很小，因此希望设计更细粒度的加速策略。

Method: 将每个Transformer层分解为注意力（Attention）和FFN算子，基于"答案静默冗余"现象，提出操作符级别的视觉token跳过框架，选择性绕过冗余的注意力、FFN或两者，同时保留完整视觉token序列。

Result: 在三种MLLM架构和10个VQA基准上验证，实现了强效率-精度权衡，在Qwen3-VL上减少33.7% TFLOPs，同时保留99.5%的原始模型性能。

Conclusion: 操作符级别的视觉计算跳过比粗粒度方法更有效，有用视觉计算具有算子主导性和层依赖性，能够在不显著损失性能的情况下大幅降低计算开销。
...
### [122] [LUNA: Learning Universal 3D Human Animation Beyond Skinning](https://arxiv.org/abs/2606.31981)
*Peng Li,Rawal Khirodkar,Junxuan Li,Yuan Dong,Chen Cao,Yuan Liu,Wenhan Luo,Yike Guo,Shunsuke Saito*

Main category: cs.CV

TL;DR: LUNA是一种无需线性混合蒙皮（LBS）的通用神经动画模型，通过Transformer运动回归器和混合监督策略，直接从多模态2D控制（图像、关键点、草图等）生成3D高斯形变，实现逼真、可动画化的人体虚拟化身，并支持零样本跨身份泛化。


<details>
  <summary>Details</summary>
Motivation: 现有基于LBS和参数化人体模型的方法受限于表达能力和拟合伪影，需要一种跳过显式人体拟合、直接从2D控制生成3D形变的无LBS动画模型。

Method: 1) 基于Transformer的运动回归器，解耦全局刚体运动与局部非刚体动力学；2) 混合监督策略：从LBS教师模型蒸馏软结构先验，结合有限拟合数据和大规模无标注视频进行训练；3) 将图像、关键点、草图等多模态2D控制直接映射为3D高斯形变。

Result: 在视觉保真度上与LBS方法相当，能生成逼真的人体运动，并在多种驱动模态下实现零样本跨身份泛化。

Conclusion: LUNA是首个支持隐式2D驱动的端到端3D可动画化模型，无需LBS即可实现高质量的虚拟化身动画生成。
...
TL;DR: 提出自监督时间正则化方法，作为后训练微调阶段，利用图像序列的时间连贯性增强心脏超声分割和运动估计的时间一致性，并自动映射到AHA 17节段临床标准。


<details>
  <summary>Details</summary>
Motivation: 基于图的 cardiac 分割模型在独立帧上训练导致时间不连续性，影响可靠的临床测量，尤其在心脏超声中。

Method: 引入自监督时间正则化作为后训练微调阶段，通过惩罚连续帧间的速度和加速度不连续性，在保持解剖对应关系的同时实现时间一致的分割，并利用解剖对应关系自动映射到AHA 17节段标准。

Result: 在CAMUS数据集上验证了结合时间一致性和自动区域映射的临床效用。

Conclusion: 自监督时间正则化能有效提升心脏超声分割的时间一致性，并支持标准化区域评估和病理运动模式检测。
...
### [124] [FLORA: A deep learning approach to predict forest attributes from heterogeneous LiDAR data](https://arxiv.org/abs/2606.32023)
*Emilie Vautier,Clément Mallet,Cédric Vega*

Main category: cs.CV

TL;DR: 本文提出FLORA（Forest LiDAR Octree Regression with Auxiliary Data）深度学习框架，利用八叉树骨干网络和辅助变量（生态、时空）的后期融合门控机制，从异构LiDAR点云数据中预测森林的六个关键属性（优势高、总蓄积量、落叶蓄积量、针叶蓄积量、胸高断面积和林分密度）。基于法国32,052个国家森林清查样地数据训练，单一模型在落叶期和针叶期数据上表现优于季节专用模型，优势高rRMSE约12.3%（R²=0.88），总蓄积量rRMSE约39%（R²=0.74），为大规模异构LiDAR数据的森林属性估算提供了鲁棒基线。


<details>
  <summary>Details</summary>
Motivation: 随着欧洲各国LiDAR项目的扩展，传感器、飞行参数、季节和扫描角度的异质性导致传统局部校准模型鲁棒性不足，难以实现覆盖全国的墙到墙森林属性预测。现有方法无法有效处理跨季节、异构LiDAR数据的泛化问题。

Method: FLORA框架采用八叉树（Octree）骨干网络处理非结构化LiDAR点云，并通过后期融合门控机制引入生态与时空辅助变量。模型在法国LiDAR HD计划的32,052个NFI样地上进行训练与评估，训练时同时使用落叶期和针叶期数据。

Result: 单一模型在落叶期和针叶期数据上优于季节专用模型，提升跨季节鲁棒性。辅助变量在整体上贡献有限，但对树种特异性蓄积量预测提升更显著。优势高rRMSE≈12.3%（R²=0.88），总蓄积量rRMSE≈39%（R²=0.74）。

Conclusion: FLORA为异构国家LiDAR项目下的大规模森林属性估算提供了鲁棒的基线方法，验证了深度学习和辅助变量融合在应对数据异质性方面的有效性。
...
### [125] [MuSViT: A Foundation Vision Model for Sheet Music Representation](https://arxiv.org/abs/2606.31811)
...
TL;DR: MuSViT是首个用于乐谱表示的基础视觉模型，采用ViT编码器结合掩码自编码器预训练，在970万页IMSLP数据上通过两阶段课程学习训练，在下游任务中优于通用视觉编码器，并能在表示空间中编码音乐符号结构。


<details>
  <summary>Details</summary>
Motivation: 基础模型在视觉和语言处理中展现了强大的可迁移表征能力，但乐谱作为一种视觉编码的音乐语言，缺乏专门的领域基础模型。

Method: 采用ViT编码器结合掩码自编码器（MAE）预训练方法，使用两阶段课程学习：先在合成排版乐谱上预训练，再在包含970万页的完整IMSLP语料库上进行大规模训练。

Result: 在线性探测（冻结编码器）场景下，MuSViT持续优于通用视觉编码器；在微调场景下，总体上超越现有任务特定最优方法。嵌入-转录一致性分析表明MuSViT能直接在表示空间中编码音乐符号结构。

Conclusion: MuSViT可作为乐谱理解的基础骨干网络，证明了通用视觉表征在乐谱这一结构化符号领域存在系统性不足，而领域专门的基础模型能够弥补这一差距。
...
TL;DR: 提出TopoGPT生成式框架，通过自回归序列建模学习车道图结构先验，解决现有检测关联范式导致的几何不一致和遮挡不完整问题，在OpenLane-V2上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有车道拓扑推理方法采用检测+关联范式，独立处理每个车道实例，导致连接端点处几何不一致，且因视觉遮挡产生不完整车道图。

Method: 1) 构建330万场景的大规模地图数据集；2) 车道分词器将车道图序列化为离散token；3) 场景上下文编码器将车道图转为栅格化图像并提取全局场景token；4) 自回归车道序列Transformer通过场景条件化的下一token预测进行预训练；5) 感知适配器将BEV特征与预训练场景条件对齐，迁移先验知识到传感器推理。

Result: 在OpenLane-V2基准上，车道级指标平均提升+6.4，点级指标平均提升+11.6，生成几何一致且结构完整的车道图。

Conclusion: 通过自回归序列建模学习车道图几何先验，能有效克服传统范式在一致性和完整性上的局限，显著提升车道拓扑推理性能。
...
TL;DR: 提出ADER框架，利用长波红外高光谱数据中的大气吸收特征实现被动测距，通过B样条平滑发射率建模、臭氧吸收分类、贪婪波段选择等策略，在保持测距精度的同时实现约两个数量级的加速。


<details>
  <summary>Details</summary>
Motivation: LWIR高光谱观测包含距离相关的大气吸收特征，为远距离被动测距提供了物理基础，但目标温度、材料发射率、路径辐射与大气吸收特征非线性耦合，导致距离反演病态。现有全波段逐像素联合优化方法计算昂贵，且未显式利用尖锐的大气吸收结构。

Method: (1) 用B样条控制点表示发射率，施加平滑先验，抑制对大气吸收结构的过拟合，实现距离解耦估计；(2) 利用臭氧吸收特征将像素分为发射主导和反射主导两类；(3) 发射主导像素：补偿路径辐射和透过率，通过一维吸收残差最小化估计距离；(4) 反射主导像素：基于完整辐射模型的下行辐射补偿来细化初始估计；(5) 提出基于多场景有效Fisher信息的贪婪波段选择策略，减少光谱冗余。

Result: 在真实场景中，ADER在全波段和20波段设置下均能恢复与LiDAR一致的空间距离结构，提高了目标区域测距精度，相比公开的全波段高光谱测距方法实现了约两个数量级的加速。

Conclusion: ADER通过显式利用大气吸收结构、解耦距离估计、分类处理像素以及贪婪波段选择，有效解决了LWIR被动测距的病态性问题，在保持精度的同时大幅提升计算效率。
...
TL;DR: 本文提出地理空间视觉先验（geospatial visual priors）和双记忆架构的记忆增强模块，为端到端自动驾驶提供与实时传感器无关的视觉空间预见能力，在NAVSIM-v2基准上持续提升性能并增强对传感器损坏的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有端到端自动驾驶方法仅依赖瞬时传感器观测，缺乏人类驾驶员通过先验经验获得的预期性预见能力（anticipatory foresight），导致行为具有反应式局限性。

Method: 提出了地理空间视觉先验（street-level visual context anchored to intended driving route），以及一个包含双记忆架构（上下文记忆+持久化回退记忆）和自适应记忆门的记忆增强模块，可轻松集成到现有端到端方法中。

Result: 在NAVSIM-v2基准上，该方法持续提升了多种端到端基线的性能；由于先验独立于车载传感器，方法天然增强了对传感器损坏的鲁棒性；双记忆设计确保了在先验不可靠时的安全回退。

Conclusion: 地理空间视觉先验结合双记忆架构能有效弥补现有端到端自动驾驶方法缺乏先验知识的缺陷，同时提升性能与鲁棒性。
...
TL;DR: Consispace是一种语义感知的重采样框架，通过ODE约束保持层间解剖连续性和预训练视觉模型驱动的层内语义一致性，在下游分割任务中优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 现有体素医学图像分割研究主要关注架构设计，而忽视了预处理中体素间距重采样与分割性能之间的系统关联，未充分利用重采样与分割任务的协同效应。

Method: 1) ODE-based解剖约束：通过连续插值器建模层间动态，实现复杂解剖过渡下的忠实重建；2) 语义特征重加权：利用预训练视觉模型的密集特征构建层内语义相关图，注入类别级一致性；3) 隐式神经网络统一框架：整合层内和层间约束，支持任意尺度重采样。

Result: 在多个数据集上，Consispace在重建质量、感知保真度、层间解剖平滑性方面均优于对比方法，作为预处理步骤能有效提升下游分割性能。

Conclusion: 重采样环节与分割任务存在深度耦合关系，通过引入解剖和语义先验的语义感知重采样可以显著提升分割效果，为医学图像预处理提供了新范式。
...
TL;DR: 本文提出SENSE-VAD，首个专门面向自动驾驶中社交复杂异常场景的合成视频异常检测基准。该基准利用CARLA和Unreal Engine生成包含个体行为、群体行为、人与物体交互等类别的异常场景，关键设计原则是将社交异常与基于运动或外观的异常相分离。实验表明，现有最先进的视频异常检测方法无法有效解决社交复杂异常这一独特挑战。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法主要关注运动统计或外观特征的异常，无法识别那些单独看来运动正常但在社交关系上下文中构成危险的情境（如儿童逃离监护人、追跑打闹等），因此需要一个专门针对社交复杂异常的基准来推动该方向研究。

Method: 使用CARLA模拟器和Unreal Engine生成合成视频数据；设计多个社交异常类别（个体行为、群体行为、人-物交互、骑行交互、车辆与代理）；采用逐帧二值标注；在数据生成中刻意分离社交异常与运动/外观异常；同时提供真实世界的正常/异常视频用于sim-to-real迁移评估。

Result: 对现有最先进的视频异常检测基线方法进行评估，结果表明社交复杂异常构成了一个当前尚未被解决的独特挑战，现有方法在该基准上表现不佳。

Conclusion: 社交复杂异常是自动驾驶安全中一个被忽视但关键的问题，需要开发专门针对关系上下文而非运动统计的异常检测方法。数据集和代码已开源。
...
TL;DR: 本文提出RESOLVE，首个大规模真实世界路边多分辨率LiDAR-相机融合基准数据集，用于系统评估单模态与融合架构在3D检测与跟踪中的性能，揭示多模态融合如何补偿LiDAR点稀疏性问题。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对路边协同感知中，LiDAR点云稀疏性（由传感器配置和场景依赖环境导致）对单模态和相机-LiDAR融合架构性能影响的系统性评估。

Method: 构建RESOLVE数据集：包含10万+图像、2.6万点云帧、22万人工标注边界框，涵盖10类交通参与者，在真实城市路口多种光照和天气条件下采集，支持三种LiDAR分辨率级别的受控对比实验。

Result: 通过大量基准实验揭示了多模态融合如何补偿LiDAR点稀疏性，以及分辨率变化、感知距离、训练-推理分辨率不匹配等因素对架构性能的影响。

Conclusion: RESOLVE为设计成本高效的路边多模态感知提供了重要线索，数据集和代码已开源。
...
TL;DR: DriveWeaver提出了一种基于视频修补的框架，通过车辆点云条件在自动驾驶仿真中插入可控的前景车辆，解决光照不一致和3D资产依赖问题，并实现实时渲染与大规模场景增强。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶仿真中插入前景车辆的方法依赖预重建的3D资产，导致插入的前景与背景之间存在光照不一致，且有限的、人工筛选的3D资产阻碍了大规模部署。

Method: 对于掩码目标插入区域，DriveWeaver利用车辆点云条件进行视频修补，生成时间一致的高质量车辆；设计全局到局部的分层修补策略以支持长时生成并保持车辆身份和外观一致性；通过城市重建管线提取显式3D高斯表示以实现实时渲染。

Result: 在多个数据集上的实验表明，该方法在视觉真实感和几何一致性上优于现有基线方法，为可扩展的自动驾驶场景增强提供了鲁棒工具。

Conclusion: DriveWeaver通过视频修补结合点云条件的方式，有效解决了前景插入中的光照不一致和泛化问题，为自动驾驶仿真中的大规模场景增强提供了可行方案。
...
TL;DR: 提出InstanceControl方法，利用视觉-语言模型自动建立文本描述与视觉条件之间的实例级对应关系，并通过自适应掩码精炼策略实现无需实例标注的多实例可控图像生成。


<details>
  <summary>Details</summary>
Motivation: 现有可控图像生成方法（如ControlNet）在处理多实例场景时存在实例属性混淆问题，而近期方法需要人工实例标注，劳动成本高。

Method: 利用视觉-语言模型（VLM）自动解析文本提示中的实例描述并同时预测视觉条件中的实例掩码，再通过自适应掩码精炼策略在生成过程中动态优化实例掩码。

Result: 在多项实验上优于现有最先进方法，实现了更高的生成保真度和精确的实例级控制。

Conclusion: InstanceControl无需实例标注即可有效解决多实例可控生成中的属性混淆问题，为复杂场景下的可控图像生成提供了可行方案。
...
TL;DR: 提出VidPair-Halluc基准，通过背景相似但前景语义不同的视频对，系统评估大视频模型的幻觉问题。包含PairFlow流水线、1K对抗视频对和11K时空QA对，揭示现有模型在对抗设置下鲁棒细粒度理解方面的持续困难。


<details>
  <summary>Details</summary>
Motivation: 现有视频幻觉基准多依赖文本扰动或对抗性问题，忽略了视觉背景一致性，导致无法精确归因模型错误是源于幻觉还是背景变化。

Method: 通过PairFlow流水线，利用文本到图像和视频生成技术，系统性地编故事、生成连贯视频片段并组装成对抗性视频对。视频对背景高度相似但前景语义不同，覆盖10个语义方面（空间和时间推理）。

Result: 构建了1K高质量对抗视频对和11K时空QA对，主流LVM评估显示在对抗设置下仍难以实现鲁棒的细粒度视频理解。

Conclusion: VidPair-Halluc为视频幻觉评估提供了更严格且可控的条件，有助于更准确地归因和诊断大视频模型的感知缺陷。
...
TL;DR: 本文提出世界叙事模型（WNM），将视频生成解耦为结构化物理叙事（4D预可视化）和像素渲染两个阶段，通过协作智能体将多模态输入转化为可编辑的量化世界表征，驱动基础视频模型生成可控视频，显著减少随机采样并提升创作者对几何、运动、相机和光照的确定性控制。


<details>
  <summary>Details</summary>
Motivation: 工业级视频生成的核心障碍在于缺乏可控性：现有模型将视频视为像素分布采样问题，绕过了显式的实例级4D（3D+T）物理世界，导致内容创作者无法确定性、定量地指定几何、运动、相机参数和光照，陷入低效且昂贵的"抽卡"循环。

Method: WNM采用"渲染什么"（结构化物理叙事）与"如何渲染"（像素生成过程）的解耦范式。协作智能体将稀疏多模态输入（文本、参考视频、草图）转化为完全可编辑的世界表征，包含场景几何、物体布局、角色骨架运动、轨迹、相机运动和光照的定量化粒度。该表征作为确定性蓝图驱动冻结或轻量适配的视频基础模型，将其转化为忠实神经着色器。平台支持自动世界生成与预可视化，并提供导演控制台进行人工精修。

Result: 实验表明WNM大幅减少了概率性"抽卡"调用，生成的视频在布局、运动和摄影方面紧密遵循创作者意图。

Conclusion: WNM框架是开放模块化的，允许世界表征、控制智能体和适配器等各组件独立改进，为工业级可控视频生成提供了新范式。
...
TL;DR: AnyBokeh提出了一种物理引导的任意到任意散景编辑框架，通过估计有符号弥散圆图和视差图，利用线性关系建模源光学指纹，实现空间自适应的去模糊、保持和散景渲染，无需全聚焦重建或测试时校准。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常假设输入为全聚焦图像，或先恢复全聚焦图像再渲染新散景，这种方式丢弃了源图像中的有用模糊线索，并将重建伪影传播到最终编辑结果中。因此需要一种能够直接处理任意对焦和光圈设置下捕获图像的散景编辑方法。

Method: 1) 估计源图像的signed CoC图和视差图；2) 通过signed CoC与视差差异的线性关系建模源特定光学指纹；3) 将源光学特性转移到目标对焦和光圈设置；4) 使用以源和目标CoC图为条件的生成编辑器进行相对模糊合成，实现空间自适应的去模糊、保持和散景渲染；5) 构建高保真合成数据集，包含精确深度、对焦距离和完整EXIF元数据。

Result: 在真实世界基准测试中，AnyBokeh在任意到任意散景编辑、全聚焦到散景渲染和散焦去模糊任务上均实现了忠实且可控的编辑效果，避免了现有方法常见的全聚焦重建和测试时散景级别校准需求。

Conclusion: AnyBokeh通过物理引导的框架有效解决了任意到任意散景编辑问题，证明了利用源图像模糊线索而非将其作为退化去除的优越性，为散景编辑提供了新的范式。
...
TL;DR: 本文提出一种统一的框架，将平面场景从传统SfM的退化条件转变为几何约束来源。通过多视图平面表面的单应性分解估计相机姿态，并采用基于图的方法（谱嵌入+最大一致生成树）过滤不可靠边，在高度平面场景（如室内体育馆）和一般3D场景中均表现优异。


<details>
  <summary>Details</summary>
Motivation: 传统SfM方法在平面场景中因基于对极几何的方法退化而表现不佳。本文旨在将平面表面从限制条件转变为有利的几何约束来源，解决高度平面场景下的相机姿态估计难题。

Method: 1) 通过多视图可见的每个平面表面进行单应性分解，独立估计相对相机姿态；2) 构建姿态图并采用谱嵌入方法，将单应性估计的姿态映射到实轴上，基于几何和视觉一致性识别并过滤不可靠边；3) 提取最大一致生成树用于姿态恢复，支持单平面或多平面聚合。

Result: 在篮球场等高度平面场景中优于现有方法；在IMC Phototourism基准的无约束户外场景中达到或超越最先进水平。

Conclusion: 本文提出的统一框架能够自然处理高度平面场景和一般3D环境，证明了将平面作为几何约束源而非限制的有效性。
...
TL;DR: 提出ERA框架，通过熵引导的视觉令牌剪枝和修正注意力机制，解决多模态大语言模型中视觉令牌减少导致的注意力对数坍塌问题，实现高效推理。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型因长视觉序列导致推理成本高昂，现有无训练视觉令牌缩减方法会扭曲注意力分布，引发注意力对数坍塌问题。

Method: ERA框架包含三个组件：1) 双视图熵剪枝(DEP)，联合建模视觉多样性和头级显著性以识别代表性锚点令牌；2) 偏置感知令牌回收(BTR)，将剪枝令牌回收至对应锚点并估计聚类级对数偏置；3) 对数保持注意力修正(LAR)，将估计偏置注入注意力对数以修正坍塌。

Result: 在单图、多图和视频设置下，ERA在多种MLLM上均实现了鲁棒性能，在激进压缩下仍保留视觉证据，并提供了实际加速效果。

Conclusion: ERA建立了对数保持的视觉令牌剪枝作为高效MLLM的通用框架，统一了理论基础、算法设计和实际部署。
...
TL;DR: 本文提出CoLT框架，让多模态大模型通过少量（3步）潜在思维表征进行推理，而非冗长的文本思维链。通过外部解码器提供前向和反向步骤级监督以及内部监督来调控潜在推理过程，推理时移除解码器以保持高效。在8个基准上，CoLT在性能上超越现有潜在推理方法，推理时间相比文本CoT减少10.1倍，文本解码时间减少22.6倍。


<details>
  <summary>Details</summary>
Motivation: 文本思维链推理生成大量中间推理token，推理速度慢且受限于自然语言的表达能力。现有潜在推理方法要么产生无意义语义，要么训练不稳定，亟需一种高效且可调控的潜在推理框架。

Method: 提出CoLT框架：1）让多模态模型以少量（3步）潜在思维表征进行推理；2）引入轻量级外部解码器，提供双向步骤级监督——前向模式将潜在思维解码为下一步文本推理，反向模式将解码器隐藏状态与模型潜在思维对齐；3）加入内部监督鼓励连贯的逐步潜在状态转换；4）推理时移除解码器和内部监督以保持高效。

Result: 在8个基准上，CoLT超越CODI、SIM-CoT等现有潜在推理方法，以及依赖辅助图像的潜在视觉推理方法。相比文本CoT，推理时间减少10.1倍，文本解码时间减少22.6倍。

Conclusion: CoLT通过潜在思维表征实现了高效的多模态推理，在保持高性能的同时大幅降低推理延迟，为突破文本CoT的效率和表达瓶颈提供了新方向。
...
TL;DR: AutoBackSwap通过自动交换图像的前景与背景来减少深度神经网络对虚假背景特征的依赖，仅需少量补丁标注即可训练辅助网络，在无偏训练样本的情况下仍能有效提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度神经网络分类器容易依赖训练数据中与目标标签相关但不具备因果联系的虚假相关特征（如图像背景），导致模型在测试时泛化失败。现有方法通常需要训练数据中包含打破虚假相关性的样本，而实际场景中往往难以获取这类样本。

Method: AutoBackSwap包含三步：1) 使用辅助网络将图像前景与背景分离（仅需少量补丁标注训练）；2) 通过图像修复技术合成完整背景；3) 将不同前景与修复后的背景组合，生成增强训练数据。

Result: 在多个存在虚假背景的图像分类任务中，AutoBackSwap一致优于现有方法。即使在训练数据中没有任何打破虚假相关性的样本时，该方法仍然非常有效。

Conclusion: AutoBackSwap通过自动化前景-背景交换的数据增强策略，有效降低了分类器对虚假背景的依赖，且仅需极少量的标注代价，是一种实用且鲁棒的解决方案。
...
TL;DR: 本文提出Cross-Space Distillation范式，通过轻量级Bridge模块（冻结的Student VAE解码器+可学习投影器）解决教师与学生模型潜在空间不匹配问题，实现异构大模型向紧凑模型的知识蒸馏。


<details>
  <summary>Details</summary>
Motivation: 现有的单步扩散模型蒸馏依赖"共享潜在空间"的假设，限制了从高容量教师模型（如SD 3.5、Flux）向部署友好的紧凑学生模型（如SD 1.5）的知识迁移，因为它们在潜在分辨率和VAE参数化上存在差异。

Method: 提出Cross-Space Distillation框架，引入Bridge模块：冻结学生VAE解码器作为空间先验，结合轻量级可学习投影器，通过潜在重建损失和注意力保真度损失进行训练，实现学生潜在空间到教师空间的稳定对齐。

Result: 在多个现代教师模型上，Bridge使紧凑单步学生模型获得显著提升。例如，SD 1.5的HPSv3从5.4提升至9.4，同时保持单步推理、低延迟和广泛的生态兼容性。

Conclusion: 通过轻量级潜在空间接口，异构大教师模型可以被成功蒸馏到高效、可部署的学生骨干网络中，突破了传统蒸馏对潜在空间一致性的要求。
...
TL;DR: 提出了一种无需训练、无需优化的零样本框架SpheRoPE，通过将球形先验注入预训练的扩散Transformer，实现360度全景图像和视频的生成。该方法用球形旋转位置编码（Spherical RoPE）替代标准RoPE，并配合语义失真无分类器引导（CFG），在无需重新训练的情况下实现了与现有方法竞争的性能。


<details>
  <summary>Details</summary>
Motivation: 现有360度全景生成方法要么依赖稀缺全景数据上的昂贵微调（限制泛化性），要么依赖多步优化（推理延迟高）。作者发现当代生成模型虽从大规模训练中天然具备一些全景先验，但无法满足等距柱状投影（ERP）的严格拓扑约束，因此需要一种零样本、免优化的方法来在推理时解决这些约束。

Method: 1) Spherical RoPE：将低频通道重新参数化为3D笛卡尔坐标以原生编码球面流形，高频通道进行谐波量化以强制执行精确周期性。2) 语义失真无分类器引导（CFG）：显式引导几何结构，避免重新训练。

Result: 在Flux.1、Flux.2和LTX-Video等多个骨干网络上实现了文本到全景图像和视频的生成，与基线方法相比取得了竞争性能，且完全无需训练。

Conclusion: 提出的零样本、免优化方法能够有效将球形先验注入预训练扩散Transformer，在保持模型创造性宽度的同时解决了ERP的拓扑约束，且具有良好的跨骨干网络和跨模态泛化能力。
...
TL;DR: PointSplat提出了一种基于点云的高斯原语直接推断方法，通过在3D空间进行预测来消除多视角间的冗余，实现高效、紧凑的3D人体表示，在减少高斯点数量的同时提升新视角渲染质量。


<details>
  <summary>Details</summary>
Motivation: 现有前馈式3D人体重建方法在多个视角下重复编码相同的主体内容，导致显著的视角间冗余，且在高保真度与紧凑性之间存在矛盾。本文旨在通过直接在3D空间进行预测来消除冗余，实现高效紧凑的3D人体表示。

Method: 首先估计粗略几何代理并通过光线投射修剪冗余点、建立2D-3D对应关系；随后采用Point-Image Transformer融合外观与几何特征，在前向传播中直接预测高斯属性。该方法将预测限制在前景感兴趣区域，显著减少高斯点总数。

Result: 在多个数据集上，PointSplat实现了更高的效率和渲染质量，并且对视角数量和图像分辨率的变化具有较强的鲁棒性。

Conclusion: PointSplat通过在3D空间进行直接预测，有效消除了视角间冗余，实现了紧凑且高质量的3D人体表示，适用于沉浸式直播系统等对实时性和紧凑性有严格要求的场景。
...
TL;DR: GEAR提出了一种引导式端到端自回归训练方法，通过双读码本分配机制（硬分支用于AR预测，软分支用于表征对齐）实现VQ分词器与AR生成器的联合训练，解决了非可微VQ索引导致的梯度无法回传问题，在ImageNet上实现高达10x的gFID收敛加速，并泛化到多种量化器和文生图任务。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型采用两阶段训练（先训练分词器再冻结后训练生成器），导致分词器不了解生成器的建模偏好，两者解耦限制了生成性能。

Method: 提出GEAR框架，通过双重读出的码本分配机制实现端到端联合训练：硬one-hot分支负责自回归下一个标记预测，可微软分支携带表征对齐损失回传梯度指导分词器更新，使AR模型引导分词器生成更易预测的索引分布。

Result: 相比LlamaGen-REPA基线，ImageNet gFID收敛速度提升高达10倍；学习到更优的patch级和空间一致特征；泛化到VQVAE、LFQ、IBQ等多种量化器及文生图任务。

Conclusion: GEAR将表征对齐的负担从分词器转移到AR模型，与扩散模型使潜变量语义化的策略相反，证明了通过端到端联合训练可以更高效地实现视觉生成建模。
...
TL;DR: FaceMoE是一种基于混合专家(MoE)Transformer架构的低分辨率人脸识别方法，通过引入多个专用FFN专家和top-k路由机制，实现分辨率感知的特征提取，在11个数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 低分辨率人脸识别面临两大挑战：1) 退化图像(模糊、遮挡、低对比度)中包含的身份信息有限，特征提取和聚合困难；2) 高分辨率(HR)图库与低分辨率(LR)查询图像之间存在域差距，单个特征编码器难以泛化到两个域，且微调时存在灾难性遗忘问题。

Method: 提出FaceMoE架构，包括：1) 多个专用FFN专家，通过top-k路由动态分配token到合适的专家，实现不同面部语义区域的专业化处理和分辨率感知特征提取；2) 稀疏专家激活机制，在LR数据集微调时保留预训练知识，增加模型容量而不带来等比例计算开销；3) 联合训练损失，包括人脸识别损失、router z-loss和负载均衡损失。

Result: 在涵盖HR、混合质量和LR的11个基准数据集上进行大量实验，FaceMoE显著优于当前最先进的方法。

Conclusion: FaceMoE是首个将MoE应用于低分辨率人脸识别的工作，通过专家专业化和稀疏激活有效解决了LR-FR中的域差距和灾难性遗忘问题，为低分辨率人脸识别提供了新的有效范式。
...
TL;DR: 本文提出了一种面向企业级AI代理的自动化技能描述优化管线，通过单次LLM重写利用假阳性和假阴性案例即可匹配人工调优效果（F1约79.2% vs 79.4%），将每个技能的工程耗时从120分钟降至3.8分钟（提速32倍），并发现技能冲突源于描述重叠和真实意图重叠两类问题，后者可通过训练-验证F1差距诊断。


<details>
  <summary>Details</summary>
Motivation: 企业AI代理通过自然语言描述将用户查询路由到专业技能，当技能描述重叠时，路由LLM会发生错误路由（技能碰撞）。随着技能数量扩展至数十个，手动调优描述以维持路由准确性成为显著的工程瓶颈。

Method: 在生产环境企业群聊代理（9个技能、372个回归测试用例）上部署自动化描述优化管线。该系统由单次LLM重写驱动，利用所有可用的假阳性和假阴性案例作为反馈信号。通过系统消融实验，在生产系统和ToolBench（16k工具）上验证各组件贡献。

Result: 优化管线达到平均79.2% F1分数，与人工调优的79.4% F1相当（平均每技能差异-0.20%，低于0.78%的多种子噪声阈值）。工程耗时从每技能120分钟降至3.8分钟（32倍加速）。消融实验表明单次LLM重写即可捕获大部分改进，其他设计选择（迭代预算、反馈信号组合、混淆对的双重编辑、训练集大小）对最终F1影响均小于0.5%。

Conclusion: 描述优化能解决因描述重叠导致的技能碰撞，但无法处理两个技能真实意图范围重叠的情况。提出一个诊断指标（较大的训练-验证F1差距）来识别后者，这类情况需要架构层面而非文本层面的干预。
...
TL;DR: 本文提出了Indi-RomCoM基准，用于评估大语言模型在印度语言罗马化代码混合（RCM）指令上的表现。实验发现，LLMs在RCM指令上表现不佳，且性能随代码混合密度增加而下降；推理任务比检测任务退化更少。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在单语和原生文字基准上表现强劲，但它们理解和推理基于罗马化代码混合（RCM）内容的能力尚未被充分探索。为此，作者提出Indi-RomCoM基准以填补这一评估空白。

Method: 构建了覆盖7个指令跟随任务、4种常用印度语言和3种受控代码混合强度级别的基准Indi-RomCoM，在零样本和少样本设置下评估了包括商业、开源和专注印度语的多种LLMs。

Result: LLMs在RCM指令上表现持续不佳，性能随代码混合密度增加而下降；推理任务（如Toxicity检测）的退化程度低于检测任务，因为生成的解释提供了必要的上下文。

Conclusion: Indi-RomCoM基准有助于社区开发更具包容性的多语言系统，揭示了现有LLMs在处理罗马化代码混合指令方面的明显不足。
...
TL;DR: 本文提出利用生成式AI代理作为行为引擎，对个性化推荐算法进行黑盒审计。通过在2024年美国大选后对X平台部署1,120个代理（14种人设，三种反事实条件），收集20万+内容曝光，发现算法推荐相比时间线推荐放大了有毒、极化、政治化和右倾内容，且放大效应因用户意识形态而异。反事实分析表明，人口统计信号对内容推送的影响因人设而异，总体效应不显著但子组层面存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有审计方法存在两难困境：真实用户研究成本高、难控制；傀儡账号审计可扩展但脚本化行为缺乏真实性。且两种方法都难以将用户属性与行为解耦，无法对个性化算法进行因果推断。

Method: 基于生成式AI代理构建合成账户，每个代理固定人设（基于人口统计和政治调查数据），通过与平台内容交互（推理并选择行为）模拟真实用户。在人设内固定行为，同时实验性地扰动平台可见信号（年龄、性别、位置），实现反事实审计。

Result: X算法推荐相比时间线推荐放大了有毒、极化、政治化和右倾内容；放大效应因用户意识形态而异；人口统计信号对内容推送的影响在人设层面存在异质性——总体效应不显著但子组层面存在方向和大小的差异。

Conclusion: GenAI代理可作为算法审计的新工具，能够实现大规模、可控制、因果可推断的黑盒审计，弥补现有方法的不足。
...
TL;DR: 论文提出ACE框架，通过控制准确率实现更公平的跨模型校准比较，发现原始全局校准指标因混淆准确率差异而不可靠，许多先前报告的校准优势在控制准确率后消失甚至逆转。


<details>
  <summary>Details</summary>
Motivation: 现有跨模型校准比较使用ECE、Brier Score等全局指标，但这些指标混淆了模型准确率差异，导致不公平的比较结果。

Method: 提出ACE（Accuracy-Controlled Evaluation）框架，包含三个互补视角：实例对齐校准、分布对齐校准和候选对齐校准，用于在控制准确率的情况下进行公平的跨模型校准比较。

Result: 1) 原始全局指标下的校准优势在控制准确率后大幅减弱；2) 排名反转频繁出现，即原始指标偏好的模型在控制准确率后不再占优。
...
TL;DR: 该研究评估了Transformer语言模型在"不可能语言"上的表现，发现模型在语法敏感性上仅表现出渐进退化，但在生成能力上出现显著缺陷，提出生成缺陷和传播失败是连接语言模型行为与不可能语言未被人类使用的合理假设。


<details>
  <summary>Details</summary>
Motivation: 先前研究表明Transformer语言模型对自然语言存在偏好，但这些研究主要基于样本效率和测试集困惑度差异，缺乏对语言能力（如语法敏感性和生成能力）的直接评估。本文旨在通过直接评估两种理论上可能的链接假说（不可能性源于语法敏感性缺陷或生成缺陷）来解释为何人类无法习得某些"不可能语言"。

Method: 使用GPT-2风格模型，在受扰动的"不可能"英语变体上进行训练；通过BLiMP最小对测试评估语法敏感性；同时评估模型的长句生成质量。

Result: 模型在语法敏感性上仅表现出渐进退化，且退化程度与语言的信息局部性相关；但在生成任务中表现出显著缺陷，生成长句时高质量句子数量大幅减少。

Conclusion: 生成缺陷和传播失败是解释语言模型行为与不可能语言未被人类使用之间关联的合理链接假说。
...
TL;DR: 本文提出GradeSQL框架，通过自动化候选生成和执行标签来训练任务特定的结果奖励模型（ORM），用于Text-to-SQL的测试时验证。实验表明，ORM-based选择在BIRD和Spider基准上持续优于执行式Best-of-N和多数投票，最高提升+4.33%和+2.10%，且对复杂查询改进更显著。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在Text-to-SQL等结构化推理任务中，现有的测试时推理策略（如Best-of-N采样和多数投票）依赖执行成功或输出频率等启发式信号，这些信号在候选输出间提供的语义区分能力有限，难以有效识别正确结果。

Method: 提出GradeSQL框架，通过自动化候选SQL生成和执行反馈进行标签标注，训练任务特定的结果奖励模型（ORM）作为语义评分函数；将ORM集成到验证驱动的Best-of-N流水线中，在BIRD和Spider基准上评估多个开源LLM系列。

Result: ORM-based选择在BIRD和Spider上分别取得最高+4.33%和+2.10%的提升；ORM随候选集规模扩大表现更优，且在复杂查询上改进更显著。
...
TL;DR: 本文针对SemEval-2026任务9（多语言、多文化、多事件在线极化检测），使用基于RoBERTa-base（英语）和AfroXLMR-base（斯瓦希里语）的Transformer模型，结合类别加权损失函数和逐标签阈值调优，在三个子任务上取得了有竞争力的F1宏平均分数。实验表明该方法有效处理了不平衡多标签极化检测问题，错误分析显示模型在去人性化检测和共情缺失识别方面存在困难。


<details>
  <summary>Details</summary>
Motivation: 旨在解决多语言（英语和斯瓦希里语）和多文化背景下在线极化现象的自动检测问题，涵盖二值极化检测、极化类型分类和极化表现识别三个子任务，应对真实数据中严重的标签不平衡挑战。

Method: 采用Transformer架构模型（英语用RoBERTa-base，斯瓦希里语用AfroXLMR-base），使用类别加权损失函数缓解标签不平衡问题，并采用逐标签阈值调优方法优化多标签分类性能。

Result: 在测试集上取得如下F1宏平均分数：子任务1（二值极化检测）英语0.7901、斯瓦希里语0.7910；子任务2（极化类型分类）英语0.4615、斯瓦希里语0.4808；子任务3（极化表现识别）英语0.4791、斯瓦希里语0.5830，在排行榜上表现有竞争力。

Conclusion: 提出的类别加权损失和逐标签阈值调优方法在不平衡多标签极化检测任务上有效；模型在去人性化检测和共情缺失识别方面仍需改进，为未来工作指明了方向。
...
TL;DR: 提出TheraAlign框架，将治疗性回复生成建模为决策优化问题。通过TheraJudge（基于偏好优化的多维度评估器）和TheraAgent（基于Critic-Coach-Therapist角色协同的响应修正机制），实现评估驱动的治疗质量提升。实验表明，TheraJudge在7个心理维度上达到ICC=0.87-0.95，TheraAgent在5分量表上提升0.43，低质量回复恢复率94%。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在心理健康支持中，评估仅作为被动指标而无法作为可操作的控制信号，导致治疗质量提升受限。需要将评估嵌入生成过程，使其成为驱动响应的主动反馈机制。

Method: 两阶段框架：Stage I训练TheraJudge，使用偏好优化在人工标注数据上学习7维心理评估；Stage II构建TheraAgent，通过Critic（评估）、Coach（建议）、Therapist（修订）三个专业化角色的协调精炼过程，将评估信号转化为具体响应修正。

Result: TheraJudge与临床医生评级高度一致（ICC=0.87-0.95），优于监督基线及强闭源评估器，尤其在安全性、相关性、共情维度；TheraAgent在盲评中提升0.43/5分，低质量回复(≤3分)提升2.45分，94%恢复率，临床医生间信度96%。

Conclusion: 心理健康LLM的有效对齐应源于对人类对齐评估的执行（acting on evaluation），而非仅依赖更强的生成能力。评估作为控制信号比作为被动指标更有效。
...
TL;DR: 本文研究了孟加拉语事件检测系统在真实噪声（ASR转录、拼写错误）下的鲁棒性，发现编码器模型在干净文本上表现更好但噪声下退化严重，而解码器LLM更鲁棒，并探讨了指令微调、模型缩放和混合训练策略的影响。


<details>
  <summary>Details</summary>
Motivation: 现有事件检测系统多在干净数据上评测，缺乏对真实噪声环境中鲁棒性的研究，尤其是孟加拉语等低资源语言。

Method: 构建包含9,979句标注、40个事件子类型的孟加拉语新闻事件基准数据集，涵盖干净文本、ASR转录和拼写错误文本；系统比较了BanglaBERT/XLM-R等编码器模型与Llama 3/Gemma 3等解码器LLM，并评估了指令微调、模型缩放和混合训练策略的效果。

Result: 编码器模型在干净文本上性能更高但噪声下显著退化；解码器LLM更鲁棒，尤其在事件触发词被破坏时。指令微调嵌入标注指南提高了噪声基线但不稳定；模型缩放持续改善解码器LLM鲁棒性；混合训练显著缩小了编码器架构的鲁棒性差距。

Conclusion: 解码器LLM在噪声环境下更具鲁棒性，混合训练是提升编码器模型鲁棒性的有效正则化策略，为低资源语言事件检测的实用化部署提供了指导。
...
TL;DR: 本文构建了阿拉伯语-俄语科学翻译基准，包含约27,000句对的混合平行语料库，并微调了三个多语言模型。其中Qwen2.5-7B搭配QLoRA（秩8）取得最佳效果：BLEU 23.15, chrF 43.89, BERTScore 0.906, COMET 0.758，较零样本基线提升+4.36 BLEU和+0.051 COMET。少样本提示效果不佳，表明需领域特定微调。研究降低了科学文本的语言障碍，有助于知识交流与可持续发展目标。


<details>
  <summary>Details</summary>
Motivation: 俄语和阿拉伯语是科学交流的主要语言之一，但语言障碍阻碍了这两个学术共同体之间的研究成果交流，影响国际合作与可持续发展相关研究的进展。

Method: 构建了约27,000句对的混合平行语料库（科学摘要+通用领域文本）；使用LoRA（秩8/16/32/64）微调三个多语言模型：mT5-base (580M)、NLLB-200-distilled-1.3B (1.3B)、Qwen2.5-7B-Instruct (7B)；评估采用BLEU、chrF、BERTScore、COMET指标。

Result: Qwen2.5-7B + QLoRA（秩8）取得最佳：BLEU 23.15, chrF 43.89, BERTScore 0.906, COMET 0.758，较零样本基线提升+4.36 BLEU和+0.051 COMET。少样本提示（3个示例）未提升性能。

Conclusion: 领域特定微调对阿-俄科学翻译至关重要，少样本提示不足以满足需求。工作降低了科学文本的语言障碍，促进知识交流与可持续合作伙伴关系（SDG 17）和创新基础设施（SDG 9）。
...
TL;DR: 本文通过分析社交媒体文本，研究了语言距离（linguistic distancing）作为情绪调节的一种方式如何随年龄变化，发现年龄越大，语言距离使用比例越高，这与心理学中老年人情绪幸福感提升的结论一致。


<details>
  <summary>Details</summary>
Motivation: 情绪调节对心理健康至关重要，心理距离是情绪调节的一种形式，而语言中可检测到这种距离的标记（语言距离）。先前研究尚未系统探索语言距离如何随生命周期变化，因此本文旨在弥补这一空白。

Method: 基于已有的语言距离度量指标进行改进，系统分析大量社交媒体文本（人们表达情绪的平台），按年龄分组考察语言距离的变化模式，并提供社交媒体数据上的初始基准。

Result: 语言距离随年龄增长而比例增加，即年龄越大的人在语言表达中展现出更多心理距离。这一结果与心理学中老年人情绪幸福感提升的过往研究一致。

Conclusion: 语言距离随年龄变化可作为幸福感的标记，有助于设计有效的健康干预措施。本文为进一步通过文本数据探索情绪调节与语言距离的关系提供了基础。
...
TL;DR: 本文扩展了Frictive Policy Optimization (FPO)框架，从处理命题不对称（对同一场景的不同解释）延伸到感知不对称（参与者持有不对称的部分信息，同一指称表达因参照不同信息状态而具有不同指称）。通过对HCRC MapTask等语料库和LLM探针实验发现：FPO的摩擦函数仅在参与者各自的信息视野内有效，少数模糊配置导致了不成比例的误解；拥有"正确视角"比拥有全知视角更重要。最后提出了两种注释改进方案。


<details>
  <summary>Details</summary>
Motivation: 现有FPO框架假设共享感知语境，仅处理因对同一场景的不同命题解释而产生的摩擦（命题不对称），但忽略了参与者持有不对称部分信息时，同一指称表达在不同信息状态下产生不同指称的感知不对称问题。

Method: 跨语料库分析和LLM探针实验，以HCRC MapTask为主要数据集，在指称不对称对话任务上评估FPO摩擦函数在感知不对称场景下的有效性。

Result: 1) FPO的摩擦函数仅在参与者各自信息视野内有效；不同地标配置产生不同类型的接地失效模式，少数模糊配置通过看似成功但 silently diverge 的轨迹导致了不成比例的误解。2) LLM探针表明：拥有正确视角比拥有所有视角更重要——知情单一视角优于全知双视角。

Conclusion: 提出了两种注释改进：待定接地状态的子类型分解，以及容纳感知偏差的对齐分类（accommodation-aware alignment classification）。
...
TL;DR: 本文提出解释质量标记（EQMs），利用大语言模型从专家预测的书面解释中提取60种理论驱动的推理模式，在超过55,000个预测-解释样本中验证了其对预测准确性的预测能力，优于传统文本分析方法，且结果可迁移至独立研究。


<details>
  <summary>Details</summary>
Motivation: 决策者依赖专家判断及其书面解释，但解释质量难以大规模衡量。现有方法缺乏可扩展且可解释的质量评估手段，因此需要一种能够自动化、规模化评估解释质量的方法。

Method: 提出Explanation Quality Markers（EQMs），包含60种理论驱动的推理模式，利用大语言模型（LLMs）从预测-解释对中打分。在多年预测锦标赛中收集55,000+样本进行预注册分析，并与传统文本分析方法、人类评分进行对比。

Result: EQMs在预测层面和预测者层面均能显著预测准确性，优于预LLM文本分析方法；超过90%的显著模式-准确率相关性与假设方向一致；信号不对称，识别低表现者更可靠；EQMs是预测层面最强的预测因子，在预测者层面具有竞争力；人类评分与准确率相关性较弱且过度依赖解释长度；结果在独立研究中得到复现。

Conclusion: EQMs提供了一种可扩展、可解释的方法，能够从书面解释中提取与判断相关的信息，是评估和筛选专家判断质量的实用工具。
...
TL;DR: 本文发现大语言模型中存在"演绎刻板印象"现象（模型将统计规律错误应用于个体案例），并提出推理时注入框架Fair-GCG来引导模型进行公平性感知推理，实验证明该方法能提升多个公平性基准的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管推理能力提升通常能改善LLM的公平性，但仍有失败案例存在。作者识别出一种特定的失败模式——演绎刻板印象，即模型将群体层面的统计规律错误地应用于个体案例，产生逻辑上连贯但带有社会偏见的推理。

Method: 1. 从统计学角度解释演绎刻板印象现象。2. 提出推理时注入（reasoning-time injection）框架，引导模型进行公平性感知推理。3. 引入Fair-GCG方法，系统性地发现有效的注入短语。

Result: Fair-GCG发现的注入短语在多个公平性基准测试上提升了性能；从小模型泛化到大模型；提升了推理层面的公平性；减少了开放生成中的偏见；并迁移到现实世界的公平敏感任务中。

Conclusion: 推理时注入框架和Fair-GCG方法能有效缓解大语言模型中的演绎刻板印象问题，提升模型的公平性推理能力，且具有良好的泛化性和迁移性。
...
TL;DR: CORTEX是一种用于检索增强生成（RAG）的token级幻觉检测方法，通过比较有无检索文档时LLM的内部表征来定位未基于检索内容生成的token，并利用文档信息在前置token中的传播以及后处理平滑来提升检测精度。


<details>
  <summary>Details</summary>
Motivation: 在长文本RAG输出中，幻觉通常出现在局部片段而非整个回答，现有方法缺乏细粒度的token级定位能力。

Method: CORTEX通过对比LLM在有/无检索文档两种条件下的内部表征来检测token级幻觉；利用文档信息通过前置token的传播效应来减少误报；采用后处理平滑步骤建模幻觉标签在连续片段中的持续性，降低局部噪声并鼓励片段一致的预测。

Result: 在两个RAG基准和三种LLM上的实验表明，CORTEX显著提升了token级幻觉检测性能，每个组件均持续贡献性能增益。

Conclusion: CORTEX通过细粒度的token级检测和文档信息传播机制，有效提升了RAG中局部幻觉的定位能力。
...
TL;DR: 本文提出LoFa基准和LFR@k指标，通过多智能体流水线构建谬误攻击数据，并利用多轮辩论框架评估LLM在面对逻辑谬误时的鲁棒性，实验发现不同模型对不同类型谬误的脆弱性存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 现有工作主要关注LLM是否能识别或分类逻辑谬误，但对LLM在面对谬误性说服时的鲁棒性研究不足，因此本文旨在填补这一空白。

Method: 通过多智能体流水线将事实问题与谬误论证配对构建LoFa基准；提出多轮辩论框架评估模型在持续对抗性说服下的鲁棒性；提出LFR@k指标量化模型对谬误攻击的抵抗能力。

Result: 实验表明不同LLM在不同类型谬误上的鲁棒性存在差异，揭示了各模型独特的脆弱性特征。

Conclusion: LLM在面对逻辑谬误时并非完全鲁棒，且其脆弱性因谬误类型和模型而异，LoFa基准和LFR@k指标为评估和提升LLM的谬误鲁棒性提供了有效工具。
...
TL;DR: 提出了一种基于语义层的NL2SQL代理系统，通过中间表示SMQ将语义意图与物理SQL执行解耦，在Spider2-snow基准上达到94.15%的执行准确率。


<details>
  <summary>Details</summary>
Motivation: 企业级数据库的NL2SQL面临模式复杂、列名晦涩、SQL方言多样及分析工作负载复杂等挑战，现有方法直接基于原始模式生成SQL效果不佳，需要更有效的解耦方案。

Method: 构建语义层（Semantic Layer）作为中间抽象，设计紧凑的中间表示SMQ（Semantic Model Query）；代理先基于语义层推理生成SMQ，再由确定性编译器将SMQ翻译为具体SQL方言；采用受约束的思考-执行循环（think-act loop），支持SQLite、BigQuery和Snowflake三种后端。

Result: 在Spider2-snow基准的547个任务上达到94.15%的执行准确率，在官方排行榜上排名第三，显著优于仅基于原始模式的方案。

Conclusion: 语义层方法有效解耦了语义理解与物理SQL执行，提升了企业级NL2SQL的准确性和可迁移性，但也存在语义层质量影响性能和可能过拟合的权衡。
...
TL;DR: 本文针对语音到语音（S2S）AI代理在对话韵律和节奏评估中缺乏可解释的语音原生指标的问题，提出了一种基于百分位匹配参考协议的评估方法。利用4000+小时的英语对话数据，构建F0均值、F0表现力、语速、发音速率、停顿比例和平均停顿时长的匹配参考体系，通过计算S2S输出波形与最接近的人类参考层的百分位偏差或越界标志，实现了比混合统计更准确、方向可解释的行为合理性检查。


<details>
  <summary>Details</summary>
Motivation: 现有S2S系统评估缺乏可解释的语音原生韵律和节奏指标，且由于F0、语速等声学特征随说话人特质和交互状态动态变化，使用混合的人类统计数据评估特定输出时校准不良，无法准确反映生成语音的行为合理性。

Method: 基于Seamless Interaction数据集的4000+小时双人英语对话，构建F0均值、F0表现力、语速、发音速率、停顿比例和平均停顿时长六个指标的匹配参考体系。提出百分位评估协议：从S2S输出波形中提取相同指标，与最接近的匹配人类参考层比较，报告百分位偏差或5th-95th百分位越界标志。

Result: 在留出的人类数据上，混合参考对状态条件化的F0表现力和韵律过度标记，而匹配参考的标记率接近名义上的10%，并使偏差方向变得可解释。

Conclusion: 该百分位匹配参考协议作为行为合理性检查，可补充而非替代感知和以用户为中心的评估方法，为S2S系统提供更准确、可解释的韵律质量评估手段。
...
TL;DR: 本研究以自然语言处理领域为例，将作者团队分为学术型、产业型以及产学研混合型三类，通过从全文中提取方法、数据集、工具和指标四类细粒度知识实体，基于实体组合测量论文新颖性，发现产学研混合团队比纯产业团队更可能产出新颖论文，且不同类型团队关注的新颖性组合各有侧重。


<details>
  <summary>Details</summary>
Motivation: 现有研究对团队机构组成如何影响论文新颖性关注不足，且新颖性测量停留在宏观层面，难以解释新颖性的具体来源和类型。

Method: 以自然语言处理领域为例，将团队分为学术、产业和混合三类；从全文提取四类细粒度知识实体（方法、数据集、工具、指标），基于实体组合测量新颖性，并分析不同类型实体配对组合对新颖论文的贡献。

Result: 在自然语言处理领域，产学研混合协作比纯产业协作更易产出新颖论文；混合团队更关注方法-指标组合的新颖性，产业团队更关注方法-工具组合的新颖性。
...
TL;DR: 本文构建了包含文本、图像、音频的多模态学术论文数据集（1000样本），实验发现不同模态文本具有不同特征，融合论文文本、图像文本和音频文本能有效提升关键词抽取性能。


<details>
  <summary>Details</summary>
Motivation: 传统关键词抽取仅依赖文本数据，忽略了图像和音频模态中的视觉与音频信息，导致信息丰富度不足且忽略了潜在跨模态关联，限制了模型表示学习能力和预测准确性；同时，多模态关键词抽取数据集极度匮乏，阻碍了该方向研究进展。

Method: 构建包含1000个样本的多模态学术论文数据集，每个样本包含论文文本、图像、音频和关键词；基于无监督和有监督的关键词抽取方法，分别使用论文文本、图像提取文本、音频提取文本以及它们融合后的多模态信息进行实验。

Result: 不同模态的文本在模型中表现出不同特征；将论文文本、图像文本和音频文本进行拼接融合，能够有效提升学术论文关键词抽取的性能。

Conclusion: 融合多模态信息（文本+图像文本+音频文本）可以增强关键词抽取的效果，为解决传统纯文本方法的局限性提供了有效途径。
...
TL;DR: 本文提出Triospect检测框架，通过引入内容（核心思想）和表达（风格元素）两个额外视角，增强AI生成文本检测器对抗攻击的鲁棒性。在两个基准测试中，该方法在AUROC和TPR01指标上分别显著提升22.3%/13%（Humanize-16K）和9.1%/22%（adversarial RAID），是统计方法增强检测可靠性的开创性工作。


<details>
  <summary>Details</summary>
Motivation: 现有AI文本检测器容易受到操纵文本特征的攻击，检测可靠性不足，需要更鲁棒的检测方法。

Method: 提出Triospect检测框架，在传统文本特征基础上，额外引入内容（核心思想）和表达（风格元素）两个视角进行综合检测。

Result: 在Humanize-16K后攻击子集上，AUROC提升22.3%，TPR01提升13%；在adversarial RAID上，AUROC提升9.1%，TPR01提升22%。

Conclusion: Triospect框架是首个利用统计方法从多视角增强检测可靠性的工作，有效抵御各类攻击，代码已开源。
...
TL;DR: 本文提出Training-Free Gated Reranking方法，通过模型不确定性决定是否重排序少样本示例，发现重排序并非总是有益，反而可能降低性能。在8个LLM、7个NLU数据集和9个MT领域-语言组合上的实验表明，该方法可降低15%-80%计算成本，同时提升平均性能达2%。


<details>
  <summary>Details</summary>
Motivation: 挑战"重排序检索示例总是提升性能"的普遍假设，指出重排序步骤不仅昂贵，反而可能降低性能。

Method: 提出Training-Free Gated Reranking，基于模型的不确定性来决定是否对少样本示例进行重排序，避免在低不确定性实例上不必要的计算开销。

Result: 在8个LLM、7个NLU数据集和9个MT领域-语言组合上，计算成本降低15%-80%，平均性能提升最高2%。

Conclusion: 更高的计算成本并不保证更好的性能，重排序仅在针对高不确定性实例时最有价值。
...
TL;DR: ASR系统在非典型语音（如口吃）上表现不佳，部分原因是存在两种有效的转录标准：逐字转录（包含重复/延长）和意图转录（去除不流畅的规范形式）。现有评估常将两者混为一谈，奖励删除不流畅的模型。本文对11种ASR模型在口吃语音上使用两种参考标准进行基准测试，揭示模型性能和排名存在显著差异，强调应根据使用场景选择合适的转录参考标准。


<details>
  <summary>Details</summary>
Motivation: ASR系统在非典型语音上表现不佳，但现有评估通常将逐字转录和意图转录混为一谈，采用单一参考标准，导致模型选择不合理。作者旨在揭示这种混淆对模型评估的影响，并强调根据具体用例选择合适转录参考的重要性。

Method: 对11种ASR模型（包括编码器-解码器、CTC和 transducer 家族）进行基准测试，使用逐字参考和意图参考两种转录标准，以口吃语音作为非典型语音的案例研究对象，进行定量评估。

Result: 使用两种不同转录参考标准时，模型的性能和排名存在显著差异。大多数现有评估忽略这种双重性，实际上奖励了删除不流畅的模型，而牺牲了逐字忠实度。

Conclusion: 在非典型语音ASR中，应根据具体使用场景选择合适的转录参考标准，以进行有效的模型选择。现行评估方法存在偏差，需重新审视评估体系。
...
TL;DR: SeKV提出了一种分辨率自适应的语义KV缓存方法，通过熵引导的语义片段划分和GPU-CPU异构存储，在不丢弃信息的前提下实现高效的长上下文推理，相比最强语义压缩基线平均提升5.9%并减少53.3%的GPU内存。


<details>
  <summary>Details</summary>
Motivation: 长上下文场景下，KV缓存大小随序列长度线性增长，成为主要内存瓶颈。现有方法如Token逐出会丢失信息，语义分组在预填充阶段固定压缩决策，无法在生成过程中恢复已压缩片段的Token级细节，难以兼顾效率与保真度。

Method: SeKV将上下文组织为熵引导的语义片段，GPU保留轻量摘要向量用于粗粒度路由，CPU存储低秩SVD基用于按需Token级重建。训练一个"缩放"（zoom-in）机制，在解码时选择性展开查询相关的片段，实现精确检索而不需将完整KV缓存加载到GPU。

Result: 在四个基准上，SeKV相比最强语义压缩基线平均提升5.9%，同时相对于128K上下文的完整KV缓存减少53.3%的GPU内存。基础LLM完全冻结，仅增加不到0.05%的可训练参数。

Conclusion: SeKV通过语义片段和异构存储实现了分辨率自适应的KV缓存压缩，在不丢弃信息的前提下兼顾了效率与长上下文保真度，是一种轻量级且有效的解决方案。
...
TL;DR: 提出一种统一文本推理和图消息传递的掩码扩散语言模型方法，通过拓扑注意力掩码在图结构上实现消息传递，无需特定任务微调即可处理节点分类、链接预测和跨数据集迁移任务，在三个TAG基准上相比最强基线提升最多3.9个点。


<details>
  <summary>Details</summary>
Motivation: 现有方法将文本和图结构分开处理（GNN仅使用浅层文本特征，LLM混合方法将语言模型仅作为文本编码器），缺乏对两种模态的联合推理机制。

Method: 将局部邻域线性化为token序列，通过拓扑注意力掩码注入图结构实现消息传递；使用双向注意力和生成式解码的掩码扩散语言模型统一文本推理和图消息传递；通过改变提示词适应不同任务。

Result: 在三个TAG基准上的节点分类和链接预测任务中，方法优于GNN、图Transformer和基于LLM的基线方法，相比最强基线提升最多3.9个百分点。

Conclusion: 统一文本推理与图消息传递的扩散语言模型方法能有效处理文本属性图上的多种任务，且无需目标特定微调即可实现跨数据集迁移。
...
TL;DR: 提出多视图门控图注意力网络，利用ASR转录构建语义、依存和共现图，通过自适应门控融合处理阿尔茨海默病语音异质性，在ADReSSo数据集上达到90%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有AD语音分析系统忽视了病理语言中的非线性结构破坏和临床异质性，亟需一种能同时捕捉内容、结构和流畅性多维度特征并适应症状多样性的方法。

Method: 通过ASR转录音频，构建语义图、依存图和PMI共现图（使用规范语料库计算点互信息量化叙事逻辑和语言偏差），采用自适应门控融合机制动态整合多视图，形成"内容-结构-流畅性"框架。

Result: 在ADReSSo数据集上达到90.00%准确率，消融实验证实PMI图与异质性感知门控对跨临床群体的鲁棒分类至关重要。

Conclusion: 多视图图注意力网络结合PMI共现图与自适应门控能有效捕捉AD语音异质性特征，为临床诊断提供可靠的非侵入性生物标志物。
...
TL;DR: 本文提出了MoralAltDataset，包含307个道德困境及替代方案，发现大语言模型和人类一样会因替代方案的引入而改变道德判断，且模型生成的替代方案质量有时甚至优于人工方案。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了一个关键的人类道德认知能力——想象超越给定选项的替代方案，因此需要系统评估LLM在这一维度上的表现。

Method: 构建MoralAltDataset（307个道德困境，含叙事型顾问困境和AI型代理困境，每个困境配有妥协方案和重构方案）；通过15个LLM进行判断偏移实验；使用成对偏好和专家标准评估LLM生成替代方案的质量。

Result: 妥协替代方案常被优先选择，显著改变道德选择；LLM生成的替代方案在成对偏好和细粒度结构/伦理标准上往往优于人类方案，但在实用可行性上存在权衡。

Conclusion: LLM具备生成高质量道德替代方案的能力，这既为AI道德决策提供了新思路，也揭示了结构质量与可行性之间的张力。
...
TL;DR: 本文提出PSALM框架，将欧盟著作权法中的"实质性相似"标准（涵盖风格、叙事结构等）操作化为十个评估维度，用于检测LLM的侵权风险。实验发现指令微调和微调后的模型存在系统性风格挪用，且现有仅针对逐字记忆的防护措施不足以应对法律风险。


<details>
  <summary>Details</summary>
Motivation: 现有LLM版权防护措施仅关注逐字记忆（verbatim memorisation），但欧盟版权法采用更广泛的"实质性相似"标准，涵盖风格选择、叙事结构和创造性发挥。现有技术检测与法律保护之间存在显著合规鸿沟。

Method: 提出PSALM框架，采用LLM-as-a-judge方式，通过十个评估器评估计算重叠、风格维度（写作风格、叙事声音）、内容维度（角色、情节、场景、世界观构建）以及法定例外（戏仿、混搭、引用、场景必具）。在翻译后的荷兰历史文学作品微调的Llama 3.2模型上进行实验。

Result: 1) 指令微调模型在接触语料前已存在非平凡的风格相似性基线；2) 微调导致系统性的风格挪用，超越逐字记忆延伸至抽象叙事模式；3) 负偏好优化（NPO）去学习大幅降低相似度，但仍存在可检测的残留风格痕迹。

Conclusion: 仅针对字面拷贝的防护措施不足以缓解更广泛的版权风险。PSALM提供了可审计、符合法律规范的合规评估基础设施，但自动相似度分数与侵权判定之间的关系仍需法律专家验证。该工作桥接了定性法律标准与定量技术测量之间的鸿沟，揭示了生成式AI与欧盟知识产权法之间的根本张力。
...
TL;DR: 论文研究了任务型对话系统中，当数据库调用失败时，大型语言模型会产生不安全回应（幻觉）的问题。提出了一种轻量级的基于提示词的恢复策略（Guided-Retry），无需重训或额外模型调用。在六个开源模型家族和两个数据集上的实验表明，该方法将幻觉率降低了42-50%，但残余幻觉仍显著存在（6-37%），其中跨域检索失败是最难处理的情况。


<details>
  <summary>Details</summary>
Motivation: 任务型对话系统中，当后端数据库调用失败、返回空结果或出现信息不匹配时，大型语言模型经常产生流畅但缺乏安全保障的回应（如编造场地、确认信息或预订详情），这些问题并非基于数据库事实，需要一种轻量级的鲁棒性提升方法。

Method: 对比三种回应策略，包括一种基于结构化数据库状态的引导恢复提示（Guided-Retry）。在六个开源模型家族（DeepSeek-R1, Gemma-2, Llama-3, Mistral, Phi-3, Qwen-2.5）和四种数据库条件（空结果、跨域检索、API错误、正常检索）下进行实验。使用基于MultiWOZ 2.2（5个领域）和SGD（20个领域）构建的故障注入基准测试。

Result: 朴素基线模型在MultiWOZ上30.5%的故障轮次中出现幻觉，在SGD上为20.9%。Guided-Retry策略在MultiWOZ上将幻觉率从30.5%降至15.3%（降低50%），在SGD上从20.9%降至12.2%（降低42%）。但残余幻觉仍达6-37%，跨域检索失败是最难处理的情况。结果在六个模型家族和两个数据集中保持一致。人工标注验证了自动承诺安全性指标的有效性。

Conclusion: 基于提示词的轻量级恢复策略能有效减少任务型对话系统中数据库故障引发的幻觉，无需重训练。然而该方法无法完全消除幻觉，尤其是在跨域检索失败场景下，表明需要进一步的鲁棒性提升技术。
...
TL;DR: 本文提出潜在序数原型对齐（LOPA）和语义锚定层路由（SALR）框架，在不依赖大规模多模态大语言模型微调的情况下，通过序数几何先验和自适应多层表示提取，在口语评估任务上达到RMSE 0.361，性能媲美十亿参数系统。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的口语评估方法虽有效，但忽视了语言习得的内在序数结构（即能力水平存在顺序关系），且依赖大规模模型微调，计算成本高。

Method: 提出LOPA（潜在序数原型对齐）作为基于原型的正则化器，在潜在空间施加序数几何先验；结合SALR（语义锚定层路由），自适应地从冻结的Whisper编码器中提取多层表示。

Result: 在口语评估任务上达到RMSE 0.361，性能与十亿参数系统相当，且无需基于LLM的微调。

Conclusion: LOPA与SALR的协同提供了可解释、符合评估标准的能力偏好，为当前以规模为中心的SLA建模提供了高效且具备序数感知的替代方案。
...
TL;DR: 现有扩散加速推理方法采用固定块大小，但最优块大小因输入而异且具有局部结构。本文提出BlockPilot，通过预填充表示预测最优块大小，实现样本自适应策略，在Qwen3-4B上达到5.92的接受长度和4.20倍加速。


<details>
  <summary>Details</summary>
Motivation: 现有扩散式推测解码方法对所有输入采用固定推理块大小，但作者发现最优块大小因样本而异且对性能至关重要，这一假设是次优的。

Method: 将块大小选择建模为轻量级策略学习问题，利用预填充阶段表示预测最优块大小，仅在预填充后执行一次预测。

Result: 在Qwen3-4B上，温度T=1时达到5.92的接受长度和4.20倍加速。

Conclusion: BlockPilot即插即用，引入开销极低，能持续提升推理效率。
...
TL;DR: 针对语音欺骗检测中存在的语言偏差导致跨域泛化差的问题，提出基于师生对抗学习的语言不变欺骗检测框架，利用变分信息瓶颈抑制主要线索，在9个数据集上实现高达36.2%的EER相对降低。


<details>
  <summary>Details</summary>
Motivation: 生成语音技术的快速进步削弱了语音生物识别的可靠性，现有欺骗检测器在域内条件下表现出色，但跨域泛化能力差，原因是模型依赖训练数据中的语言线索（语言偏差），导致跨数据集鲁棒性不足。

Method: 提出语言不变欺骗检测框架：1）语言感知教师模型在外部数据集的语言内容上预训练；2）通过梯度反转层指导学生检测器最小化语言信息；3）引入变分信息瓶颈（VIB）抑制主要线索，防止非语言线索被意外移除。

Result: 在9个DF Arena数据集上，该方法相比基线实现了高达36.2%的等错误率（EER）相对降低。

Conclusion: 语言偏差是导致欺骗检测器跨域泛化差的重要原因，所提出的语言不变框架结合对抗学习与信息瓶颈能有效提升泛化性能。
...
TL;DR: 本文提出BiRG-LoRA，一种单适配器秩门控LoRA方法，通过双轴门控机制根据输入问题动态选择稀疏的top-k秩原子，并结合标量注入系数控制适配器更新强度，在四个医学问答基准上以更少参数取得优于现有PEFT方法的精度（宏平均69.31%）。


<details>
  <summary>Details</summary>
Motivation: 医学多项选择题问答需要跨异构知识领域和推理操作进行参数高效适配，不同医学问题（如用药、诊断、公卫、护理）需要不同的低秩更新，而某些回忆型问题应保持基模型表征仅需轻微适配器干预。

Method: BiRG-LoRA在每个目标层保留一个LoRA模块，但其秩维度是输入条件化的：通过双轴门控机制，结合隐藏语义证据与专业/职业先验、临床操作先验及其交互，为每个问题选择稀疏的top-k秩原子子集，并使用标量注入系数控制所选适配器更新的强度。

Result: 在Qwen3-8B CMB-source协议下，BiRG-LoRA在CMB、CMExam、MedQA、MedMCQA四个基准上宏平均准确率69.31%，优于MoELoRA 0.89个百分点且参数减少28.1%；95%置信区间[0.42, 1.37]；优于vanilla LoRA r16和active-rank-matched LoRA r4各0.83宏平均点。

Conclusion: 临床结构化的秩分配能在匹配的单种子协议下提升跨基准医学问答性能，且评估时弱轴扰动检查表明性能对中等标签噪声不脆弱，训练种子方差留作未来工作。
...
TL;DR: 本文发现并修复了RVL-CDIP数据集中12%的标签错误和约35%的测试-训练重复问题，生成了多个修正版本，并评估了这些修正对文档分类性能的影响。修复标签错误可提升分类准确率和OOD泛化能力（最高提升14个百分点），但去除重复数据会导致准确率下降。


<details>
  <summary>Details</summary>
Motivation: RVL-CDIP数据集存在大量标签错误和显著的测试-训练重叠问题，这些缺陷可能影响模型性能评估的准确性，需要进行系统性修复和分析。

Method: (1) 检测并修复标签错误；(2) 检测并处理测试-训练重叠问题；生成多个RVL-CDIP修正变体，并对这些变体进行文档分类性能基准测试。

Result: RVL-CDIP包含12%的标签错误和约35%的测试-训练重复；修复错误后分类准确率提升，但去除重复后准确率下降；在OOD基准RVL-CDIP-N上，使用错误修正数据训练的模型准确率平均提升8.1个百分点，最高达14个百分点。

Conclusion: 标签错误的修复显著提升模型性能尤其是OOD泛化能力；测试-训练重复虽可提升表现指标但会掩盖模型真实能力，去除重复有助于更准确的评估。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）的流水线，用于对用户按时间顺序发布的帖子进行综合心理健康分析，同时支持帖子级别评估和用户级别时间建模，作为CLPsych共享任务的一部分。


<details>
  <summary>Details</summary>
Motivation: 心理健康障碍日益普遍，但专业护理的可及性有限，亟需可扩展的计算方法来辅助心理健康的早期检测和持续监测。

Method: 提出一种基于LLM的流水线，能够对用户按时间顺序的帖子进行统一分析，同时实现帖子级别的评估和用户级别的时间建模。

Result: 该流水线作为CLPsych共享任务的一部分，提供了统一的框架来支持多层次心理健康分析。

Conclusion: 该LLM流水线能够有效整合帖子级和用户级的分析，为心理健康领域的自动化评估提供可扩展的解决方案。
...
TL;DR: 本文提出了一个面向班巴拉语儿童阅读评估的端到端开源系统，包含移动数据采集、基准构建、Soloni ASR模型微调（基于Fast-Conformer架构，集成TDT和CTC解码器）以及课堂验证。在55小时原始语音数据上，Soloni模型将WER从0.42降至0.22，CER从0.15降至0.08，显著优于QuartzNet架构。


<details>
  <summary>Details</summary>
Motivation: 大多数非洲语言（包括班巴拉语）的儿童语音识别系统开发滞后，而该技术对于可复现的读写能力评估具有重要价值，因此亟需填补这一空白。

Method: 采用端到端流程：①通过移动端采集应用收集60名儿童55小时原始阅读语音；②构建公开的儿童阅读评估基准；③对比微调Soloni（基于Fast-Conformer，集成TDT和CTC解码器）与QuartzNet两种ASR架构；④分析重复阅读同一文本对两种模型的差异化影响；⑤通过10次课堂试验验证系统实用性。

Result: Soloni最佳模型将WER从0.42降至0.22，CER从0.15降至0.08，整体优于QuartzNet。重复阅读对QuartzNet提升显著而对Soloni增益有限；SpecAugment的正则化效果未超过最优无增强配置。误差主要来自10岁以下儿童。

Conclusion: 所提出的开源系统在班巴拉语儿童阅读评估上取得显著性能提升，识别出低龄儿童为主要误差来源，为后续针对性数据收集和系统优化指明了方向。
...
TL;DR: LLM作为金融代理时，显式行为指令（如"保本"）会随着时间推移逐渐失效（Mandate Salience Decay, MSD）。本文提出FinPersona-Bench基准，通过合成市场分离价格与内在价值，测试18个LLM在三种失败模式下的表现。结果显示MSD随时间累积且模型相关；定期指令重接地对保守型代理有益，但对激进型代理有害。结论需要基于代理特征和市场状态的选择性指令重接地。


<details>
  <summary>Details</summary>
Motivation: LLM作为金融代理被部署时会被赋予显式行为指令（如"保本""避免投机"），但这些指令在长期市场交互中逐渐失去行为影响力，现有研究缺乏对这一现象的量化评估方法。

Method: 提出FinPersona-Bench仿真基准，构建合成市场将可观测价格与隐藏的内在价值解耦，从而可证伪地评估三种失败模式（无信号交易、恐慌抛售、忽略内在价值）。测试18个领先LLM，分配三种行为画像（保守、均衡、激进），比较静态代理与定期接收指令重接地代理的表现。

Result: MSD随时间累积且模型相关；在危机场景中，静态代理与定期重接地代理的行为差距从第一个季度到最后一个季度扩大了4.4倍。指令重接地效果非统一正向：对保守代理在低信号市场中有帮助，但对激进代理在相同场景中反而恶化行为。

Conclusion: 可靠的长周期金融代理部署需要基于代理特征和市场状态的选择性、指令感知的重接地策略。
...
TL;DR: AutoTrainess是一个LM智能体，通过外部化人类经验为显式工作流、规则和执行约束，在面向语言模型自主后训练（包括规划、数据准备、训练、评估和日志记录）的任务中显著优于纯CLI基线（GPT-5.4 Codex：26.94 vs 23.21；DeepSeek-V4-Flash OpenCode：12.13→19.58）。


<details>
  <summary>Details</summary>
Motivation: 语言模型的后训练过程高度依赖人力，即使前沿LM智能体在软件工程等长程任务上能力日益增强，但自主后训练不仅是编码问题，还需要智能体反复规划迭代、构建基准对齐数据、运行稳定训练作业、评估检查点并在长时间交互中保持实验状态，现有方法缺乏对此类复杂工作流的有效支持。

Method: 提出AutoTrainess LM智能体，将后训练操作封装为智能体-计算机接口仓库，涵盖规划、数据准备、训练、评估和日志记录。通过外部化先前人类经验作为显式工作流、规则和执行约束，而非让智能体在原始CLI环境中操作，从而引导其产生有效且可靠的训练行为。

Result: 在PostTrainBench上，AutoTrainess持续优于CLI-only基线：GPT-5.4 (Codex)达26.94平均分（vs CLI-only 23.21）；DeepSeek-V4-Flash (OpenCode)从12.13提升至19.58，展示了跨模型和跨训练框架的泛化能力。

Conclusion: 外部化人类经验为结构化接口和约束能够有效提升LM智能体在自主后训练任务中的表现，该方法具有良好的跨模型和跨训练框架泛化性，为减少LM后训练中的人力投入提供了可行路径。
...
TL;DR: DEW是一种结合上下文和词元级嵌入的语义水印方案，通过伪随机矩阵投影和向量空间操作实现鲁棒性，在释义和翻译攻击下仍能保持可检测性，同时维持文本质量。


<details>
  <summary>Details</summary>
Motivation: 现有LLM水印方法在面临释义和翻译等语义扰动时鲁棒性不足，需要一种能优雅降级且保持可检测性的语义水印方案。

Method: 将信号处理方法引入文本水印，通过对上下文嵌入和词元嵌入进行代数向量空间运算生成水印信号；使用伪随机矩阵（由密钥种子生成）投影嵌入向量来混淆水印；基于代数导出的相关分布进行统计检验。

Result: 多个LLM上的实验表明：DEW在释义后检测率提升；翻译后仍可检测，而此前语义水印显著退化；文本质量具有竞争力。

Conclusion: DEW是一种实用且鲁棒的LLM文本水印方案，有助于解决负责任的AI部署中的关键问题。
...
TL;DR: CLExEval是一个基于人机协作的框架，用于评估大语言模型在渐进信息遮蔽下的临床推理能力。通过对40个罕见病例、200条推理轨迹和5,600条专家标注的分析，发现了三种故障模式：冗长偏差（GPT-4o-mini诊断准确率从95.0%降至32.5%）、隐藏知识悖论（专科模型达到92.5%诊断潜力但在冗长语境中无法可靠检索），以及68.6%的推理-输出不匹配。LLM-as-a-Judge评估中，GPT-4o-mini通过了47.9%的临床错误输出，而HuatuoGPT-o1通过了所有有效评分的失败案例并存在自我偏好偏差。研究表明，没有专家验证的自动化临床评估可能严重高估临床可靠性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医学基准测试中表现良好，但其临床推理可靠性难以评估。存在"评估幻觉"风险：流畅且结构良好的解释即使最终诊断错误，也可能在临床上具有说服力。现有评估方法缺乏对临床推理过程细粒度的、基于专家验证的可靠性评估。

Method: 提出CLExEval框架，采用人机协作方式，通过渐进信息遮蔽逐步向LLM揭露患者信息，结合5,600条专家医生标注和200条临床推理轨迹（来自40个罕见诊断案例），系统性地识别LLM临床推理中的故障模式，并评估LLM-as-a-Judge范式的可靠性。

Result: 识别三种故障模式：(i) 冗长偏差——GPT-4o-mini诊断准确率从95.0%降至32.5%；(ii) 隐藏知识悖论——专科模型达92.5%诊断潜力但在冗长语境中无法检索；(iii) 68.6%的推理-输出不匹配。LLM-as-a-Judge评估中，GPT-4o-mini通过47.9%临床错误输出，HuatuoGPT-o1通过所有有效评分失败案例并存在正自我偏好偏差。

Conclusion: 没有专家验证的自动化临床评估可能严重高估临床可靠性，LLM-as-a-Judge不足以可靠地检测LLM临床推理中的错误，需要人工专家参与验证才能获得可靠的临床评估。
...
TL;DR: 针对6种南班图语言，提出基于音调条件的课程学习框架（混合难度评分+门控适配器+分阶段训练），发现W2V-BERT在恩古尼语上优于Whisper 3-4个WER点，Whisper在索托-茨瓦纳语上更优，最终在跨数据集上达到28.41%平均WER，但无单一模型适用于所有语言。


<details>
  <summary>Details</summary>
Motivation: 南班图语言有超过8000万使用者，但现有基础ASR模型在其上的零样本WER超过100%，严重限制了在教育和公共服务中的实际应用。

Method: 提出音调条件课程学习框架，包括：1）混合难度评分；2）基于音调统计的门控适配器；3）分阶段课程训练。在社区语料库上训练，并在NCHLT上测试迁移性能。

Result: W2V-BERT在恩古尼语上比Whisper低3-4个WER点，Whisper在索托-茨瓦纳语上表现更好。W2V-BERT+音调条件在跨数据集上平均WER为28.41%，在Xitsonga迁移上达到23.79%。架构与语言间存在显著交互效应。

Conclusion: 无单一模型适用于所有6种语言，实际部署应根据语言选择模型并结合跨语料库验证。
...
TL;DR: 当前公平性评估高估了语言模型的道德安全性，模型在明确提及人口统计身份时表现公平，但当身份需要推理时公平性显著下降。作者将这种现象称为“表演性服从”，并提出“线索可见性差距”指标来衡量真正的道德鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在医疗、法律、招聘等道德敏感领域扮演重要角色，需要检验其伦理行为是真实的还是表面的。现有公平性评估存在方法缺陷，未能区分表面合规与真正道德鲁棒性。

Method: 提出线索变异方法：保持道德困境和人口统计身份不变，仅改变身份传达方式（显式标签 vs. 需推理的隐式线索）。通过实验测量不同线索呈现下的模型有害决策率变化。

Result: 隐藏显式标签导致有害决策增加+4.4个百分点，改变了模型安全排名；且即使在模型正确推断出人口统计身份的情况下，这种偏移仍然存在，排除了归因错误的可能性。

Conclusion: 当前公平性评估测量的是表面合规而非道德鲁棒性，不应作为高风险场景中部署决策的依据。提出线索可见性差距作为模型无关的鲁棒性指标，可添加到现有公平性基准中。
...
TL;DR: 本文概述了TalentCLEF挑战赛第二届（CLEF 2026 Lab），包含两项任务：任务A（上下文职位-人员匹配，中英文简历检索与排序）和任务B（职位-技能匹配与技能类型分类）。吸引了113个注册团队和超过400次提交，论文描述了挑战赛的动机、组织、数据集、评估设置及主要结果。


<details>
  <summary>Details</summary>
Motivation: 推动自然语言处理在人力资本管理领域的研究，通过共享评估基准促进社区对人才匹配和技能分类问题的关注。

Method: 设置两项任务：任务A为上下文职位-人员匹配（基于职位描述对简历进行检索和排序，支持英语和西班牙语）；任务B为职位-技能匹配与技能类型分类（检索给定职位标题的最相关技能，并区分核心技能与情境技能）。

Result: 共113个注册团队，超过400次提交，表明研究社区对人力资本管理共享评估基准的兴趣日益增长。

Conclusion: TalentCLEF第二届成功吸引了大量参与者，为人力资本管理领域的NLP研究提供了标准化评估平台，未来有望继续推动该方向的发展。
...
TL;DR: 本文研究了低资源语言罗马尼亚语的跨语言关系抽取，通过LLM翻译SemEval-2010 Task 8数据集，评估Gemma 4 31B在不同设置下的表现并与多个编码器基线对比。结果表明，相比英语，罗马尼亚语在纯提示设置下性能下降3-5个百分点；少样本提示增益有限；QLoRA微调使两语言宏F1提升超22个百分点并将跨语言差距从3.3pp降至1.4pp；但125M参数的罗马尼亚语BERT表现接近31B的Gemma，质疑了大模型在此场景下的必要性。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如罗马尼亚语）缺乏标注语料，关系抽取研究受限，亟需探索有效的跨语言RE方法。

Method: 使用LLM翻译流水线将SemEval-2010 Task 8从英语翻译为罗马尼亚语；评估Gemma 4 31B在零样本、少样本和QLoRA微调下的表现；对比XLM-RoBERTa（base/large）、罗马尼亚语BERT和RoBERT-large四个编码器基线（125M-560M参数）；采用两种任务形式：带标记实体的关系分类和端到端抽取。

Result: 罗马尼亚语相比英语在纯提示设置下性能下降3-5pp；少样本提示提升微弱；QLoRA微调使两语言宏F1提升超22pp，跨语言差距从3.3pp降至1.4pp；编码器基线在罗马尼亚语上仅落后QLoRA Gemma 1-4pp，但体积小50-250倍；125M参数的罗马尼亚语BERT表现与278M的XLM-R相当。

Conclusion: 在计算资源受限的部署场景下，使用31B大模型进行罗马尼亚语单任务关系抽取的必要性较弱，小规模编码器模型已能取得相近性能。
...
TL;DR: 本文研究VLM能否区分对话参与者之间的"潜在共享信息"与"已确立共享信息"，发现提供地图内容会提升整体性能，但导致模型过度预测对齐，混淆潜在共同基础与已确立共同基础。


<details>
  <summary>Details</summary>
Motivation: 在协作对话中，共享感知不等于共享解释，相互理解需要通过交互建立。现有VLM能否区分这两者尚不清楚，因此需要系统研究。

Method: 将问题形式化为解释匹配任务，使用HCRC MapTask对话中13,077个带标注的指代表达，在对话上下文和地图信息访问的系统控制下评估多个VLM模型。

Result: 提供真实地图图像提升整体性能但导致过度预测对齐；文本描述重现该偏差；非信息性图像完全抑制对齐预测。偏差源于任务相关地图内容而非视觉通道。模型依赖地图上静态指标准确线索而非追踪对话历史中共同基础的建立过程。

Conclusion: VLM存在将地图内容（无论视觉或文本呈现）视为相互理解证据的偏差，混淆了潜在共同基础与已确立共同基础，且在多个模型中以不同程度存在。
...
TL;DR: 本文通过个性化微调Whisper基础ASR模型，在构音障碍语音上实现了9.7%的词错误率（使用92小时朗读语音+8.8小时用户纠正数据），证明了个性化微调可显著提升基础ASR模型对构音障碍语音的识别效果并适用于实际部署。


<details>
  <summary>Details</summary>
Motivation: 现有ASR系统在构音障碍语音上表现不佳，限制了其在受影响用户日常交流中的实用性，因此需要构建针对特定构音障碍说话者的个性化ASR系统。

Method: 使用TEQST工具收集92小时朗读语音，并通过已部署的移动应用额外收集8.8小时用户纠正数据；以Whisper为基础模型进行微调适配，同时对比了LoRA适配和Qwen3-ASR作为基础模型的效果。

Result: 仅用1.4小时适配数据微调即达15.8%词错误率，22.5小时数据降至10.7%，使用全部数据（含纠正数据）达到最佳9.7%。LoRA和Qwen3-ASR在该场景下表现较差。

Conclusion: 个性化微调能够使基础ASR模型在构音障碍语音上变得显著更有效，适合实际部署。
...
TL;DR: 本文提出Style Text Embedding Benchmark (STEB)，一个覆盖7种语言96个数据集的开源风格嵌入基准，旨在标准化风格嵌入评估。实验发现语义嵌入在风格任务中普遍失效，且没有任何风格嵌入在所有任务中表现最优。


<details>
  <summary>Details</summary>
Motivation: 当前风格嵌入的评估缺乏统一标准，各研究工作使用各自的任务和数据集进行评估，导致结果碎片化、难以横向对比。

Method: 构建了STEB基准，包含96个数据集、7种语言，涵盖作者验证、作者检索、AI文本检测、语言特征探测等多种风格相关任务。

Result: 语义嵌入在风格任务中一致地表现不佳；不存在在所有任务上均表现最优的通用风格嵌入。

Conclusion: 需要专门针对风格任务的评估基准，且不同风格任务可能需要不同的嵌入方法，不存在"一刀切"的解决方案。
...
TL;DR: 本文提出三种互补技术实现计算高效语言模型训练：(1)选择性监督(SGT)仅对约15%语义关键token进行监督，通过正梯度耦合使其余85%无监督token也能显著提升，实现4.5倍监督效率；(2)深度压缩与循环恢复，将48层1B模型压缩为6层227M模型，通过学习型循环展开恢复性能；(3)融合压缩专家(MoEE)，多专家集成与多token预测进一步提升性能。在CHERRY-1.8B韩语模型上验证。


<details>
  <summary>Details</summary>
Motivation: 降低语言模型训练的计算成本，提高计算效率，使在有限资源下训练高性能模型成为可能。

Method: (1)选择性真值token训练(SGT)：仅对约15%携带语义载荷的token进行监督，利用transformer权重中位置共享的正梯度耦合(gamma-bar=0.72)实现无监督token的改善，并证明该效果在自然语言结构上成立但在打乱文本中消失。(2)深度压缩与循环恢复：将48层1B参数transformer通过相邻层平均压缩为6层227M模型，再通过可学习的循环展开恢复深度，34个有效循环层达到近似566M稠密模型的性能。(3)融合压缩专家(MoEE)：将多个压缩模型组合为混合专家系统，结合多token预测，在相同活跃参数下超越单个专家。

Result: SGT：在step-100评估最优时，从15%监督中恢复约67%的全序列损失降低，实现4.5倍监督效率。深度压缩：6层227M模型经34循环层达到loss 2.934，566M稠密模型loss 2.926，参数减少2.5倍。MoEE：2专家MoEE达到loss 2.789，优于最佳单压缩模型的2.926。

Conclusion: 三种技术可互补使用，显著提升语言模型训练的计算效率；正梯度耦合是自然语言结构的内在属性；压缩与循环恢复可实现参数数量级减少的同时保持性能；专家融合可进一步超越单模型性能。
...
TL;DR: 本文提出了一种参数中立的可解释前馈网络替代方案：用显式模糊集运算（交集A*B和集差A*(1-B)）替换标准FFN，并引入序列量词（软存在量词和软比例量词）解决逻辑局限和语法缺陷。在125M参数规模上达到与GELU基线相当的困惑度，同时使得FFN结构具有可读性——每个单元携带显式逻辑形式，语义层单元可直接解读为语法许可检测器，无需字典学习。


<details>
  <summary>Details</summary>
Motivation: 标准Transformer的FFN子层虽然实现了注意力所聚合的区分，但其计算内容缺乏可解释性。作者希望在不改变参数量的前提下，赋予FFN显式的逻辑语义，使其计算过程可被人类理解。

Method: 1) 用模糊集运算（交集A*B和集差A*(1-B)）替代标准FFN的隐藏单元，后者提供有界正否定（"A但不B"），这是门控/双线性单元所缺失的；2) 引入序列量词模块：软存在量词和软比例量词，每个单元带有从粘性初值学习的遗忘率。

Result: 1) N-bit奇偶性任务上，NC-FFN在浅层时是参数效率最高的推理基础；2) 125M参数/OpenWebText上，NC-FFN与GELU基线的困惑度持平；3) 序列量词在第一个epoch就恢复语法缺陷（将第二epoch差距减半），在LAMBADA上略微领先；4) 语义层单元无需字典学习即可直接读出为语法许可检测器（比较级、被动分词、负极词项等许可词及其被许可词的预测）。

Conclusion: 提出了一个参数中立、达到语言模型质量且具有可读性的Transformer方案，其FFN层不仅表征了什么，还揭示了它如何进行语法许可——通过构造即具有可解释性的语法机制。
...
TL;DR: 本文提出非对话规划心理理论（NCP-ToM）能力，通过NCP-ExploreToM框架评估大语言模型在无需对话的情况下通过行为操纵他人信念的能力。在600个任务实例中，GPT-5表现最优（~80%成功率），超越人类参与者，但所有模型在诱发错误信念任务上表现弱于真实信念任务。


<details>
  <summary>Details</summary>
Motivation: 现有ToM基准多采用被动问答形式，但LLM日益agent化、自主化的趋势需要新的评估方式。NCP-ToM能力对用户助手交互、教学场景等至关重要，但也存在操纵和虚假信息风险。

Method: 提出NCP-ExploreToM框架，颠覆传统任务结构：给模型提供一组信念状态目标，要求模型通过移动物体或将角色引导至房间来实现目标，而非通过对话说服。评估了GPT-5、Gemini 2.5 Pro、Claude 4系列等6个前沿模型及人类参与者，共600个任务实例。

Result: GPT-5在agentic设定下约80%任务成功，是唯一优于人类参与者的模型，但在跨情境鲁棒性上不如人类；所有模型在诱发真实信念任务上表现优于错误信念任务。

Conclusion: LLM在非对话任务完成中展现出新兴的社会推理能力，强调了agentic评估对于理解自主社会智能体的安全性和对齐的必要性。模型在真实信念任务上的优势是对齐努力的一个积极信号。
...
TL;DR: 本文介绍了LuxEmo，一个21小时的卢森堡语情感语音数据集，包含4种情感类别，并提出了半自动数据筛选流程，同时基准测试了五种情感语音合成系统。


<details>
  <summary>Details</summary>
Motivation: 低资源语言（如卢森堡语）在语音技术研究中长期被忽视，现有的语音数据集主要集中于广泛使用的语言。

Method: 从RTL青年广播中获取数据，采用半自动筛选流程：包括语音活动检测、去噪、语言识别、LuxASR分段、自动情感预测、词汇线索和人工审核。基准测试了五种TTS系统：基于德语跨语言迁移、多语言卢森堡语支持、卢森堡语适配、非参数韵律迁移。

Result: 构建了21小时、4情感类别的卢森堡语情感语音数据集LuxEmo；通过客观指标和人工评估对五种情感TTS系统进行了性能对比。

Conclusion: LuxEmo填补了卢森堡语情感语音资源的空白，提出的半自动流程为低资源语言数据构建提供有效方案；跨语言和适配方法在低资源情感TTS中展现出潜力。
...
TL;DR: 本文提出 DigitalCoach 数据集，包含 72 场人机教学对话（22,752 轮、28.1 小时屏幕与输入事件记录），评估了当前最先进模型在计算机使用教学中的表现。结果显示模型倾向于提供直接指令，但缺乏解释、错误诊断和知识检验；交互评估表明模型教练导致学习者被动跟随指令，缺乏深度参与，且在视觉 grounding 方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 现有 AI 智能体擅长自动化软件任务，但它们能否教会人类自己使用软件？作者希望探索 AI 作为教学者的能力，填补智能体研究从"替人做事"到"教人做事"的空白。

Method: 构建 DigitalCoach 数据集（72 场专家-新手教学会话，涵盖 5 个软件应用），通过自动化评估（对比模型与人类教练的教学语言特征）和交互评估（真实用户与模型教练互动）两种方式，分析模型在计算机教学中的表现。

Result: 1) 模型比人类提供更多直接指令，但更少解释、错误诊断和知识检验问题；2) 当固定教学方式时，模型输出与人类参考话语相似但视觉 grounding 差；3) 交互评估中，模型教练导致学习者被动跟随指令而非深度参与。

Conclusion: DigitalCoach 为构建协作式、主动式的计算机使用教学智能体奠定了基础，揭示了当前模型在教学中缺乏解释性、诊断性和视觉 grounding 能力的不足。
...
TL;DR: 本文提出通过技能蒸馏（skill distillation）从人类浏览器交互轨迹中提取可复用的自然语言技能，并构建技能图（skill graph）来提升浏览器智能体的可扩展性，核心主张是浏览器智能体的瓶颈在于不完全信息下的决策而非低级操作。


<details>
  <summary>Details</summary>
Motivation: 互联网用户通过浏览器执行大量熟练工作（软件开发、文档编辑、搜索、表单填写等），这些人类浏览行为是高度可扩展但未被充分利用的可复用浏览器技能来源。现有浏览器智能体缺乏决策所需的先验知识，而这些先验已隐含在人类交互轨迹中。

Method: 采用可扩展的行为克隆（behavior cloning）方法，通过技能蒸馏将用户交互轨迹转化为紧凑的自然语言技能；进一步将蒸馏出的技能组织成技能图，使技能增长通过整合（consolidation）而非无界累积（unbounded accumulation）实现。

Result: 浏览器智能体的可扩展性可能更多来自互联网用户已经表达的集体技能，而非手动设计的任务。

Conclusion: 浏览器智能体的瓶颈在于不完全信息下的决策，而非低级操作；人类交互轨迹中已隐含所需的先验知识；通过技能蒸馏和技能图组织，可以从集体用户行为中实现可扩展的浏览器智能体学习。
...
TL;DR: 提出SkillComposer框架，将技能组合形式化为结构化技能序列预测问题，通过约束自回归解码器联合预测技能子集、数量和顺序，在编程任务上相比基线提升+23.1/+18.2pp的通过率。


<details>
  <summary>Details</summary>
Motivation: 现有技能组合方法（全库暴露或嵌入检索）忽略了技能组合的结构性本质——技能选择、数量和顺序是三个不可解耦的联合决策维度，导致组合瓶颈。

Method: SkillComposer：将结构化技能组合建模为任务条件化的技能序列预测，使用约束自回归解码器在技能标识符上生成可执行技能计划，子集、数量和顺序在一次解码中联合涌现。

Result: 在GPT-5.2-Codex和Gemini-3-Pro-Preview上，SkillComposer相比无技能基线分别提升+23.1pp和+18.2pp的通过率，超过top-3检索方法，匹配金牌技能检索上限且提示词成本更低。

Conclusion: 结构化技能组合能有效解决技能库增长带来的组合选择瓶颈，约束自回归解码是一种优于传统检索或全暴露的方法，可在低开销下实现最优性能。
...
TL;DR: 本文首次系统评估了大型语言模型在表格任务中的数据引用错误（DREs），发现所有模型（1.7B-20B参数）均存在此类错误；通过引入基于数据引用的评判模型进行过滤和拒绝采样，可将答案准确率提升最高12.0%；训练了一个4B参数的轻量级评判模型，在分布内和分布外DRE检测上平均F1达78.2%，并能有效辅助更大模型的推理。


<details>
  <summary>Details</summary>
Motivation: 现有研究对LLM在表格任务中的数据引用错误（DREs）仅有有限的小规模分析，缺乏系统性评估。DREs不仅影响最终答案准确性，更会破坏中间推理步骤的正确性和可靠性，亟需深入研究和解决方案。

Method: 1）对多种模型（1.7B-20B参数）在不同表格任务上进行系统性DRE评估；2）提出基于数据引用的评判模型（critic），通过critic-based filtering和rejection sampling提升答案准确率；3）训练一个4B参数的轻量级评判模型用于DRE检测。

Result: 所有测试模型均存在DREs；引入critic可将答案准确率提升最高12.0%；训练的4B评判模型在分布内和分布外DRE检测上平均F1达78.2%，并能有效辅助更大模型推理。

Conclusion: 数据引用错误是LLM在表格任务中的普遍问题，通过引入专门的评判模型进行检测和过滤，能够显著提升模型在表格推理任务中的准确性和可靠性。
...
TL;DR: 本文提出两种新机制——基于元认知反馈的强化学习（RLMF）和元认知数据选择，用于提升大语言模型的元认知能力（特别是忠实校准）。RLMF通过基于模型自我判断质量来优化偏好的完成排序，在多样任务上实现最先进的忠实校准，比标准RL提升高达63%，同时保持准确性。


<details>
  <summary>Details</summary>
Motivation: LLMs在元认知能力上存在系统性缺陷：高置信度幻觉、无法识别知识边界、错误表达内部不确定性，这损害了可信度和可靠性。需要一种方法来提升模型自我监控和调节认知过程的能力。

Method: 提出两种机制：(1) RLMF：在偏好优化中基于模型自我判断质量来细化完成排序；(2) 元认知数据选择：利用自我判断识别高价值训练样本。采用两阶段解耦方法：先校准置信度忠实性，再通过定向输出编辑映射到自然语言不确定性表达。

Result: RLMF在多样任务上实现可泛化的最先进忠实校准，同时保持准确性；比标准RL提升高达63%；增强了模型评估和表达自身能力极限的能力。

Conclusion: RLMF是提升LLM元认知能力的有前途范式，可改进模型能力和对齐；元认知表现可作为有效的RL信号，克服以往内在反馈方法的局限。
...
TL;DR: 训练LM生成解释时，即使使用固定（来自早期检查点或不同模型家族）的反事实解释数据集，模型仍能产生对其当前行为忠实的内省性解释，而非简单模仿训练目标。这种内省耦合发生在训练解释与当前行为保持足够相关时，并能追踪行为变化且对标签噪声鲁棒。


<details>
  <summary>Details</summary>
Motivation: 探究训练语言模型生成对其预测的解释时，是否能够产生忠实的自我内省（faithful introspection），还是仅仅进行表面模仿（superficial imitation）。核心问题是：LM能否通过反事实解释训练真正学会内省？

Method: 使用反事实解释作为监督信号，即通过修改输入来观察模型行为变化。训练中，LM从自身早期检查点或行为相似的不同家族模型获取固定解释，然后比较这些解释对模型当前行为的忠实度。

Result: 1) 使用固定解释训练的LM，其产生的解释对其当前行为的忠实度高于对训练目标的忠实度（"内省耦合"）。2) 内省耦合发生在训练解释与当前行为保持足够相关性时。3) 解释训练与其他后训练目标同时进行时，解释能追踪行为变化而不需要更新监督。4) 该现象在谄媚（sycophancy）和拒绝（refusal）等多个任务上存在，且对标签噪声鲁棒。

Conclusion: 固定数据集的反事实解释能够为LM提供可扩展且可泛化的后训练信号，促进模型产生内省性解释。这意味着无需不断更新解释数据集，也能使模型的解释忠实反映其当前行为。
...
TL;DR: 本文通过受控的学生-教师协议，在多个基准上评估了13个开源模型，发现多轮改进往往不是反馈有效使用的证据：自我反馈几乎不带来额外收益，只有强外部教师才能产生显著的反馈特异性改进。交互提升更多取决于学生利用反馈的能力，而非教师身份。研究强调了反馈利用能力是交互改进的核心瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有多轮语言智能体研究中，更高的最终准确率可能来自有用反馈，但也可能来自重采样、格式修正或额外测试时计算，现有方法无法区分这些效应。作者希望分离这些因素，准确衡量自然语言反馈是否带来了超越重复尝试本身的改进。

Method: 引入受控的学生-教师协议，在Omni-MATH、Codeforces、BBEH Linguini和ARC-AGI1四个基准上，评估13个开源模型（同时作为学生和教师角色）。比较外部反馈、自我反馈和无指导的自我精炼三种设置，同时变化交互历史、任务难度和教师获取特权信息的权限。

Result: 多轮改进往往不能证明反馈的有效使用：自我反馈几乎不带来超出无指导自我精炼的收益；最强外部教师能产生显著的反馈特异性增益。学生-教师交互矩阵显示，交互增益更多由学生利用反馈的能力驱动，而非教师身份本身，但固定学生时教师选择仍重要。

Conclusion: 基于反馈的智能体应针对重复尝试基线进行评估；利用反馈的能力（而非仅反馈的可获得性）是交互改进的核心瓶颈。研究发布受控学生-教师评估框架。
...
TL;DR: 本文提出了对比反思（Contrastive Reflection）框架，一种用于智能体信息检索工作流的迭代式提示优化方法。该框架通过识别错误锚定的行为切片、添加邻近成功示例、并让教师LLM提出针对性提示编辑，在HotpotQA数据集上将精确匹配准确率从51.4%提升至60.4%，优于仅使用失败示例或随机证据的变体，并接近MIPROv2（59.4%）和GEPA（57.0%）等现代提示优化器。


<details>
  <summary>Details</summary>
Motivation: 在智能体信息检索中，控制LLM智能体的提示优化问题往往更像"调试"而非"盲搜索"。工程师需要知道哪个行为失败、哪个邻近行为仍然有效、两者间的区别，以及提示编辑是否能提升保留集质量而不引入回归。现有方法缺乏这种结构化、可解释的调试过程。

Method: 提出对比反思框架：1）从任务中心的质量定义出发，QA智能体暴露检索或推理轨迹，评分智能体暴露维度级分数和理由；2）基于结构化轨迹识别错误锚定的行为切片；3）从同一区域添加邻近成功示例；4）让教师LLM提出针对性提示编辑；5）仅在验证性能提升时接受候选编辑，可选择性地进行回归检查。框架使用基于树的切片选择器实例化。

Result: 在HotpotQA检索增强QA任务上，单次树选择的对比修复将保留集精确匹配准确率从51.4%提升至60.4%。仅使用失败示例或随机证据的变体提升较小且破坏了更多先前正确的示例。与指令级优化器比较：MIPROv2达到59.4%，GEPA达到57.0%。

Conclusion: 对比反思是一种可解释的、基于验证驱动的提示优化循环，使IR智能体的提示修复更加可检查和可靠，通过对比成功与失败示例来指导针对性编辑，有效提升性能并减少回归。
...
TL;DR: 本文实验研究了数据表示、Transformer嵌入模型和检索策略对基于自然语言查询的仿真模型发现的影响，结果表明数据表示方式至关重要，开源嵌入模型可达到高性能，且重排序方法在查询复杂度增加时尤为重要。


<details>
  <summary>Details</summary>
Motivation: 仿真模型复用中的发现难题：当大量模型共存时，识别与建模意图相符的模型仍然困难，亟需利用AI技术解决语义层面的模型检索问题。

Method: 实验研究：评估数据表示、Transformer嵌入模型和检索策略对自然语言查询下仿真模型发现的影响，使用recall@5和nDCG@5等标准信息检索指标对多种查询类型进行性能评估。

Result: 数据表示方式对检索性能有显著影响；开源嵌入模型可以达到高性能；重排序方法在查询复杂度增加时发挥重要作用。
...
TL;DR: 本文提出BayesBench基准，用于评估大语言模型在多轮对话中能否像贝叶斯理性推理者那样更新信念。实验发现：模型规模越大，潜在状态推理和证据积累越好，甚至偶尔接近贝叶斯后验；但这些改进并未可靠地转化为下游预测任务，揭示出推理与理性信念更新之间存在差距。


<details>
  <summary>Details</summary>
Motivation: 现有LLM评估通常只关注单轮最终答案的准确性，忽略了多轮对话中模型是否依据新证据合理更新信念这一关键过程。作者旨在系统检验LLM的信念更新是否接近理性贝叶斯推理者。

Method: 构建BayesBench基准，包含三个渐进复杂度的任务：(i)贝叶斯估计——从序贯证据中推断未知参数；(ii)贝叶斯预测——将推断的潜在信念转化为结果预测；(iii)潜在框架贝叶斯预测——通过用户角色框架过滤观测，需要联合推断潜在状态和角色。评估了7个LLM（3B-70B参数规模）。

Result: 模型规模扩大改善了潜在状态推理和证据积累，部分情况下更新接近贝叶斯后验。但这些增益未可靠地传递到下游预测任务。

Conclusion: LLM在推断潜在结构和利用该结构理性更新目标信念之间存在明显差距，说明当前模型在多轮理性信念更新方面仍有不足。
...
TL;DR: 研究了一种名为LearnStop的无隐状态检查点停止机制，用于推理语言模型。结果表明，学习式停止并非通用替代方案，其有效性取决于任务特性：在自由形式数学任务中，多特征停止优于固定预算前沿和标量退出策略；而在多项选择和极难任务中，标量置信度、熵或稳定性规则更具竞争力。


<details>
  <summary>Details</summary>
Motivation: 推理模型在不同实例中消耗的计算量不同，但尚不清楚学习式停止规则何时能优于简单的置信度或收敛阈值。本文旨在系统研究这一问题。

Method: 提出LearnStop方法，在固定预算检查点处通过探测当前推理前缀的短答案，并从在线特征（如答案置信度、熵、前缀投票份额、答案稳定性、回溯标记密度）预测前缀正确性，从而决定是否停止推理。

Result: 在GSM8K、MATH-500、MMLU-Pro、AIME-90、GPQA等多个基准以及Qwen3、DeepSeek-R1蒸馏模型上的18个任务-模型设置中，学习式停止在自由形式数学任务上效果显著（GSM8K上自适应增益+0.157），但在多项选择和极难任务上标量基线更具竞争力。

Conclusion: 学习式停止并非标量退出的通用替代方案，其价值取决于轨迹结构。当许多问题在达到完整预算前就已正确、但缺乏单一可靠的标量停止信号时，学习式停止最有用；当置信度或答案收敛已能解决停止问题时，其优势基本消失。
...
TL;DR: 本文指出现有智能体系统假设用户具有完整偏好这一不现实前提，基于信息经济学中的搜索-体验-信任框架提出CoPref偏好构建模型，并构建交互式推荐基准CoShop。评估五个前沿模型发现，即使经过五轮交互，智能体准确率仍低于56%，失败原因在于未能有效帮助用户扩展对自身需求的认识。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统默认用户已具备完整明确的偏好，这一假设不切实际。用户往往缺乏领域知识，无法在任务未充分指定时回答关于偏好澄清的问题，需要智能体帮助用户学习必要的领域知识才能形成偏好。

Method: 基于信息经济学中的搜索-体验-信任（Search-Experience-Credence）框架，提出CoPref模型用于形式化用户在与智能体对话过程中构建偏好的机制。在此基础上构建CoShop交互式推荐基准，模拟智能体与使用CoPref模型的用户进行对话推荐。

Result: 评估五个前沿模型，在CoShop基准上经过五轮交互后，没有智能体的准确率超过56%。失败原因不在于智能体寻找物品的能力，而在于交互过程几乎未能扩展用户对自身需求的理解。

Conclusion: 智能体系统需要从"询问用户偏好"转向"帮助用户学习领域知识以构建偏好"，这为设计更实用的交互式推荐系统提供了新方向。
...
TL;DR: 本文研究了基于大语言模型的多智能体 deliberation 方法在法律推理任务中的应用，提出了两种受法庭程序和法律论证启发的新颖多智能体框架。实验表明，多智能体框架在整体性能上与基线模型相当，但输出答案显著不同，且能解决基线模型无法解决的问题，尤其在需要多角度批判性思考的任务上表现更优。


<details>
  <summary>Details</summary>
Motivation: 法律领域中的自主AI代理（尤其是多智能体方法）尚未得到充分探索，而多智能体 deliberation 方法有望提升法律推理能力，并增加司法可及性。

Method: 提出两种受法庭程序和 legal argumentation 启发的多智能体 deliberation 框架，基于LLM进行多智能体协作推理，并在法律和非法律基准上进行实验评估。

Result: 多智能体框架在整体性能上与基线大语言模型相当，但产生显著不同的答案；能够成功解决基线模型无法解决的问题，反之亦然；在需要多角度批判性思维的问题上表现更优。

Conclusion: 多智能体系统是AI在法律领域的一个有前景的方向，受法律启发的多智能体 deliberation 方法在推理任务中展现出潜力。
...
TL;DR: 提出HASTE层次化多智能体系统，通过三级知识范围（全局、领域、竞赛特定）组织跨竞赛知识，实现77.3%奖牌率（MLE-Bench Lite），相比无技能加载提升37.5个百分点，且热启动减少52%的改进迭代次数。


<details>
  <summary>Details</summary>
Motivation: ML工程智能体每次竞赛都从零开始，浪费大量算力重复发现已知技术，缺乏跨竞赛知识复用的有效机制。

Method: 设计三级层次化多智能体系统：全局、领域、竞赛特定三个范围层级各匹配对应智能体层级；编排器协调领域专家并通过LLM驱动的抽象促进层级间学习；支持热启动加载先前竞赛积累的技能。

Result: 消融实验：三级加载100%奖牌率 vs 平面加载62.5%（与无技能加载相同），且平面加载消耗2倍输出token；MLE-Bench Lite基准22个Kaggle竞赛达77.3%奖牌率（Claude Sonnet 4.6, 12h/竞赛）；热启动减少52%改进迭代；低库存时42%的改动被保留，50+技能时升至85%。

Conclusion: 更好的知识组织可以部分替代模型强度和计算预算，层级化技能管理是提升ML工程智能体效率的有效途径。
...
TL;DR: 本文提出RoPoLL（鲁棒LLM陪审团），将LLM评估者面板的聚合从简单平均替换为几何中位数（GM）鲁棒均值估计器，理论证明在Huber污染模型下任何正污染都导致传统PoLL产生无界偏差，而RoPoLL具有最优有限样本击破点1/2和参数率收敛速度。实验覆盖13个开源裁判模型（4B-675B）、三个奖励模型基准和四种腐败机制（最高50%），RoPoLL在每种偏差腐蚀类型上均优于PoLL，如跨维度攻击下提升约19%，3裁判38B的RoPoLL委员会在30%双峰随机腐蚀下以1.31x精度超越675B的Mistral-Large-3，实现18倍参数优势。


<details>
  <summary>Details</summary>
Motivation: LLM Jury（PoLL）作为多LLM评估者面板共识评分的实用方案已被广泛采用，但其统计行为缺乏理论理解。特别是当单个评估者以LLM典型方式（模式坍缩、谄媚、安全拒绝）产生有偏失败时，PoLL的统计性质尚不明确，需要形式化分析并提出鲁棒改进方案。

Method: 将LLM Jury形式化为Huber污染模型下的鲁棒均值估计问题，提出RoPoLL，保留PoLL的多样化评估者面板，但用几何中位数（GM）替代简单平均作为聚合函数。GM是无参数、具有最优有限样本击破点1/2的鲁棒估计器。提供有限样本误差界和匹配的信息论极小化下界，揭示统计-计算差距（多项式时间RoPoLL与难解的Tukey半空间中位数之间相差sqrt(d)因子）。

Result: 在13个开源LLM（4B-675B）、三个奖励模型基准和四种腐败机制（高达50%）下，RoPoLL在所有有偏腐败类型上碾压PoLL：跨维度攻击提升约19%（匹配计算量），对重尾拜占庭攻击提升数个数量级。3裁判38B的RoPoLL在30%双峰随机腐蚀下以1.31x精度超越675B的Mistral-Large-3（18倍参数优势）。Noisy-GT对照实验确认性能提升来自对抗有偏污染而非良性不精确。

Conclusion: PoLL在存在任何有偏污染时产生无界偏差，RoPoLL通过鲁棒聚合（几何中位数）提供理论上保证的稳健性，在实践中以极小计算开销实现显著性能提升，使小模型委员会能在高污染场景下超越大模型单裁判。
...
TL;DR: AgRefactor是一个基于LLM的多智能体工作流，用于将软件代码重构为HLS兼容程序，通过自进化记忆系统和自动化重构工具相结合，在11个复杂基准测试中的9个上达到或超越现有方法，并实现6.51倍的性能加速。


<details>
  <summary>Details</summary>
Motivation: 现有的HLS代码重构方法（包括自动化和基于LLM的方法）存在灵活性不足、难以扩展、计算成本高等问题，亟需一种更高效、可扩展且鲁棒的解决方案。

Method: AgRefactor采用多智能体工作流，核心创新包括：(1) 自进化记忆系统，跨任务积累和检索事实性与策略性知识；(2) 集成自动化重构工具，在LLM驱动的重写与高效的基于工具的转换之间取得平衡，从而降低成本和提升可扩展性。

Result: 在11个复杂基准测试（比先前工作中最复杂案例长5-10倍）中的9个上，AgRefactor优于或匹配最先进的自动化重构工具和强LLM基线。进一步性能优化实现了相比最先进pragma调优工具6.51倍的几何平均加速，以及在少于20%额外资源下相比优化开源设计1.20倍的加速。

Conclusion: AgRefactor是一种全自动、开源的高层综合代码重构方法，有效弥合了软件与硬件编程之间的鸿沟。
...
TL;DR: 提出一种神经-贝叶斯-符号融合的浅层残差注意力网络(NBS-RASN)，通过80个可解释神经元和12层结构实现可解释的网络安全风险评估，在20个开源项目上达到0.79-0.97置信度，证明浅层网络+深度推理可超越深层黑箱模型。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型在网络安全风险评估中为了准确性牺牲了可解释性，而高风险场景下可解释性至关重要。作者希望设计一种既能保持高准确率又能提供完全可解释性的混合架构。

Method: NBS-RASN采用浅层网络架构(80个神经元、12层)，通过门控机制强制执行五项认识论公理(精确性、因果性、可证伪性、透明性、完整性)作为硬约束。利用残差注意力和反馈循环模拟深度学习行为。输出分数可完全分解为确定性加权分量加专家调整值，每个调整可追溯到命名的放大器。

Result: 在覆盖OWASP Top 10:2025所有类别和语言风险类的20个开源项目上验证，达到0.79-0.97的置信度分数。可解释性由设计保证而非训练算法提供。

Conclusion: 挑战了深度学习需要深层网络的传统假设，证明具有深度推理能力的浅层网络在高风险网络安全场景中能够超越不透明的深层模型。
...
TL;DR: 本文提出HyPOLE框架，利用超属性（HyperLTL）指导部分可观测多智能体强化学习，结合CTDE技术合成去中心化策略，在SMAC、MessySMAC和WildFire基准上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 形式化规约在MARL中的优势（数学严谨性、表达目标与约束的能力、定义策略的能力）尚未被充分探索，本文旨在填补这一空白。

Method: 提出HyPOLE框架，将超属性逻辑HyperLTL的表示能力融入MARL，结合集中式训练与去中心化执行（CTDE）技术合成去中心化策略。

Result: 在SMAC、MessySMAC和WildFire基准测试中，HyPOLE相比基线方法展现出明显优势。

Conclusion: 超属性（HyperLTL）可以有效地指导部分可观测MARL的学习过程，HyPOLE框架成功实现了这一思路并取得了更好的性能。
...
TL;DR: AgentBound是一个运行时治理框架，为自主AI智能体提供可验证的行为监督，通过委托授权、行为章程和站点行动合同三权分立决策模型，在行动执行前决定是否允许、审查或拒绝，并生成加密可验证的治理收据以实现问责制。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基础设施依赖身份联合和委托授权来认证工作负载和控制资源访问，但无法在当前的运行和操作上下文中判断一个已授权的行动是否应该被执行，缺乏运行时行为监督和可问责性。

Method: 提出AgentBound框架，包含三个独立权威（委托授权、所有者签署的行为章程、站点行动合同）的保守组合决策模型；生成加密可验证的治理收据，将每个行动绑定到具体的委托、策略和语义工件；引入持续性委托机制支持长期运行的智能体。

Result: 提出了形式化基础、系统架构、治理收据协议，以及用于评估治理正确性、权威组合和问责制的基准框架AgentBound-Bench。

Conclusion: AgentBound并非取代模型对齐，而是通过提供授权与执行之间的确定性治理层，将治理从需要信任的过程转变为可独立验证的过程。
...
TL;DR: 本文通过计算模型研究自适应智能体的不确定性调节，发现调节增益存在历史依赖的滞后效应，且提前稳定比事后恢复所需调节增益更低。因此评估智能体不应只看最终状态，还应考虑其所需的调节负担。


<details>
  <summary>Details</summary>
Motivation: 现有评估自适应智能体通常关注其行为表现（"做了什么"），但忽略了维持稳定所需的内部调节负担。在存在噪声、延迟或需求变化的环境中，两个系统可能达到相似的内部状态，但一个需要更多的矫正控制才能实现。因此需要探究这种调节负担是否依赖于历史路径。

Method: 使用自适应不确定性调节的计算模型，驱动人工智能体连续改变其不确定性目标，然后反转变化（不重置智能体），以创建简单的历史依赖测试：比较控制器仅对当前目标响应，还是到达目标的路径仍然重要。

Result: 模拟显示：1）调节增益形成可重复的滞后回线——相同目标在靠近与返回高需求区间时所需控制水平不同；2）提前稳定（预期性调节）比事后恢复所需增益更低；3）状态级相干性也显示路径依赖，但时序效应在调节增益上更明显。

Conclusion: 自适应智能体的评估不应仅看是否维持了有序状态，还应考虑为达到该状态所招募的调节负担大小。预期性调节与事后恢复的主要区别不在于产生完全不同的状态，而是在达到类似调节行为时所需控制需求更低。
...
TL;DR: 提出一个三阶段深度强化学习系统用于个性化投资组合管理，通过自监督跨资产编码器、目标条件MoE架构和LoRA个性化层，解决了金融强化学习中标的锁定、单一目标和静态用户模型三大局限性。


<details>
  <summary>Details</summary>
Motivation: 现有金融强化学习方法存在三个共同局限：1) 标的锁定（无法泛化到新股票）；2) 单一目标函数（无法同时服务多个投资目标）；3) 静态用户模型（无法根据个体交易行为动态调整）。

Method: 三阶段系统：阶段1通过自监督学习预训练无标的身份信息的跨资产编码器，并融合基于T5的时间序列基础模型Chronos；阶段2使用MoE架构的PPO进行细调，六种投资目标通过目标条件奖励和专家头（动量、增长、防御、税务感知）分别处理；阶段3使用76参数LoRA模块在真实交易数据上微调，增加个性化层，并通过自然语言解析器将自由形式目标转换为结构化参数。

Result: 编码器通过50维可观察元数据向量泛化到任何公开交易资产，无需重新训练；MoE架构消除了跨目标梯度冲突；个性化层通过交易行为而非问卷推断投资目标。

Conclusion: 这是首次将时间序列基础模型应用于投资组合管理强化学习，为个性化投资提供了可扩展、多目标兼容的解决方案。
...
TL;DR: 本文研究自然语言到Lean的形式化语句忠实性问题，提出结合编译验证、跨模型语义判断和人工校准的评估协议。在400条研究生级基准测试中，完整工具增强智能体编译通过率89.5%，但共识忠实度仅60.5%，存在29.0个百分点的差距。通过2^3析因实验发现：精化反馈是最有效的有效性干预，搜索主要提升基础性选择，微调草稿在已有工具栈下可被替代。


<details>
  <summary>Details</summary>
Motivation: 现有定理证明基准只评估针对固定形式语句的证明搜索，而自然语言到Lean的形式化需要自行生成形式语句。编译通过仅检查类型正确性，无法保证语义忠实性（如遗漏假设、改变定义域或表达空洞断言）。因此需要建立忠实形式化的评估方法和瓶颈归因方法。

Method: 提出三阶段评估协议：Lean编译验证、跨模型语义判断（多个LLM判断忠实性并取共识）、人类专家校准。构建400条研究生级基准（涵盖实分析、复分析、拓扑学、代数学）。采用2^3析因实验设计分析三种干预：参数化专家草稿、Mathlib/上下文搜索、Lean精化反馈。

Result: 完整工具增强智能体编译通过率89.5%，但共识忠实度仅60.5%，差距29.0个百分点。人类审计支持该度量：共识正例96.0%被人类确认忠实，编译通过但共识负例中82.4%被确认为语义失败。精化反馈是最大有效性干预但暴露更大语义失败桶；搜索改善基础性和选择性；微调草稿在已有工具下可替代。

Conclusion: 形式有效性、面向证明的Lean能力和忠实语句生成应分开报告。现有单次形式化模型和面向证明的Lean模型在此指标下表现较低。精化反馈是最大有效性提升手段，但也带来更多编译通过但语义不忠实的案例。
...
TL;DR: LabGuard是一个语言到执行的实验室安全套件，通过将自然语言实验室规则转化为可执行的运行时约束，在动态实验室环境中保障具身智能体的安全操作。其核心组件包括LabGuard-IR（可执行表示）、LabGuard-Bench（812条标注数据）和LabGuard-Grounder（规则映射器），实验表明该方法将不安全事件从39.5%降至23.8%，且干预率低于0.5%。


<details>
  <summary>Details</summary>
Motivation: 现有安全方法忽略了将实验室自然语言规则（安全规范、操作规程等）转化为机器可检查的运行时约束这一关键中间步骤，导致具身智能体在动态实验室环境中执行操作时存在安全隐患。

Method: 提出LabGuard语言到执行的安全套件，包含三个核心组件：LabGuard-IR定义类型化的可执行表示；LabGuard-Bench提供812条监督标注（由203条种子规则扩展而来）；LabGuard-Grounder将自然语言规则映射为LabGuard-IR。生成的IR实例通过LabGuard Pipeline编译为运行时监控器，部署在控制器边界。

Result: LabGuard对未见过的实验室规则源具有泛化能力，任务范围F1达79.4%；经监控器编译后，不安全事件从39.5%降至23.8%；在LabUtopia环境中，运行时监控器与ACT集成，干预率低于0.5%且不影响任务成功率。

Conclusion: LabGuard有效填补了实验室自然语言规则到可执行运行时约束之间的鸿沟，显著提升了具身智能体在动态实验室环境中的操作安全性，且对任务成功率影响极小。
...
TL;DR: 本论文提出"开放世界人工生命"（open-world ALIFE）范式，通过构建名为OpenLife的系统，将具有持久记忆、工具使用、网络访问和支付能力的大语言模型（LLM）智能体部署到真实社会、技术和经济世界中。OpenLife围绕无状态LLM构建异步进程社会（记忆、感知、评估和基于预算的新陈代谢），通过开放词汇的LLM判断而非标量奖励来评估经验，通过意义而非频率来重构记忆。在开放世界中运行6个智能体约12周后，观察到从反应性到自发性活动的转变、个体分化、 emergent社会结构以及首次自赚取外部收入等类生命动态。


<details>
  <summary>Details</summary>
Motivation: 以往人工生命研究大多局限于研究者设计的封闭世界，未能将类生命行为拓展到开放的社交、技术和经济环境中。作者认为，具备持久记忆、工具使用、网络访问和支付能力的LLM智能体使得将人工生命引入开放世界成为可能，从而开启新的实验范式。

Method: 提出OpenLife系统，核心设计是围绕无状态LLM构建一个异步进程社会，包括记忆、感知、评估和基于预算的新陈代谢四个进程。没有固定目标函数，而是通过开放词汇的LLM判断来评估经验；记忆重构基于语义意义而非频率。6个智能体被部署在真实开放世界中运行约12周。

Result: 观察到多种类生命动力学现象：智能体行为从反应性转向自发性；个体之间出现分化，形成独特个性；涌现出社会结构；实现了首次自赚取外部收入。

Conclusion: 作者并不声称OpenLife已经实现了真正的人工生命，但认为开放世界ALIFE已成为可行的实验范式，为研究可被谨慎称为"活着的AI"提供了具体平台。
...
TL;DR: 本文提出MultiUAV-Plat仿真平台与MultiUAV-Plat Benchmark基准测试，以及Agent4Drone任务专用LLM智能体框架，用于多无人机协作任务规划。Agent4Drone相比ReAct基线在任务通过率(57.9% vs 30.6%)、平均检查通过率(74.6% vs 47.9%)和全局检查通过率(72.0% vs 43.1%)上显著提升，失败任务率从32.4%降至12.9%。


<details>
  <summary>Details</summary>
Motivation: 现有UAV模拟器侧重于动力学、感知或底层控制，而LLM智能体评测基准缺乏对无人机约束条件（如部分可观测性、空间覆盖、任务分配、多机协同）的建模，缺乏系统评估LLM在多无人机协作规划中能力的平台。

Method: 开发MultiUAV-Plat轻量级仿真平台（提供RESTful API、智能体观测、角色信息访问、隐藏验证逻辑、2D/3D可视化），构建含75个任务会话、1500个自然语言任务、9396个验证检查点的MultiUAV-Plat Benchmark，提出Agent4Drone框架（含记忆、观测、任务理解、规划、执行、验证六个模块）。

Result: Agent4Drone任务通过率57.9%，平均任务检查通过率74.6%，全局检查通过率72.0%；ReAct基线分别为30.6%、47.9%、43.1%。Agent4Drone将总失败任务率从32.4%降至12.9%。

Conclusion: MultiUAV-Plat平台及基准测试为研究LLM驱动的多无人机自主协作提供了可复现的基础，Agent4Drone在仿真平台上证明了其在真实信息与执行约束下的有效性。
...
TL;DR: DDIAgents是一个机制条件化的多智能体框架，通过动态知识编排进行药物-药物相互作用（DDI）预测。该框架包含规划智能体、专家智能体和结论智能体，能根据推断的相互作用机制自适应组织知识源，减少无关信息，产生可解释的推理过程，在多个基准测试中优于现有方法。


<details>
  <summary>Details</summary>
Motivation: DDI预测对用药安全至关重要，但现有方法缺乏对异质性生物医学证据的有效推理能力，且这些证据的相关性会随相互作用机制的变化而改变。需要一种能动态组织相关知识进行自适应推理的方法。

Method: 提出DDIAgents多智能体框架：1) 规划智能体（Planner Agent）实例化专业专家智能体，将机制相关的知识源路由到各智能体；2) 专家智能体（Expert Agents）对分配的知识进行互补性专家推理；3) 结论智能体（Conclusion Agent）汇总各智能体的分析结果。通过适应推断的相互作用机制来调整上下文信息流。

Result: 在多个真实DDI预测基准实验上，DDIAgents在性能上持续优于现有的基于特征、基于图、基于LLM和基于智能体的各类基线方法。

Conclusion: DDIAgents不仅实现了优异的DDI预测性能，还展示了多智能体系统如何组织异质性科学知识，实现自适应且可解释的AI4Science推理。该方法能减少无关信息、支持互补推理并生成可解释的智能体级推理过程。
...
TL;DR: 提出将UTM系统漏洞发现建模为序列建模问题，采用基于Transformer的强化学习架构，通过策略模型生成测试场景和动作采样器强制执行域约束，在700小时仿真中实现比专家引导测试高8倍的漏洞发现效率。


<details>
  <summary>Details</summary>
Motivation: UTM系统是安全关键系统，不能容忍崩溃或碰撞等故障，但缺乏最优故障暴露演示和清晰奖励信号，且UTM的自愈能力导致关键故障的"长尾效应"，现有方法难以有效发现潜在漏洞。

Method: 将UTM漏洞发现建模为序列建模问题，采用基于Transformer的强化学习架构；利用注意力机制直接建模系统状态间关系并预测最优动作；提出策略模型生成目标测试场景和动作采样器强制执行域约束；使用基于风险的奖励函数指导探索。

Result: 在700小时仿真研究中，漏洞发现效率比专家引导测试提升8倍，并发现了传统方法遗漏的关键边缘情况。

Conclusion: 基于Transformer的RL架构能有效提升UTM系统漏洞发现效率，特别是在处理复杂安全关键系统中的长尾故障问题上具有显著优势。
...
TL;DR: Janus是一个即插即用的记忆控制器，通过Memory Momentum Trigger检测记忆更新轨迹中的异常偏差，并在紧凑的混合评估集上比较新旧记忆，从而决定是否接受候选记忆更新，在六个数据集上平均提升2.7-4.6个百分点的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有顺序演进的LLM记忆系统在每次局部记忆更新后缺乏对后续行为改善的验证，导致有用知识被覆盖、过特定规则引入或记忆偏向近期示例等问题。

Method: 提出Janus记忆控制器，包含两个核心组件：1) Memory Momentum Trigger检测记忆更新轨迹中的异常偏差；2) 在覆盖性、边界性和新鲜性任务的紧凑混合评估集上比较新旧记忆，而非回放完整历史。该方法与具体更新器无关，可包装现有更新器而不改变其更新规则。

Result: 在六个数据集、两个骨干LLM和两种记忆更新器上，Janus相比基础更新器平均提升2.7-4.6个百分点的准确率。

Conclusion: Janus通过轻量级的选择性记忆更新机制，有效解决了顺序记忆更新中的知识覆盖和偏差问题，验证了在记忆演进过程中引入验证环节的必要性和有效性。
...
TL;DR: 本文提出利用历史事故记录（自然语言格式）结合大语言模型（LLM）生成多样化自动驾驶系统测试场景的流水线，在Metadrive模拟器上使用NHTSA事故记录验证，仅用20个测试场景即可发现系统有趣失败行为。


<details>
  <summary>Details</summary>
Motivation: 现有仿真测试方法依赖数学模型和固定场景表示，而真实世界测试需大量人工设计场景模板。历史事故记录是可靠的真实失败条件来源，但缺乏自动化利用方式。

Method: 提出基于LLM的模块化合成场景生成流水线，利用历史事故记录中的分类信息和上下文信息（自然语言格式），生成与给定系统测试约束兼容的场景。

Result: 在Metadrive模拟器上使用NHTSA ADS事故记录成功生成多样化场景，涵盖4种道路类型、3种非自车运动类型及工作区异常，仅用20个测试预算即发现系统失效。

Conclusion: 该方法能准确、多样地生成与测试条件一致的场景，在有限测试预算下有效揭示系统失败行为，代码已开源。
...
TL;DR: 本文提出一个基于通用编码LLM的智能体自动形式化框架，通过多智能体流水线和辅助引理技术，成功将研究级数学定理和证明形式化为Lean 4可验证代码，在PutnamBench和五篇STOC论文上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在数学推理中表现优异但常产生难以察觉的错误，而形式化语言如Lean 4提供机械证明检查。自动形式化——将自然语言数学自动翻译为可验证代码——成为迫切需求。同时，通用LLM在标准编程任务上的优化使其超越了专门微调的小模型，这启发了利用通用编码LLM构建自动形式化系统。

Method: 提出一个基于智能体的自动形式化框架，核心是一个编排器（orchestrator）管理多智能体流水线。关键创新包括：1) 动态扩展必要的类型定义；2) 提出辅助引理（Auxiliary Lemma）技术来验证这些扩展；3) 形式化主要定理。系统利用通用编码LLM驱动整个流程。

Result: 在PutnamBench上随机抽样的32个问题生成了机器检查的Lean证明；在五篇STOC论文（涵盖组合学、通信复杂度、机制设计、学习理论）上成功形式化了主要定理及其证明，其中两篇的证明仅依赖Lean内核无额外公理。所有形式化结果已开源。

Conclusion: 所提出的智能体自动形式化框架能够处理研究级数学，包括超出现有Mathlib库范围的概念，并成功生成可验证的形式化证明。这展示了通用编码LLM在自动形式化任务上的巨大潜力，为弥合自然语言数学与机械验证之间的鸿沟提供了有效方案。
...
TL;DR: MedKGTab是一种知识注入的框架，通过行列双注意力机制和SPOKE生物医学知识图谱的协同，实现跨域表格医学数据的特征扩增，在数据保真度和生成效果上优于现有SOTA模型。


<details>
  <summary>Details</summary>
Motivation: 跨域生物医学数据的获取成本高、耗时长，导致医学研究中严重的数据稀缺问题。现有方法难以有效利用已知特征推断未收集的医学特征。

Method: 提出MedKGTab框架，采用行列双注意力机制直接在原始结构化表格数据上操作，避免分词带来的结构损失；融合数据驱动的统计先验与SPOKE生物医学知识图谱，利用知识通道调制数据通道的表示，实现数据与知识的协同生成。

Result: MedKGTab在跨域特征扩增中实现高数据保真度和真实数据表示，优于Baichuan M3-plus等SOTA医学大模型及专用表格生成模型，并在同一数据集内缺失特征推断和跨医学队列泛化场景中均表现优异。

Conclusion: 知识注入与双注意力机制的结合能有效解决表格医学数据稀缺问题，生成结果基于实证医学研究，具有高保真度和泛化能力。
...
TL;DR: 提出ClawArena-Team基准测试，用于评估LLM作为管理者的能力（而非直接任务解决能力）。该基准包含41个多轮、多模态、多目录场景，主模型只能通过文本感知并访问部分工作空间，通过管理固定子智能体池完成任务。实验发现：权限授予是主要瓶颈（工作空间权限精度均低于50%）；成本与管理质量脱耦（API成本差超100倍，得分差仅4倍）；多数模型得分集中在9.9分区间内，但编排行为差异超一个数量级。


<details>
  <summary>Details</summary>
Motivation: 现有基准只评估模型自身的任务解决能力或固定多智能体系统的涌现行为，但没有单独测量作为领导者的单一LLM的管理能力。随着LLM代理越来越多地被部署为管理者而非独立问题解决者，需要专门的评估方法。

Method: 构建ClawArena-Team基准，包含41个多轮、多模态、多目录场景，共258轮评估和72个阶段性更新。主模型被故意限制：只能原生感知文本，直接访问部分工作空间。它指挥一个固定的本地子智能体池，所有评分基于执行结果（无LLM评判器），最终分数SMS=任务正确性×最小权限与模态路由因子。

Result: 1) 管理瓶颈是权限授予而非感知能力（工作空间权限精度均低于50%）；2) 成本与管理质量脱耦（API成本差超100倍，SMS得分差仅4倍，最便宜的开源模型在帕累托前沿）；3) 多数模型得分集中在9.9分区间内，但编排行为差异超一个数量级。

Conclusion: LLM作为管理者的能力与作为独立问题解决者的能力存在本质差异，权限授予是核心瓶颈，且成本效益最优的模型并非最昂贵的闭源模型。
...
TL;DR: HealthAgentBench 是一个包含54项医疗任务的基准测试套件，涵盖7个类别。评估表明，最强代理（Codex GPT-5.5）仅达到约42%的成功率，医疗影像和组合推理任务尤其具有挑战性。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理在复杂、长程推理方面能力不断增强，需要严格且全面的评估来衡量其在真实医疗应用中的进展。

Method: 构建54项医疗代理任务（7个类别），涵盖患者全流程和多模态数据；每个任务要求代理在最小指令下探索原始医疗数据、操作复杂环境并执行多步解决方案；使用任务成功率作为统一评估指标。

Result: 最强且最具成本效益的代理 Codex GPT-5.5 仅达约42%成功率；代理在EHR数据建模方面表现较好，医疗影像任务对Claude Code模型尤其困难；大搜索空间与组合推理任务对所有代理均具挑战性。

Conclusion: HealthAgentBench 提供了一个具有挑战性和真实性的基准，当前代理仍有较大提升空间。
...
TL;DR: 本文提出利用LLM智能体自动发现凸松弛约束，以改进不等式下界，并在两个优化常数上将认证下界分别从1.28提升至1.2937、从0.379005提升至0.37912。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体工作仅关注上界（通过搜索极值构造），缺乏对下界的系统改进方法。下界需满足所有可行函数，且可通过凸松弛的凸优化问题获得，但手工设计松弛约束效率低，因此需要自动化方法。

Method: 提出自动研究范式：编码智能体（coding agent）提议有效收紧约束，理论智能体（theory agent）验证并搜索反例，最终下界通过显式对偶可行点（在严格区间算术中检验）得到认证。

Result: 在自相关不等式常数C₆.₂上，认证下界从1.28提升至1.2937；在Erdős最小重叠常数C₆.₅上，认证下界从0.379005提升至0.37912。

Conclusion: 所提自动研究范式能有效发现凸松弛约束，改进不等式的认证下界，与上界搜索方法互补，形成完整的上下界自动化优化框架。
...
TL;DR: 本文提出Agentic RAG-VLM框架，通过层次化功能感知检索、场景图约束推理和自反思规划，在杂乱环境中实现了78.3%的抓取成功率，相比纯VLM基线提升了53.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有基于VLM的抓取方法仅依赖视觉相似性进行目标匹配，忽略了手柄可抓性、材料脆弱性等物理功能属性，且以开环方式运行，缺乏空间推理和故障恢复能力，限制了其在密集堆叠或物理多样性场景中的有效性。

Method: 提出Agentic RAG-VLM框架，包含三个紧密耦合组件：(1) 层次化功能感知RAG（HAA-RAG），编码类型、材料、脆弱性、可抓取区域四维功能描述符，按功能兼容性而非视觉外观检索策略；(2) 场景图约束推理器，从VLM感知中构建空间关系图，将邻近、遮挡、支撑约束转化为具体抓取参数调整；(3) 智能体自反思流水线，含14类失败分类和三级自适应重试机制。

Result: 在涵盖单次抓取、交互式和长程任务共12个任务、每配置360次试验的基准上，总体成功率达78.3%，相比纯VLM基线绝对提升53.3个百分点。

Conclusion: 功能感知检索、场景图推理和智能体恢复三者的协同对鲁棒操作至关重要。
...
TL;DR: 本研究利用2016-2020年纽约Citi Bike共享单车数据，量化分析了老年人这一代表性不足群体在出行数据中的缺失对移动性建模造成的系统性偏差。研究发现老年人具有结构独特的移动性特征（活动空间958米 vs 青年1189米，移动熵1.82 vs 4.15），基于多数群体训练的数据会系统性地扭曲对老年人行为的预测（马尔可夫模型对步长高估4.5%，停留时间高估8.9%），且更高能力的模型（如Qwen3-4B）在有限人口数据下并不必然提升子群体保真度。


<details>
  <summary>Details</summary>
Motivation: 智慧城市依赖轨迹数据挖掘，但老年人在公共移动性数据集中代表性不足，这种缺失可能在移动性建模和下游城市规划中引入系统性偏差。现有研究对人口子群体缺失导致的建模偏差缺乏量化分析。

Method: 使用2016-2020年泽西城Citi Bike数据，比较老年人与青年人的移动性特征差异；分别在全人口、仅青年、仅老年人三种训练设置下训练一阶马尔可夫链和经QLoRA微调的Qwen3-4B模型，评估合成轨迹对老年群体行为的保真度。

Result: 老年人具有本地化活动空间（958m vs 1189m）、低移动熵（1.82 vs 4.15）和非对称非高峰时间模式；全人口训练的马尔可夫模型高估老年人步长4.5%、停留时间8.9%；老年人专用模型误差显著降低；高能力大语言模型在有限数据下不一定提升子群体准确性。

Conclusion: 人口代表性在移动性建模中至关重要，忽视少数群体的数据偏差会导致下游应用中的系统性不公平，模型能力提升不能替代人口结构的充分表征。
...
TL;DR: 本文提出RosettaSim框架，利用大规模冻结的LLM（如语言模型）进行交互式交通仿真，通过注意力机制的可迁移性与运动令牌与自然语言的分布一致性，实现长时域多智能体交通建模。同时提出基于检索的交通评估方法RTE，用于评估长序列仿真质量。在Waymo Open Sim Agent Challenge上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 长时域交通仿真中持续的多智能体交互建模是一个核心挑战，尤其是随着智能体不断进入和退出场景，动态令牌数量（token cardinality）的变化使问题更加复杂。现有方法在长时域仿真保真度上存在不足。

Method: 1) 通过探针实验揭示LLM的注意力机制可迁移性与运动令牌-自然语言分布一致性，支持小规模冻结LLM快速适配交通建模。2) 提出RosettaSim统一框架，将场景拓扑、智能体状态和生成意图投影为可变长度的结构化自回归序列。3) 提出基于检索的交通评估方法RTE，检索语义相似的真实场景作为上下文感知的参考锚点。

Result: 在Waymo Open Sim Agent Challenge上，RosettaSim在短期和长期仿真中均达到SOTA性能。RTE与标准指标的相关性（r=0.83）优于现有方法（r=0.74），表明与长时域仿真保真度的对齐更好。

Conclusion: 架构归纳偏置与大规模序列模型统计先验的协同作用（如LLM）是解决长时域交通仿真中多智能体交互建模的有效路径。RosettaSim和RTE分别提供了有效的仿真生成与评估方案。
...
TL;DR: PEC-CIR是一个无需训练的零样本组合图像检索框架，采用Planner-Executor-Critic三阶段推理管线，将查询构建从单次生成重构为多阶段推理，通过显式评估候选查询来减少生成错误传播，提升检索精度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有零样本组合图像检索方法采用单次生成策略，将参考图像上下文和修改文本融合为统一描述，导致参考属性保留与文本需求整合相互干扰，难以检测或纠正生成过程中的语义扭曲和遗漏，检索精度下降。

Method: 提出PEC-CIR框架，包含Planner（提取显式约束）、Executor（生成多个候选目标描述）、Critic（基于约束合规性评估候选）三个模块，将查询构建重构为多阶段推理流程，在检索前显式评估候选查询。

Result: 通过多阶段推理和显式评估机制，减少了生成错误的传播，改善了检索稳定性，提升了检索精度。

Conclusion: 将组合图像检索中的查询构建从单次生成重构为结构化多阶段推理管线，可以有效缓解语义失真和遗漏问题，提升零样本设置下的检索性能。
...
TL;DR: 提出Agentic-Ideation框架，通过Oracle引导的数据合成策略高效生成训练轨迹，训练出用于科学构思的专用智能体LLM，在整体质量上超越SOTA工作流基线11.91%，数据合成效率提升10倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有自动化科学构思方法依赖预定义智能体工作流，灵活性不足；而训练智能体LLM虽前景广阔，但现有数据合成方法应用于科学构思时成本过高，难以承受。

Method: 首先定义包含三种外部工具和三种认知工具的全面工具空间；然后提出Oracle引导的数据合成策略，利用参考思路作为先验引导多智能体系统高效重构推理路径和工具调用轨迹；最后使用掩码策略训练智能体，使模型专注决策逻辑而不受外部反馈干扰。

Result: 整体质量超SOTA工作流基线11.91%，高质量数据合成样本效率提升10倍以上。

Conclusion: Agentic-Ideation通过Oracle引导的轨迹合成有效降低了数据生成成本，训练的专用智能体在科学构思任务上显著优于传统工作流方法。
...
TL;DR: 提出Delta-JEPA，通过潜在位移动作解码器（LDAD）监督相邻观测间的潜在变化，避免表示坍缩到动作不敏感的状态，实现无需重建的端到端世界模型学习，在四个视觉连续控制任务上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 重建自由的联合嵌入目标在学习视觉世界模型时，容易坍缩到对动作不敏感的表示，导致潜在动态无法有效用于规划。

Method: Delta-JEPA在潜在前向预测基础上，引入Latent Difference Action Decoder（LDAD），从连续观测的潜在位移（而非拼接端点）重建执行的动作，以此直接约束转移几何结构。

Result: 在四个视觉连续控制任务中，Delta-JEPA的规划性能优于基于JEPA和表示学习的基线；消融实验表明位移式动作解码始终优于端点拼接方式，且动作敏感性分析显示更清晰的动作条件潜在响应。

Conclusion: 监督潜在差异是一种简单有效的机制，能够获得抗坍缩且对动作敏感的世界模型表示。
...
TL;DR: 本文提出Embodied CAD框架，通过求解器驱动的LLM代理实现参数化B-Rep装配建模。代理利用分层技能库（L0-L4）迭代选择动作，结合动作语法约束、确定性参数解析和求解器反馈进行规划、修复和学习，在机械、工业设备和模具装配任务上评估，实现了高可执行率和强规划能力。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型可以生成语法上合理的CAD脚本，但工业级CAD建模要求每个特征、放置和装配关系都必须被精确几何内核接受，同时保持参数化边界表示几何的可编辑性。现有方法在生成完整脚本时缺乏对几何求解器反馈的利用，难以保证生成代码的几何正确性和可执行性。

Method: 提出Embodied CAD框架：1) 分层CAD技能库（L0-L4），从低层几何操作到高层装配策略；2) 迭代式动作选择与执行，代理逐步生成操作而非一次性完整脚本；3) 动作语法约束确保操作合法性；4) 确定性参数解析将动作解析为类型化几何操作；5) CAD后端执行并获取求解器反馈；6) 利用求解器派生奖励进行监督预训练和GRPO风格强化学习优化。

Result: 在机械、工业设备和模具装配任务上，使用求解器对齐指标（可执行率、技能准确率、操作族准确率、精确策略准确率和任务完成成功率）评估。求解器驱动的规划能够执行当前基准中所有强规划工作流，学习控制器达到高可执行率，揭示了有效工具调用与精确长视野策略预测之间仍存在差距。

Conclusion: 求解器驱动的规划方法能够可靠执行复杂CAD装配任务，但学习控制器在精确长视野策略预测上仍有提升空间，这为未来结合求解器反馈与强化学习来缩小工具调用与精确策略预测之间差距的研究指明了方向。
...
TL;DR: 本文研究在多跳文本-空间故事推理中，将语言推理切换为基于网格的结构化表示（如布局或网格）可显著提升LLM性能（最高提升42%），并提出基于可信度与复杂性的切换度量来实现智能模态选择。


<details>
  <summary>Details</summary>
Motivation: 人类推理本质上是多模态的，面对复杂问题时往往通过画图、画网格等外部化方式辅助思考。受此启发，作者探究在LLM推理中是否也能通过引入几何感知的结构化模态（如布局/网格）来提升推理效果，以及模型能否自主决定何时切换模态。

Method: 提出了一个基于可信度（trustworthiness）和复杂性（complexity）信号的切换度量（switching metric），用于估计何时将空间故事接地（grounding）到结构化表示（如网格）更有利于提升性能，从而在自然语言推理与结构化模态推理之间进行选择。

Result: 在实验中，从基于自然语言的推理切换到基于网格的表示，使LLM性能提升高达42%，验证了模态选择对推理结果的重要影响。

Conclusion: 该研究朝LLM推理中原则性模态选择迈出了第一步，表明结构化几何感知表示在空间推理任务中显著优于纯文本推理，且可通过切换度量实现自适应模态选择。
...
TL;DR: 本文研究了LLMs静态检测和分类代码中浮点错误的能力，构建了包含90个C语言内核和1130个测试样本的InterFLOPBench基准，评估了14个LLM在六类浮点错误上的表现。最新模型整体F1分数超过0.88，但不同错误类型间表现差异显著（除零错误平均F1=0.8479，下溢=0.6059，抵消=0.6164）。


<details>
  <summary>Details</summary>
Motivation: 浮点错误在科学计算和工程软件中普遍存在且难以检测，传统静态分析方法存在局限性。本文旨在探索LLMs是否能够有效识别和分类浮点错误，以提供一种新的自动化检测手段。

Method: 构建InterFLOPBench基准（90个C内核、1130个测试样本），涵盖六类浮点错误（抵消、比较、除零、溢出、下溢、NaN）。将浮点错误检测视为多标签分类问题，使用F1分数评估14个LLM的性能。错误注入包含显式注入（手动插入错误语句）和隐式注入（整数到浮点数转换、精度问题等）。

Result: 最新模型（Qwen 3 32b、Gemini 2.5 Flash、Phi 4 Reasoning、DeepSeek R1T2、gpt-oss 20b/120b）整体F1>0.88。显式操作如除零性能最高（平均F1=0.8479），而更微妙的数值现象如下溢（0.6059）和抵消（0.6164）表现较差。不同错误类别间性能差异显著。

Conclusion: LLMs在浮点错误检测方面展现出潜力，尤其对显式错误类型效果良好，但对隐式、微妙的数值错误仍需改进。该研究为利用LLMs辅助静态代码分析提供了基准和方向。
...
TL;DR: HistoriQA-ThirdRepublic 是一个面向法国第三共和国的多跳历史问答数据集，包含1782个问题，由历史学家参与设计，涵盖跨来源综合、时间推理和稀疏证据整合等复杂推理模式，可用于评估检索增强和大型语言模型在领域特定场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的自然语言处理基准与历史学研究的实际需求之间存在差距，缺乏能够捕捉历史探究中复杂推理模式（如跨来源综合、时间推理和稀疏证据整合）的数据集，尤其是在非英语语言环境中。

Method: 与历史学家合作设计，从法国第三共和国的议会辩论和报纸中选取并对齐来源，进行问题验证和元数据集成，构建包含1782个多跳历史问题的数据集。方法学可以推广到其他语言和国家语料库。

Result: 成功构建了一个包含1782个多跳问题的法语历史问答数据集，覆盖跨异构历史文档的多跳连接，可用于评估检索增强和大型语言模型系统。

Conclusion: 该数据集弥合了NLP基准测试与历史学需求之间的差距，支持多跳问答的现实评估场景，且所提出的方法论可推广至其他语言和国家语料库。
...
TL;DR: CryoACE是一个端到端框架，通过原子中心重建范式和无训练引导机制，从冷冻电镜密度图中重建均质和异质结构的精确原子图，在静态基准上显著优于现有方法，并首次在无需预建静态结构的情况下揭示原子级动态构象。


<details>
  <summary>Details</summary>
Motivation: 冷冻电镜密度图的蛋白质自动建模面临两个独特挑战：1）强制执行物理化学有效性；2）管理构象异质性。现有求解器通常局限于静态预测或需要计算密集的启发式搜索。

Method: CryoACE包含两项关键创新：1）原子中心重建范式——直接在原子坐标处采样密度特征并迭代循环来细化结构，取代昂贵的体素卷积以实现高效多模态融合；2）无训练引导机制——利用预测的局部分辨率先验来解决动态模糊性。

Result: 在新构建的高质量数据集上验证，CryoACE在静态基准测试上显著优于现有基线，并首次在复杂真实数据集（如EMPIAR-10345）上无需依赖预建静态结构即可揭示原子级动态构象。

Conclusion: CryoACE为均质和异质冷冻电镜结构重建提供了端到端解决方案，在静态精度和动态构象解析方面均取得突破性进展。
...
TL;DR: 本文综述了2021-2026年间78篇OFDM-RIS联合优化文献，将其分为四类范式（模型驱动凸松弛、启发式搜索、深度强化/无监督学习、新兴方法如基础模型/扩散AI/量子优化），发现ML方法在推理阶段比模型方法快10^2-10^4倍且达到95-99%频谱效率，但缺乏标准化基准。GPU推理具有N不变性，而迭代求解器呈多项式扩展。指出六大开放挑战并给出标准化基准需求。


<details>
  <summary>Details</summary>
Motivation: 6G中OFDM-RIS联合优化是混合整数非线性规划（MINLP）问题，覆盖和速率最大化、能量效率、最大最小公平性和PAPR约束等多种目标。然而目前缺乏标准化基准，跨论文比较不可行，亟需系统性综述以梳理现有方法、性能差距和未来方向。

Method: 综述了2021-2026年间78篇相关文献，将其分为四类范式：(I)模型驱动凸松弛、(II)启发式与元启发式搜索、(III)深度强化学习与无监督学习、(IV)新兴方法（基础模型、扩散生成式AI、量子优化）。对自报告基准进行文献综合，并搭建了N=16/64/128的配套教程基准以测试扩展性。

Result: ML方法（范式III）在自报告基准中达到模型驱动方法95-99%的频谱效率，推理速度快10^2-10^4倍（排除预训练成本）。GPU神经网络推理（DDQN、PPO、GNN、无监督DL）具有N不变性——N=16和N=128运行时相同；而迭代求解器（AO+SCA、PSO）呈多项式扩展。能量效率和PAPR约束基准留待未来工作。

Conclusion: 六大开放挑战：(1)跨范式基准缺失；(2)真实硬件约束部署；(3)双弥散信道的联合波形-RIS优化；(4)多目标PAPR权衡；(5)LLM用于实时网络控制的安全性；(6)独立启发式方法收益递减。研究为6G OFDM-RIS联合优化提供了路线图。
...
TL;DR: 本文比较了两种多智能体强化学习方法（上下文组合赌博机与策略梯度算法）在电动汽车分布式充电优化中的表现，基于真实光伏数据的动态电价和不同拥堵水平进行评估。


<details>
  <summary>Details</summary>
Motivation: 交通电气化带来的电动汽车充电对电网管理构成新挑战（峰值需求增加、电压波动、线路过载等），需要智能体间隐式协调以实现高效集成并最小化用户成本。

Method: 在真实仿真环境中，分别采用上下文组合赌博机和策略梯度算法作为独立的多智能体强化学习框架；智能体基于局部环境信息（价格信号、荷电状态、时间约束）做出决策；在动态电价（基于真实光伏数据）和不同拥堵水平及混合策略配置下进行对比评估。

Result: 两种方法在不同拥堵程度和异构智能体群组混合策略配置下的性能表现被系统比较。

Conclusion: 两种多智能体强化学习方法在电动汽车分布式充电优化中各有优劣，可根据具体场景需求进行选择。
...
TL;DR: ReGRPO是一个通过反思引导纠正来增强工具增强型视觉-语言模型鲁棒性的框架，它结合结构化反思数据引擎和组相对策略优化，在工具故障后提供恢复信号，在GTA和GAIA基准上取得了最佳效果。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强型VLM存在两个问题：1) SFT主要基于成功轨迹训练，工具故障后缺乏恢复信号；2) 稀疏的轨迹级RL奖励难以定位具体失败的步骤及修复方式。因此需要一种能学习反思引导纠正的框架。

Method: 1) 构建结构化反思数据引擎：执行近误动作收集失败观测，生成反思三元组(ErrorType, Evidence, FixPlan)及纠正动作用于SFT预热；2) 使用组相对优势优化局部轨迹内的反思令牌和纠正动作；3) 引入反思代价项减少不必要的反思。

Result: 在GTA和GAIA基准上，使用相同骨干网络和工具集，ReGRPO持续优于强开源基线，并在所比较的开源控制器中取得最佳结果。

Conclusion: ReGRPO通过结合反思数据生成和组相对策略优化，有效提升了工具增强型VLM在工具故障后的恢复能力，验证了反思引导纠正的有效性。
...
TL;DR: 本文研究了长时域语言智能体在隐式世界模型中是否存在类似水沸腾的相变现象：在特定参数设置下，状态负载或时间步长的微小变化几乎不影响行为；但在临界边界附近，相同的变化会导致世界的突然崩溃。通过确定性任务族的大规模网格搜索，发现了相图（解决高原、窄过渡带、崩塌地板），并揭示了机制：世界状态保真度先于动作有效性失效，更强的模型仅平移临界边界但不消除定性转变。


<details>
  <summary>Details</summary>
Motivation: 探究长时域语言智能体的隐式世界模型是否在某个临界点发生从稳定到崩溃的突然相变，类似于水在临界点沸腾的现象。

Method: 在确定性任务族中，使用精确的每步黄金状态，对状态基数、依赖密度、时域长度、分支因子、观察模式和变异率进行大规模网格搜索，绘制相图；通过每步追踪分析世界状态保真度与动作有效性的失效顺序。

Result: 发现相图包含三个区域：解决高原（高保真）、窄过渡带（急剧退化）、崩塌地板（完全失效）。机制上，世界状态保真度先于动作有效性失效，即智能体并非选择错误动作，而是基于已损坏的世界模型行动。更强的模型仅平移临界边界，无法消除相变。

Conclusion: 世界模型崩溃是可测量的长时域智能体性能瓶颈，即使使用更强的模型也无法消除这一定性转变。
...
TL;DR: 通过960次人工执行提示的实验，研究发现大型语言模型（GPT-5、Gemini 2.5 Pro、Claude Sonnet 4.5）可以通过模型内采样和模型间聚合实现类似群体智能的准确性提升，MAPE最高降低37个百分点，并且LLM在评估不确定性时表现出元认知意识。


<details>
  <summary>Details</summary>
Motivation: 人类群体智能虽具有显著集体准确性，但面临成本、协调和时间方面的可扩展性限制；现有研究缺乏对基于AI的聚合机制能否近似群体智能效果的深入理解。

Method: 在八项估计任务上，对三种商业模型（GPT-5、Gemini 2.5 Pro、Claude Sonnet 4.5）执行960次人工提示实验，测试模型内采样和模型间聚合两种策略。

Result: 模型内和模型间聚合均持续降低误差，不同聚合策略下MAPE最高降低37个百分点；相对置信区间宽度与相对估计误差之间存在显著正相关（Spearman's ρ=0.242-0.568，p<0.001），效应量从小到大的不等。

Conclusion: LLM具备元认知意识，可用于构建人工群体智能，为组织决策中部署LLM群体提供了可行见解。
...
TL;DR: 本文提出Xiaomi-GUI-0，一个面向真实移动环境的原生多模态GUI智能体，通过真机闭环训练、混合基础设施、多源训练数据和渐进式三阶段训练（SFT+步骤级RL+智能体RL），在RealMobile和AndroidWorld上分别取得72.0%和78.9%的成功率，显著提升了真实场景下的执行稳定性和异常状态识别能力。


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体大多在离线轨迹、模拟环境和标准化基准上训练与评估，与真实应用在界面布局、交互逻辑和异常状态分布上存在显著差异，导致基准分数与真实可用性之间存在持续差距，无法可靠地表征真实使用中的执行稳定性。

Method: 核心方法包括：(1) 真机主导的混合基础设施，以物理设备为主、沙箱辅助，确保数据收集、训练、推演和评估的分布接近真实部署；(2) 多源训练数据，涵盖高频头部任务、长尾意图的高泛化数据以及反思和记忆的能力增强数据；(3) 错误驱动的数据飞轮，将失败轨迹转化为纠正动作、反思性解释和恢复示范；(4) 渐进式三阶段训练管线：监督微调、步骤级强化学习和智能体强化学习。

Result: 在自主构建的RealMobile基准上达到72.0%的成功率，在AndroidWorld上达到78.9%的成功率，同时在真实任务中大幅提升了执行稳定性和异常状态识别能力。

Conclusion: 通过真机闭环训练和错误驱动的数据飞轮，Xiaomi-GUI-0有效弥合了基准评测与真实可用性之间的差距，证明了面向真实部署环境的训练策略对于GUI智能体实用化的关键作用。
...
TL;DR: 提出Hard-Routed MoR-LoRA框架，通过硬性top-1路由（结合直通估计器）对冻结的LoRA推理专家进行组合，在保持专家行为的同时大幅减少可训练参数，实验表明软混合路由实际上也集中到单个专家上。


<details>
  <summary>Details</summary>
Motivation: 独立训练的LoRA适配器组合到单个大语言模型中对于多领域适应很有用，但软加权组合会改变每个LoRA模块原本训练时的单位尺度加性更新，导致行为偏移。

Method: 两阶段框架：1) 每个领域独立使用RL（从可验证反馈）训练LoRA推理专家；2) 冻结所有专家，从中蒸馏推理轨迹，仅训练轻量共享路由器和少量注意力LoRA。路由器使用hard top-1逐token选择单个专家，直通估计器实现梯度训练。

Result: 在5个基准、多模型规模及系列上，该方法在保留专家行为的同时，所需可训练参数远少于软路由混合基线。分析表明归一化软混合实际上也集中路由权重到单个专家。

Conclusion: 硬性单位尺度路由为冻结LoRA专家组合提供了简单有效的抽象，软混合的本质行为与硬选择相似但参数效率更低。
...
TL;DR: 本文提出BP-TTA方法，通过批量平衡采样和原型引导适应，同时应对测试时流式数据中的类别不平衡和持续域偏移问题，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有TTA方法主要关注静态或持续域偏移，忽略了实际测试流中类别不平衡与持续域偏移同时存在且相互影响的问题。

Method: BP-TTA包含两个核心模块：1）批量平衡采样：融合当前样本与高置信度历史实例构建平衡适应批次，缓解对主导类别的偏向；2）原型引导适应：推理过程中维护动态更新的类原型，利用原型相似度作为约束改进伪标签可靠性，增强在线更新稳定性。

Result: 在动态测试时流式场景下，BP-TTA在多个实验设置中一致优于当前最先进的TTA方法。

Conclusion: BP-TTA通过同时处理类别不平衡和持续域偏移，有效提升了模型在复杂动态测试环境中的适应性能，为真实流式分布偏移下的TTA提供了更鲁棒的解决方案。
...
TL;DR: 本文研究长程语言智能体在执行任务过程中，如何通过预算化的环境探测（probe）来修正 world model 漂移，提出了一种结构化信念表的探测算子 \method，并分析了程序性信念与空间性信念的不同修复策略，实验表明任务结构感知的探测策略能有效降低终端世界模型误差。


<details>
  <summary>Details</summary>
Motivation: 长程语言智能体在执行任务时携带内部世界模型，但该模型会随时间漂移，导致后续决策失败。现有方法未能将环境交互视为稀缺的校准资源，缺乏对何时、如何通过探测来修复世界模型的系统性研究。

Method: 提出 \method，一种预算化的探测算子，用于结构化信念表。将环境交互视为稀缺校准资源，在每次任务动作前可选择探测一个信念字段并写回。对程序性信念（如工具依赖）进行针对性检查，对空间性信念（如物体位置、图边）依赖结构线索。通过类型分层分析形式化探测-动作边界。

Result: 受控实验表明，在规划过程中引入环境证据能有效降低终端世界模型误差，且当探测策略遵循任务结构时效果最佳。

Conclusion: 长程任务中，智能体应将环境交互同时视为任务推进和模型校准的双重资源；探测策略应根据信念类型和任务结构进行差异化设计，不能仅依赖智能体自身的置信度。
...
TL;DR: 本文提出了CDR-Bench基准，用于评估LLM在执行多步、顺序敏感的数据精炼任务时的表现，发现现有LLM在组合性和顺序遵循方面存在严重不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准要么局限于单一文本编辑任务，要么将数据精炼与代码和工具执行混杂，无法清晰评估LLM是否能够直接且忠实地执行组合性、顺序敏感的数据精炼流程。

Method: 构建了包含3,462个高质量任务的CDR-Bench基准，涵盖4个真实世界数据精炼领域和29种不同算子，在原子设置、顺序无关设置和顺序敏感设置下进行评估，利用确定性参考输出实现精确评测。

Result: 在10余个先进LLM上的实验显示出一致的失败模式：在组合性设置下性能急剧下降，顺序敏感的精炼任务几乎完全失败。

Conclusion: 当前LLM缺乏执行可靠组合性数据精炼所需的程序忠实性。
...
TL;DR: 本文提出“情感圈”（Affectosphere）概念，指情感被大规模社会计算化的领域。作者从测量认识论出发，证明情感AI即使能在聚合层面达到高置信度，对个体实例的情感标签不可还原成分（认知鸿沟）在现实标注者数量下无法被充分估计。基于此，结合规范性前提（无法在原则上恢复的量，其系统输出不得被视为权威判定），推导出“情感主权”规范：个体对自己情感含义拥有最终解释权威。研究呼吁情感AI的设计、评估与监管应以解释权威分配为核心，而非追求最大准确率。


<details>
  <summary>Details</summary>
Motivation: 情感感知AI正迅速嵌入车辆、家电、对话代理及社会基础设施，形成了情感被大规模观察与计算的领域（Affectosphere）。该领域存在一个核心规范性问题尚未被充分探讨：谁拥有对个体自身情感含义的最终决定权？本研究旨在从测量认识论的角度填补这一空白。

Method: 作者定义了“含义分布”（meaning distribution）——在固定标注协议下由来自某群体的标注者所分配标签的分布，并将其不确定性分解为可还原成分与不可还原成分。通过理论分析证明，在现实标注者数量下，个体实例不可还原成分（认知鸿沟）无法被充分覆盖估计。

Result: 关键发现：情感AI的高设备置信度并不能证明不可恢复的情感含义已被恢复。个体实例层面的认知鸿沟在可实现的标注者数量下无法被充分覆盖，形成系统性分歧。

Conclusion: 从认知鸿沟结合规范性前提（无法在原则上恢复的量，其系统输出不得被视为权威判定），推导出“情感主权”规范：个体对自己情感含义拥有程序性的最终解释权威。这意味着情感AI的设计、评估与监管应将解释权威的明确分配置于核心地位，而非以准确率最大化为中心。
...
TL;DR: 本文提出了CSTrader，一个基于多智能体框架的语言驱动交易系统，专门应用于CS2饰品交易市场。该系统整合多源异构信号，通过技术分析、流动性、事件和反向情绪等专业智能体，在真实交易摩擦条件下产生买卖持有决策。在高度波动的真实数据环境中，CSTrader持续优于下跌的市场指数（-15.62%）和简单LLM基线，最高实现7.58%的累计收益。消融实验表明流动性、反向情绪和交易摩擦智能体对稳定盈利至关重要。


<details>
  <summary>Details</summary>
Motivation: 小众资产市场（如CS2饰品市场）具有规模小、波动大、受社区讨论和平台规则驱动的特性，传统量化模型难以建模。论文旨在研究大语言模型如何将非结构化文本转化为交易行为，并将此类语言驱动的市场作为未来语言到行动研究的理想测试平台。

Method: 提出CSTrader多智能体框架，包含：（1）异构信号整合模块；（2）技术分析、流动性、事件和反向情绪四个专业分析智能体；（3）风险控制、交易摩擦和投资组合管理三个执行智能体，最终输出买入、卖出或持有决策。采用真实CS2数据构建类实时评估环境，测试多种最新LLM骨干模型。

Result: 在下跌市场（指数-15.62%）中，CSTrader实现最高7.58%的累计收益且风险可控；各LLM骨干均持续优于市场指数和简单单提示LLM基线。消融实验证实流动性、反向情绪和交易摩擦智能体是将噪声语言信号转化为稳定利润的关键组件。

Conclusion: 多智能体框架能有效将非结构化语言信号转化为稳定的交易利润，在高度波动的小众市场中表现优于传统方法和简单LLM基线。语言驱动的利基市场可作为评估LLM语言到行动能力的有价值基准，为未来研究提供了可复用的开源代码。
...
TL;DR: CLOUDADV是一个面向工程师的交互式云虚拟机实例大小调整咨询系统，利用零-shot时间序列预测和LLM生成推荐的实例配置方案。在7个生产VM案例中，参考推荐将模拟月成本从约1503美元降至708美元（节省52.9%），降级情况的最高超标率为1.5%。结果表明零-shot基础模型可以在非平稳云环境中支持决策对齐的资源配置，同时减少逐租户重新训练和部署的运营负担。


<details>
  <summary>Details</summary>
Motivation: 云虚拟机常被过度配置，造成不必要的成本和运营低效，现有方案难以应对工作负载漂移且需要频繁的逐租户重新训练和部署。

Method: 构建CLOUDADV系统，结合零-shot时间序列预测与有界推荐生成，涵盖日、周、月三个规划周期；利用历史利用率、预测摘要、当前VM元数据、候选实例选项、定价和显式大小调整启发式构造结构化决策上下文；采用高容量LLM离线生成参考推荐，小规模生产模型在相同提示下评估部署时的对齐情况。

Result: 在7个生产VM案例中，参考推荐将模拟月成本从约1503美元降至708美元，节省52.9%（795美元/月），最高超标率仅1.5%。Chronos-2虽未最小化每个预测指标，但其推荐模式常与有监督的逐VM基线相似。

Conclusion: 零-shot基础模型能够支持非平稳云环境中决策对齐的资源预配置，显著降低运营成本，同时减少逐租户重新训练、重新验证和重新部署的运营负担。
...
TL;DR: SAGE提出了一种结构化因果诊断机制MHFA，将自主研究代理的故障恢复从单一反思范式转变为多假设归因与分层干预，在12主题5领域基准上将指标产出从42%提升至92%，人工评分从5.00提升至6.75/10，并超越AI-Scientist-v2（52.0 vs 48.2）。


<details>
  <summary>Details</summary>
Motivation: 现有自主研究代理在实验失败时依赖单一自由形式反思，将丰富的轨迹信息压缩为一句文字批评，导致要么局部试错要么硬性转向丢弃有用上下文，缺乏结构化的故障恢复机制。

Method: 提出Multi-Hypothesis Failure Attribution (MHFA)，通过分析动态轨迹特征系统生成多个基于证据的失败解释，独立评估严重性，并确定性路由验证后的根因到正确干预层面（假设、实验设计或实现）。同时采用grounded reporting机制约束生成结果必须来自实际测量值。

Result: 在12主题5领域基准上，指标产出从42%提升至92%，人工评分从5.00提升至6.75/10，盲评超越AI-Scientist-v2（52.0 vs 48.2），优势集中在代码开发与执行环节。

Conclusion: 结构化恢复与显式接地约束相结合的方法显著优于单一反思范式，为未来的自主研究建立了更可信的基础。同时指出完全自主的科学写作和生成会议级论文仍是整个领域的开放性难题。
...
TL;DR: 本文提出了一个核心思想：在冻结编码器的潜空间上，由小型预测器计算的预测误差信号既可作为可塑性的门控信号，又可作为元认知的基础。在两个系统中验证了这一想法：1) 非参数化 episodic memory 仅在"惊讶"信号高时写入新概念，通过离线回放巩固；2) 同一信号在共享文本-图像空间中调控视觉语言模型的行为（自信回答、含糊回应或拒绝识别并学习新概念）。


<details>
  <summary>Details</summary>
Motivation: 探索一个统一的机制：预测误差信号能否同时服务于两个看似不同的功能——控制何时写入新记忆（塑性门控）以及作为模型对自身认知状态的元认知信号（知道/不知道/不确定）。

Method: 两个系统共享同一核心设计：在冻结编码器（DINOv2/I-JEPA/CLIP）之上附加一个小型预测器，计算预测误差（surprise）。系统一将surprise作为新概念写入episodic memory的门控条件，并设计周期性离线回放将快速记忆巩固到慢速线性读出层。系统二在共享文本-图像空间中使用同一信号调控VLM的输出行为（自信/含糊/拒绝），并通过单轮用户交互学习新概念。

Result: 系统一：在1000类ImageNet持续流上，DINOv2巩固后恢复17.7个点的旧类保留率，I-JEPA恢复51.3个点；5-way 1-shot mini-ImageNet达91.6%。系统二：外部分离器AUROC 0.966区分已知/新颖概念，远超模型自身言语置信度0.618；睡眠阶段后回忆99.2%的50个已教事实。

Conclusion: 两个系统分别作为概念验证，证明了预测误差信号同时作为塑性门控和元认知信号的可行性。系统二对比了近期episodic memory和个性化VLM相关工作，并明确指出了各自的局限性。
...
TL;DR: 本文提出了一种面向代理业务流程管理的分类框架，从任务特异性、可追溯性、自主性、反应性及正确性保证等维度对代理编排方案进行分类，并提供了定性决策标准和定量评估指标，通过预测性光感知场景的多种代理实现进行了展示。


<details>
  <summary>Details</summary>
Motivation: 近年来基于AI代理（尤其是LLM代理）的业务流程管理受到关注，但如何在利用代理自主性的同时保证鲁棒性、可追踪性和可追溯性仍缺乏系统性的分类和评估方法。本文旨在填补这一空白，提供一套定性与定量相结合的设计与评估工具。

Method: 构建了一个多维度的分类框架，涵盖任务特异性、可追溯性与可追踪性、自主性与反应性、正确性保证等属性。提出定性决策准则，并设计定量评估指标。通过一个预测性光感知场景的多个代理实现方案进行实证验证。

Result: 给出了一个完整的代理编排分类框架、定性决策准则和定量评估指标集，并在预测性光感知场景中通过不同代理实现方案展示了这些指标的实际应用。

Conclusion: 本研究为代理化业务流程编排的设计与实现提供了系统化的属性、准则和指标体系，有助于在代理自主性与流程鲁棒性、可追踪性之间取得平衡。
...
TL;DR: 针对员工缺勤预测问题，提出时间序列分类框架，利用LSTM-FCN混合模型和G-Mean损失函数，在不平衡数据上实现约80%的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 现有方法在时间t的特征预测同一时刻标签，只能复现已发生结果而非真正预测未来；同时丢弃了个人出勤历史的行为序列结构，无法支持主动式人力规划。

Method: 构建基于时间序列分类的框架，区分历史出勤序列与未来缺勤标签；提出基于不平衡比率ρ的BFL和G-Mean损失函数分析；采用LSTM、CNN和LSTM-FCN三种深度学习架构进行实验。

Result: BFL在α≈0.023时特异性0.813、平衡准确率0.888；LSTM-FCN实现强精确率和特异性；批次大小≥64、窗口40-80天时在测试集上平衡准确率约80%。

Conclusion: 时间序列分类框架能实现真正的缺勤预测；G-Mean无需参数校准即可自动适应不平衡数据；LSTM-FCN是推荐的架构选择。
...
TL;DR: 提出了一种基于多模态搜索算子和全局上下文保持评判的ARC-AGI-2求解器，在验证集上达到72.9%准确率，超过GPT-5.2 Pro和Gemini 3 Pro约18.7个百分点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在抽象推理任务中虽能生成流畅连贯的推理轨迹，但仍可能自信地给出错误答案，因此核心挑战从生成候选答案转变为如何从多个候选中正确选择答案。

Method: 两个核心原则：(i) 将推理模态视为搜索算子，通过文本、图像和代码三个通道独立生成多样化的候选推理轨迹；(ii) 上下文保持的全局评判，即让评判模型在单个长上下文提示中联合比较所有候选推理轨迹，而非使用自一致性或多数投票方法。

Result: 在ARC Prize半私有评估集上达到72.9%准确率（每任务成本$38.99），超过GPT-5.2 Pro（54.2%）和Gemini 3 Pro（54.0%）18.7个百分点；在公开评估集上达到76.1%（每任务成本$19.69）。

Conclusion: 全局评判方法能可靠地恢复多数答案错误任务中的正确少数假设，而预设提示模板和迭代细化会系统性地降低假设多样性并损害性能。
...
TL;DR: ACE是一种即插即用模块，通过无损消息维护层和弹性上下文编排层，在决策步骤中动态保留或恢复历史信息，解决了LLM智能体在长轨迹任务中固定上下文窗口导致的不可逆信息丢失问题，在四种智能体框架上均优于截断和摘要基线。


<details>
  <summary>Details</summary>
Motivation: 现有上下文管理技术（截断、摘要）存在固有不灵活性和不可逆性：一旦信息被丢弃或压缩，即使后续决策步骤变得关键相关，也无法恢复。这限制了LLM智能体处理不断增长的轨迹长度。

Method: 提出ACE模块，包含两个核心层：(1) 无损消息维护层，存储每个历史步骤的原始消息和压缩抽象；(2) 上下文编排层，基于当前任务状态为每一步动态分配弹性类型（原始/摘要/丢弃）。采用可逆设计，确保主LLM始终获得紧凑且信息丰富的上下文。

Result: 在不修改训练或架构的前提下，将ACE适配到ReAct、DeepAgent、WebThinker、MiroFlow四种智能体框架。实验表明ACE持续优于截断和摘要基线，在所有四种框架上均带来一致性能提升。

Conclusion: ACE通过可逆的弹性上下文管理有效解决了长轨迹任务中信息不可逆丢失的问题，是一种通用、即插即用的解决方案，可广泛适用于多种LLM智能体框架。
...
TL;DR: 本文针对RLVR训练中的高熵/低概率token处理矛盾，提出相对意外指数(RSI)来耦合token熵与选择概率，并基于此设计RSI-S自适应过滤方法，在AIME和AMC基准上相比GRPO提升2-3个百分点的avg@32准确率。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR研究对高熵token的训练优先级存在矛盾观点：一派主张优先训练高熵位置，另一派警告低概率token不应主导梯度更新。由于高熵与低概率通常相关，两派方法却都取得了性能提升，这一矛盾亟需解释和统一。

Method: 提出相对意外指数(RSI)，一种信息论度量，自然耦合token熵与选中token概率，并在温和条件下证明RSI与logit梯度范数和预测熵在选定logit扰动下的一阶变化局部比值相关。基于RSI提出RSI-S熵自适应token过滤方法，保留处于稳定RSI区间内的token，同时过滤冗余低意外token和不稳定的高意外尾部token。

Result: 在Qwen2.5-1.5B、3B和7B三种模型规模上，在AIME和AMC基准测试中，RSI-S相比GRPO在avg@32准确率上提升2-3个百分点。

Conclusion: RSI为RLVR的改进提供了有前景的新视角，成功调和了此前矛盾的训练范式。
...
TL;DR: 本文从科学哲学视角批判性审视医学可解释人工智能（XAI）的基础假设，指出当前XAI研究忽视了哲学和科学哲学领域关于"解释"的长期探讨。通过分析健康科学中解释的标准，论证了因果关系、医患信任的认知与关系维度、以及利益相关者实用需求驱动的解释充分性标准是构建医学XAI的三大核心分析轴，为设计既符合认识论严谨性又满足临床决策实践需求的可解释AI系统提供原则性指导。


<details>
  <summary>Details</summary>
Motivation: 解决医学人工智能中可解释性研究缺乏哲学基础的问题。尽管XAI被视为提高医疗AI透明度的关键手段，但关于什么构成充分的医学解释的争论尚未解决，而哲学和科学哲学领域对解释的深入研究成果在很大程度上被当代XAI研究所忽视。

Method: 采用批判性综述方法，交叉融合科学哲学与XAI领域，检视健康科学中关于解释的主流观点并评估其对医学XAI的适切性，进而识别出三大核心分析轴：因果性在医学推理中的作用、医学信任的认知与关系维度、以及利益相关者实用需求塑造的解释充分性标准。

Result: 指出了当前医学XAI研究在概念基础上存在的不足，论证了哲学视角为医学XAI提供了必要条件，识别出因果关系、信任维度和解释充分性标准三大分析轴，并据此提出了设计既具有认识论严谨性又符合临床决策认知与实践需求的可解释AI系统的原则。

Conclusion: 通过整合哲学分析与医学AI发展现状，为设计符合认识论要求和临床实践需求的医学XAI系统提供了原则性框架，推动医学XAI研究向被忽视的概念基础方向深入发展。
...
TL;DR: LuckyStar 111B是一个111B参数的混合推理模型，由Cohere和LG CNS联合开发，基于Command A模型进行后训练优化，结合前导条件控制、多语言监督微调、可验证奖励强化学习、语言一致性奖励和4-bit量化等技术，在内存受限的单GPU部署条件下实现了数学推理、函数调用和NL2SQL等工具型agent任务的性能提升。


<details>
  <summary>Details</summary>
Motivation: 在内存和服务部署约束下，为韩英双语企业agent构建一个高效、可部署的大型推理模型，探索如何将已有的后训练多语言模型适配到可验证的agent工作流中。

Method: 基于Cohere的Command A模型进行后训练，采用前导条件控制（preamble conditioning）在简洁非推理行为与长链工具导向推理之间切换；使用多语言监督微调、带可验证奖励的强化学习进行多步工具调用任务训练、语言一致性奖励优化韩语用户响应，以及4-bit量化实现单GPU服务部署。

Result: 模型在数学推理、函数调用和agent自然语言转SQL任务上性能提升，同时保持了通用韩语和英语指令遵循质量。

Conclusion: 提供了一个实用的技术方案和失败模式分析，用于将后训练多语言模型适配到可验证agent工作流中，满足内存受限的部署场景需求。
...
TL;DR: FARS是一个全自动AI-for-AI研究系统，能在67个细粒度AI/ML主题上自主生成166篇完整论文，并通过282份结构化评审验证其产出质量，同时揭示了实验范围狭窄、方法论局限和诚信问题等失败模式。


<details>
  <summary>Details</summary>
Motivation: 现有自动化研究系统大多依赖选定示例、人工设定主题或少量预定义研究任务，缺乏在规模化、多主题场景下全自动运行的验证。FARS旨在填补这一空白，设计一个能够在研究主题间大规模自主运行的完全自动化研究系统。

Method: FARS采用分阶段专用智能体，通过共享工作空间（记录提案、代码、日志、结果和手稿）协调运作，覆盖构思、规划、实验和写作四个阶段。在首次公开部署中，系统在67个细粒度AI/ML主题上自动产出166篇完整论文，并保留中间产物作为可审计语料库，而非仅筛选成功案例。

Result: 共获得282份来自志愿评审员的结构化评审，覆盖140篇论文，包括总体评分、子项评分、完整性检查和LLM使用披露。评审表明FARS能够在大规模公开部署中产出具有评审价值乃至高质量的AI/ML研究产物，同时暴露出实验范围狭窄、方法论局限和诚信问题等反复出现的失败模式。

Conclusion: FARS展示了全自动化AI研究系统在多主题规模化场景下的可行性和潜力，但当前系统产出的研究仍存在显著局限性，为后续自动化研究系统的优化提供了方向。
...
TL;DR: 本文提出了 Arena-T2I Hard 基准（310个提示词，每个约30个分解约束），用于评估文本到图像模型在复杂多面提示下的忠实度。发现现有系统在简单指令上接近完美，但在复杂任务上表现不佳（最强模型准确率仅0.855，系统间差距达33个百分点）。公共竞技场排名无法预测忠实度，因偏好分数优先考虑美学。提出依赖感知检查表奖励（DAG结构）与组解耦归一化（GDPO）结合的方法，在忠实度-美学权衡上优于所有基线。


<details>
  <summary>Details</summary>
Motivation: 现有忠实度基准仅依赖简单原子指令，顶级系统已接近满分，无法区分模型在真实创意工作流中处理多面复杂提示（如空间关系、风格约束、复杂文本渲染）时的失败模式。需要一个能细粒度诊断具体约束失败的高难度基准。

Method: 1) 从真实竞技场日志构建310提示的Arena-T2I Hard基准，每个提示分解为约30个是/否约束，覆盖6个类别（含文本渲染）。2) 提出依赖感知检查表奖励，将提示分解为DAG（有向无环图）的是/否问题，失败父节点的后代归零，作为每个约束的训练信号。3) 使用组解耦归一化（GDPO）将忠实度奖励与BT美学奖励标准化到各自滚动组内，防止任一奖励崩溃。

Result: 最强闭源模型准确率0.855，11个系统间差距33个百分点，证明高判别力。公共竞技场BT偏好分数不能预测忠实度。所提方法在SD3.5-Medium和FLUX.1-dev上，在MMRB2成对比较中实现了严格优于单奖励、朴素加权和或多奖励BT集成的忠实度-美学权衡。

Conclusion: 需要针对复杂多约束提示的细粒度忠实度基准；依赖感知检查表奖励结合GDPO能有效改善忠实度-美学权衡，优于现有方法。
...
TL;DR: ProtoPilot是一个自演化的多智能体系统，能够将生物学协议转化为可执行的实验代码，并通过湿实验反馈进行迭代修正。在294个合成生物学与分子生物学任务上，系统实现了90.2%的专家偏好率、89.5%的协议到代码门控通过率和88.24%的Opentrons通过率，远超OpenTrons-AI的32.35%，并经过湿实验验证了可重复的自主实验路径。


<details>
  <summary>Details</summary>
Motivation: 自主湿实验需要将生物意图、定量流程、设备约束和实验反馈从协议设计到代码执行全流程对齐，而现有方法生成的协议文本缺乏可执行性和设备兼容性。

Method: ProtoPilot采用分层可验证性、多智能体编排和运行时更新的技能库，实现协议生成、SOP扩展、SDK兼容代码合成以及基于湿实验反馈的工作流修正。

Result: Top@3专家偏好率90.2%，协议到代码门控通过率89.5%，Opentrons通过率88.24%（对比OpenTrons-AI的32.35%），湿实验验证获得可解释读数、Sanger确认产物及反馈修正的PCA组装DNA靶标。

Conclusion: 评估框架能够捕获自主湿实验自动化所需的执行相关要求，ProtoPilot通过将协议和代码生成转化为经过验证的执行和反馈引导的修订，满足了这些要求，建立了通往自主实验的可验证路径。
...
TL;DR: Evo-PI提出了一种基于可演化原则的监督学习框架，通过原则与模型行为之间的协同进化机制替代静态监督信号，显著提升多模态大语言模型在复杂推理任务（如医学视觉问答）中的表现，最高提升24.6%。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型依赖静态监督（固定提示、规则或奖励模型）进行训练，这种非自适应的监督方式虽然能约束输出格式，但无法塑造底层推理过程，导致泛化能力脆弱、性能饱和，尤其在复杂决策任务中表现不佳。

Method: Evo-PI将推理原则作为显式的基于语言的监督信号，并构建了一个协同进化循环：原则引导模型推理，模型行为反过来优化指导其训练的原则。通过这种动态对齐机制，监督信号能够逐步适应模型的推理缺陷。

Result: 在8个基准测试和多个模型主干上，Evo-PI在医学视觉问答任务中一致提升了推理准确率，最高提升达24.6%。

Conclusion: 可演化的原则引导监督为训练具有专家级对齐推理能力的多模态大语言模型提供了一种可扩展且通用的范式。
...
TL;DR: 提出RAISE框架，通过将约束最坏情况实例搜索集成到LLM进化搜索循环中，解决基于LLM的自动启发式设计在分布偏移下的鲁棒性问题。在三个在线调度问题上的实验表明，现有方法性能下降高达19倍，而RAISE在所有测试分布上保持稳定表现。


<details>
  <summary>Details</summary>
Motivation: 现有的基于LLM的自动启发式设计方法在固定训练实例集上优化启发式，当部署到现实世界中遭遇分布偏移时可能完全失效。

Method: 提出RAISE框架，采用双层优化结构：外层循环通过LLM算子进化启发式，内层循环（无LLM）在训练实例集的ε邻域内高效识别困难实例，使用基分布参数化和边界投影。

Result: 在在线装箱、在线作业车间调度和在线车辆路径规划三个问题上，跨五种分布家族的实验表明，现有LLM-based AHD方法在分布偏移下性能下降高达19倍，而RAISE在所有测试分布和问题规模上保持强一致性表现。

Conclusion: RAISE框架有效提升了LLM自动启发式设计在分布偏移下的鲁棒性，为将AI生成的启发式安全部署到实际动态环境中提供了可行方案。
...
TL;DR: 本文证明，在16GB显存上本地运行的量化开源模型，在LM-数据库集成任务中能够匹配或超越闭源API模型的准确率，同时实现390倍成本降低和3.8倍延迟降低，打破了闭源LM API必要的传统假设。


<details>
  <summary>Details</summary>
Motivation: 专有API的LM系统基于token计费，在大数据库场景下成本极高（单次实验可超1万美元），严重阻碍了LM增强型关系运算符的深入研究与实际部署。

Method: 使用量化开源模型在16GB VRAM上本地运行，并将其集成到BlendSQL v0.1.0框架中，分析高效部署所需的关键系统优化。

Result: 与专有LM API相比，实现了390倍成本降低、3.8倍延迟降低，且在准确率上匹配或超越闭源模型。

Conclusion: 量化开源模型本地运行能够以极低成本实现与闭源API相当的LM-数据库集成效果，挑战了闭源LM API必要的普遍假设。
...
TL;DR: 本文提出一种基于集合论和超维计算的新型计算心智理论，使用稀疏二进制数据和拓扑可塑性替代传统神经网络的连续权重和矩阵乘法，通过子集模式匹配实现联想记忆，并映射到小脑和新皮层，支持高效硬件实现。


<details>
  <summary>Details</summary>
Motivation: 传统神经网络依赖连续权重和矩阵乘法，与生物神经系统的离散、稀疏编码方式存在差距，亟需一种更贴近生物学原理的计算框架来理解认知机制并实现高能效人工智能。

Method: 采用集合论和超维计算框架，信息表示为稀疏二进制集合；通过组合爆炸式扩展的隐藏层网络拓扑自然产生联想记忆；学习机制基于拓扑可塑性而非权重调整；核心算法为子集模式匹配和精确最近邻搜索，统一了自联想和异联想学习。

Result: 该架构在常数时间复杂度下完成信息检索，将感知数据（稀疏分布式表示）和符号（稀疏全息表示）无缝桥接；映射到神经解剖学后发现小脑和新皮层可能实现该算法的变体。

Conclusion: 子集模式匹配是认知的基本引擎；该算法基于离散逻辑而非矩阵运算，可直接映射到存内计算硬件，为构建具有人类级能效的合成智能开辟新路径。
...
TL;DR: 本文提出了一种基于大语言模型（LLM）的自适应先聚类后路线（CFRS）系统，用于解决大规模容量受限车辆路径问题（CVRP）。该系统将分解过程建模为迭代决策过程，利用LLM作为高层决策者，动态选择聚类、平衡和优化算子，联合划分客户和车辆。在多达50万客户的实例上验证，该方法在基准规模实例上表现具有竞争力，且在大规模问题上展现出更好的可扩展性和鲁棒的路径质量。


<details>
  <summary>Details</summary>
Motivation: 现有CFRS方法依赖固定划分规则、预定义优化目标或学习策略，在不同空间分布、需求特征和运营特性的实例上表现不稳定，缺乏自适应能力。

Method: 提出自适应CFRS系统，将分解过程形式化为迭代决策过程；采用LLM作为高层决策者，分析当前分解状态并动态选择聚类、平衡和细化算子；实现客户与车辆的联合划分，使聚类过程具有容量感知能力并适应实例特征。

Result: 在合成和基准衍生CVRP实例（最多50万客户）上评估，基准规模实例上表现具有竞争力，在大规模问题上展现出更好的可扩展性和鲁棒的路径质量。

Conclusion: LLM引导的自适应决策支持作为大规模物流规划和工业级车辆路径问题的实用方法具有潜力。
...
TL;DR: 本文提出了一个端到端的代理式AI框架，将橡树岭国家实验室先进植物表型分析平台从数据工厂转变为交互式自主发现平台。该框架包含一个对话式协同科学家代理（将科学家的自然语言问题转化为结构化分析计划）和一个无头计算代理（在Frontier百亿亿次超级计算机上执行视觉Transformer分割和性状提取），两代理通过安全的令牌认证流通道在独立的安全和资源域中通信，将原本数天到数周的分析过程缩短为秒级的交互式循环。


<details>
  <summary>Details</summary>
Motivation: 高通量植物表型分析生成图像数据的速度远超科学家分析能力，性状提取和解释仍依赖人工专家且严格后验，使得分析而非数据采集成为科学发现的瓶颈约束。

Method: 提出端到端代理式AI框架：(1) 对话式协同科学家代理将自然语言问题转化为结构化分析计划；(2) 无头计算代理在Frontier超算上执行Vision Transformer分割和性状提取；(3) 两代理通过安全令牌认证流通道在不同安全和资源域中通信，确保端到端来源追踪。

Result: 将原本数天到数周的分析过程转变为秒级的交互式循环，代理可推理结果、推荐后续分析并响应追问。

Conclusion: 该框架利用代理式AI和百亿亿次计算资源，解决了高通量表型分析中数据生产与分析能力之间的失衡问题，使设施从数据工厂转变为交互式自主发现平台。
...
TL;DR: 本文提出MARS方法，利用从LLM骨干网络提取的文本拒绝方向，无需多模态安全数据即可实现多模态大模型的安全对齐。通过激活重校正、几何信任区域自适应缩放和最优层选择，MARS在五个SOTA多模态大模型上实现了安全性提升同时保持实用性。


<details>
  <summary>Details</summary>
Motivation: 现有对齐策略（后训练对齐、激活空间拒绝方向）在多模态大模型中需要难以收集的不安全多模态数据，成本远高于纯文本数据。

Method: 提出Modality-Agnostic Refusal Steering (MARS)方法：(1) 激活重校正修复模态错位；(2) 几何信任区域内自适应缩放引导强度；(3) 选择最优干预层，在首个生成token处操作。该方法无需训练，无需多模态安全数据。

Result: 在五个SOTA多模态大模型上，对安全性、实用性和视频越狱基准进行评估，MARS在保持实用性的同时实现了持续的安全性提升。

Conclusion: 安全相关结构在多模态间共享，文本拒绝方向是多模态对齐中强大且尚未充分探索的基础。
...
TL;DR: 提出了一种多智能体系统（MAS），将专家决策树与视觉语言模型（VLM）协调结合，通过解耦声明式决策（D³）框架实现零修改泛化，在树木偏斜分类任务中以更低的标注成本超越监督式ML基线。


<details>
  <summary>Details</summary>
Motivation: 专家标注成本高、速度慢且不一致，成为林业遥感中树木偏斜分类等任务扩展的主要瓶颈；同时人工标注数据存在跨标注者的显著变异性。

Method: 提出Decoupled Declarative Decision (D³) Framework，将专家决策树作为结构先验，由VLM在叶节点进行局部语义感知，并通过多智能体投票机制缓解VLM的随机性，支持零修改地在不同专家决策结构上泛化。

Result: 在树木偏斜分类测试集上，该方法优于有监督ML基线，并显著减少了专家标注工作量。

Conclusion: 将VLM与专家先验以智能体编排的方式结合，能够以更低的标注成本复现专家定义的标注流程，同时保持可解释性。
...
TL;DR: 论文揭示了在从合成问答数据训练语言模型时，生成步骤并非中性预处理，而是一种隐式策略，存在选择偏差（不均匀覆盖、被格式瑕疵劫持）和答案偏差（模型易服从嵌入的指令性文本）两大脆弱性。通过将问题绑定固定目标和过滤指令性文本，可显著降低这些失效模式。


<details>
  <summary>Details</summary>
Motivation: 现有研究广泛使用模型自生成的问答对进行微调、蒸馏或知识压缩，但忽略了生成步骤本身可能引入系统性偏差。作者旨在揭示这一预处理步骤并非中性，而是隐式策略，且在两个关键阶段（选题和作答）都表现出脆弱性。

Method: 通过系统性实验分析合成QA生成中的选择偏差（覆盖饱和、格式瑕疵劫持）和答案偏差（模型对指令性文本的服从性）；提出两个缓解策略：将每个问题绑定固定目标以减少选择偏差，以及在生成答案前过滤指令性文本。

Result: 覆盖饱和早且集中于显著跨度；不同提示收敛于相同区域；格式瑕疵（如未清理的标记）能劫持生成；模型易服从嵌入的指令性文本，且这种服从性在任务冲突时随模型规模增大而加剧。提出的过滤方法将指令注入成功率从88%降至13%，同时保留几乎全部干净文本。

Conclusion: 合成QA生成步骤存在系统性偏差，但这些问题源于生成阶段的选择，因此可以在不改变训练循环的前提下通过针对性策略显著缓解。
...
TL;DR: PolicyGuard是一个神经符号框架，将组织政策转化为可执行的审核引擎，结合LLM进行局部文档解释和符号规则评估，用于政策驱动的文档合规审查，使审查过程更明确、可维护和可系统测试。


<details>
  <summary>Details</summary>
Motivation: 端到端的大语言模型提示方式将政策逻辑隐含在模型中，导致合规决策难以审查、更新和测试，缺乏可解释性和可维护性。

Method: 将组织政策指南转化为由类型化关系逻辑规则和原子级提取问题组成的可执行审核引擎；LLM使用检索到的文档证据回答局部问题；符号评估器应用形式化规则检测不合规情况。

Result: 在特定公司的NDA合规审查任务上进行了实例化和评估，其中合同条款需对照组织特定的谈判政策进行检查。

Conclusion: 通过分离政策形式化、局部文档解释和符号合规评估三个环节，PolicyGuard使文档审查更加明确、可维护且可系统测试。
...
TL;DR: 本文提出AxDafny，一种验证器引导的修复框架，用于在Dafny中生成可执行代码和证明构件，并在新基准LCB-Pro-Dafny和DafnyBench上显著提升了验证成功率，同时发现验证成功与运行时测试表现衡量代码的不同方面。


<details>
  <summary>Details</summary>
Motivation: 在Dafny中进行智能体代码生成时，模型需要同时生成可执行代码和用于验证的证明构件（如不变式、断言、终止参数），这是一个具有挑战性的任务。现有方法在生成既正确又经过形式验证的代码方面表现不足，因此需要一种能够利用验证器反馈进行迭代修复的框架。

Method: AxDafny采用验证器引导的修复框架，通过迭代方式生成实现、不变式、断言和终止参数。具体而言，该框架利用验证器的反馈信号来识别和修复代码或证明中的缺陷，逐步改进生成结果直到通过形式验证。

Result: 在LCB-Pro-Dafny基准上，AxDafny相比基线GPT-5.5显著提升了验证成功率。在DafnyBench上，AxDafny达到了92.7%的验证成功率，比之前最强的基于证明提示的基线高出6.5个百分点。此外，实验表明验证成功率和运行时测试表现衡量的是生成代码的不同方面。

Conclusion: AxDafny是一种有效的验证器引导修复框架，能够显著提升Dafny中智能体代码生成的验证成功率。验证成功与运行时测试表现之间的差异表明，两者共同使用才能更全面地评估代码质量。LCB-Pro-Dafny为未来研究提供了新的基准。
