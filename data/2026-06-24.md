...
TL;DR: 本文提出了一种基于几何约束的计算机视觉流程，利用自行车单摄像头和RT-DETR+ByteTrack实现超车事件的自动检测，在315个真实事件上达到97.8%召回率与零误报，并实现了无需标定的侧向距离估算（MAE 13-14cm），解决了人工标注瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 传统仪器自行车研究依赖人工逐帧标注超车事件，这一瓶颈限制了样本量和自然场景骑行安全研究的规模，亟需自动化方法替代人工标注。

Method: 采用RT-DETR目标检测与ByteTrack多目标跟踪，结合三阶段几何验证模块（方位角趋势、表观尺寸增长、空间确认），基于透视投影原理实现超车事件检测；通过边界框几何特征实现免标定的侧向距离估算。

Result: 在315个手动标注的真实事件上达到97.8%召回率、零误报；平均在通过前2.44秒识别超车意图，84.1%的事件超过1.5秒人类反应阈值；96个事件的侧向距离测量显示33.3%低于5英尺（152.4cm）；距离估算MAE为13-14cm。

Conclusion: 该系统消除了仪器自行车研究的主要标注瓶颈，为基于消费级视频的车辆-自行车交互分析提供了可扩展的基础，支持更大数据集和多样化城市环境的研究。
...
TL;DR: 本文提出Sol Video Inference Engine，一个面向视频扩散模型的智能体推理加速框架。该框架将缓存、稀疏注意力、剪枝、量化和算子融合五种技术组织为智能体加速栈，针对具体模型、硬件和推理配置进行实例化优化。在64B Cosmos3-Super、22B LTX-2.3和2B SANA-Video三个模型上实现了2倍以上端到端加速，且质量近乎无损。


<details>
  <summary>Details</summary>
Motivation: 视频扩散模型通过扩展规模提升了生成质量，但推理成本也随之增加。现有的加速方法存在一个核心挑战：最优加速策略高度依赖于具体实例——针对某模型、硬件和推理配置有效的方案，往往无法迁移到其他组合。不同模型在架构、数值敏感性和注意力集中模式上各不相同；推理设置在时空分辨率和视频时长上存在差异；硬件平台在内存层次、支持的数值格式和内核吞吐量方面也有区别。这些因素产生了巨大的调优空间，使得手动性能优化成本高昂。

Method: 提出Sol Video Inference Engine，一种基于智能体的、原生无训练的加速框架。框架包含五个广泛适用的技术（缓存、稀疏注意力、token剪枝、量化和算子融合），组织为智能体加速栈进行实例化优化。具体流程为：并行技能智能体优化各项技术的实现，智能体集成器将其组合为全局加速栈，人工验证者提供生成质量反馈。

Result: 在三种不同规模和架构的视频模型（64B Cosmos3-Super、22B LTX-2.3、2B SANA-Video）上，以少量人工参与实现了超过2倍的端到端加速，同时保持了近乎无损的VBench生成质量。

Conclusion: 该智能体框架在视频扩散加速方面展现了有效性，能够针对不同模型、硬件和推理配置自动组合最优加速策略，实现显著的端到端加速且质量损失极小，为解决实例特异性的加速优化问题提供了一种可扩展的解决方案。
...
TL;DR: 针对大视觉语言模型中视觉-语言一致性问题，提出基于提示侧语义的PV-TAM方法，通过滤除模态边界标记的系统偏差，利用注意力峰值分布衡量提示与视觉区域的对齐程度，在多个数据集上优于传统答案侧基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有评估视觉-语言一致性的方法依赖答案侧标记的注意力分布，但存在两个问题：(1) 解码漂移导致语言先验累积，使高注意力区域与语义标记不匹配；(2) 结构标记（如模态边界符）会捕获全局上下文，产生与目标无关的高注意力。这些扭曲影响了大型VLMs的一致性评估。

Method: 提出Prompt-Vision Token Activation Map (PV-TAM)：采用提示侧语义替代答案侧注意力；引入滤波器去除模态边界标记引起的系统偏差；利用注意力峰值分布（而非传统掩码重叠率）度量提示与视觉区域的对齐程度。

Result: 在多个数据集上，PV-TAM在基于注意力的指标和IoU式定位指标上均一致优于答案侧基线方法。

Conclusion: PV-TAM有效解决了答案侧注意力分布中的解码漂移和结构标记干扰问题，为大型VLMs提供了更可靠的视觉-语言一致性评估方法。
...
TL;DR: 提出从空间域转向频域特征处理的小目标检测范式，通过频域引导特征表示框架和轻量级DER算子（WDG、LGE、FDHead三个即插即用模块），在不增加参数冗余的情况下有效恢复高频细节，DERNet系列以仅YOLOv11 1/6的参数取得一致性能提升。


<details>
  <summary>Details</summary>
Motivation: 小目标检测受限于目标特征稀疏，且空间域检测器在下采样过程中无差别丢弃关键高频细节。在空间域恢复这些脆弱线索通常需要昂贵且会放大背景噪声的架构扩展。

Method: (1) 频域引导特征表示框架，CNN和Transformer通用；(2) DER算子（分解-增强-重构），包含三个即插即用轻量模块：Wavelet-Difference Gate (WDG)、Log-Gabor Enhancer (LGE)、Frequency-Driven Head (FDHead)，在骨干、颈部、头部注入频域感知调制，将特征建模与分辨率降低解耦。

Result: 在VisDrone2019、UAVDT、TinyPerson、DOTAv1等多领域基准上取得一致提升。DERNet系列以YOLOv11仅1/6的参数实现同等尺度下的更优性能。

Conclusion: 频域特征处理是解决小目标检测中高频细节丢失问题的有效范式，频域感知调制能够以显著降低的参数冗余实现精准定位。
...
TL;DR: ABACUS是一个统一的视觉-语言模型，通过密度感知自适应缩放、边界感知计数策略和循环一致性GRPO三大创新，在无需特定基准训练的情况下，在物体计数、人群计数、指代表达计数和计数忠实图像生成任务上均达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型在处理多种计数任务（物体计数、人群计数、指代表达计数）以及计数忠实图像生成时，通常需要针对不同基准进行专门训练，缺乏统一的解决方案。作者希望构建一个无需特定基准训练即可通用于多种计数相关任务的统一模型。

Method: 基于现有3B参数统一基础模型，引入三项关键创新：(1) 密度感知自适应缩放与目标性图（objectness maps）实现空间定位；(2) 基于GRPO的边界感知计数策略消除裁剪边界错误；(3) 循环一致性GRPO策略：理解分支自我批判生成输出，无需外部标注即可弥合理解与生成之间的差距。

Result: 在七个基准测试上达到SOTA结果，性能超越任务专用模型和更大的通用模型。

Conclusion: ABACUS证明了通过精心设计的训练策略（密度感知缩放、边界感知计数、循环一致性自我批判），可以在不依赖任务特定训练数据的情况下，用一个统一模型有效解决多种计数相关任务，并且理解分支与生成分支可以相互促进。
...
TL;DR: 本文提出HANCLIP系列模型，通过双曲空间建模和角度三元组目标，在仅2万条数据上训练，使VLM学会区分否定描述（"不是什么"）与肯定描述（"是什么"），在保持标准任务性能的同时显著提升对否定语义的敏感性。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLM）对否定语义（negation）非常脆弱，容易被浅层词共现误导；直接微调否定数据会干扰已有知识，导致标准基准性能下降。

Method: 提出HANCLIP框架，包含两个核心设计：(1) 双曲空间（hyperbolic）公式建模层级语义关系与非对称性；(2) 角度三元组目标（angular triplet objective）驱动否定描述与对应肯定描述在嵌入空间中的系统性分离。在2万条图像-文本四元组上训练，模型无关，可即插即用于CLIP等现有VLM。

Result: 在NegBench否定基准上取得一致提升，同时保持或提升了标准分类和图像-文本检索基准的性能。验证了在CLIP、LongCLIP、SmartCLIP、HiMo-CLIP上的有效性。

Conclusion: 精心设计的几何目标函数可以在仅使用少量数据的情况下，显著扩展现有VLM的推理能力，且不会覆盖预训练表示的整体结构。
...
TL;DR: HeRA提出了一种头级别的表示对齐方法，通过对比学习目标对齐多模态大语言模型中各注意力头部的局部拓扑结构，并发现对齐最不匹配的头部效果最佳，能有效提升视觉任务性能并减少幻觉。


<details>
  <summary>Details</summary>
Motivation: 现有表示对齐方法通常固定对齐语言骨干网络的某一层，忽略了Transformer模型的细粒度结构（如注意力头级别），导致对齐不够精细，难以充分发挥跨模态对齐的潜力。

Method: 基于柏拉图表示假说，HeRA在注意力头级别上通过互K近邻（MKNN）度量评估对齐程度，并引入可微分的对比学习目标作为局部结构匹配的代理。在训练中选择对齐分数最低的头部进行对齐，以实现最大收益。

Result: 在多个MLLM和18个基准上的广泛评估表明，HeRA在视觉中心任务上持续提升性能，并有效减少视觉幻觉，缓解了对语言先验的过度依赖。

Conclusion: HeRA通过头级对齐策略证明了细粒度跨模态表示对齐的有效性，为MLLM的表示正则化提供了新思路。
...
TL;DR: REALM是首个面向物理世界VLM的统一红队测试基准，整合了12种攻击方法、3种防御和13个VLM，通过智能体目标生成管道实现公平比较。实验发现文本和排版注入攻击最有效，多模态联合优化产生最强视觉扰动迁移，单次攻击接近迭代方法效果且成本更低，模型规模本身不保证对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有VLM红队测试方法在数据集、指标和威胁模型上高度碎片化，缺乏统一标准来直接比较不同攻击方法；现有基准主要面向聊天机器人场景，未能系统覆盖物理世界VLM的功能性故障和红队测试方法。

Method: REALM整合12种红队测试方法、3种模型无关防御和13个VLM，采用统一的黑盒威胁模型、共享数据集和指标；引入智能体目标生成管道（agentic target-generation pipeline），为每个场景构建共享的、场景特定且物理世界可执行的攻击目标，使不同攻击方法在一致对抗目标下可公平比较。

Result: 文本和排版注入攻击造成最多故障；多模态联合优化产生最强视觉扰动迁移能力；单次攻击在成本远低于迭代方法的情况下能达到相近效果；模型规模本身并不赋予对抗鲁棒性。

Conclusion: REALM作为首个面向物理世界VLM的统一红队测试基准，有效解决了攻击方法间公平比较的难题，揭示了不同类型攻击的相对威胁程度，并为物理世界VLMs的安全性评估提供了标准化框架。
...
TL;DR: 提出TheProfessor方法，通过多教师集成蒸馏（固定域微调PromptSRC教师+零样本EVA-CLIP教师）改进PromptKD的单教师范式，在四个base-to-novel数据集上，置信度加权集成使平均HM从87.52提升至89.28（+1.77），在域偏移场景下增益最大（EuroSAT +5.78）。


<details>
  <summary>Details</summary>
Motivation: PromptKD单教师蒸馏性能有限，多教师集成可以引入互补的监督信号，尤其在域偏移场景下提升学生模型的泛化能力。

Method: 固定双教师集成：一个域微调PromptSRC ViT-L/14教师和一个零样本EVA-CLIP-L/14教师（每数据集预计算logits），采用等概率集成和置信度加权集成两种策略，蒸馏至ViT-B/16学生模型。

Result: 在Caltech-101、DTD、UCF101、EuroSAT四个数据集上，置信度加权集成平均HM 89.28（+1.77），等概率集成平均HM 88.88（+1.37）。Caltech-101增益最小（+0.16），EuroSAT增益最大（+5.78）。

Conclusion: 多教师提示蒸馏在第二教师提供互补监督时最有效，尤其在域偏移场景下收益显著；更新了此前仅基于Caltech-101的分析结果。
...
TL;DR: 提出法医知识图谱（FKG）统一框架，通过整合证据提取、结构化推理和可解释性，在图像伪造检测和定位任务上超越现有法医检测器和视觉语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有法医检测器缺乏可解释性，而视觉语言模型虽能提供解释但无法利用法医痕迹进行可靠检测，亟需一个兼具检测准确性和可解释性的统一框架。

Method: 构建法医知识图谱（FKG），编码法医痕迹及其因果依赖关系并链接场景内容；提出新型法医认证网络和迭代上下文精炼策略，引导VLM生成忠实且可追溯的解释；构建FKG-50K数据集。

Result: 在检测、伪造识别与定位、法医理由说明任务上，FKG均优于现有法医检测器和视觉语言模型。

Conclusion: FKG框架成功统一了法医证据提取、结构化推理和人类可解释性，为图像伪造认证提供了可靠且可解释的解决方案。
...
TL;DR: 提出DivRL框架，通过解耦视觉特征联合优化身份一致性与结构多样性，采用负自相似性度量(nSSM)量化多样性、视觉语义匹配(VSM)评估一致性，并设计"探索-抑制"策略将身份保持从竞争目标转化为可行性约束。


<details>
  <summary>Details</summary>
Motivation: 主体驱动图像生成面临"身份-多样性悖论"：强身份保持往往导致输出僵化且缺乏多样性，现有方法难以同时优化这两个相互冲突的目标。

Method: 提出DivRL后训练框架：(1) 从鲁棒相似性模型中提取解耦视觉特征；(2) 引入nSSM度量结构多样性，VSM评估身份一致性；(3) 设计"Explore-and-Suppress"策略，将VSM作为门控约束，仅在样本违反身份阈值时通过二次铰链损失进行惩罚，从而将身份保持转化为可行性约束而非竞争目标。

Result: 该方法有效推动模型生成既一致又多样的图像，在保持可比的身份一致性前提下，显著提升了结构多样性。

Conclusion: 通过门控优化公式，DivRL成功实现了nSSM和VSM的联合改进，打破了身份保持与多样性之间的权衡困境。
...
TL;DR: 本文提出Token-to-Token对齐框架，通过结构对齐（将提示改写为共享结构形式）和嵌入级对齐（基于语义相似性匹配token表示），解决文本到图像生成中token序列空间缺乏一致语义结构的问题，使得线性插值能产生平滑连续的语义过渡，实现图像混合与连续编辑等应用。


<details>
  <summary>Details</summary>
Motivation: 文本到图像生成模型中，token序列空间缺乏一致的、可访问的语义结构：语义相似的图像可能对应不同措辞、顺序的token序列，而相似的token序列却可能编码截然不同的语义。这种结构缺失使得在该空间中进行平滑过渡（如图像混合、连续编辑）变得困难。

Method: 提出Token-to-Token对齐框架，包含两个阶段：(1) 结构对齐：将不同提示改写为共享的结构化形式，使语义对应的概念映射到一致的位置；(2) 嵌入级对齐：基于语义相似性匹配提示间的token表示，对齐token嵌入。

Result: 对齐后的表示空间中，简单的线性插值成为有意义的操作，能产生平滑且连贯的语义过渡，成功实现图像混合和连续编辑等应用。

Conclusion: 文本到图像模型中的文本嵌入空间隐式编码了连续的语义结构，只需对齐表示而非修改生成模型本身，即可实现语义控制。
...
TL;DR: DriveStack-VLA通过注入BEV表示到LLM解码器、Render-Teacher对齐和自批评模块，增强了VLA驾驶模型的空间感知能力，在多个驾驶基准上取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA驾驶模型缺乏驾驶导向的空间智能，其策略主要基于透视图像token和语言先验，缺乏度量几何、俯视场景结构和安全关键感知线索的建模，导致对专家演示中的弱视觉几何建模和感知覆盖存在脆弱性。

Method: 1) 通过DeepStack式连接将鸟类视角(BEV)表示注入到大语言模型解码器中；2) 提出Render-Teacher对齐机制，对齐真实图像与栅格化图像的感知焦点；3) 引入基于头的自批评模块，对采样轨迹进行排序并有条件地优化最佳轨迹。

Result: NAVSIMv1上91.6 PDMS，NAVSIMv2上91.0 EPDMS（启用人类惩罚过滤器），闭环Bench2Drive上驾驶得分79.49，成功率56.36%。

Conclusion: DriveStack-VLA通过增强空间感知能力有效提升了VLA驾驶模型在开放环和闭环场景下的性能，验证了多模态空间表示融合和轨迹优化对驾驶决策的重要性。
...
TL;DR: EPEdit是一个集成零样本图像编辑算法的应用，基于Stable Diffusion模型，无需微调即可通过文本指令和遮罩完成多种图像编辑任务，在用户评估中优于现有方案。


<details>
  <summary>Details</summary>
Motivation: 传统图像编辑工具（如Photoshop）需要专业技能，而现有生成式AI方案（如Stable Diffusion）需要大量重训练和微调，成本高昂。为降低门槛和成本，提出EPEdit。

Method: EPEdit采用后端框架与前端界面结合的设计，核心基于Stable Diffusion模型的零样本图像编辑算法，无需额外微调。用户通过文本指令或区域标记进行交互，支持图像生成、对象替换/移除、背景修改、姿态/视角变化、区域编辑等多种任务。

Result: 在图像编辑、主题设计和系统性能的用户评估中，EPEdit在各项任务上均优于现有解决方案。

Conclusion: EPEdit提供了一种用户友好且成本效益高的综合图像编辑方案，使非技术用户也能高效完成复杂的图像编辑和主题集合创作。
...
TL;DR: 本文提出VisChronos框架，利用大语言模型和密集字幕模型从单张图像中识别和描述真实世界事件，并构建了EventCap数据集，以弥合视觉内容与自然语言理解之间的语义鸿沟，生成更准确、连贯、关注事件的描述。


<details>
  <summary>Details</summary>
Motivation: 传统图像描述方法在捕捉上下文叙事方面存在局限，难以弥合视觉内容与自然语言理解之间的语义鸿沟，无法生成具有事件感知能力的描述。

Method: 提出VisChronos框架，结合大语言模型和密集字幕模型，从单张输入图像中自动识别并描述真实世界事件；同时构建EventCap数据集以增强模型对复杂事件的识别与理解能力。

Result: 用户研究表明，该方案能生成准确、连贯且聚焦于事件的描述，有效提升了描述的上下文相关性和质量。

Conclusion: VisChronos框架和EventCap数据集为未来事件中心图像理解研究奠定了基础，推动了从视觉内容中提取叙事性事件描述的发展。
...
TL;DR: 本文对FoodSeg103数据集进行食材级语义分割，微调SegFormer-B0和B1模型，B1在像素准确率(0.7929)和平均IoU(0.3204)上均优于B0，并将预测掩码转换为食材面积占比作为简单的营养意识提示。


<details>
  <summary>Details</summary>
Motivation: 食品图像通常包含多种可见食材，仅分配一个菜肴标签会丢失重要的视觉结构信息，因此需要研究食材级别的语义分割来捕获更细粒度的视觉信息。

Method: 在FoodSeg103数据集上微调两种SegFormer变体（B0和B1），使用ImageNet预训练的MiT骨干网络，初始化新的104类输出层，并将预测掩码转换为食材面积百分比。

Result: 在2135张测试图像上，B0像素准确率0.7709、平均IoU 0.2521；B1像素准确率0.7929、平均IoU 0.3204，B1在各项指标上均提升，平均IoU绝对增益+0.0683。

Conclusion: 该方法可提供餐盘视觉成分摘要作为初步营养意识提示，但不能直接估计卡路里、宏量营养素、食物质量、体积、密度或真实份量。
...
TL;DR: 提出ObsGraph，一种以观测为中心的层次化场景图，通过房间-视角-对象三层结构统一场景表示、检索与探索，并在有限预算下进行由粗到精的层次化检索，利用检索结果指导多尺度探索决策，在具身推理和探索基准上提升了成功率和效率。


<details>
  <summary>Details</summary>
Motivation: 机器人在复杂陌生环境中需要自主识别并获取任务所需信息，现有方法在场景表示、信息检索与探索策略之间缺乏紧耦合，导致信息获取效率低下。

Method: 构建ObsGraph层次场景图（房间-视角-对象三层），在有限预算下执行由粗到精的层次化检索，并根据检索结果动态激活三种探索模式：房间级探索、视角精化或前沿探索，实现表示、检索与自适应多尺度探索的紧密结合。

Result: 在具身推理和探索基准测试中，ObsGraph相比基线方法在任务成功率和探索效率上均有显著提升。

Conclusion: 结构化场景表示与基于信息缺失驱动的目标导向探索相结合，能有效提升机器人在复杂环境中的推理与探索能力。
...
TL;DR: 本文首次提出织物图像去摩尔纹的标准化基准，通过物理合成框架构建16,050对多分辨率数据集，并定制了具有强泛化能力的基线模型。


<details>
  <summary>Details</summary>
Motivation: 织物摩尔纹由精细纺织图案与相机传感器网格交互产生，其宽带半周期特性导致频谱严重重叠，使去摩尔纹问题高度病态。现有基于屏幕摩尔纹数据集训练的模型泛化性差，且该领域缺乏标准化基准。

Method: 提出物理驱动的合成框架以解决真实像素对齐数据难以获取的问题，构建包含16,050对多分辨率、可控混叠程度的织物图像数据集，并定制基线模型。

Result: 构建了首个织物去摩尔纹标准化基准数据集，基线模型在该基准上展现出有竞争力的性能和强泛化能力。
...
TL;DR: 本文提出EMRFormer，一种用于自动调制识别的端到端脉冲神经网络架构，在KA200神经形态芯片上实现超过90%的理论能耗降低和最高5倍的实际功耗削减，同时在多个数据集上达到最先进的分类精度。


<details>
  <summary>Details</summary>
Motivation: 基于深度学习的方法在自动调制识别任务中虽能实现高精度，但其高计算成本导致准确率与功耗之间难以平衡，限制了在资源受限平台上的应用。神经形态架构在视觉和时间序列任务上展现出低能耗潜力，但尚未被充分探索用于AMR任务。

Method: 提出EMRFormer架构，包含：1) 自适应脉冲编码器，减少有效信息退化；2) Integer Leaky Integrate-and-Fire (LIF) 神经元，增强表示能力；3) 将脉冲可分离卷积神经网络(SSCNN)集成到脉冲驱动Transformer(SpikeFormer)中，从原始IQ波形中提取多尺度时间特征。

Result: 在主流数据集上达到最先进精度，优于所有基线；在低信噪比环境下保持强性能；理论能耗降低超90%；在KA200神经形态芯片上相比3090 GPU或Orin NX实现最高5倍功耗降低。

Conclusion: EMRFormer为资源受限设备上的自动调制识别提供了一条有前景的路径，证明了脉冲神经网络结合神经形态硬件在低功耗高精度AMR任务中的可行性。
...
TL;DR: 本文提出PNAFusion，一种渐进式像素邻域可变形交叉注意力机制，用于多光谱特征融合。通过像素邻域交叉注意力(PNCA)和自适应可变形对齐(ADA)模块，结合迭代反馈机制，实现高效且精确的可见光-热红外图像特征对齐与融合。在FLIR、M3FD和DroneVehicle数据集上达到SOTA性能，计算效率优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多光谱目标检测中，现有全局交叉注意力虽具强长程建模能力，但其二次复杂度限制了在资源受限平台上的部署。同时，可见光与热红外图像之间既存在局部邻域内的弱未对齐问题，又存在跨模态非线性空间映射关系，固定感受野难以有效建模。

Method: 提出PNAFusion框架，包含两个核心模块：(1) 像素邻域交叉注意力(PNCA)，将注意力限制在局部邻域内，避免冗余全局匹配并抑制背景噪声；(2) 自适应可变形对齐(ADA)，通过学习逐像素偏移捕捉非线性空间对应关系。两者通过迭代反馈机制渐进式精炼跨模态特征对齐。

Result: 在YOLOv5检测器下，FLIR、M3FD和DroneVehicle上分别达到84.2、90.5和85.5 mAP@0.5；迁移至Co-DETR后，FLIR达86.8，M3FD达90.8 mAP@0.5。相比ICAFusion，GPU内存减少33%，理论FLOPs从194.8G降至156.4G。

Conclusion: PNAFusion通过将局部空间先验融入设计，在保持高效计算的同时实现了优异的跨模态特征对齐与融合性能，验证了局部邻域注意力与可变形对齐结合的有效性。
...
TL;DR: 提出了一种基于文本指导的通用图像聚类框架（Guideline-Driven Image Clustering Agent），通过生成式概念代理建模和基于最小生成树的LLM遍历，统一了不同聚类场景（通用/细粒度、全局/局部、均衡/长尾）下的图像聚类任务。


<details>
  <summary>Details</summary>
Motivation: 不同图像聚类场景（如通用分类、细粒度分类、全局/局部标准、均衡/长尾分布）之间存在根本性差异，现有方法难以统一处理这些多样化的聚类任务。

Method: 1) Guideline-Driven Image Clustering Agent：首个通过文本指导桥接不同聚类场景的统一框架；2) Generative Concept Proxy Modeling：通过概念代理提取生成包含指导信息的嵌入表示，无需任务特定训练；3) LLM Traversal based on Minimum Spanning Tree：基于最小生成树选择性应用LLM推理进行复杂语义判断，实现自动聚类发现。

Result: 在多种聚类场景（从通用到细粒度分类、从全局到局部标准、从均衡到长尾分布）中，该方法一致性地优于专门的聚类方法。

Conclusion: 该框架通过文本指导统一了不同聚类场景，无需任务特定训练即可处理复杂指导，展现出良好的通用性和泛化能力。
...
TL;DR: 本文提出从传感器层面优化自动驾驶感知能力，通过可微RAW到任务管线，发现学习彩色滤波阵列（CFA）权重是提升密集预测性能的关键（mIoU提升+0.017至+0.023），而光学点扩散函数协同设计反而有害（-0.020 mIoU），噪声优化效果微弱，扩大CFA tile尺寸持续降低性能。最终建议：学习2x2 CFA权重，保持单位PSF。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶感知研究多聚焦于扩大模型规模、基础模型和多智能体融合，本文提出一个互补但更上游的问题：相机传感器本身应该测量什么？即从传感器层面探索哪些自由度对密集预测任务有益。

Method: 构建可微RAW到任务（RAW-to-task）管线，将传感器参数（CFA权重、点扩散函数PSF、噪声）纳入端到端可微优化，分解各自由度对密集预测性能的影响。

Result: 学习CFA权重是最优杠杆（KITTI-360 +0.017 mIoU, ACDC +0.023 mIoU）；PSF协同设计为负收益（-0.020 mIoU），受数据处理不等式约束；噪声优化效果可忽略；扩大CFA tile尺寸（超过2x2）一致降低性能。

Conclusion: 传感器级干预具有模型无关性，在不同天气条件下（雾、夜、雨、雪）验证了鲁棒性，最终推荐简单有效的方案：学习2x2 CFA权重，保持单位PSF。
...
TL;DR: 本文提出DramaDirector框架，通过从真实短剧镜头库中索引深度和姿态来借用电影摄影几何，结合模式约束的SFT和GRPO训练规划器，实现从剧情到多镜头短剧的生成。同时构建了包含35部真人剧、2.8K集、81K镜头的DramaBoard基准，实验表明该方法在忠实度、一致性和可控性上优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 短剧具有快节奏镜头切换、对话驱动的焦点转移和苛刻的摄影基础要求，现有的提示级别或纯文本视频生成流程难以满足这些需求。

Method: 提出DramaDirector框架：1) 从真实短剧镜头库中按深度和姿态索引检索几何参考；2) 将每个镜头解耦为静态视觉条件和动态叙事条件；3) 在学得的文本-视觉对齐奖励下，通过模式约束的SFT和GRPO训练规划器；4) 检索深度-姿态参考以指导首帧生成和图像到视频合成。

Result: 在DramaBoard基准上，DramaDirector在忠实度、一致性和可控性方面优于代表性多智能体和视频生成基线。

Conclusion: 通过借用真实短剧的摄影几何知识并结合结构化的训练策略，可以显著提升剧情到多镜头短剧生成的质量，代码已开源。
...
TL;DR: 本文在胃肠内镜VQA数据集Gut-VLM上系统评估了9种幻觉检测方法（覆盖黑盒、灰盒、白盒三类）在5个VLM上的表现，发现白盒方法ReXTrust显著优于其他方法（平均AUC提升19.5点），并识别了"自信幻觉"这一系统性失效模式。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在临床部署中面临幻觉问题的重大障碍，现有幻觉检测方法主要集中在放射学领域（如MIMIC-CXR、VQA-RAD），而胃肠内镜领域几乎未被探索，亟需填补这一空白。

Method: 在Gut-VLM数据集（4,392个VQA对）上，对5个VLM（MedGemma-4B/27B、LLaVA-Med-7B、LLaVA-v1.6-7B、Lingshu-32B）基准测试9种幻觉检测方法，涵盖黑盒（RadFlag、SelfCheckGPT-NLI）、灰盒（AvgProb、AvgEnt、MaxProb、MaxEnt、Semantic Entropy、VASE）和白盒（ReXTrust）三类，使用配对置换检验评估统计显著性。

Result: ReXTrust（白盒方法）在所有5个模型上AUC最高，峰值达93.0（MedGemma-4B），平均比最强替代方法高19.5 AUC点（范围9.5-33.5）；在LLaVA-v1.6-7B上AUC达79.9，而黑盒与聚类灰盒方法接近随机水平；非白盒方法中，token级灰盒统计（MaxEnt、MaxProb）优于聚类灰盒（Semantic Entropy、VASE）和黑盒方法。

Conclusion: 白盒隐状态访问在幻觉检测中提供一致且显著的性能优势；token级不确定性统计是无需白盒访问时的最佳替代方案；"自信幻觉"（模型在高一致性或高概率下仍产生幻觉）是现有方法面临的系统性失效模式。
...
TL;DR: 本文提出PreciseDoc，一种专为文档图像中精确元素定位设计的大规模多模态模型，通过构建高质量合成训练数据（含手写文档和相机效果）和联合强化学习的视觉定位推理训练范式，显著提升了文档空间定位和文档理解任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LMM在文本密集的文档图像中定位精度差，无法准确找到关键文档元素以支持可靠推理，亟需提升文档视觉定位的精确性。

Method: 1）构建两条合成数据流水线，批量生成带细粒度坐标标注的高质量文档（含相机效果的合成手写文档）；2）引入视觉定位推理训练范式，通过强化学习联合监督定位与推理过程。

Result: 在多个基准测试上的综合评估表明，所提出的数据和方法在文档空间定位和文档理解方面具有优势。

Conclusion: PreciseDoc通过合成数据增强和强化学习联合训练，有效提升了文档图像中精确元素定位能力，并可扩展至文档VQA等下游任务。
...
TL;DR: 本文研究了如何将卫星预训练的地球观测基础模型（Prithvi-EO-2.0-600M Vision Transformer + UPerNet解码器）适配到厘米级RGB航空影像的洪水分割任务。实验表明，该模型在两个洪水数据集上达到SOTA；零样本迁移到新洪水事件时优于基线；仅用少量新数据微调即可快速接近全量训练性能，展现了良好的迁移能力。


<details>
  <summary>Details</summary>
Motivation: 应急响应和灾害评估需要及时、高分辨率的洪水范围地图。虽然深度学习模型可用于航空RGB影像的水体分割，但CNN和小型ViT在场景变化（不同洪水事件）时需要大量数据进行适配。而地球观测基础模型在卫星数据上预训练，其分辨率、视角和辐射度与RGB影像不同，因此需要研究如何有效适配。

Method: 使用Prithvi-EO-2.0-600M Vision Transformer作为骨干，结合UPerNet解码器构建Prithvi-2.0-UPN模型，在BlessemFlood21和NeuenahrFlood两个RGB数据集上进行二值水体分割的微调。设计三个实验：1) 在各自数据集上训练并评估SOTA性能；2) 零样本跨事件迁移（BlessemFlood21训练→NeuenahrFlood测试）；3) 使用少量NeuenahrFlood数据进一步微调，评估性能提升速度。

Result: 实验1：Prithvi-2.0-UPN在两个数据集上均达到SOTA；实验2：零样本迁移到新洪水事件时优于现有基线模型；实验3：仅用少量新数据微调，Prithvi-2.0-UPN性能提升最快，且接近全量训练水平，表明其良好的迁移能力。

Conclusion: 卫星预训练的地球观测基础模型能够通过微调有效适配到厘米级RGB航空洪水分割任务，并在跨事件迁移和小样本微调场景下展现出显著优势，为快速部署洪水应急响应提供了可行方案。
...
TL;DR: 本文提出了Bengal-HP_RU，首个面向孟加拉人群体的公开头部姿态数据集，包含12,894张标注了连续偏航角、俯仰角和滚转角的头部图像，数据来自Wikimedia Commons，按上传者身份划分训练/测试集以避免数据污染。


<details>
  <summary>Details</summary>
Motivation: 现有头部姿态数据集主要涵盖西方或东亚人群，南亚人群（尤其是孟加拉人）严重缺乏代表性，亟需一个面向该人群的数据集以促进相关研究的公平性和泛化能力。

Method: 从Wikimedia Commons收集图像，经自动标注流水线处理后进行人工标签校正；按Wikimedia上传者身份划分数据集（10,494训练/2,400测试，共296个独立上传者）以防止数据污染；标注了连续的yaw、pitch、roll值。

Result: 构建了包含12,894张标注图像的Bengal-HP_RU数据集，训练集10,494张、测试集2,400张，覆盖296个不同上传者，在年龄、性别、遮挡、光照和背景方面具有显著多样性。

Conclusion: Bengal-HP_RU填补了孟加拉人群头部姿态估计数据集的空白，提供了真实场景下的多样化数据，公开可用以推动该领域的公平研究。
...
TL;DR: Sat2City v2 提出了一种从单张卫星图像生成显式纹理化3D城市资产的方法，通过适配预训练的原生结构化潜空间3D基础模型，结合真实世界卫星-网格配对数据集，实现了几何可控、外观可控的高质量城市网格生成，在多项基准上达到最佳性能。


<details>
  <summary>Details</summary>
Motivation: 现有 Sat2City 框架存在外观随机、训练数据为合成数据、任务专用VAE难以扩展至真实噪声重建等问题。迫切需要一种能从单张卫星图像生成具有合理几何结构和可控外观的可复用纹理网格的方法，以满足数字孪生、城市模拟和地理空间智能等实际应用需求。

Method: 1) 构建包含16,241个卫星-网格对的大规模真实世界数据集，覆盖9个城市24个区域；2) 利用预训练的原生结构化潜空间3D基础模型将网格编码至3D潜空间；3) 微调卫星条件几何流模型以生成几何潜码；4) 使用解码后的几何形状作为锚点，进行卫星条件纹理生成，保持几何到外观的级联架构。

Result: 在度量尺度DSM重建和生成式城市资产基准测试（包括几何和外观）中，Sat2City v2在所有评估基线中取得了最佳综合性能。此外，该工作是首个为资产级任务构建的、从匹配地理裁剪区域收集的真实卫星-网格配对数据集。

Conclusion: Sat2City v2 将卫星到城市生成从面向渲染的3D代理推进到显式纹理化网格资产，实现了外观可控的生成，为卫星图像驱动的3D城市重建提供了更实用、可扩展的方案。
...
TL;DR: 本文提出了一种面向3D高斯泼溅的几何感知风格迁移框架（GAST），通过解耦优化交替更新颜色与几何参数，并引入几何感知对比特征匹配（GCFM）融合RGB、深度和边缘线索，实现了外观与几何结构的同步风格迁移，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS风格化方法仅关注颜色迁移而忽略几何结构适配，导致外观与几何更新相互干扰，缺乏稳定的场景级几何变换能力。

Method: 提出解耦优化方案交替更新颜色和几何参数，并设计几何感知对比特征匹配（GCFM）融合RGB、深度和边缘线索，在优化两个阶段中传递结构特征。

Result: 在定性保真度和定量指标上均显著优于现有3DGS风格化方法。

Conclusion: 解耦优化结合多模态对比特征匹配能有效实现3D高斯泼溅的几何感知风格迁移。
...
TL;DR: 本文批判了"视频生成本质就是世界建模"的流行观点，指出仅靠视觉预测缩放无法实现真正的物理智能体。作者提出，视频生成模型只能学习部分、隐式的时空世界模型，而缺乏完全接地和可控的能力。真正的关键在于反事实可控性——即能够回答"在某个行动下会发生什么"的问题，并通过这种能力实现自进化的世界模型。因此，论文提出具有反事实可控性的自主视频生成是实现自进化世界模型的一条有前途的路径。


<details>
  <summary>Details</summary>
Motivation: 现有文献声称视频生成本质上是世界建模，这一观点虽然推动了生成式AI从静态图像向时间扩展的物理场景发展，但危险地依赖于"仅靠缩放视觉预测就能自动产生物理智能体"的信念。作者认为需要更准确的表述和更严格的判据来界定视频生成与世界模型的关系。

Method: 通过概念分析和对现有视频生成模型局限性的批判性论证，提出反事实可控性作为决定性判据，并以此构建自进化世界模型的新视角。

Result: 指出视频生成模型仅学习部分、隐式的时空世界模型，缺乏对可控变量、身体约束和有效未来的完整认知。反事实可控性——即测试在特定行动下未来是否能够承受具身约束并将行动知识反馈回生成过程——才是真正的界标。

Conclusion: 具有反事实可控性的自主视频生成是实现自进化世界模型的一条有前途的路径，超越了单纯的预测现实主义。
...
TL;DR: 提出差分展开网络（DU），通过异构框架替代传统深度展开网络的均匀重复设计，将优化过程分为结构锚定和差分进化两个互补角色，稀疏部署高复杂度阶段生成基础特征，再通过轻量差分阶段以差分表示先验（DRP）跨阶段传播和精炼特征，实现精度与效率的更好权衡，在视频快照压缩成像中取得SOTA结果且显著降低计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有深度展开网络（DUNs）采用统一的均匀设计哲学，重复堆叠相同结构的高复杂度先验模块，忽略了优化轨迹会向静态状态收敛的事实，导致表示停滞（representation stagnation），大量高成本计算浪费在微小的特征更新上，效率低下。

Method: 提出差分展开（DU）异构框架，核心为差分进化框架（DEF），将展开过程分为两个互补角色：1）结构锚定：稀疏部署高参数通用阶段生成高保真特征基础；2）差分进化：轻量差分阶段采用差分表示先验（DRP），通过差分表示注意力（DRA）演化注意力图以及差分调制FFN（DM-FFN）进行特征矫正，以极低开销建模跨阶段变化。

Result: 在视频快照压缩成像任务上取得新的最先进结果，同时显著削减了计算开销，实现了精度与效率的优越平衡。

Conclusion: 通过将计算资源聚焦于动态演化而非静态冗余，DU是一种更高效的深度展开设计范式，能够在不牺牲性能的前提下大幅降低计算成本。
...
TL;DR: 提出PriorTR方法，通过分离模型先验注意力与任务条件注意力来改进视觉令牌剪枝，在无需额外训练的情况下提升多模态大模型的效率与准确性平衡。


<details>
  <summary>Details</summary>
Motivation: 现有基于注意力得分的视觉令牌剪枝方法中，注意力被模型固有先验主导（即使无文本指令，模型也会关注某些与任务无关的区域），导致任务条件令牌的注意力得分被抑制，在剪枝时被错误丢弃。

Method: 提出PriorTR方法：1) 引入空令牌（null token）作为指令无关探针，在单次前向传播中同时估计模型先验注意力和任务条件后验注意力；2) 通过对比先验与后验注意力分布，计算每个视觉令牌贡献的额外可用信息；3) 基于此进行令牌剪枝，无需额外训练。

Result: 在多个多模态基准和MLLM上的实验表明，PriorTR在准确性与效率之间取得了更优的平衡，特别是在激进令牌预算下表现突出，优于已有的强训练无关基线方法。

Conclusion: PriorTR通过显式分离模型先验与任务条件注意力，有效解决了注意力主导偏差导致的令牌误剪问题，是一种高效的训练无关令牌缩减方法。
...
TL;DR: 本文提出了一种无需组标签的偏差缓解方法，通过CBCM（置信度引导的偏差概念挖掘）利用扩散模型解耦的语义概念识别虚假属性，以及DCD（双分支交叉投影去偏）通过双分支提示微调框架分离目标与虚假表征，在四个基准数据集上达到了最差的组准确率SOTA，仅微调0.22%参数。


<details>
  <summary>Details</summary>
Motivation: 现有无组标签的偏差缓解方法面临两个挑战：1）隐式推断虚假属性难以与真实世界偏差语义对齐；2）单分支设计在共享特征空间中无法解耦目标与虚假属性。

Method: 提出两阶段方法：CBCM利用扩散模型的解耦能力挖掘语义对齐的虚假概念；DCD采用双分支提示微调框架，通过交叉零空间投影显式去除虚假信息同时保留目标相关语义。

Result: 在四个基准数据集上，最差组准确率超越现有无组监督方法，且仅微调0.22%模型参数。

Conclusion: 通过语义级别的虚假概念挖掘和双分支解耦设计，有效解决了无组标签场景下的偏差缓解问题，实现了高效参数微调下的SOTA性能。
...
TL;DR: 提出一种基于跨层频谱演化（CLSE）的无训练token剪枝框架，通过分析视觉token在Transformer层间从高频结构细节到低频语义抽象的频谱重分布，量化token重要性，在保持性能的同时大幅降低MLLMs的计算和内存开销。


<details>
  <summary>Details</summary>
Motivation: 现有token剪枝方法依赖单层信号（如注意力分数、token相似度），忽略跨层视觉表征变换，且在多模态token序列中存在位置偏置，导致在激进剪枝下性能严重下降。

Method: 提出CLSE框架，在频域中量化token表示在Transformer层间的演化程度（频谱重分布），认为跨层频谱变化越强的token语义越活跃，应予以保留；无需训练，直接基于层间频谱动态产生稳定性重要性准则，消除位置偏置。

Result: 在多个图像和视频基准测试上，CLSE在激进token缩减下实现了更好的效率-精度权衡，显著降低FLOPs、KV缓存和延迟，同时保持甚至提升性能。

Conclusion: 跨层频谱演化提供了一种稳定、无偏的token重要性度量，为MLLM的高效部署提供了无需重新训练的即插即用方案。
...
TL;DR: 针对点云视频理解中标注成本高和内存瓶颈两大问题，提出PoinTriE框架，通过伪运动轨迹合成、几何-运动对偶网络和多模态对比学习实现数据高效预训练，并采用冻结主干+轻量级时空侧网络（LoRA+梯度流掩码）的参数/内存高效微调，在动作识别和语义分割任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法在点云视频理解中存在两个关键局限：大规模点云数据集的标注成本过高，以及微调过程中的严重内存瓶颈。本文旨在从现有数据中挖掘更丰富的监督信号，而非盲目扩展数据集，同时大幅降低微调的内存占用。

Method: 预训练阶段：通过刚性变换合成伪运动轨迹，结合原始点云生成的文本语料和2D投影；提出几何-运动对偶网络，通过多模态对比学习、刚性旋转预测和运动分布散度损失产生密集自监督信号。微调阶段：冻结预训练主干，仅更新由LoRA单元构建的轻量级时空侧网络，配合梯度流掩码策略同时降低内存消耗和参数开销。

Result: 在动作识别和语义分割任务上建立了新的SOTA结果。

Conclusion: PoinTriE通过数据、参数和内存三个维度的联合优化，提供了一种统一的点云视频理解框架，有效解决了现有PEFT方法在标注成本和内存瓶颈方面的局限性。
...
TL;DR: 本文提出将测试时规范化为分布外检测问题，系统评估了约20种OOD分数和9种搜索算法，发现基于距离的分数配合随机搜索与局部微调效果最佳，并引入门控机制以保留分布内精度。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉模型对仿射变换（旋转、缩放、剪切）的鲁棒性不足，现有解决方案（等变架构或数据增强）需要修改或重新训练模型，而测试时规范化虽无需改动分类器，但现有方法仅依赖少量基于logit的能量分数和特制搜索流程，缺乏对评分函数和优化器的系统性探索。

Method: (1) 将测试时规范化重新定义为OOD检测问题，使任意OOD分数可作为变换空间上最小化的能量函数；(2) 系统评估约20种OOD分数（包括基于距离、密度、能量等方法）与9种搜索算法（含随机搜索、局部微调、贝叶斯优化等）；(3) 引入门控机制，仅在输入OOD分数较高时进行规范化，避免对已对齐输入的精度损害。

Result: 在多种基准（手写字符、草图、自然图像、3D点云）上，基于距离的OOD分数（如Mahalanobis距离）结合随机搜索与局部微调的整体性能最优；门控机制有效保留了分布内精度，同时保持了对变换输入的鲁棒性增益。

Conclusion: 测试时规范化可借助通用的OOD检测框架实现，无需修改或重新训练分类器；该框架具有通用性，可推广至不同数据模态（图像、点云）和变换类型。
...
TL;DR: 本文系统综述了2016-2026年间三维场景补全领域的发展，涵盖从以SSCNet为代表的体素语义补全范式到结合生成式扩散先验与高斯泼溅实时渲染的最新技术，梳理了体素网格、点云学习、隐式神经场、Transformer、扩散网络及基于渲染感知的3D高斯基元等表征范式的演进，并提出了分类体系和未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 三维场景补全在自动驾驶导航和增强现实等领域具有广泛应用，但该领域在过去十年中经历了快速范式变革，缺乏系统性的综述来梳理从体素语义补全到扩散模型+高斯泼溅等最新范式的完整演进脉络，因此有必要对该领域的研究贡献进行全面的分类与总结。

Method: 采用系统性文献综述方法，对2016-2026年间三维场景补全领域的研究工作进行汇编，重点分析表征范式的演变（包括体素网格、点云学习、隐式神经场、Transformer、扩散网络、3D高斯基元），并构建分类体系对已有贡献进行结构化整理。

Result: 建立了涵盖六大表征范式（体素、点云、隐式神经场、Transformer、扩散网络、3D高斯基元）的完整分类体系；揭示了该领域从SSCNet到扩散先验+高斯泼溅的范式演进路径；识别了当前仍待解决的关键挑战。

Conclusion: 提出了面向下一代三维场景补全系统的研究议程，为未来发展方向提供了清晰指引；指出了该领域虽已取得显著进展，但在多个方面仍存在亟需突破的难题。
...
TL;DR: 本文提出了一种名为AdaQ的自适应、无需训练的帧采样方法，将长视频帧选择定义为准高斯采样问题，利用3-σ规则实现自适应帧采样，显著提升了MLLM在长视频理解任务中的性能与效率。


<details>
  <summary>Details</summary>
Motivation: 长视频理解中，MLLM面临计算量和内存消耗过大的问题。现有的关键帧选择方法采用硬采样原则，存在灵活性低、噪声大的缺陷。

Method: 将视频帧选择定义为准高斯采样问题，提出AdaQ方法。受高斯分布3-σ规则启发，为不同示例自动确定最优的3-σ区间：局部查询使用较小区间，全局查询使用较宽区间，从而实现鲁棒且自适应的帧采样。

Result: 在4种MLLM和3种嵌入模型上的实验表明，AdaQ显著优于默认MLLM和现有SOTA关键帧选择方法。例如，仅用64帧即可使Qwen3-VL-8B平均超越GPT4o 15.8%。

Conclusion: AdaQ是一种高效、鲁棒且实用的长视频帧采样方法，仅需设置1个超参数，具有优越的鲁棒性和高效性。
...
TL;DR: 本文针对扩散模型概念擦除中良性共现概念被连带抑制的问题，提出了CARE分数作为通用评估指标，并设计了ReCARE框架，通过自动构建CARE词表并在训练中加以保护，实现了仅擦除目标概念而保留共现概念的效果，在多个任务上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型的概念擦除方法在移除目标概念（如裸体）时，会连带抑制与之良性共现的概念（如"人"），导致模型无法正常生成包含这些概念的内容。作者希望解决这一"附带损害"问题。

Method: 提出CARE分数作为量化共现概念保留程度的通用指标；设计ReCARE框架，自动从目标图像中提取良性共现词元构建CARE集，并在训练过程中利用该词表来稳定擦除过程，确保只擦除目标概念。

Result: 在裸体、梵高风格、丁鳜物体等多个目标概念上，ReCARE在概念擦除效果、整体效用和CARE保留三个维度上均达到最先进水平。

Conclusion: 通过显式定义并保护良性共现概念，ReCARE有效解决了概念擦除中的附带损害问题，为扩散模型的安全可控生成提供了更精细的解决方案。
...
TL;DR: 提出I2C-3D方法，通过交互碰撞策略和多视图自适应评分蒸馏采样，解决现有3D生成方法在多对象组合场景生成和跨视图一致性问题。


<details>
  <summary>Details</summary>
Motivation: 现有3D生成方法存在两个实际挑战：(1)主要生成单个3D对象，难以生成多对象组合3D资产，缺乏高斯原语在合理交互中的建模；(2)在3D优化过程中常出现跨视图不一致问题，因为评分蒸馏采样固有地在每个单视图上执行，导致跨视图幻觉。

Method: (1) Inclusive Interactive Collisions策略：引导高斯原语自然地出现在合理的交互区域，确保组合场景中对象以物理合理且视觉连贯的方式交互。(2) Multi-View Adaptive Score Distillation Sampling：通过跨视角调制实例标记和空间标记的注意力图，从预训练扩散模型中蒸馏多视图一致性先验和布局先验。

Result: I2C-3D不仅能生成高保真多视图一致性的组合3D资产，还支持灵活的3D编辑和复杂场景生成。大量实验证明在生成质量和多视图一致性方面优于现有方法。

Conclusion: 通过精心设计的交互碰撞策略和多视图自适应评分蒸馏采样，有效解决了组合3D生成中的对象交互合理性和跨视图一致性问题。
...
TL;DR: MorVess是一种融合可微几何先验与大规模基础模型（SAM）的形态感知分割框架，通过联合预测血管掩膜、距离图和厚度图，并引入2.5D适配器和全局-局部融合模块，在肺部CT血管分割任务中显著提升了细小血管恢复和拓扑完整性，取得了领先的Dice、clDice和HD95指标。
...
Method: MorVess框架包含三个核心设计：(1) 联合预测血管掩膜、距离图和厚度图，提供边界、中心线一致性及直径平滑过渡的显式监督；(2) 轻量级2.5D适配器桥接3D空间上下文与2D SAM表示；(3) 全局-局部融合模块聚合多层级语义与几何线索，实现高保真拓扑重建。

Result: 在两个挑战性肺部CT基准上，MorVess在Dice、clDice和HD95指标上均取得领先结果，显著改善了小血管恢复和全局连通性。

Conclusion: 将几何智能嵌入预训练视觉模型为精确血管分析和临床可靠的定量分析提供了一条有原则且可扩展的路径。
...
TL;DR: GIVE是一个基于几何指令的视频编辑框架，通过统一的对象状态公式（深度框+方向框）实现物体级几何编辑（平移、旋转、缩放、复制、删除），并利用图形引擎管线生成成对监督数据，实现具有时间一致性和一致二次效应（阴影、反射）的生成式视频编辑。


<details>
  <summary>Details</summary>
Motivation: 物体级几何编辑（平移、旋转、缩放、复制、删除）是数字内容创作中的常规操作，但在生成式视频编辑中仍不可靠。关键挑战在于如何在视角和时间变化下明确指定目标物体的3D状态变化，同时保持阴影、反射等几何相关的二次效应的一致性。

Method: 提出GIVE框架，通过统一的对象状态公式表示编辑：两个与视频对齐的几何流——深度框（depth-box）编码粗略的3D位置和范围，方向框（orientation-box）提供与外观无关的方向线索。同时构建可扩展的图形引擎管线，执行物体级编辑程序，渲染受控的前/后视频对，隔离几何编辑的同时保持二次效应与变换一致。

Result: 实验表明GIVE在统一框架中能产生具有时间一致性和一致二次效应的忠实几何编辑，并在野外视频上展现出有前景的迁移能力。

Conclusion: GIVE通过几何指令驱动的统一框架，有效解决了生成式视频编辑中物体级几何变换的3D状态指定和二次效应一致性问题，为数字内容创作提供了可靠的工具。
...
TL;DR: FiCA是一个前馈式、即时的3D高斯编码头像生成管线，能从单张肖像图像生成逼真且可驱动的3D高斯头像，无需测试时优化。


<details>
  <summary>Details</summary>
Motivation: 单张图像重建逼真的3D人头外观和几何结构极具挑战，因为有限的视觉信息难以准确推断完整的3D头部形状和外观。现有方法通常需要测试时优化或多视图输入，效率较低。

Method: 结合人类中心视觉基础模型与扩散模型，学习从局部观察到完整3D网格的生成映射；设计前馈式网格精炼网络增强保真度与身份保持能力；利用通用先验模型将生成的网格解码为一组3D高斯，实现照片级渲染和实时表情驱动。

Result: 生成的化身能忠实代表多样身份，在视觉质量上超越近期竞争方法，且无需测试时优化即可实时驱动新表情。

Conclusion: FiCA首次实现了从单张图像到可驱动3D高斯头像的完全前馈式生成，兼顾了逼真度、身份保持和实时驱动能力，为单视图化身生成提供了高效且高质量的解决方案。
...
TL;DR: 本文提出EVA框架，通过生成连续的潜在视觉表征（Latent_slot tokens）替代传统离散输出，实现多模态模型的高效推理。针对Latent_slot tokens后出现的策略偏移问题，提出D-GSPO解耦优化方法，并构建EVA-230K数据集。实验表明EVA在多个基准上取得显著性能提升和推理效率改进。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型依赖生成离散输出（如代码或坐标）来调用外部工具，导致刚性依赖和高延迟问题，限制了多模态模型的推理效率和灵活性。

Method: 提出EVA框架，原生生成连续的潜在视觉表征（Latent_slot tokens）作为推理过程中的中间视觉思维；提出D-GSPO方法解耦潜在表征与离散文本的优化，解决transition window中的策略偏移问题；构建EVA-230K高质量文本-图像交错Chain-of-Thought数据集。

Result: 在多个基准测试上，EVA实现了显著性能提升，同时增强了推理效率。

Conclusion: 连续潜在视觉表征相比离散输出能够更高效地支持多模态推理，解耦优化策略有效解决了潜在与离散成分联合训练中的策略偏移问题。
...
TL;DR: 提出ProteusVPR，一种两阶段检索-精化框架，用于解决船舶环境下跨场景视觉定位问题，结合几何-视觉估计网络和时序信息，在自建XHZ数据集上将定位误差平均降低60%以上。


<details>
  <summary>Details</summary>
Motivation: 自主船舶机器人在海事环境中的视觉定位面临跨场景感知偏移问题，开放甲板与封闭舱室之间的视觉特征差异极大，现有VPR方法无法可靠泛化到这些截然不同的场景。

Method: 提出两阶段检索-精化框架ProteusVPR：第一阶段使用任意标准VPR模型进行初始图像检索；第二阶段引入几何-视觉估计网络，融合检索到的图像与两个时间上前序帧，利用几何描述子、局部仿射坐标系和相机方位角编码实现精确定位。同时构建了XHZ数据集，包含8K全景船舶图像、多层舱室结构和严格查询-数据库分离。

Result: 在XHZ数据集上的大量实验表明，ProteusVPR在多种VPR骨干网络上均能持续提升定位精度，平均将定位误差降低60%以上。

Conclusion: ProteusVPR为具有挑战性的跨场景海事环境提供了一种有效且鲁棒的精确视觉定位解决方案。
...
TL;DR: 本文提出M²C-EvDet框架，通过频域学习和超图计算的多域多阶跨模态知识蒸馏方法，缩小事件数据与帧数据在目标检测任务中的性能差距。


<details>
  <summary>Details</summary>
Motivation: 事件数据固有的稀疏表示和不足的视觉语义导致事件检测与帧检测之间存在显著性能差距，现有知识蒸馏工作仅关注空间语义或成对关系信息，在复杂场景下性能受限。

Method: 提出M²C-EvDet多域多阶跨模态知识蒸馏框架，包含两个核心模块：自适应频域解耦特征蒸馏（AF²D²）和多阶关系蒸馏（MORD），基于频域学习和超图计算实现。

Result: 在需要高时间分辨率和宽动态范围的场景中取得优于已有方法的检测性能。

Conclusion: 通过引入频域和多阶关系建模，M²C-EvDet有效弥合了事件检测与传统帧检测之间的性能鸿沟。
...
TL;DR: TuringViT通过线性注意力机制、VISTA数据筛选和原生动态分辨率预训练三大设计，以不到10%的数据量超越开源ViT基线，实现高效高分辨率视觉编码，并已部署于小鹏AI系统。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs/VLA系统多使用现成ViT（如SigLIP2），但下游任务对延迟、时序建模和VLM集成有定制化需求。训练SOTA级ViT需要大量图像-文本数据，且标准softmax注意力在高分辨率/动态分辨率预训练中成本过高，通常只能低分辨率预训练后做后处理适配。

Method: 1) Turing Linear Attention (TLA)：高效序列建模的线性注意力机制；2) VISTA-Curation：构建富含监督信号的图像-视频训练数据；3) 原生动态分辨率预训练：从一开始就支持灵活输入，无缝迁移到下游VLM。

Result: 仅用10%的数据量超越领先开源ViT基线；下游VLM性能更强；高分辨率输入下延迟缩放显著更优；缩放定律分析表明性能随数据量可预测提升，远未饱和。

Conclusion: TuringViT提供了一套可复现的pipeline，大幅降低了社区训练、定制和部署SOTA级ViT的成本，已在XPeng AI系统中作为统一视觉基础模型，有望使高性能Vision Transformer普惠化。
...
TL;DR: 提出Solo-to-Social框架，通过LLM规划交互结构（阶段分解、角色分配）与运动执行器（LoRA微调、自条件和伴条件）协同，实现文本驱动的人际3D交互生成。


<details>
  <summary>Details</summary>
Motivation: 文本到单人运动生成已取得进展，但扩展到文本驱动的3D人际交互（HHI）仍具挑战，因为HHI需要建模支配阶段进展、参与者角色和人际协调的社会结构。

Method: 采用规划-执行范式：LLM规划器将隐式交互语义转换为运动对齐的社会监督（阶段分解、角色分配）；运动执行器通过LoRA适配预训练单人运动模型、前一阶段自条件化和自我-相对伙伴条件化，将规划的社会结构落地为协调的双人运动。

Result: 生成的3D HHI在阶段一致性、角色对齐和伙伴感知协调方面得到改善。

Conclusion: LLM能够"思考"交互阶段和角色，但不能直接"运动"；提出的规划-执行范式有效弥合了社会组织与运动实现之间的鸿沟。
...
TL;DR: 本文提出Tailor-Bench基准测试，通过常规、非常规和不可能三种场景模式评估视觉世界模型对物理交互的长尾分布建模能力，发现现有模型在非常规和不可能场景下性能严重下降，表明它们仅能模拟常见物理交互而缺乏对物理原理的真正泛化。


<details>
  <summary>Details</summary>
Motivation: 现有视觉世界模型在常见物理交互模拟上表现优秀，但物理交互遵循长尾分布——大量稀有不寻常的交互被忽视。作者想知道当前模型是否真正内化并泛化了物理原理，还是仅仅记住了常见模式。

Method: 设计Tailor-Bench基准，包含三种渐进式场景模式：Regular（常规工具-任务对）、Unconventional（属性兼容的非常规工具替代，测试功能泛化）、Impossible（属性冲突的工具，测试约束感知）。同时设计两种统一评估设置：预测生成（无指导推断结果）和描述生成（指定目标结果要求忠实实现）。

Result: 实验显示明确的长尾鸿沟：模型性能从Regular到Unconventional再到Impossible场景依次下降，表明泛化能力有限。失败分析进一步揭示模型依赖表面视觉模式：图像模型无法实现正确的状态变化，视频模型还存在时间一致性问题。

Conclusion: 当前视觉世界模型未能真正内化物理原理，仅在常见交互上表现良好，对非常规和不可能的物理交互缺乏泛化能力和约束意识。
...
TL;DR: 本文提出3DCarGen，一种面向真实世界图像的单视图3D汽车生成框架，通过合成任意数量的3D一致多视图图像，结合前馈重建模型、多视图扩散模型和颜色-法向联合优化的网格重建算法，实现高质量、几何一致的3D车辆重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于多视图扩散的单图像重建范式存在视点有限和跨视图几何不一致问题，导致真实场景下的重建保真度降低，亟需一种可扩展的高质量3D车辆生成方法。

Method: 1) 输入单张图像，先合成固定视角的多视图图像；2) 通过前馈重建模型获得基于3D高斯泼溅的粗略3D表示；3) 以显式3D先验为条件，多视图扩散模型生成任意相机视角的3D一致图像；4) 扩展快速网格重建算法，引入颜色-法向联合优化，从稠密合成视图中恢复细节一致的三维车辆模型。

Result: 在合成数据集和真实世界数据集上的大量实验表明，与现有方法相比，该方法在几何一致性和重建保真度方面均取得了更优结果。

Conclusion: 3DCarGen通过可扩展的多视图合成与联合优化策略，有效解决了3D车辆重建中的跨视图几何不一致问题，为自动驾驶仿真提供了高质量3D资产生成方案。
...
TL;DR: 本文提出了首个能够处理多源、异质、非对齐地理空间数据的生成模型，用于热带气旋微波图像的空时插值。该模型通过自监督任务（随机掩码重建）训练，结合微波与红外数据，在CRPS指标上优于监督学习方法，且集合均值与确定性模型相当，功率谱更接近真实观测。


<details>
  <summary>Details</summary>
Motivation: 微波卫星图像对监测热带气旋降水和强度至关重要，但重访时间长，可能错过快速风暴演化阶段。现有插值方法难以处理来自不同仪器的微波数据的高度异质性（包括样本间变化、不规则时间间隔、地理错位及仪器特性差异）。

Method: 提出首个适用于多源、异质、非对齐地理空间数据的生成模型。采用自监督训练策略：随机掩码一个源并重建。模型结合微波和红外数据进行时空插值，通过生成式方法产生概率性预测。

Result: 自监督训练相比监督训练显著降低了CRPS（连续排名概率评分）。融合红外与微波数据相比仅用微波数据性能进一步提升。生成模型的集合均值与确定性模型相当，且生成的功率谱明显更接近真实观测。

Conclusion: 本文首次提出了能够利用多个微波仪器和红外观测在不规则时间间隔下插值热带气旋微波图像的生成模型，有效解决了多源异质数据的时空插值难题。
...
TL;DR: 本文提出一种统一的多模态RGB-D视频插帧框架，联合利用RGB外观、深度几何和事件相机的时间线索，通过双向光流估计与运动基优化实现高保真RGB和深度插值，并构建了新的RGB-D-Event三模态数据集。


<details>
  <summary>Details</summary>
Motivation: RGB-D相机受限于低帧率，难以捕捉快速动态场景；现有RGB插帧方法不适用于RGB-D场景，会产生模糊边界与几何不一致问题；仅靠两帧边界帧进行运动估计在复杂动态场景中约束不足。

Method: 提出统一多模态框架：1) 提取并融合RGB、深度和事件线索；2) 通过运动基优化估计RGB双向光流，通过Z轴优化估计深度流；3) 通过双向变形和软混合合成目标RGB-D帧。同时构建RGB-D-Event三模态数据集缓解训练数据稀缺问题。

Result: 在公开基准和所提数据集上的实验表明，该方法在RGB插值的光度保真度和深度插值的几何精度方面均优于现有方法。

Conclusion: 融合事件相机的高时域分辨率线索可有效提升RGB-D视频插帧质量，三模态数据集的构建为后续研究提供了基础。
...
TL;DR: ActiveScope提出了一种无需训练的框架，通过主动搜索和自校正机制增强多模态大语言模型在高分辨率图像中的细粒度感知能力，核心包含语义锚点定位（SAL）和干扰抑制细化（ISR）两个模块，分别解决语义偏差和上下文主导问题，在V* Bench等基准上达到96.34%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在高分辨率图像的细粒度感知上表现不佳，训练-free方法常被干扰物误导且无法定位多目标。作者将失败原因归因于两个关键问题：Contextual Dominance（显著干扰物压倒目标注意力导致定位不准确）和Semantic Bias（全局语义导致模型聚焦最显著概念，在多目标场景下定位不完整）。

Method: 提出ActiveScope框架，包含两个模块：(1) Semantic Anchor Localization (SAL) —— 利用细粒度语义锚点独立定位关键目标，缓解语义偏差；(2) Interference-Suppressed Refinement (ISR) —— 通过抑制对显著干扰物的注意力来细化定位，克服上下文主导。整个框架无需训练，采用主动搜索和自校正范式。

Result: 在高分辨率图像理解基准上，ActiveScope优于现有训练-free方法，在V* Bench上达到96.34%准确率。

Conclusion: 验证了主动搜索与自校正范式在提升MLLMs细粒度感知方面的有效性，代码已开源。
...
TL;DR: 提出双层次聚合网络（DHANet），通过层次化空间聚合（HSA）和层次化通道聚合（HCA）分别缓解跨域小样本分割中的语义过对齐和属性过对齐问题，并引入在线概率语义库（OPSB）利用查询预测构建伪原型以弥补支持样本不足，在四个目标域数据集上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有跨域小样本分割方法主要关注风格差异导致的特征分布偏移，忽略了跨域类别语义粒度与判别属性的显著差异，导致支持-查询匹配中出现语义过对齐和属性过对齐两种关键退化问题。

Method: 提出DHANet，包含三个模块：1) HSA模块沿空间维度进行多尺度区域聚合，生成层次化语义增强特征；2) HCA模块沿通道维度进行多尺度属性聚合，生成层次化属性增强特征；3) OPSB模块在推理阶段从查询预测中逐步构建并更新类别概率分布，采样多个伪原型作为额外支持信息。

Result: 在四个目标域数据集上的大量实验表明，所提方法达到了最先进的性能。

Conclusion: 通过层次化聚合策略缓解跨域语义和属性对齐问题，结合在线概率语义库补充支持信息，有效提升了跨域小样本分割的泛化能力。
...
TL;DR: 本文提出了一种无需训练的训练自由框架（基于DINOv3），通过语义感知特征重融合（SAFR）、自适应支持增强（ASE）和混合原型匹配（HPM）三个模块，在跨域少样本分割任务中达到了最先进性能，同时避免了训练开销和过拟合问题。


<details>
  <summary>Details</summary>
Motivation: 现有CD-FSS方法依赖训练或微调，计算成本高且易过拟合；即使引入视觉基础模型，性能提升有限甚至因过拟合而下降。因此需要一种消除可训练参数、避免训练开销和过拟合的框架。

Method: 基于自监督视觉编码器DINOv3构建训练自由框架，包含三个核心模块：(1) 语义感知特征重融合（SAFR）模块，识别并重融合强调语义模式的特征以增强语义判别性；(2) 自适应支持增强（ASE）模块，通过鲁棒的查询信息聚合缩小支持集与查询集之间的语义差距；(3) 混合原型匹配（HPM）模块，整合多种原型的匹配结果以适应不同领域的语义复杂度。

Result: 在四个目标域数据集上的大量实验表明，该方法在无需任何训练的情况下达到了CD-FSS的最先进性能。

Conclusion: 完全无需训练的训练自由框架可以有效解决跨域少样本分割问题，避免过拟合和计算开销，同时利用自监督视觉基础模型的强大表征能力实现优异的跨域泛化性能。
...
TL;DR: MM-TRELLIS是一种多模态3D车辆生成方法，通过整合多视图图像和LiDAR点云数据，在自动驾驶场景中实现高质量、几何精确的3D车辆网格重建，在Waymo数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的车辆生成方法未能充分利用多模态传感器数据（多视图图像和LiDAR点云），且依赖神经渲染重建导致网格质量低下；原生3D生成模型不适用于任意多视图输入，难以处理野外驾驶图像。

Method: 将多视图图像作为循环条件输入，LiDAR点云提供测试时引导以保证几何精度和跨视图一致性；在去噪过程中先对齐引导点云与模型先验，再强制生成几何与引导点云的一致性；最后基于3D高斯泼溅的不透明度引入体素滤波策略抑制漂浮物。

Result: 在Waymo数据集上的综合实验表明，该方法在高保真3D车辆生成方面优于现有方法。

Conclusion: MM-TRELLIS成功将自动驾驶数据集中的LiDAR和图像传感器集成到原生3D生成模型中，有效解决了野外驾驶场景下的3D车辆生成问题。
...
TL;DR: 该论文系统研究了三种可控微调策略（对比度归一化、数据增强、层冻结）对TrOCR模型适应中世纪手写文本识别的影响，通过在13世纪意大利手稿和READ-16基准上的实验，发现浅层冻结无显著伤害、去除CLAHE可获更优性能，并借助Grad-CAM和注意力图分析了错误模式。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的手写文本识别模型预训练于现代文本，微调至中世纪手稿时面临巨大的视觉域差异，现有研究缺乏对可控微调选择（对比度归一化、数据增强、层冻结）如何影响识别精度的系统分析。

Method: 在13世纪意大利手稿（I-CT 91 "Cortonese"）和READ-16基准上设计受控实验网格，对比13种配置组合；使用Grad-CAM梯度归因和解码器交叉注意力图进行错误模式诊断。

Result: Cortonese上最优配置达8.03% CER；冻结最多3个编码器层或6个解码器层不显著损害精度；去除CLAHE获7.84% CER；解码器冻结阈值跨数据集迁移更鲁棒；组合冻结需数据集特定重新验证。

Conclusion: 精心优化的微调策略可减少对图像预处理的依赖；解码器冻结迁移性优于编码器；有必要针对特定数据集重新验证组合冻结策略。
...
TL;DR: REDI-Match提出了一种旋转等变蒸馏（REDI）范式，将视觉基础模型的语义知识蒸馏到轻量级严格旋转等变编码器中，并配备熵驱动空间对齐模块，在密集特征匹配任务中实现了SOTA性能，尤其在严重面内旋转场景下，在SatAst数据集上精度提升13.89%，速度比RoMa v2快1.9倍，达到~41 FPS实时推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临两难困境：数据驱动方法需大量参数隐式学习旋转，而严格等变网络缺乏现代VFM的语义能力。当前框架通常冻结VFM并将旋转泛化负担全部推给下游解码器，形成了架构瓶颈。

Method: 提出REDI（Rotation-Equivariant Distillation）范式，将VFM的非等变语义表示蒸馏到轻量级严格旋转等变编码器中，利用等变几何架构约束高维语义；同时设计熵驱动空间对齐模块，通过评估离散旋转假设锁定规范坐标系，消除全局歧义。

Result: 在多个基准上达到SOTA，SatAst数据集上绝对位姿精度提升13.89%，速度比RoMa v2快1.9倍，单张RTX 4090上实现~41 FPS实时推理。

Conclusion: REDI-Match通过旋转等变蒸馏和熵驱动对齐有效解决了严重面内旋转下的密集特征匹配问题，打破了现有架构瓶颈，兼顾了精度与效率。
...
TL;DR: UniTranslator是一个统一的图文多模态框架，通过理解-生成对齐模块（UGAM）和空间掩码解码器（SMD）解决图像内机器翻译（IIMT）中的理解-生成冲突和空间位置错位问题，在多个基准上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态模型直接应用于IIMT时面临两大挑战：1）理解-生成冲突：翻译推断与文本生成监督不一致；2）空间位置错位：渲染文本无法准确匹配目标文本区域。

Method: 提出UniTranslator框架，包含两个核心组件：1）理解-生成对齐模块（UGAM）桥接理解与生成之间的表示差异，促进翻译内容预测与文本渲染的语义一致性；2）空间掩码解码器（SMD）通过文本区域的像素级监督，提高生成时的空间定位、几何对齐和布局可控性。

Result: 在多个基准测试上，UniTranslator在多种语言方向和复杂真实世界布局中均达到SOTA性能。实验还显示翻译理解与图像生成之间存在强相互增强效应。

Conclusion: UniTranslator通过紧耦合翻译理解与文本编辑，有效解决了IIMT中的关键挑战，验证了统一翻译多模态学习的优势。
...
TL;DR: 本文提出将单目度量物体尺寸估计作为反事实分析的诊断性任务，构建了Metric VQA数据集，评估了12个开源VLM，发现最大模型在野外场景上仍落后于纯文本LLM，并揭示了模型对场景几何信息的利用不足。


<details>
  <summary>Details</summary>
Motivation: 传统反事实分析在良态任务（多线索独立支持同一答案）中诊断价值有限，因为移除单一线索不会改变预测。需要一个病态（ill-posed）设定来有效评估视觉-语言模型的证据选择能力。

Method: 1) 提出单目度量物体尺寸估计作为病态诊断任务；2) 构建Metric VQA数据集（10,813个Objectron维度查询+331个野外真实测量场景）；3) 对12个开源VLM（3B-397B参数）进行反事实分析，分解6种视觉和语言证据通道（目标身份、目标像素、局部上下文、表观尺寸、场景几何、语言提示）。

Result: 1) 最大VLM（Qwen3-VL-235B等）在野外场景上落后于纯文本LLM；2) 目标身份是最关键的线索，目标像素和局部上下文仅对部分模型有帮助，表观尺寸能改变预测但缺乏方向性解读，全局场景几何基本未被利用；3) LoRA微调可以学习任务但未能学会利用场景几何。

Conclusion: 单目度量尺寸估计是一个有效的病态诊断任务，揭示了当前VLM在证据选择上的局限：过度依赖物体先验知识，忽视场景几何等关键视觉线索，且单纯微调不足以弥补这一缺陷。
...
TL;DR: TIGER提出了一种三先验融合框架（身份先验、几何先验、生成先验），结合渐进式三阶段训练策略，有效解决了人脸视频恢复中的身份偏移、视角纠缠引导和感知真实性问题，在保真度和时间稳定性上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有FVR方法难以同时应对三个关键挑战：身份偏移（identity shift）、视角纠缠引导（viewpoint-entangled guidance）和感知真实性（perceptual realism），亟需一种能兼顾身份保持、结构引导和生成质量的统一框架。

Method: 1) 身份先验：将主体判别性嵌入注入潜在空间，锚定身份；2) 几何先验：将2D参考线索提升至解耦的3D参数空间，通过跨源参数融合构建几何锚点；3) 生成先验：利用视频生成模型的一步整流流（one-step rectified flow）实现高效高真实感恢复；4) 渐进式三阶段训练：分阶段优化结构保真度、纹理重建和分布级真实感。

Result: 在自建大规模FVR数据集上的大量实验表明，TIGER在身份保真度和时间稳定性上均达到SOTA性能，实现了高质量、高效且身份一致的人脸视频恢复。

Conclusion: TIGER通过结构化三先验融合与渐进式训练策略，有效解决了FVR的多重挑战，为人脸视频恢复提供了一种兼顾身份一致性、时间稳定性和感知真实性的高效解决方案。
...
TL;DR: 本文提出OVBEVSeg，一种面向开放词汇BEV分割的几何感知框架，通过三阶段渐进式优化（2D-to-BEV伪标签、联合2D-BEV场景优化、3D几何蒸馏），利用高效高斯泼溅反投影和视觉-语言模型，在nuScenes数据集上实现SOTA性能：未见类别mIoU超越闭集方法15.3，推理速度比投影法快2.5倍，显存仅为其0.22倍。


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知方法局限于闭集场景，难以应对开放世界中的未知类别；将2D VLM语义提升到BEV存在严重的3D几何不一致性问题。

Method: 提出OVBEVSeg框架，采用基于高斯泼溅的高效反投影，通过三个渐进阶段增强3D几何一致性：(1) 基于可靠3D投影的2D-to-BEV伪标签；(2) 联合2D-BEV逐场景优化及BEV结构约束；(3) 3D几何蒸馏实现在线高效推理。

Result: 在nuScenes上，OVBEVSeg在未见类别上mIoU超越闭集方法15.3；无需新类标注即可与使用40%标注的半监督方法竞争；推理速度达投影法的2.5倍，显存仅为其0.22倍。

Conclusion: 通过几何感知的渐进式优化策略，OVBEVSeq有效解决了2D VLM语义到BEV的3D几何不一致问题，首次实现了开放词汇BEV分割，在精度和效率上均达到SOTA。
...
TL;DR: 本文提出SignNet-1M，一个涵盖ASL、CSL和DGS的大规模增强手语数据集，通过3D高斯泼溅、扩散模型和后期渲染增强三种轴线的数据增强方法，显著提升了手语模型在跨视角、跨背景、跨身份和后期渲染偏移下的泛化能力，同时保持原有分布性能。


<details>
  <summary>Details</summary>
Motivation: 现有手语模型通常在受限条件下采集的数据集上训练，缺乏视角、背景和手语者身份多样性，导致在真实世界分布偏移下鲁棒性差。

Method: 提出三种数据增强轴线：(i) 通过3D高斯泼溅(3DGS)实现新视角渲染（旋转和缩放）；(ii) 通过扩散模型进行场景/身份编辑（背景替换和手语者替换），同时保留手语动作和语言内容；(iii) 后期渲染增强，模拟采集和压缩伪影（如姿态/时间扰动和视频级损坏），以匹配野外录制条件。

Result: 在跨视角、跨背景、跨身份和后期渲染偏移下，使用SignNet-1M训练的模型泛化能力持续提升，同时保持强分布内性能。

Conclusion: SignNet-1M提供的大规模增强数据集和统一基准套件能有效提升手语模型在真实世界分布偏移下的鲁棒性，数据集和代码已开源。
...
TL;DR: 本文提出将KAN的可学习函数置于卷积结构层面而非每个边缘，设计了三种变体（SV-KAN、AG-KAN、RF-KAN），其中RF-KAN（基于形状的模型）和SV-KAN（基于值的简单模型）在约0.4M参数下，CIFAR-10/100上性能优于标准卷积和逐边缘KAN，且发现可学习形状是关键承载成分。


<details>
  <summary>Details</summary>
Motivation: 现有卷积KAN将可学习一元函数附着在每个卷积核条目上，导致参数量大且易过拟合。本文认为可学习函数应放在卷积的结构层面而非每个边缘，以更高效地组织设计空间。

Method: 沿单一轴组织设计空间：函数作用于像素值（SV-KAN用共享一元函数+静态空间滤波器，AG-KAN增加内容自适应高斯门控）或作用于滤波器形状（RF-KAN用定向脊剖面构建滤波器，并在局部振荡Morlet小波基中展开，辅以内容自适应振幅）。

Result: 在匹配的四层协议下（约0.4M参数），RF-KAN和SV-KAN在CIFAR-10上分别达88.47%和88.20%，CIFAR-100上达64.40%和64.57%，优于标准卷积和所有逐边缘KAN变体（包括Gram变体），参数量约为后者的1/5。移除可学习形状后精度下降超40个点。

Conclusion: 在匹配小规模下，形状模型（RF-KAN）和最简单的值模型（SV-KAN）性能并列最优；RF-KAN的增益来源于局部振荡基和内容自适应性；可学习形状是该尺度下的关键承载成分。
...
TL;DR: MATCH是首个基于Flow Matching的多视角异常检测方法，利用ODE公式估计似然度来检测异常，在Real-IAD和MANTA-Tiny数据集上达到SOTA性能，且能在消费级硬件上实时运行。


<details>
  <summary>Details</summary>
Motivation: 复杂工业对象需要从多个视角分析以检测异常，现有方法在多视角异常检测领域仍存在不足，需要更高效、准确的检测方法。

Method: 基于Flow Matching（FM）的ODE公式估计似然度，利用FM模型的架构灵活性将不同空间尺寸的特征变换到正态分布，并省略昂贵的散度项以支持实时推理。

Result: 在Real-IAD数据集上达到SOTA性能，首次在MANTA-Tiny数据集上进行了全面评估，在异常检测和分割任务上均取得最佳结果，且能在消费级硬件上实时运行。

Conclusion: MATCH证明了Flow Matching在多视角异常检测中的有效性，通过省略散度项实现了实时生产场景下的部署能力，消融研究验证了方法设计选择的合理性。
...
TL;DR: 本文提出一种针对多模态动作识别的后验混合OOD检测器，利用多模态与单模态预测之间的关系，结合特征空间分数与对数概率归一化，在MultiOOD基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在训练时考虑OOD检测，但推理时仍直接使用为单模态设计的OOD检测器，丢弃了多模态信息。作者发现多模态与单模态预测之间存在有意义的关系，希望利用这一信号构建专为多模态场景设计的后验检测器。

Method: 1) 利用多模态与单模态预测之间的关系构建后验检测信号；2) 结合特征空间分数检测多模态空间的离流形样本；3) 使用多模态logits进行归一化；4) 该混合检测器可与现有训练时方法兼容。

Result: 在MultiOOD基准的多个数据集上，平均性能超过现有SOTA方法。

Conclusion: 在推理时显式考虑不同模态信息对多模态OOD检测至关重要，所提方法有效提升了检测鲁棒性。
...
TL;DR: EgoSAT是首个面向流式设定下自我中心视频推理的综合基准，包含1,997个视频、4,800个问答对，评估VLM在回顾、在线和前瞻推理上的表现，发现现有模型存在严重的错误自信问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频推理基准缺乏对流式交互场景的统一评估，无法全面衡量VLM在帧序列到达时对过去、现在和未来事件的持续理解能力。

Method: 构建EgoSAT基准，统一回顾推理（已发生事件）、在线理解（正在发生事件）和前瞻预测（未来事件）三类任务于流式框架中，并要求模型仅基于已观测帧进行推理；评估多种开源和闭源VLM，区分可回答性并分析模型置信度校准情况。

Result: 现有VLM在前瞻和回顾建模上表现不佳，且存在严重的置信度校准问题——模型置信度与问题的固有可回答性不匹配，导致大量"自信但错误"的行为。

Conclusion: EgoSAT揭示了当前VLM在流式自我中心视频推理中的能力瓶颈和校准缺陷，为未来研究提供了重要基准。
...
TL;DR: 本文系统研究了三种编码器网络（通用型与病理专用型）对组织病理学图像中经典图像变换的鲁棒性，发现嵌入向量对变换具有一定鲁棒性但并非完全不变，且通用网络与病理专用网络存在显著差异。


<details>
  <summary>Details</summary>
Motivation: 探究编码器网络在组织病理学分类任务中对图像变换的鲁棒性及其在潜在空间中的编码特性，理解为何数据增强能提升性能，并比较通用与病理专用编码器的差异。

Method: 使用Lunit Inc.、Bioptimus（病理专用）及Meta Research Team（通用）的编码器网络，对H&E染色组织切片图像进行标准数据变换，通过比较原始与变换后图像嵌入向量的距离，以及与随机嵌入的距离，评估嵌入的方差和鲁棒性。

Result: 原始与变换后图像的嵌入向量比随机嵌入更接近，表明具有一定鲁棒性；但嵌入并非完全不变，编码器网络未完全消除变换在潜在空间中的影响，这解释了数据增强的有效性；通用与病理专用编码器之间存在显著差异。

Conclusion: 当前编码器网络对图像变换具有部分鲁棒性但非完全不变性，病理专用与通用编码器行为不同，提示需针对下游任务进一步优化编码器的变换不变性。
...
TL;DR: 本文提出PCFM，一种基于PTv3和流匹配（Flow Matching）的医疗点云补全生成模型。在SkullFix、SkullBreak和Mandibular Defect三个数据集上，PCFM在生成质量上达到SOTA，且采样步骤远少于扩散模型（PCDiff），同时PTv3骨干较PVCNN带来最高7倍加速。实验还展示了模型规模与点分辨率上的扩展趋势。


<details>
  <summary>Details</summary>
Motivation: 医疗点云补全对解剖结构重建和临床工作流至关重要，但该场景下的生成式建模研究尚不充分。

Method: 采用连续时间生成建模（流匹配Flow Matching），以PTv3为骨干网络，构建PCFM模型；同时构建确定性的PTv3编码器-解码器基线以及基于扩散的PCDiff（含PVCNN和PTv3两种去噪器）作为对比。

Result: PCFM在生成性能上达到SOTA，与确定性PTv3基线竞争力相当；采样步骤远少于扩散模型；PTv3较PVCNN带来最高7倍加速；模型规模和点分辨率增大可带来一致性的性能提升。

Conclusion: PCFM是医疗点云补全场景下高效且高质量的生成式方法，具备实际临床应用潜力。
...
TL;DR: S1-Omni-Image 是一个面向科学图像的理解、生成与编辑的统一多模态模型，采用“先思考后生成”范式，在科学推理骨干 S1-VL-32B 基础上耦合图像生成模块，通过构建 SciGenEdit 训练集，在生成和编辑基准上达到 SOTA，同时保持理解能力不降。


<details>
  <summary>Details</summary>
Motivation: 通用图像生成模型难以胜任科学图像任务，因为科学图像需要高保真合成之外，还必须理解科学语义、结构关系、领域知识和任务意图。现有方法缺乏统一的科学图像理解-生成-编辑框架。

Method: 基于科学多模态推理骨干 S1-VL-32B，耦合图像生成模块，采用“先思考后生成”范式：模型先产生任务导向的推理轨迹、文本答案和任务特殊 token，再将其隐状态注入生成模块，以条件化图像生成或编辑。统一支持理解、生成（科学插图、文本渲染、逻辑图、关系对比、数据图表、科学可视化）和编辑（分割即编辑、多轮插图编辑、医学/地理图像分割、医学图像翻译、超分辨率）。

Result: 构建了 314K 样本的 SciGenEdit 训练集。在 GenExam 和 TechImage-Bench 上超越开源模型；在 MSD、cigRockSEM、SynthRAD2025、IXI 四个编辑基准上达到 SOTA；科学图像理解评估上保持稳定性能。

Conclusion: S1-Omni-Image 在科学图像生成和编辑上取得显著提升，同时保持了继承自 S1-VL-32B 的科学图像理解能力，证明了“先思考后生成”范式在科学多模态任务中的有效性。
...
TL;DR: P-MTP通过渐进式多令牌预测和置信度门控动态草稿机制，在文档解析任务中实现高达5倍加速且几乎不损失精度。


<details>
  <summary>Details</summary>
Motivation: 现有VLM在解析密集令牌文档时存在显著延迟瓶颈，而多令牌预测（MTP）在扩展预测深度时面临优化不稳定的问题。

Method: 提出P-MTP框架，包含：(1) 渐进式课程损失（Progressive Curriculum Loss），通过累积路径可靠性和回溯目标一致性自适应调整不同预测深度的权重，实现从易到难的优化过渡；(2) 置信度门控动态草稿（Confidence-Gated Dynamic Drafting），在推理时自适应校准推测长度，最大化有效预测深度和接受率。

Result: 在多个基准和架构上，P-MTP实现高达5倍加速，且精度损失可忽略不计，首次成功验证了大规模前瞻多令牌预测在文档解析领域的有效性。

Conclusion: P-MTP有效解决了MTP在深度扩展时的优化不稳定问题，为高吞吐文档解析提供了高效推理加速方案。
...
TL;DR: SENTRY 是一种无需训练、即插即用的模块，通过在写入前验证短期时间一致性来改进 SAM2 跟踪器的内存更新机制，解决因仅依赖置信度掩码选择导致的漂移问题，在多个基准上实现零样本 SOTA，且仅增加少量计算开销。


<details>
  <summary>Details</summary>
Motivation: 论文发现现有 SAM2 跟踪器仅依赖置信度进行掩码选择，导致在遮挡、快速运动和干扰物场景下出现严重的漂移问题。

Method: 提出 SENTRY 模块，采用“先验证再写入”策略：为每帧聚合多样化的分割假设，回溯生成短轨迹，并通过邻域感知的循环一致性匹配与近期轨迹对比，筛选时空一致的掩码，替代纯置信度驱动的写入机制。

Result: 集成到五个强基线中，在九个基准上取得一致提升，在 LaSOT、LaSOT_ext、GOT-10k、VOT20、VOT22 和 DiDi 上实现零样本新 SOTA；SAM2-L 版本在 A100 上运行速度达 32.8 FPS，仅增加约 0.4–0.6 GB 显存。

Conclusion: 在写入时强制时间有效性验证能够在不重新训练的情况下稳定记忆增强跟踪器的性能，并提供了首个统一的跨尺度 SAM2 跟踪器评估。
...
TL;DR: 本文提出Lite Any Stereo V2 (LAS2)，一种面向高效零样本立体匹配的超快模型系列，通过2D-only代价聚合架构和三阶段训练策略（合成监督、自蒸馏、真实世界知识蒸馏），在保持极低延迟的同时实现了最先进的零样本泛化性能，在H200和Orin平台上分别比Fast-FoundationStereo快1.8倍和2.7倍。


<details>
  <summary>Details</summary>
Motivation: 现有立体匹配方法存在两难：高精度模型通常体积大、计算量大或依赖基础模型先验，难以部署在资源受限平台；而高效模型虽推理快，但零样本泛化能力普遍较弱。本文旨在打破这一假设，设计同时具备高效率和强零样本泛化能力的超快立体匹配模型。

Method: 架构上，重新审视高效立体匹配设计，提出纯2D代价聚合框架，针对实际推理延迟而非理论MACs进行优化；训练上，设计三阶段策略：合成数据监督预训练、自蒸馏、真实世界知识蒸馏，并引入伪标签过滤和误差钳制操作以改善合成到真实的迁移稳定性。模型系列包括前馈变体（适配不同效率预算）和迭代变体（追求更高精度）。

Result: LAS2在高效立体方法中达到最先进精度，同时保持显著更低的延迟。LAS2-H在零样本性能上全面超越迭代方法Fast-FoundationStereo，在H200和Orin上推理速度分别提升1.8倍和2.7倍。

Conclusion: 本文证明高效模型同样可以具备强大的零样本泛化能力，为资源受限平台上的立体匹配部署提供了可行的解决方案。
...
TL;DR: 本文提出GeoLaV，一个两阶段框架，通过从图像中蒸馏3D几何知识来增强文本驱动的视频目标分割。第一阶段通过单目新视角合成学习几何一致的视觉表示，第二阶段通过几何感知蒸馏将3D结构知识迁移到视频分割模型，在多个RVOS基准上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动视频目标分割（RVOS）模型通常在2D图像或视频数据集上使用简单分割损失训练，忽略了帧间的几何一致性，导致空间理解能力薄弱。

Method: 提出GeoLaV两阶段框架：第一阶段，通过单目新视角合成进行单目几何预训练，在大规模单图像数据集上通过空间对齐学习几何一致的视觉表示；第二阶段，引入几何感知蒸馏，使用通用3D先验模型将3D结构知识迁移到视频分割模型上微调，增强3D感知能力以及时空连贯性和语言定位能力。

Result: 仅使用图像分割数据即展现出显著的零样本RVOS泛化能力；结合几何感知蒸馏在视频上微调后，在多个RVOS基准上取得最先进性能。

Conclusion: GeoLaV证明了从图像中蒸馏3D几何知识能有效提升文本驱动视频分割的时空一致性和语言定位能力，且代码已开源。
...
TL;DR: 提出了video-SALMONN-R³，一种通过强化学习实现重看机制（re-watch）的端到端视频大语言模型，无需思维链冷启动，采用重新回答策略（re-answer）和重新提问机制（re-ask），在降低计算成本的同时提升视频问答性能。


<details>
  <summary>Details</summary>
Motivation: 视频LLM受限于计算和内存预算，通常使用降低的帧率和空间分辨率，导致可能错过问答所需的关键信息。现有两阶段范式（先粗看定位再细看）可行，但基于CoT的方法需要大量数据标注且会降低预训练视频理解能力。

Method: 1) 通过强化学习（而非CoT冷启动）实现端到端的重看机制；2) 提出重新回答策略（re-answer），模型先在第一次观看时给出直接答案，重看后再精炼；3) 提出重新提问机制（re-ask），在重看局部片段时重新注入查询以提升问题遵循度。

Result: video-SALMONN-R³持续优于基础模型和QA-SFT基线，且以显著更低的计算成本超越了先前基于重看的方法。

Conclusion: 强化学习驱动的重看机制是替代CoT冷启动的有效方案，重新回答和重新提问机制有效解决了重看行为与预训练模型倾向之间的不匹配问题。
...
TL;DR: MambaRaw是一种基于JPEG预览的元数据原始图像重建框架，利用状态空间模型（SSM）高效估计熵参数，通过TileMambaBlock和Energy-Aware Refinement两个轻量模块实现空间-能量耦合上下文建模，在三个相机数据集上达到SOTA，PSNR提升1.2-1.4 dB，延迟降低约9%。


<details>
  <summary>Details</summary>
Motivation: 现有基于元数据的原始图像重建方法在高分辨率（如4K）下计算开销大，注意力机制随特征图二次缩放，限制了实际应用。因此需要更高效的上下文建模方法。

Method: 提出MambaRaw框架，核心是空间-能量耦合上下文建模机制，包含两个轻量模块：(1) TileMambaBlock——仅对信息密集的tile进行Mamba式选择性扫描，提升效率；(2) Energy-Aware Refinement (EAR)——恒等初始化的残差模块，增强特征表示以匹配原始信号的长尾能量分布。

Result: 在Sony、Olympus、Samsung三个相机数据集上超越强基线，达到JPEG引导原始重建新SOTA。低元数据比特率下PSNR提升1.2-1.4 dB，端到端编码延迟降低约9%。

Conclusion: MambaRaw通过SSM和空间-能量耦合机制有效解决了高分辨率原始图像重建的效率问题，兼顾了重建质量和计算效率，具有实际应用价值。
...
TL;DR: 本文针对WordArt艺术文本识别（WATER）任务，从数据和模型两方面推进。数据上构建了2M规模的合成数据集WATER-S（含渲染管道SynthWordArt和VL模型生成两个子集），模型上提出支持任意形状输入和自回归解码器的WATERec架构，在WordArt-Bench上达到90.40%准确率，远超通用视觉语言模型和OCR专用模型。


<details>
  <summary>Details</summary>
Motivation: 现有场景文本识别（STR）数据集和方法主要针对常规场景文本和固定模板输入设计，难以扩展到高度定制化（字体、纹理、布局）的WordArt艺术文本识别任务，且缺乏大规模高质量的艺术文本数据。

Method: 数据层面：构建2M规模的WATER-S数据集，包含两个子集——1）SynthWordArt升级渲染管道生成的高可控合成数据；2）Qwen3-VL进行提示挖掘+Z-Image进行图像合成生成的多样化真实感数据。模型层面：提出WATERec，采用支持任意形状输入的视觉编码器+自回归解码器，打破固定模板STR在WordArt上的瓶颈。

Result: WATERec在WordArt-Bench上达到90.40%准确率，大幅超越通用视觉语言模型和OCR专用视觉语言模型，在艺术文本等不规则文本上达到SOTA性能。

Conclusion: 通过大规模合成数据（WATER-S）和适配不规则文本的模型架构（WATERec），有效解决了WordArt艺术文本识别难题，为WATER任务提供了强有力的基线方案。
...
TL;DR: RetiSEM是一种面向有限多模态生物医学数据的域约束结构方程建模框架，通过分块组织变量、施加禁止边约束以及分解路径效应（TE、NDE、NIE），在合成基准和真实碎片化数据（NHANES+视网膜表征）中实现了更优的因果图恢复和中介分析性能。


<details>
  <summary>Details</summary>
Motivation: 从碎片化的生物医学数据（临床、分子、影像变量常不完整或非联合观测）中学习因果模型极具挑战，现有方法缺乏对块状缺失和多模态异构数据的有效因果推断能力。

Method: 提出RetiSEM框架：(1) 将变量组织为生物学可解释的块结构；(2) 施加禁止边约束以融入领域知识；(3) 将路径级效应分解为总效应(TE)、自然直接效应(NDE)和自然间接效应(NIE)；并利用结构方程建模进行因果图恢复与中介分析。

Result: 在十种合成基准（涵盖不同维度、非线性、因果深度和路径结构）上，RetiSEM的结构误差更低、因果准确性更高；在NHANES临床变量与外部视网膜表征组成的碎片化真实数据中，视网膜变量主要表现为下游生物标志物样指标，存在较小但可检测的间接效应。

Conclusion: RetiSEM为有限资源下的生物医学AI提供了一种可解释的结构化因果假设检验框架，代码已开源。
...
TL;DR: VistaRef框架通过局部手部实体建模（LHEM）、几何射线建模（GRM）和方向一致性对齐损失（OCAL），显式增强空间定向感知，在指向性手势接地任务中相比基线提升14个百分点的绝对准确率。


<details>
  <summary>Details</summary>
Motivation: 基于Transformer的视觉模型在全局注意力机制中忽略了微几何关系，导致指向任务中方向精度下降，表现为指向漂移和定位模糊，尤其在处理远处或密集物体时。

Method: 1) LHEM模块：引入手部姿态嵌入，增强对细微手指偏差的捕获能力；2) GRM模块：借鉴多视图几何，将隐式方向信息转化为显式空间几何特征，通过注意力机制引导特征聚合与深度融合；3) OCAL损失函数：协同监督手部存在性和指向一致性。

Result: 在指向性手势接地任务中，VistaRef相比基线模型取得了14个百分点的绝对准确率提升，定性分析验证了其有效建模从手部到目标的几何相关性。
...
TL;DR: 提出了一种无标注、无学习、几何驱动的框架，通过事件流中的自运动结构（FoE模型+偏航补偿）将检测目标分类为静态或独立运动物体，在MVSEC和Prophesee数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有汽车事件数据集依赖基于帧的外观标注，不适应运动感知事件处理；缺乏利用事件流本身进行运动物体检测的无监督方法。

Method: 基于焦点展开（FoE）模型结合偏航补偿估计全局背景运动；通过尺度不变残差量化局部运动与全局预测的偏差，将目标标定为运动物体；时间稳定性机制增强跨事件窗口的鲁棒性；无需学习或人工运动标签，兼容任意输入边界框。

Result: 在MVSEC和Prophesee 1兆像素汽车检测数据集上，该方法在各种驾驶场景下表现稳定；偏航补偿在转弯场景中显著改善结果；简单的平移局部模型在精度与效率之间取得良好平衡。

Conclusion: 提出了一种几何驱动的无监督运动物体分类框架，有效利用事件流内在自运动结构，摆脱了对外观标注和学习模型的依赖，为事件视觉中的运动感知提供了一种轻量级、可迁移的解决方案。
...
TL;DR: 本文从后验传输的角度理论分析了基于流的逆问题求解器（FlowDPS、FLOWER、PnP-Flow等），证明贝叶斯条件化在确定性流先验中应通过源分布重加权而非轨迹校正实现，揭示了现有轨迹引导方法存在本质偏差并导致200-800倍更大的误差和模式坍缩，据此提出了一种廉价的原理性速度校正求解器。


<details>
  <summary>Details</summary>
Motivation: 现有基于流的逆问题求解器（FlowDPS、FLOWER、PnP-Flow等）通过在概率流ODE中添加测量引导项来近似后验采样，取得了强劲的实证结果，但这些逐步校正实际近似了什么、与真实后验p(x|y)的偏差有多大，尚未被理论刻画。

Method: 从后验传输角度出发，证明确定性流先验中贝叶斯条件化完全由源分布重加权实现（无需漂移校正）；将轨迹引导求解器解读为最小动能校正场，并揭示FlowDPS/FLOWER/PnP-Flow分别对应于该校正场的零阶/高斯/近端近似；在Wasserstein距离下界定了后验偏差。

Result: 2D可控实验证实：源重加权方法在所有指标上匹配真实后验（达蒙特卡洛下限），而轨迹引导方法产生200-800倍更大的误差并导致后验模式坍缩（与引导强度无关）。提出的速度校正求解器在域内（AFHQ、CelebA）和域外设置中均具有竞争力，且能产生与重建误差相关的不确定性估计。

Conclusion: 轨迹引导方法存在原理性缺陷，源重加权才是确定性流后验采样的正确途径；据此设计的原理性速度校正求解器兼顾了性能与不确定性量化能力。
...
TL;DR: 提出VisCritic，一种通过比较操作前后截图在视觉特征空间中进行过程奖励验证的框架，用于提升GUI智能体在长程任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有GUI智能体在长程任务中频繁失败，因为缺少步骤级验证；当前的过程奖励模型仅通过文本推理验证动作，忽视了GUI状态变化的视觉本质。

Method: 使用孪生视觉Transformer提取变化感知表示，结合动作感知批评头（联合评估动作成功、任务进度和错误类型），并通过弱监督数据构建管线从已有轨迹中生成训练数据。

Result: 在五个基准测试上的实验和离线分析表明，VisCritic作为即插即用增强模块，能够提升多种GUI智能体的基准指标，并提供视觉诊断线索。

Conclusion: 视觉过程奖励验证相比纯文本验证能更有效捕捉GUI状态变化，VisCritic可作为通用增强模块提升GUI智能体的鲁棒性。
...
TL;DR: ForensicsTok通过将图像篡改定位重构为自回归序列生成任务，利用Token Splatting解码器和层次化专家融合模块，在六个基准数据集上优于现有MLLM方法和强专家基线，同时具有更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在取证任务中依赖外部分割解码器，引入信息瓶颈导致空间信号稀释，且受限于分割器的语义先验，定位效果次优。

Method: 提出ForensicsTok，将图像篡改定位重构为自回归序列生成任务；设计Token Splatting解码器(TSD)通过码本感知的代码平滑将token映射为二值掩码；提出层次化专家融合(HEF)模块注入取证专家模型的多尺度特征。

Result: 在六个基准数据集上，显著优于现有MLLM基线，小幅优于强取证专家基线，并展现出更强的扰动鲁棒性。

Conclusion: 统一的自回归架构有效弥补了标准MLLM缺乏取证先验的问题，实现了精确的篡改定位。
...
TL;DR: PointVG-R 提出了一种基于几何感知推理的多模态大语言模型，通过强化学习和冷启动数据策略，结合视觉思维链和自适应重要性加权，在指向性视觉定位任务上实现了15.86 mIoU的显著提升，达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法将图像编码为静态特征表示，主要在语言域进行推理，忽视了图像中丰富的感知线索和显式空间几何信息，导致模型在解释手势空间关系时存在认知脆弱性。

Method: 1) 提出几何推理流水线，模拟人类解读指向手势时的迭代认知过程；2) 构建EgoPoint-CoT高质量视觉思维链数据集，通过SFT和RL引导模型；3) 提出基于组方差的自适应重要性加权策略，动态调整奖励信号以优化学习。

Result: PointVG-R达到SOTA性能，在mIoU上比基线提升15.86个点，消融实验验证了各模块的有效性。

Conclusion: 将几何感知推理引入MLLM，通过RL和冷启动数据让模型学会"用图像思考"，显著提升了指向性视觉定位的表现，展示了推理引导在多模态理解中的潜力。
...
TL;DR: 本文提出 Counterfactual-World (CF-World) 基准，通过反事实生成任务测试文本到图像模型是否具备真正的因果理解，实验表明所有模型在反事实设定下性能严重下降，原因是模型将世界知识与视觉外观编码为紧密耦合的模式，过度依赖训练数据中的共现模式。


<details>
  <summary>Details</summary>
Motivation: 探究文本到图像生成模型是否具备真正的因果理解能力，而非仅仅依赖视觉-文本相关性的模式匹配。受Russell归纳火鸡启发，通过反事实场景检验模型能否在违背现实先验的规则下生成图像。

Method: 构建CF-World基准，每场景设三个递进层次：事实生成、显式反事实生成（含直接视觉指令）、隐式反事实生成（需从规则变化中因果演绎）。使用VLM评估器（CF-Eval），引入两个指标：Prior Resistance Rate (PRR) 衡量模型克服现实先验的能力，Reasoning Retention Rate (RRR) 评估无显式视觉线索时维持反事实生成的能力。

Result: 所有模型从事实到反事实设定性能急剧下降。模型将世界知识与视觉外观编码为紧密耦合的模式，反事实任务下被迫退回到熟悉的常识先验。

Conclusion: 当前T2I模型缺乏真正的因果理解，其生成能力本质上是基于训练数据中高频视觉共现的模式匹配，无法在违背现实规则的场景下进行可靠的因果推理。
...
TL;DR: SPOFA是一个面向异构知识蒸馏的特征与梯度双稳定框架，通过LayerNorm解耦投影器解决特征范数差异问题，利用MEMA动态缩放器缓解梯度冲突，以极轻量参数实现SOTA精度。


<details>
  <summary>Details</summary>
Motivation: 异构知识蒸馏（HKD）在不同架构（如Transformer到CNN）间迁移知识时存在严重的训练不稳定性，源于特征范数差异导致的优化拖拽以及主任务与蒸馏目标间的梯度冲突。

Method: 提出SPOFA框架，包含两个核心模块：1) 基于LayerNorm的解耦投影器，显式解耦特征幅度与方向，构建有界稳定的语义对齐空间；2) 动量驱动的指数移动平均（MEMA）动态缩放器，建立优化轨迹的历史基线，评估瞬时梯度冲突并自适应惩罚有害蒸馏信号。

Result: 在两个主流基准上达到SOTA精度，显著优于计算开销大的方法，且相比标准基线仅引入极小计算开销。

Conclusion: SPOFA通过轻量级的特征-梯度双稳定机制有效解决了HKD中的训练不稳定问题，实现了高效且高性能的异构知识蒸馏。
...
TL;DR: 本文提出了一种基于动态区间编码和先验平衡优化的QUBO灰度CT重建框架，通过逐轮局部编码和边界命中引导的更新策略，在稀疏视角和有限角扇束CT实验中优于多种基线方法，并在D-Wave混合BQM求解器上验证了可行性。


<details>
  <summary>Details</summary>
Motivation: 现有QUBO-based CT重建受二进制变量预算限制：固定全局位平面编码在提高灰度精度时导致QUBO规模增大和耦合复杂度上升，而低位编码引入量化误差，亟需一种高效的灰度级表示方法。

Method: 提出动态区间编码（每轮仅对当前估计值附近局部灰度区间内的活跃像素编码）与边界命中引导的更新规则（自适应切换搜索扩展与局部精化），并引入投影域数据一致性与边缘保持二次先验的先验平衡优化以提升稳定性。

Result: 在稀疏视角和有限角扇束CT实验中，该方法在结构和灰度级分布恢复上优于解析法、迭代法、变分法及基于表示法的基线。消融实验表明改进主要来自动态局部编码和更稳定的数据保真-先验耦合。D-Wave混合BQM求解器实验验证了其在量子-经典混合后端的可执行性。

Conclusion: 动态区间编码与先验平衡优化有效解决了QUBO灰度CT中二进制变量预算受限问题，兼顾了重建精度与可扩展性，并具备在混合量子-经典计算平台上落地的潜力。
...
TL;DR: PatternGSL提出了一种模板无关、可学习的结构化服装表示语言，从单张图像直接预测完整缝纫图案（包括面板边界、参数化接缝和显式缝合拓扑），并通过视觉-语言框架实现无需优化或手动清理的确定性解码，同时发布了包含30万样本的大规模图像-GSL配对数据集PatternGSLData。


<details>
  <summary>Details</summary>
Motivation: 现有无模板方法能捕捉表面几何但缺乏缝纫结构无法仿真，而程序化系统虽可直接仿真但受限于预定义模板，存在几何重建与结构化服装构造之间的表示鸿沟。

Method: 提出PatternGSL结构化服装表示语言（模板无关、可学习、紧凑标准化），构建视觉-语言框架从单张图像直接预测GSL规格，通过轻量级确定性有效性处理解码为可仿真服装，并创建30万样本的PatternGSLData数据集用于监督训练。

Result: 相比基线方法在图案精度上更优，能恢复显式缝纫结构，支持可靠的布料仿真和图案级编辑，且通过相同确定性解码管道实现。

Conclusion: PatternGSL弥合了几何重建与结构化服装构造之间的鸿沟，将缝纫结构提升为生成建模的一等目标，实现了从单张图像到仿真就绪服装的端到端预测。
...
TL;DR: 本文提出了一种多级随机即插即用（ML-SPnP）方法用于稀疏视图CT重建，通过在多分辨分析近似空间中执行多级步骤，避免了对细层随机先验梯度的昂贵估计，从而在保持重建质量的同时显著降低运行时间。


<details>
  <summary>Details</summary>
Motivation: 现有的随机PnP方法虽然通过重噪化提升了鲁棒性，但收敛需要大量迭代，限制了实际效率。在随机设置下，直接跨层级强制先验一致性需要多次评估去噪器函数以准确估计细层先验梯度，计算成本过高。

Method: 提出多级随机PnP方法（ML-SPnP），其核心创新是在多分辨分析（MRA）近似空间中执行多级步骤。利用小波分解的结构特性，使得先验一致性校正项在期望下消失，从而避免了对细层随机先验梯度的昂贵估计。

Result: 在SVCT重建实验上，ML-SPnP方法达到了与最先进方法相当的重建质量，同时显著减少了运行时间。

Conclusion: 通过巧妙利用多分辨分析空间和小波分解特性，ML-SPnP有效解决了随机PnP方法在多级框架中计算成本高的瓶颈，为加速随机PnP重建提供了新思路。
...
TL;DR: ConQuer是一种改进的视觉-语言对比预训练方法，通过为每个解剖概念学习独立的跨注意力查询，替代CLIP的单一全局表征，在3D医学CT上实现更精细的局部对齐，训练的Jolia模型在分类、报告生成等任务上超越CLIP基线并达到SOTA。


<details>
  <summary>Details</summary>
Motivation: CLIP风格的图文对比预训练将每个模态编码为单个全局token，但医学图像涉及多器官、放射报告结构冗长且包含多个结构化章节，全局压缩会丢失重要的局部细节信息。

Method: ConQuer将放射报告拆分为概念特定的章节，为每个概念（如解剖区域/器官）学习跨注意力查询，从图像中池化匹配的特征，无需分割掩码或空间监督，并对每个概念独立进行对比学习。

Result: 在胸部与腹部CT上训练的Jolia模型在发现分类、报告生成和跨中心迁移任务上持续优于CLIP基线，在多个公开基准上达到新SOTA。

Conclusion: 通过概念级别的局部对齐增强全局对比学习，能有效保留医学图像-文本对中的细粒度信息，同时提供内置的空间可解释性。
...
TL;DR: 本文提出EERLoss，一种可微的等错误率(EER)近似函数，用于直接优化生物特征验证模型。在包含18.5万样本的击键动力学KVC-onGoing基准上，相比现有损失函数，EERLoss实现了约30%的EER相对降低，且收敛速度显著更快。


<details>
  <summary>Details</summary>
Motivation: 深度学习生物特征验证模型通常优化间接目标函数，导致训练过程与评估指标（等错误率EER）之间不一致，需要一种直接优化EER的任务对齐损失函数。

Method: 提出EERLoss，一种次可微的、可任意精确逼近EER的损失函数，并具备适应DET曲线上任意特定操作点的能力。在大规模击键动力学基准KVC-onGoing（18.5万+受试者）上进行了全面消融实验和与KVC-winning架构的对比评估。

Result: 消融研究证明EERLoss优于现有最先进损失函数；收敛速度显著快于其他损失；用EERLoss重新训练KVC-winning架构，EER相对降低约30%。

Conclusion: EERLoss作为一种任务对齐的训练目标，特别适用于高方差的生物特征（如击键动力学验证），显著提升了大规模基准上的性能。
...
TL;DR: 本文提出ViTexQA数据集和FrameThinker方法，针对多帧视频文本理解中时间依赖性问题。ViTexQA要求跨帧文本融合才能回答，FrameThinker通过两阶段训练（CoT监督微调+时间强化学习）实现显式时间建模，在ROUGE-L上提升6.3%。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在视频文本理解中表现有限，因为真实视频理解需要整合多帧中时间分布的文字线索，而现有数据集大多可以从单帧回答，无法反映真实需求。

Method: 构建ViTexQA数据集，采用质量控制的CoT标注流程并增强时间约束；提出FrameThinker，两阶段训练：CoT引导的监督微调生成帧感知推理链，然后进行时间强化学习，用多帧一致性奖励优化。

Result: 在ViTexQA上，方法超越SOTA基线，ROUGE-L提升6.3%。

Conclusion: 通过强制跨帧文本依赖的数据集和显式时间建模的训练方法，有效提升了视频文本理解中的时间推理能力。
...
TL;DR: 提出了一种协作式多智能体框架，通过规划代理补充观察视角、感知代理构建结构化认知地图，在闭环迭代中实现零样本3D理解，在6个基准上达到SOTA，ScanRefer上Acc@0.5提升11.1%。


<details>
  <summary>Details</summary>
Motivation: 现有基于MLLM的零样本3D理解方法受限于视频有限视角和隐式的3D场景感知，存在固有瓶颈。

Method: 提出协作多智能体框架：规划代理分析认知地图确定查询相关视角并补充缺失关键视角；感知代理从这些视角记录对象级属性，分配跨视角一致的实例标识符，将碎片化观察整合为结构化整体认知地图，并向规划代理提供反馈以过滤不匹配候选对象，通过闭环迭代协作直至捕获足够信息完成任务。

Result: 在6个基准上达到SOTA，ScanRefer上Acc@0.5提升11.1%，3D辅助对话上BLEU-1提升14.6%，SQA3D上EM提升2.1%。

Conclusion: 该协作多智能体框架通过显式认知地图和视角规划有效克服了传统方法的瓶颈，显著提升了零样本3D理解性能。
...
TL;DR: 本文提出了一种基于人类标注的稀疏自编码器（SAE）评估框架，通过合成基准数据集（synCUB和synCOCO）、全二值匹配追踪（FBMP）算法和目标属性扰动对齐分数（TAPAScore），量化SAE潜在变量与语义概念的对齐程度，发现适度字典大小可获得最佳可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有的SAE评估方法多依赖代理指标或定性检查，缺乏对潜在变量与人类标注概念之间语义对应关系的直接量化度量。

Method: 1) 构建synCUB和synCOCO两个合成基准数据集，每对图像仅在单一属性上不同；2) 提出FBMP算法支持SAE潜在变量与标注概念间的多对多匹配；3) 提出TAPAScore验证匹配概念在目标属性扰动下的选择性和方向正确性。

Result: 在CLIP和DINOv2上训练的SAE中，FBMP和TAPAScore是唯一能可靠区分训练与未训练SAE的指标；增加过完备性会降低扰动对齐；适度字典大小提供最佳权衡。

Conclusion: 该框架表明中等规模的字典能产生最具可解释性的SAE，为视觉模型的可解释性评估提供了标准化方法。
...
TL;DR: 本文提出语义证据奖励（SER），将时空证据定位重构为约束验证任务，利用裁判VLM评估模型生成证据的相关性与定位质量，在V-STAR基准上相比Open-o3-Video提升3.0个百分点的mLGM（49.6%），有效增强答案准确性与证据可靠性。


<details>
  <summary>Details</summary>
Motivation: 现有视频多模态大模型在细粒度时空推理中常基于无关帧或物体生成正确答案，缺乏可解释性；而基于IoU的强化学习奖励对边界扰动敏感且忽略语义对齐，难以提供可靠的时空证据。

Method: 提出语义证据奖励（SER），将时空证据定位视为约束验证任务，使用裁判视觉语言模型（VLM）作为局部检查器，从相关性和定位质量两个维度评估模型生成的证据主张，并引入时间惩罚项，无需密集框标注即可直接在标准视频QA数据上训练。

Result: 在V-STAR基准上，SER达到49.6%的mLGM，相比强证据基线Open-o3-Video提升3.0个百分点，表明其在提升答案准确性和证据定位方面的有效性。

Conclusion: SER通过语义对齐的证据验证机制替代传统几何交并比奖励，不仅减少了标注依赖，还显著提升了视频MLLM在细粒度时空推理中的可解释性和可靠性。
...
TL;DR: VSANet提出了一种基于视图感知稀疏注意力机制的光场图像去噪网络，通过局部敏感哈希稀疏注意力实现跨视图全局特征交互，并引入特征精炼模块增强空间、角度和对极子空间的特征表达，在光场去噪任务上超越了现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 光场图像去噪面临高维数据结构的挑战，噪声在子孔径图像间独立分布，而场景内容存在强跨视图相关性，现有方法未能充分利用这种相关性。

Method: 提出VSANet，核心包括：(1) 视图感知稀疏注意力(VSA)块，将4D光场特征映射为统一的空间-角度令牌空间，利用局部敏感哈希的稀疏注意力实现线性复杂度的全局特征交互；(2) 特征精炼(FR)块，在空间、角度和对极子空间中增强信息特征；(3) 将VSA和FR块集成到序列注意力精炼模块中。

Result: 实验表明VSANet在光场去噪任务上优于当前最先进的方法。

Conclusion: 通过视图感知稀疏注意力和特征精炼机制，VSANet有效利用了光场数据的跨视图相关性，实现了高效且高质量的光场去噪。
...
TL;DR: BioMedVR是首个基于视觉重编程（VR）的框架，通过紧凑的可学习VR模块、混淆最小化机制（利用LLM生成的混淆感知属性和混淆抑制损失）以及混合提示专家（正专家+负专家+自适应门控），在18个数据集（11个生物医学+7个自然图像）上实现了预训练VLM在生物医学影像上的少样本高效适配，显著提升了细粒度分类的准确率和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLM（如CLIP）在自然图像上表现优异，但直接微调计算成本高，且生物医学数据稀缺、类间差异细微。现有VR方法主要关注正类提示而忽略混淆负类，导致在细粒度医学场景中预测校准不良。

Method: 提出BioMedVR框架：(1) 紧凑可学习的VR模块实现参数高效少样本适配；(2) 混淆最小化机制：利用LLM生成混淆感知属性，结合混淆抑制损失减少假阳性对齐；(3) 混合提示专家：正类专家负责主类判别，负类专家负责混淆抑制，通过自适应门控平衡。

Result: 在18个数据集（11个生物医学+7个自然图像）上进行广泛实验，BioMedVR取得了优越的准确率和泛化性能，有效弥合了VR与VLM在生物医学领域的差距。

Conclusion: BioMedVR通过引入混淆感知和双专家机制，成功将VR范式应用于生物医学影像领域，为参数高效适配VLM处理细粒度医学任务提供了有效方案。
...
TL;DR: 本文提出自适应赫布路由（Adaptive Hebbian Routing）方法，通过轻量级MLP路由器动态控制赫布记忆的贡献、更新强度与保留程度，在少样本视觉Transformer上提升了分类性能与推理效率。


<details>
  <summary>Details</summary>
Motivation: 固定行为的赫布快速权重记忆无法适应不同少样本任务的需求，需要一种能够动态调节记忆行为的方法。

Method: 使用轻量级MLP路由器从支持集特征中学习，自适应控制赫布记忆的贡献（Adaptive Placement）、记忆更新强度（Adaptive Plasticity）及记忆保留程度（Fully Adaptive Routing）。

Result: 在Swin-Tiny上，Adaptive Plasticity将固定赫布基线的96.74%提升至96.92%，Fully Adaptive Routing达到96.94%并减少推理时间（16.51ms→14.05ms）；在CIFAR-FS上，三种骨干网络均获得提升，多样本评估显示增益可持续。

Conclusion: 自适应可塑性与自适应记忆激活能够改善少样本Transformer的表示能力，其效果优于固定赫布记忆行为。
...
TL;DR: UniDrive提出了一种结合时序推理与高分辨率感知的统一框架，通过门控交叉注意力融合多帧动态上下文和精细空间细节，同时生成自然语言风险描述和边界框，在DRAMA-Reasoning基准上实现最优性能，并在小目标定位、零样本泛化及人类可解释性方面表现突出。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶多模态大模型在时序推理与空间精度之间存在根本性权衡：单帧或低分辨率输入易遗漏小目标、远处或部分遮挡的危险目标，而以语言为中心的驾驶模型提供的解释缺乏充分的视觉证据支撑。

Method: UniDrive包含两个分支：时序推理分支（从多帧视觉输入建模场景动态）和高分辨率感知分支（从最新帧保留精细空间细节），通过门控交叉注意力融合模块对齐动态上下文与空间证据，并联合生成自然语言风险描述和风险目标的边界框输出。

Result: 在DRAMA-Reasoning基准上，UniDrive在描述生成和风险目标定位上均超越基于图像和视频的基线模型，在验证集上取得最优综合性能，并在小目标定位、零样本泛化到NuScenes和BDD100K数据集以及人工评估的可解释性和可信度方面表现出明显优势。

Conclusion: 显式结合时序语义与高分辨率感知为可解释且面向安全性的自动驾驶系统提供了更强的基础。
...
TL;DR: OpenReLoc提出一种利用物体级信息（语义、布局、几何）构建结构化地图表示，仅通过物体单元驱动相机重定位的系统，在多个数据集上实现了优越的重定位召回率和精度。


<details>
  <summary>Details</summary>
Motivation: 现有室内视觉重定位方法主要依赖低层次视觉方案，难以感知场景语义和构成，导致可解释性和适用性受限。本文探索如何将场景中丰富的物体信息（语义、布局、几何）组织成结构化地图表示，仅利用物体单元驱动相机重定位任务。

Method: 1）基于基础模型的多模态机制，集成开放词汇语义知识实现有效的2D-3D物体匹配；2）设计面向物体的参考坐标系作为位置先验，结合基于Distance-IoU的参考帧选择策略，支持可扩展场景；3）提出双路径2D迭代最近点损失（由物体形状引导），确保稳定且精确的位姿优化。

Result: 在多个数据集上，OpenReLoc实现了优越的重定位召回率和精度。

Conclusion: OpenReLoc通过纯物体级表示实现了可解释且高精度的室内视觉重定位，展示了利用物体语义信息驱动定位任务的可行性和优势。
...
TL;DR: GradAudit是一种基于梯度分析的审计框架，通过检测模型参数在训练样本上的梯度稳定性与对齐性，有效识别VLLM是否使用了特定图文对数据进行训练，在医疗和通用数据集上显著优于现有方法，并发现现有方法严重低估了未授权数据使用程度。


<details>
  <summary>Details</summary>
Motivation: 现有训练数据检测方法无法有效处理跨模态场景（如VLLM），且大多依赖浅层输出信号，判别能力不足。特别是在医疗领域，患者医学图像与临床报告的配对数据亟需严格的隐私保护，但缺乏有效的审计手段。

Method: GradAudit通过分析模型内部优化动力学（梯度签名）来区分训练与非训练样本。关键发现：模型参数收敛到训练样本梯度稳定且对齐良好的区域，而非训练样本的梯度则嘈杂且不一致。通过计算梯度特征实现跨模态图文关联检测。

Result: 在医疗和通用数据集上，GradAudit在预训练和微调VLLM场景中均显著优于现有基线方法。案例研究表明，现有方法严重低估了未授权数据使用程度，且随着模型更新更先进，这种低估更为严重。

Conclusion: GradAudit通过分析模型内部梯度特征而非表面输出，提供了一种更强力、更可靠的训练数据检测方法，尤其适用于跨模态版权与隐私审计场景。
...
TL;DR: 提出AerialFusionMapNet，一种通过结构化两阶段训练策略融合航空与车载传感器图像进行在线高清地图构建的方法，在nuScenes数据集上达到54.7 mAP，超越先前基线模型12.1%。


<details>
  <summary>Details</summary>
Motivation: 现有航空-车载融合方法未能充分利用航空图像中的结构信息，端到端融合方式对航空特征的挖掘不够充分。

Method: 提出AerialFusionMapNet框架，采用结构化两阶段训练策略，在统一流水线中显式增强航空特征的贡献，实现更有效的结构先验融合。

Result: 在nuScenes地理分割数据集上达到54.7 mAP，相比先前航空-车载融合基线48.8 mAP提升了5.9个绝对点（+12.1%相对提升）。

Conclusion: 结构化训练设计比增加架构复杂度更能发挥航空图像在在线HD地图构建中的潜力，代码和模型已开源。
...
TL;DR: 本文提出将树木计数问题建模为空间密度匹配问题，通过非平衡最优传输（Unbalanced Optimal Transport）进行监督，并引入自校正机制利用传输残差逐步优化噪声标注。在覆盖三大洲、三个卫星传感器、超2.15亿棵树标注（含77.3万人工验证）的新基准TinyTrees上，该方法优于检测、回归及其他传输匹配基线。


<details>
  <summary>Details</summary>
Motivation: 卫星影像中的单棵树计数在密集森林中边界模糊、标注昂贵且LiDAR派生标注噪声大，现有方法难以有效处理这些挑战。

Method: 将树计数建模为空间密度匹配问题，使用非平衡最优传输（Unbalanced Optimal Transport）作为监督信号；提出基于传输残差的自校正机制，在训练过程中逐步优化噪声标注。

Result: 在TinyTrees基准（覆盖三大洲、三个卫星传感器、超2.15亿棵树标注，含77.3万人工验证）上，方法一致优于检测、回归及其他传输匹配基线。

Conclusion: 非平衡传输和可靠性感知监督对大规模卫星影像树计数有效，代码、数据和模型已开源。
...
TL;DR: 本文提出了一种渲染区域感知的剪枝策略（Pocket-SLAM），通过移除对有效渲染区域贡献低的高斯点，显著降低3DGS-SLAM在大型场景（如自动驾驶）中的内存占用，在EuRoC和KITTI数据集上实现超过60%的内存减少和2倍以上的FPS提升，同时保持定位与建图精度。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS-SLAM在大规模场景中面临内存消耗随时间持续增长的关键限制，高斯点不断累积导致内存效率低下，限制了其在自动驾驶等真实场景中的应用。

Method: 提出一种渲染区域感知的剪枝策略，根据高斯点对有效渲染区域的贡献度进行选择性移除，而非仅依赖不透明度或梯度幅值等基于高斯点本身的启发式指标。

Result: 在EuRoC和KITTI数据集上，该方法在大型室外场景中优于现有剪枝方法：实现超过60%的内存缩减、2倍以上的FPS提升，同时保持定位与建图精度。

Conclusion: 渲染区域感知剪枝是推动3DGS-SLAM扩展至真实世界自动驾驶场景的一个有前景的方向。
...
TL;DR: 本文提出EG-VQA基准，用于评估视频问答中的证据定位能力，并引入EG-F1指标联合衡量时间对齐与语义一致性。实验发现现有模型在答案正确性与证据定位间存在根本性差异，为此提出EG-Reasoner模型通过显式证据监督训练，在开源模型中达到SOTA，在反事实等推理密集型任务上表现突出，证明了结构化证据监督对可靠视频理解的重要性。


<details>
  <summary>Details</summary>
Motivation: 现有视频问答基准仅评估答案正确性，忽视预测结果是否扎根于视频证据，导致答案生成与证据理解之间存在脱节。

Method: 构建EG-VQA基准（含2,067个视频和11,838个QA对，每个QA对标注了细粒度的时间证据）；提出EG-F1指标联合衡量时间对齐与语义一致性；提出EG-Reasoner模型通过显式证据监督训练。

Result: 强闭源模型在证据定位上表现不佳，存在答案正确性与证据定位间的根本差异；EG-Reasoner在开源模型中达到SOTA，与闭源系统竞争力相当，在反事实等推理密集型任务上提升尤为显著。

Conclusion: 仅靠模型缩放不足以实现鲁棒视频理解，结构化证据监督对于开发更可靠和可解释的视频问答系统至关重要。
...
TL;DR: OrbitForge利用冻结的文本到视频先验与每提示3D高斯泼溅重建优化，将单次文本生成的视频转换为标准闭环轨道的3D高斯泼溅场景，无需特定任务微调或分数蒸馏优化，并通过覆盖感知评估在T3Bench基准上达到359度中位跨度，将Q10 ImageReward从8.07提升至16.36。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频模型虽能作为开放世界场景先验，但生成的视频存在相机运动难控制、视角覆盖不全、帧间不一致等问题，无法直接得到可靠的3D资产。因此需要一种方法将单次生成的视频转换为一致的3D场景。

Method: OrbitForge包含三个步骤：1) 从首轮生成视频通过可变形高斯泼溅和鲁棒MedianGS代理获得初步3D重建；2) 从预定轨道渲染视图检测缺失视角；3) 利用文本到视频模型仅补全缺失视角，再将补全后的轨道重建为最终高斯泼溅场景。该方法无需任务特定视频或多视图微调，也无需每提示分数蒸馏优化。

Result: 在冻结的300提示T3Bench审计集上，OrbitForge重建达到359.0度中位测量跨度，将原本未支持区间的Q10 ImageReward从8.07提升至16.36（相对MedianGS仅重建），同时在覆盖质量上与VideoMV保持竞争力。

Conclusion: 通过将3D重建作为锚点来改进生成视频的3D一致性，并采用覆盖感知评估，OrbitForge有效解决了从文本生成视频到3D场景的转换问题，证明了无需特定任务微调即可实现高质量闭环轨道3D场景重建。
...
TL;DR: 本文提出DDStereo，一种基于双解码器立体Transformer的实时开放集3D目标检测方法，通过轻量级双解码器架构（一个用于开放集前景2D检测，另一个用于3D属性回归）和紧凑的视差特征提取器，在保持高精度的同时首次实现与单目方法相当的实时推理速度，填补了立体3D开放集检测的空白。


<details>
  <summary>Details</summary>
Motivation: 现有立体3D检测方法精度高于单目方法，但推理速度慢，无法满足实时应用需求；同时，开放集/开放词汇检测在单目2D/3D中已有进展，但立体3D开放集检测尚未探索。

Method: 提出DDStereo，包含两个轻量化解码器分支：一个用于开放集前景2D检测，另一个用于3D属性回归；两个解码器共享目标级查询以实现统一对齐；设计了紧凑的视差特征提取器和精简的解码器架构以提升推理效率。

Result: 在公共立体3D基准测试上，DDStereo在封闭集和开放集协议下均达到最先进精度；推理速度超越现有立体3D检测器，首次实现与单目方法相当的实时性能。

Conclusion: DDStereo成功弥合了立体3D检测在实时性和开放集泛化方面的差距，为立体3D开放集检测提供了有效的解决方案。
...
TL;DR: 本文提出了一种基于去噪扩散概率模型（DDPM）的框架，在极端数据稀缺（仅15张样本）条件下生成合成透射电子显微镜（TEM）图像，通过渐进式补丁训练、数据增强、域迁移、分类器引导和修复技术，生成符合FAB计量要求的全尺寸图像，并利用DDPM特征表示进行分割，合成图像MS-SSIM > 0.98，支持下游缺陷检测、分割和计量任务。


<details>
  <summary>Details</summary>
Motivation: 先进半导体节点对TEM的需求激增，但破坏性样品制备、缓慢成像和高成本严重限制了多样化数据集的获取，阻碍了机器学习模型的训练。现有生成模型缺乏TEM特有的噪声、结构细节和随机变异性，亟需高质量合成数据生成方法。

Method: 采用DDPM框架，提出渐进式补丁训练策略（从低分辨率补丁扩展到全图像），集成自定义TrivialAugment适应、跨过程域迁移、分类器引导和RePaint风格修复；并将DDPM编码器特征图分区用于分割任务。

Result: 合成图像MS-SSIM > 0.98，专家评估与结构相似性结果一致，有效支持下游ML训练（缺陷检测、分割、计量），同时保持统计和物理真实性。

Conclusion: 提出的DDPM框架能够在极端数据稀缺条件下生成高质量TEM合成图像，并复用特征表示实现分割，为半导体计量领域的机器学习提供了可行的数据增强方案。
...
TL;DR: GeoT2V-Bench是一个基于重建的诊断基准，用于评估相机提示的文本到视频(T2V)模型是否能生成支持刚性3D重建的视频片段。通过在80个静态场景提示和12个模型配置上的3840次重建实验，发现可见运动、静态渲染误差、光流一致性和灵活vs静态拟合行为往往不一致，揭示了T2V视频作为全局静态场景采集时的互补失效模式。


<details>
  <summary>Details</summary>
Motivation: 现有的相机提示T2V模型生成的视频在视觉上看似合理，但缺乏对生成帧是否能够提供一致的、支持单一静态3D场景的多视角证据的评估方法。作者旨在建立一种基于3D重建的诊断工具来系统评估T2V视频的几何一致性。

Method: 提出GeoT2V-Bench流水线：使用VGGT风格几何估计预测每帧相机内参和位姿，拟合DeformableGS（可变形3D高斯泼溅），通过时间中值聚合推导静态MedianGS代理，然后沿估计的相机路径渲染该代理。最终输出一个连续的“重建剖面”，包含表观图像运动、估计轨迹行为、MedianGS静态渲染误差、静态渲染光流一致性以及灵活拟合与静态拟合之间的差距。

Result: 在80个GeCo-Eval静态场景提示、12个开源模型配置、4个随机种子的3840次重建实验中，发现可见运动、静态渲染误差、光流一致性和灵活vs静态拟合行为往往相互不一致，表明这些指标捕获了互补的失效模式。

Conclusion: GeoT2V-Bench能够捕获T2V视频作为全局静态场景采集时出现的多种互补失效模式，为评估相机提示T2V模型的3D几何一致性提供了有效的诊断工具。
...
TL;DR: 本文提出黎曼残差线搜索方法，通过将一步扩散编辑中的保真度-编辑能力矛盾转化为候选选择问题，利用局部时间曲率估计构建更强编辑并沿原始更新方向投影，最后通过CLIP对齐选择最佳候选，在PIE-Bench++上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 一步扩散编辑器速度快但面临根本性矛盾：单次传输更新必须足够激进以实现目标提示，又必须足够保守以保留源图像，而固定的更新强度无法同时满足所有编辑类型的需求。

Method: 提出黎曼残差线搜索方法：(1)通过估计提示差场(prompt-delta field)的局部时间曲率构建更强编辑，并将校正方向投影回原始一阶能量场传输估计的更新范数上；(2)从源图像到该强编辑形成小残差路径；(3)保留原始一阶输出作为候选之一；(4)通过最大化目标提示CLIP对齐度选择最终图像。

Result: 在包含10种编辑类型ID、700样本的PIE-Bench++评估中，该方法在现有一步更新算法中达到SOTA性能。

Conclusion: 将编辑中的保真度-编辑能力张力视为能量场传输上的事后候选选择问题（而非设计新编辑模型）是有效的，黎曼残差线搜索能显著提升一步扩散编辑器的性能。
...
TL;DR: 本文提出将球面立体图像通过ERP投影预处理，结合RAFT+EACS框架进行视差估计，在合成鱼眼立体数据集上实现了实时、准确的视差图生成。


<details>
  <summary>Details</summary>
Motivation: 球面立体图像（如鱼眼或全景）违反了经典视差估计的几何假设——极线对应沿大圆曲线产生二维位移，无法在几何校正前作为单轴视差处理。

Method: 采用球面到等距柱状投影（ERP）作为预处理步骤，将极线曲线拉直恢复一维视差结构；然后使用RAFT密集光流提取对应关系，通过保留基线对齐的流分量还原为视差，并结合Epipolar-Aligned Channel Selection（EACS）框架。

Result: 在合成鱼眼立体数据集上，该管道能够实时生成准确、平滑且结构一致的视差图。

Conclusion: 已建立的ERP预处理可与RAFT+EACS方法有效结合，为将传统立体管道扩展到360°成像提供了一条直接可行的路径。
...
TL;DR: 提出隐式视觉思维链（IV-CoT）框架，将视觉条件查询分解为结构→语义级联，通过训练时草图监督引导结构查询捕捉布局信息，在单个前向传播中实现隐式推理，显著提升多模态大模型的结构感知文本到图像生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有统一多模态大模型在文本到图像生成中虽质量高，但在结构感知提示遵循（如物体计数、空间关系、属性绑定、粗略布局）方面表现不佳，原因在于结构规划与外观渲染被纠缠在单一条件流中。

Method: 提出IV-CoT框架，将视觉条件查询分解为结构→语义级联：结构查询先形成隐式视觉规划，语义查询再基于该规划渲染外观。引入仅训练阶段的草图监督，引导结构查询从草图中捕捉结构信息，推理时无需草图提取或中间解码。

Result: 在GenEval和T2I-CompBench上取得优异结果，可视化与分析表明结构和语义查询在结构感知生成中发挥互补作用。

Conclusion: IV-CoT通过解耦结构规划与外观渲染，在单一前向传播中实现隐式思维链推理，有效提升了多模态大模型对结构感知提示的遵循能力。
...
TL;DR: FLUX3D提出了一种面向图像转3D高斯泼溅的框架，通过扩散对齐的结构化潜变量（DA-SLAT）和稀疏结构感知扩散框架（含SMDiT和MARoPE），解决了现有方法中的表征瓶颈和跨模态对齐瓶颈，在保真度上显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有稀疏体素表示方法在图像到3DGS生成中存在两个结构性瓶颈：（1）使用面向语义抽象的判别式2D特征构建稀疏体素潜变量，抑制了重建线索，导致表征瓶颈；（2）标准扩散Transformer缺乏有效机制对齐密集2D图像token与稀疏3D体素潜变量，导致跨模态对齐瓶颈。

Method: （1）提出Diffusion-Aligned Structured Latents (DA-SLAT)，重新审视2D特征选择，并结合仅解码器架构提升3DGS重建保真度；（2）设计稀疏结构感知扩散框架，包含Sparse-structure Multimodal Diffusion Transformer (SMDiT) 和 Modal-Aware Rotary Positional Embedding (MARoPE)，实现几何无关的2D-3D对齐。

Result: 在多个基准数据集上的实验表明，FLUX3D在外观保真度上取得显著提升，全面超越所有现有最先进方法，生成高质量的3DGS资产。

Conclusion: 通过同时改进表征学习和生成过程中的跨模态对齐，FLUX3D有效解决了稀疏体素方法在高频细节保留上的核心瓶颈，为图像到3DGS生成提供了可扩展且高性能的解决方案。
...
TL;DR: FLAT首次从视频扩散潜空间中直接解码出三角形splat（三角面片），实现了从单张图片生成可探索3D场景，相比3D高斯在几何精度上显著提升，且支持实时渲染。


<details>
  <summary>Details</summary>
Motivation: 现有前馈潜空间解码器通常输出体素3D高斯，缺乏明确定义的表面，无法直接用于仿真或标准图形管线。作者探索能否将压缩的视频扩散潜变量直接映射到显式表面基元。

Method: 提出FLAT框架，核心包含两个关键设计：(1) 射线中心旋转参数化（ray-centered rotation parameterization）用于三角形回归；(2) 新颖的乘积窗口函数（product window function）改善可微三角形渲染中的梯度流。

Result: 在标准基准上，FLAT在几何精度上显著优于现有最先进的前馈基线，同时保持有竞争力的视觉质量。轻量级测试时精炼可将预测的三角形网格转换为完全不透明的、支持游戏引擎实时渲染的表示。

Conclusion: 首次系统分析了3DGS、2DGS和三角形splatting在前馈场景生成中的表示权衡，证明三角形splat是连接生成模型与显式几何资产的可行桥梁。
...
TL;DR: 本文构建了一个包含85,355次CT扫描的大规模开放基准，系统评估了12个肿瘤检测AI模型在不同肿瘤大小、位置、患者亚组和成像协议下的表现。利用大语言模型自动提取和整理临床数据中的亚组信息，发现当前最先进的AI模型在稀有或代表性不足的亚组（如年轻、女性、非裔美国人）中表现不佳，强调了在医学影像中进行亚组级评估的必要性。


<details>
  <summary>Details</summary>
Motivation: AI模型在医学影像中取得了显著成功，但在不同临床环境下（如患者人群、成像协议变化时）表现不一致，尤其是在检测小肿瘤、分析不同对比相扫描或评估不同年龄/性别患者时。现有研究缺乏系统量化这些不一致性的基准，因此作者开发了一个大规模开放基准来全面评估肿瘤检测AI模型在不同亚组中的表现。

Method: 构建了包含85,355次CT扫描的大规模开放基准，系统评估12个肿瘤检测AI模型。利用大语言模型从临床数据中提取和组织亚组信息（如肿瘤大小、位置、患者年龄/性别/种族、成像协议），实现了可扩展且可复现的分析。

Result: 当前最先进的AI模型虽然在平均准确率上表现优异，但在稀有或代表性不足的亚组（如年轻、女性、非裔美国人）中表现不佳。收集这些稀有病例的充足标注数据在实践中往往不可行。
...
TL;DR: NanoGen是一个统一的DiT训练和评估框架，支持ImageNet和文本到图像（T2I）两种设置。通过训练21个潜在扩散模型发现，ImageNet上的方法排名与T2I生成之间无强相关性（Pearson相关系数在-0.377到-0.580之间），表明仅用ImageNet评估不足以反映真实进展。因此提出DiffusionBench作为替代性综合基准。


<details>
  <summary>Details</summary>
Motivation: 当前DiT研究仅依赖ImageNet上的类条件生成进行评价，但这一单一评估设置可能无法反映生成建模的真实进展，而T2I评估因成本问题被忽视，本文旨在验证这一认知是否合理并推动更全面的评估。

Method: 提出NanoGen统一训练与评估框架，支持RAE、VAE、像素空间和MeanFlow四种扩散方法，仅需12行配置变更即可从ImageNet切换至T2I设置。训练21个潜在扩散模型，计算ImageNet与T2I指标之间的Pearson相关系数，并构建DiffusionBench综合基准。

Result: NanoGen在ImageNet上匹配SOTA DiT基线；ImageNet与T2I之间的方法排名Pearson相关系数为-0.377到-0.580，表明两者无强相关性；T2I训练所需算力与ImageNet相当。

Conclusion: 单一ImageNet评估无法代表DiT方法的整体进展，建议使用涵盖ImageNet和T2I的DiffusionBench作为替代基准，提升结果的方法更可能反映广泛进步。
...
TL;DR: EXPO-SQL提出基于子句级奖励的强化学习方法，通过对SQL查询中正确/错误子句进行细粒度区分，解决了传统RL方法中统一查询级奖励导致学习信号不足的问题，在Text-to-SQL基准测试中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的强化学习方法在Text-to-SQL任务中给所有子句分配统一的查询级奖励，无法区分正确子句和错误子句，导致学习信号粗粒度、训练效率低。

Method: 提出EXPO-SQL方法，通过分析执行结果（包括错误信息和子句增量执行）识别错误子句，为每个子句分配细粒度的子句级奖励，从而实现基于执行的子句级策略优化。

Result: 在多个Text-to-SQL基准测试上，EXPO-SQL在性能上显著超越了现有的监督微调、提示工程和基于强化学习的方法。

Conclusion: 细粒度的子句级奖励信号能够更有效地指导模型学习正确的SQL生成，是提升Text-to-SQL任务性能的关键方向。
...
TL;DR: 提出ModTGCN，一种联合优化交叉熵和模块度辅助目标的图神经网络文本分类方法，通过促进文档社区与类别一致来缓解过平滑问题，并在五个基准数据集上取得一致提升。


<details>
  <summary>Details</summary>
Motivation: 传统基于图的文本分类模型仅依赖局部邻域聚合，忽略了语义文档图中存在的全局社区结构（类别一致性聚类），导致类别边界模糊和过平滑问题。

Method: 1) 提出ModTGCN，联合优化交叉熵损失和模块度辅助目标；2) 模块度项基于Transformer嵌入（预训练或微调）构建的文档-文档相似度图计算；3) 将异质TextGCN图解耦为文档-词和词-词两个独立组件，实现2-10倍训练加速；4) 研究图构建策略、标签感知边重加权和模块度优化的监督选择。

Result: 在五个基准数据集上取得一致性能提升，在复杂、低同质性数据集（如Ohsumed和20NG）上改进更为显著。

Conclusion: 显式引入模块度感知的社区结构约束可以有效提升图神经网络文本分类性能，尤其适用于同质性较低的复杂数据集。
...
TL;DR: 提出归一化上下文利用（NCU）指标来量化RAG中模型对上下文信息的真实利用程度，发现严格事实提取任务中，小语言模型（SLMs）在上下文遵循方面优于大模型，而大模型和商用API存在先验主导、信心崩溃等问题。


<details>
  <summary>Details</summary>
Motivation: 现有RAG评估依赖离散启发式方法，存在"认知盲区"，无法区分模型是真正从外部上下文中提取信息，还是依赖其参数化记忆，亟需一种更严格的量化指标。

Method: 提出归一化上下文利用（NCU）指标，利用连续token对数概率，在零样本、oracle和对抗条件下严格量化上下文信息增益；在1.5B到72B参数规模及商用API上进行评估。

Result: 在严格事实提取任务中，小语言模型（SLMs）表现与大模型相当或更优；商用API在近半数对抗冲突中覆盖外部证据，且频繁出现系统性的信心崩溃（负迁移）。

Conclusion: SLMs在严格事实提取中具有结构性的认知优势和更好的上下文遵循能力；传统扩展定律在该任务中存在极端边际收益递减；先验主导与模型规模和商业对齐相关。
...
TL;DR: 本文发现涌现错位（EM）与模型对齐人格向量的激活和邪恶角色特征相关，提出自我生成文本识别（SGTR）微调作为针对角色的防御方法。实验表明SGTR在预防EM方面优于传统微调，且EM的本质是对齐角色的失稳而非采纳错位人格。


<details>
  <summary>Details</summary>
Motivation: 已有工作将涌现错位（EM）与错位人格向量和邪恶角色特征的激活联系起来，暗示EM通过破坏模型的对齐角色运作，而非直接学习有害内容。受此启发，研究针对角色的干预方法。

Method: 在GPT-4.1、Qwen2.5-32B-Instruct、Seed-OSS-36B-Instruct三个模型上，跨多个EM数据集进行两阶段微调实验，比较SGTR微调与良性微调基线（正确领域数据、通用知识、单词计数）在逆转和预防EM上的效果。

Result: 所有干预在逆转EM上效果相当，但仅当恢复EM退化的能力时才有效。预防方面，仅SGTR微调能一致降低错位而不恶化任何指标。EM微调会诱导模型身份自我报告的多样性，人为破坏自我识别会加剧EM，移除身份承载的系统提示显著降低EM效果。

Conclusion: 涌现错位不是采纳一致的错位人格，而是对齐角色的失稳。角色强化（SGTR）是有效的预防策略。
...
TL;DR: 本文提出了一个可扩展且可解释的框架，利用大语言模型（LLM）从定性产品反馈中量化产品期望度。在ZORQ和CARMA两个PDT数据集上，零样本数值情感评分与专家标签的Pearson相关性最高达0.97，分类准确率最高达94%。GPT-4o-mini以94%更低的成本达到与更大模型相当的性能。框架还整合了模型置信度和可解释AI解释，提升了透明度和可信度。


<details>
  <summary>Details</summary>
Motivation: 定性产品反馈包含细微的用户体验信息，但其内隐情感难以量化测量。现有方法缺乏可扩展性和解释性，且依赖显式评分数据。

Method: 使用LLM进行零样本连续数值情感评分和分类情感评估；结合模型置信度评分和人类可读的理性解释（xAI）；与基于词典和Transformer的基线方法对比。

Result: LLM生成的数值情感评分与专家标签Pearson相关性高达0.97，分类准确率高达94%；GPT-4o-mini以94%更低成本达到与大型模型相当的性能；LLM在多形式数据下保持鲁棒性；基线方法未产生统计显著结果。

Conclusion: 将PDT工具与成本高效的LLM结合，能够提供丰富的数值和分类情感评分以及高层次用户印象，可用于产品开发改进和营销目标群体识别。
...
TL;DR: 本文提出IBA框架，通过解耦实体识别与段落重排序，利用MLLM在候选名称中选择实体，再配合文本重排序器，在KB-VQA任务上超越现有MM-RAG方法，同时降低复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有MM-RAG方法将实体判别和段落级证据排序耦合在单一重排序阶段，导致成本高、泛化性差；MLLM在开放实体命名场景下表现不佳，但在候选集中选择实体时准确率更高。

Method: 提出IBA框架：第一阶段用MLLM从候选名称中选择高置信度实体；第二阶段用现成的文本重排序器进行证据选择。整个流程无需训练。

Result: 在Encyclopedic-VQA和InfoSeek上，IBA方法一致优于微调的多模态重排序基线，同时降低了训练和推理复杂度。消融实验表明性能提升来源于更好的实体识别和更有效的证据选择。

Conclusion: 解耦实体识别与证据重排序是提升KB-VQA性能的有效策略，IBA框架简单、无需训练且具有良好泛化性。
...
TL;DR: 本研究评估了六种通用大语言模型在16种DSM-5精神障碍条件下的安全防护能力，发现除自杀和自伤行为外，其他疾病（如进食障碍、物质使用障碍、重度抑郁障碍）的安全防护失败率高达100%，并提出了八维伤害分类学和多维度评估框架。


<details>
  <summary>Details</summary>
Motivation: 通用大语言模型越来越多地被用于心理健康相关对话，但其安全防护措施在不同临床条件下存在不足且不一致，可能对弱势群体造成严重风险。

Method: 对六种专有LLM进行了评估，覆盖16种DSM-5精神障碍条件，使用四种对抗性攻击变体，引入八维伤害分类学和多维度评估框架。

Result: 安全防护仅在自杀和自伤行为方面可靠，而进食障碍、物质使用障碍和重度抑郁障碍等条件下的失败率高达100%。

Conclusion: LLM的伦理设计和部署需要明确定义不同临床条件下的伤害类别并实施相应安全防护，否则这些模型将对弱势群体构成重大风险，特别是在教育环境中的整合令人担忧。
...
TL;DR: 本文系统审计了八种自动评估指标在RAG归因任务中的表现，发现没有一种指标能在多数据集间稳定迁移：最佳指标随数据集反转（如AttributedQA上NLI最好AUROC 0.90，LFQA上跌至随机0.53，BERTScore反而0.91），且"平均最优"选择策略存在显著后悔值（留一法后悔均值0.172 AUROC）。基于LLM的评判避免了自动指标的随机崩溃问题，但成本高出~100倍且非确定性，仅转移了验证负担而非消除。


<details>
  <summary>Details</summary>
Motivation: 当前实践将LLM检索增强生成中的归因自动评估指标视为可互换使用，但缺乏系统性验证这种假设是否成立。研究者希望通过审计多种评估指标在不同评估构造（数据集）下的表现稳定性，揭示指标互换性的真实程度及其决策代价。

Method: 审计八种自动评分器（词法、嵌入、BERTScore基线，以及基于蕴含/基础训练的模型如clean/FEVER NLI、MiniCheck），覆盖三种评估构造（来源/主题性、生成答案归因、事实验证蕴含）。以95%置信区间内是否保持最优表现为标准检验迁移性，并使用Kendall tau相关系数衡量排名反转。采用留一数据集验证评估"平均最优"选择策略的后悔值。

Result: 在生成答案归因构造中，无任何自动指标能在所有数据集上保持最优；数据集间最优指标排名显著反转（Kendall tau=-0.64, p=0.031）；"平均最优"选择策略在留一验证中后悔均值0.172 AUROC，劣于固定单一评分器。基于LLM的评判避免了自动指标的随机水平崩溃，但成本高~100倍且非确定性。

Conclusion: 自动归因评估指标不能互换使用，指标选择必须在目标数据集上单独验证，而非从其他数据集推断。LLM评判虽解决了部分问题但带来新挑战（成本、非确定性），本质上是将验证负担转移而非消除。
...
TL;DR: 本文研究了tau-bench环境中预行动策略分类任务，发现在该基准设置下，精确匹配的条款召回率会低估检索器对下游策略分类的实际效用。使用Qwen2.5-3B/7B分类器，检索到的策略条款与黄金标准条款在macro-F1上无显著差异（3B: 0.58 vs 0.60），而精确匹配召回率仅7%（航空公司状态）。建议在分类循环中使用检索到的策略进行评估，而非仅依赖召回率。


<details>
  <summary>Details</summary>
Motivation: 精确匹配的检索召回率常被用作检索器是否为下游决策模型提供有用策略背景的代理指标，但这种代理的有效性尚未得到充分验证。本文旨在测试该代理指标在预行动策略分类任务中的可靠性。

Method: 在tau-bench环境中，使用Qwen2.5-3B/7B分类器进行预行动策略分类。比较黄金策略条件、检索到的策略条款、不匹配策略和无策略控制条件下的macro-F1性能。使用两种不同的检索器，并在不同微调配置下进行测试。

Result: 精确匹配条款召回率仅7%（航空公司状态），但检索到的策略条款与黄金标准条款在macro-F1上无显著差异（3B: 0.58 vs 0.60，Delta=-0.02，95% CI [-0.23,+0.21]）。不匹配策略和无策略控制得分分别为0.32和0.21。紧凑结构化状态相比原始轨迹提升macro-F1 0.13-0.17。

Conclusion: 在该基准设置下，精确匹配条款召回率会低估检索器对下游策略分类的实际效用，建议在分类循环中使用检索到的策略进行评估而非仅依赖召回率。
...
TL;DR: 本文提出QuechuaTok基准，系统比较四种分词策略（BPE、Unigram LM、WordPiece、PRPE）在南盖丘亚语上的表现，发现PRPE在形态边界准确性上达到83.33%，远超BPE的6.67%，证明生育率指标不足以评估粘着语的分词器质量。


<details>
  <summary>Details</summary>
Motivation: 标准分词评估指标（如生育率）无法捕捉粘着语的形态正确性，尤其针对南盖丘亚语这类低资源粘着语缺乏系统性的分词基准。

Method: 使用20万句语料库和SQUOIA有限状态形态分析器作为银标准，采用生育率、OOV率和形态边界准确性（MorphAcc）三个指标，比较BPE、Unigram LM、WordPiece和形态感知PRPE四种分词策略。

Result: BPE在16k词表下取得最低生育率（1.636），但MorphAcc仅6.67%；PRPE取得最高MorphAcc达83.33%，表明生育率单独不足以评估粘着语分词器性能。

Conclusion: 对于粘着语，形态感知的分词策略（如PRPE）显著优于基于频率的方法，需引入形态边界准确性等指标来全面评估分词质量。
...
TL;DR: 本文探究自监督（wav2vec2-base）和监督（Whisper-small）语音模型对非裔美国人英语（AAE）中辅音丛简化（CCR）现象的编码方式。通过两层探测任务（缩减检测和底层丛身份恢复），发现两个模型均能高精度区分缩减与规范形式，且缩减后的音段仍保留底层塞音线索，表明模型将CCR编码为结构化的梯度 phonological 变异，而非简单的音段删除。


<details>
  <summary>Details</summary>
Motivation: CCR是AAE中广泛存在的音系过程，也是自动语音识别（ASR）性能差异的重要来源，但语音模型如何表征这一现象尚未被充分研究。理解模型对CCR的编码方式有助于揭示语音模型对方言变异的结构化表征能力。

Method: 采用说话者无关的逐层探测方法，对wav2vec2-base和Whisper-small两个模型进行两项任务：（1）缩减检测——判断是否发生CCR；（2）底层丛身份恢复——识别被缩减的底层辅音丛中缺失的塞音身份。通过分析模型各层的表征来评估其对CCR的编码深度。

Result: 两个模型均能以高准确率区分缩减形式与规范形式。更重要的是，缩减后的音段中依然保留着关于底层塞音身份的线索，说明模型并非简单检测到音段缺失，而是编码了结构化的梯度音系变异信息。

Conclusion: 现代语音模型对AAE的CCR模式具有结构化的音系编码能力，CCR被表征为有结构的梯度音系变异，而非简单的音段删除。这一发现为理解语音模型如何处理方言音系变异提供了新的视角。
...
TL;DR: 本文构建了MELD数据集，发现现有嵌入模型倾向于按术语而非数学本质对语句分组，并提出了一种对比学习方法，通过对齐非正式表述与不同形式化表达来改善数学等价性嵌入，实验证明该方法在检索任务和MELD数据集上均有提升。


<details>
  <summary>Details</summary>
Motivation: 数学的抽象性导致同一命题在不同子领域以截然不同的形式呈现，许多突破源于研究者发现某问题已在其他领域被解决。同时，形式化资源的增长需要能够在数学"语言"（如从Lean到自然语言）之间高效可靠导航的工具。

Method: 提出MELD数据集（数学等价但词汇不同的语句对），评估现有嵌入模型；提出一种对比学习方法，将非正式陈述与不同的形式化表达进行对齐，从而学习数学文本的嵌入。

Result: 当前最先进的嵌入模型倾向于按使用的术语而非底层数学对语句进行分组；所提出的对比学习方法不仅在非正式-形式化检索任务上有改进，在仅包含自然语言语句的MELD数据集上也有提升。

Conclusion: 现有嵌入模型未能捕捉数学等价性，而提出的对比对齐方法能够有效改善这一缺陷，增强跨数学子领域的语义理解能力。
...
TL;DR: CAMS是一个基于Claim锚点的多文档摘要框架，通过提取原子声明、跨文档聚类、支持度感知选择和有约束重写，实现了细粒度的多源归因和事实一致性，在质量与基线相当的情况下显著提升了忠实度和引用精确度。


<details>
  <summary>Details</summary>
Motivation: 端到端大语言模型生成的多文档摘要虽流畅但易产生幻觉，且归因粒度粗（整篇文档或段落）、事后生成，导致每个摘要陈述难以验证。作者希望重新审视模块化的Extract-Select-Rewrite范式，将其中间表示重构为归因单元，实现结构化的细粒度归因与忠实度保障。

Method: CAMS框架包含四个步骤：(i) 从每篇源文档中提取带有token级来源的原子声明；(ii) 跨文档聚类等值声明并标记冲突；(iii) 选择支持度感知且显著的声明子集；(iv) 将所选声明重写为摘要，每个句子锚定于经过支持检查的声明，并链接回一个或多个源文本片段。

Result: 在MultiNews上与强端到端和跨度归因基线相比，CAMS在摘要质量上相当，但显著提升了忠实度和引用精确度，多源归因准确率提高了约三分之二。在DiverseSumm上分析了冲突处理能力，在WCEP上测试了零样本迁移。

Conclusion: CAMS通过构建时即定位内容的结构化方法，实现了可控制的忠实度-覆盖率权衡，这是端到端模型隐式处理的。框架在保留细粒度多源可追溯性的同时，通过支持感知选择、约束重写和验证来鼓励事实忠实度。
...
TL;DR: 本研究针对临床值集补全任务，提出了一种两阶段方法：第一阶段通过Qwen3检索、词汇感知扩展和代码显示救援检索构建高召回率候选池；第二阶段使用约束GPT-5裁决进行候选选择。在RASC基准上，该方法将候选池召回率从0.553提升至0.730，完整测试宏F1从0.287提升至0.549，同时确保返回代码来自可审计的候选池。


<details>
  <summary>Details</summary>
Motivation: 临床值集定义标准化术语代码，用于质量测量、表型构建、队列建设和临床决策支持。RASC基准显示直接零样本LLM生成不适合此任务，因为临床代码系统规模大、有版本控制且LLM无法可靠记忆。需要一种阶段式替代方案来优化候选池构建（召回率）和约束LLM裁决（选择）。

Method: 两阶段方法：阶段1候选池构建，使用Qwen3检索结合词汇感知扩展（vocabulary-aware expansion）和代码显示救援检索（code-display rescue retrieval），优化召回率；阶段2约束裁决，用盲化GPT-5从候选池中选择，而非直接LLM生成，确保所有返回代码来自可审计候选池。

Result: 在RASC完整3,744值集测试集上：候选池召回率从基线0.553提升至0.730（保留发布者层为0.655）。原始SAPBert交叉编码器在扩展池上宏F1为0.287/0.233；替换为GPT-5裁决后，宏F1提升至0.549/0.533。

Conclusion: 检索约束的LLM裁决能显著改善值集补全性能，同时保持安全性约束（所有返回代码必须来自可审计候选池），证明两阶段方法优于直接LLM生成。
...
TL;DR: 提出"规范学习"(spec learning)框架，通过少量用户偏好判断编译为自然语言规范，在推理时引导LLM行为，无需参数更新，且在密集偏好信号的专业领域上常优于DPO。


<details>
  <summary>Details</summary>
Motivation: 传统通过人工迭代调整prompt来引导LLM行为的过程繁琐、脆弱且易错，而基于偏好的微调（如DPO）虽更严谨但成本过高。需要一种既高效又可控的方法。

Method: 用户提供简短指令和少量偏好判断，将这些偏好编译为自然语言形式的规范（specifications），在推理时作为条件输入LLM，无需更新模型参数。

Result: 在偏好信号密集的专业领域数据集上，基于编译规范生成的响应通常优于直接偏好优化（DPO）；规范具有人类可读性，可作为偏好信号的可解释、透明的书面体现。

Conclusion: 规范学习是一种高效、可解释的LLM行为引导方法，避免了昂贵的参数更新，同时提供了优于DPO的性能和透明度优势。
...
TL;DR: MeMo提出使用显式多层关联矩阵记忆（CMM）的语言模型，通过记忆编辑而非重新训练来更新知识，支持替换、废弃、保留历史、回滚和追踪等版本感知操作。


<details>
  <summary>Details</summary>
Motivation: 减少语言模型在知识发生变化时重新训练的需求，通过显式记忆编辑实现高效的知识更新。

Method: 提出版本感知操作层，将高级操作编译为MeMo原生原语调用；引入版本CMM（V-CMM）和事务CMM（T-CMM）两个辅助记忆矩阵，分别映射版本转换和存储可复用变更内容与逆程序；支持序列级编辑和结构化差异输入。

Result: 支持更新成功、回滚、可追溯性、局部性和事务复用等维度的评估路线。

Conclusion: 版本感知操作通常不是单一MeMo关联，而是有序的原语编辑事务（如遗忘某序列-令牌链、记忆另一链、保留历史链、记录逆程序），通过显式记忆编辑实现知识更新。
...
TL;DR: 本文系统研究了Twitter情感分析中预处理技术的执行顺序对模型性能的影响，发现分词（tokenisation）影响最大，拼写纠正影响最小，并提出了最优预处理顺序：分词→文本清洗→词干提取→停用词移除（保留否定词）。


<details>
  <summary>Details</summary>
Motivation: 尽管预处理在情感分析中至关重要，但现有研究缺乏对预处理技术执行顺序的系统性探讨，导致实践者需耗费大量成本进行探索性实验。

Method: 通过控制预处理技术的执行顺序，系统比较了不同顺序组合下各技术（拼写纠正、分词、文本清洗、词干提取、停用词移除）对Twitter情感分析模型输出的影响。

Result: 拼写纠正是影响最小的预处理技术；分词影响最大；词干提取与停用词移除可互换；移除停用词时应保留否定词；最优顺序为分词→文本清洗→词干提取→停用词移除。

Conclusion: 为从业者提供了系统化的预处理执行顺序方案，可显著提升模型性能并省去昂贵的预处理探索阶段。
...
TL;DR: 本文指出在自回归语音语言理解（SLU）模型中，即便抑制了目标意图的输出，模型仍保留了基于该意图生成槽位的条件映射（称为“能力持续性”）。为此，提出绑定子空间（BSU）方法，在表示层面隔离并衰减意图相关方向，从而在保持原有性能的同时，显著降低通过强制前缀恢复被抑制意图的能力。


<details>
  <summary>Details</summary>
Motivation: 现代SLU系统在实际部署中因政策或安全原因需要移除特定功能（意图及关联的槽位生成），但自回归模型在抑制目标意图后，仍然保留了“意图→槽位”的条件映射，导致外部提供意图前缀时可重建原意图-槽结构，存在安全隐患。

Method: 提出绑定子空间（Binding Subspace, BSU）方法，在表示层面（representation-level）隔离并衰减与意图条件映射相关的方向，从而削弱强制前缀条件下的意图恢复能力。

Result: 在多个SLU基准上，BSU在保持原始任务性能的前提下，显著降低了强制前缀下被抑制意图的可恢复性。

Conclusion: BSU有效解决了SLU中功能移除后的能力持续性问题，提升了模型在安全约束下的部署可靠性。
...
TL;DR: 该研究将大语言模型中的"上下文诱发性"现象从词元层面扩展到句子层面，发现即使提示中包含反事实句子，也会显著提高该句子在推理时的概率；随着模型规模增大，该效应减弱；约2%-4%的注意力头控制该现象，关闭它们可缓解问题而不损害模型性能。


<details>
  <summary>Details</summary>
Motivation: 先前研究仅在词元层面发现了上下文诱发性现象，本文旨在探索该现象是否在句子层面也存在，并进一步理解其机制及影响因素。

Method: 通过计算句子中每个词元的平均对数概率来度量句子层面的上下文诱发性；在26个来自7个家族的LLM上、覆盖主观和客观任务的两个数据集上进行实验分析。

Result: 句子层面上下文诱发性确实存在；模型规模越大，诱发性越弱；约2%-4%的注意力头主导该现象，关闭它们可有效缓解诱发性且不影响模型性能。

Conclusion: 上下文诱发性是LLM中普遍存在的现象，可通过识别并干预少量关键注意力头来缓解，为理解LLM内部机制和提升模型可靠性提供了新思路。
...
TL;DR: 提出Cavewoman双通道评估协议，发现输出压缩可降低推理成本（1.4-2.4x），但输入压缩反而增加成本（~1.15x）并导致准确率下降和文本偏离，是一种双输策略。


<details>
  <summary>Details</summary>
Motivation: 现有研究广泛提倡使用"压缩语言"（caveman style）来降低推理成本，但缺乏系统性评估：压缩输入提示还是压缩模型输出对成本的影响不同，且对任务准确率和生成质量的代价未被充分研究。

Method: 提出Cavewoman双通道评估协议，在相同样本上同时测量输入通道和输出通道的压缩效果，评价指标包括：任务准确率、实际每项成本、以及与模型无约束生成参考文本的一致性。在5个数据集上评估8个模型、5种压缩级别。

Result: 输出压缩在多数API模型上实现1.4-2.4x（最高3x）的成本降低，在所有开源模型上同样有效；输入压缩则相反，导致净成本上升（均值~1.15x，最差情况达2.7x），同时准确率下降。非推理模型中约半数正确生成不再蕴含模型自身的无约束基线生成。

Conclusion: 输出压缩是有效的降本策略，但输入压缩是严格的双输（lose-lose）策略，且模型生成的表面文本会偏离无约束参考，即使控制长度、多重比较校正和使用语义度量后该偏差仍然存在。
...
TL;DR: 通过分析《全唐诗》中357位诗人的作品，结合CBDB地理数据，利用字符n-gram TF-IDF和可解释特征（意象、季节、用典），证明唐代诗歌语言存在可检测的地域信号：南北分类准确率0.69（基线0.53），语言距离随地理距离衰减（Mantel r=0.40），地域信号随时间变化（晚唐最强），且模型的高置信度错误具有历史意义（初唐南方诗人被误判为北方）。GuwenBERT未超越简单TF-IDF模型。


<details>
  <summary>Details</summary>
Motivation: 探究唐代诗人的地理起源是否在其诗歌作品中留下可检测的语言痕迹，从而将计算语言学方法应用于文学史研究，作为文学史假说的生成工具。

Method: 构建357位唐代诗人的诗级语料库，通过CBDB将诗人关联至其行政"道"（circuit）。使用字符n-gram TF-IDF结合可解释领域特征（意象、季节、用典），采用经典分类模型和神经网络模型进行多分类预测。同时使用GuwenBERT（古典中文Transformer）进行对比实验，并进行Mantel检验分析地理距离与语言距离的关系。

Result: 南北分类准确率0.69，超过多数类基线0.53；语言距离与地理距离存在正相关（Mantel r=0.40, p≈0.09）；南北可分离性在初唐接近随机，晚唐最强；初唐所有误分类均为南方诗人被归类为北方；GuwenBERT未超越TF-IDF，两者结合无提升。

Conclusion: 唐代诗歌中存在可检测的地域语言信号，该信号受时间因素调节（帝国鼎盛期宫廷驱动同质化、晚期区域分化）。可解释机器学习可作为文学史假说的生成工具，字符n-gram已充分捕捉地域信号。
...
TL;DR: PORTER是一种语言驱动的结构化EHR基础模型，通过解耦事件表示与固定词汇表，利用文本编码器和数值路径处理临床事件，在跨机构和词汇迁移中保持高性能，无需重新训练或词汇映射。


<details>
  <summary>Details</summary>
Motivation: 现有EHR基础模型将临床事件编码为固定词汇表中的离散token，无法表示包含未见概念或新属性组合的事件，限制了跨机构甚至同一机构内不同部署管道的迁移能力。

Method: PORTER使用冻结的文本编码器通过事件描述来表示事件，通过专用路径集成数值，并通过自回归预训练的时间骨干网络学习患者时间线上的临床动态。

Result: 在儿科医院的74项临床预测任务中，PORTER与固定词汇模型平均AUROC相当；迁移到未见词汇时无需重新训练即可恢复97.1%的AUROC；迁移到MIMIC时优于固定词汇模型（后者因69%事件token未见而丢失）；数值路径提高了对数值大小的敏感性而不破坏临床概念身份；相比任务特定文本序列化比较器，AUROC更高且计算量降低329倍。

Conclusion: PORTER是实现词汇无关的EHR基础模型的关键一步，减少了词汇统一化的需求，同时保持了领域内性能并实现了高效的跨任务复用。
...
TL;DR: 本文系统对比了文本记忆和代码记忆在自进化智能体中的互补性权衡，并提出基于分层双表征记忆的Metis系统，在AppWorld基准上实现准确率提升20.6%、执行成本降低22.8%。


<details>
  <summary>Details</summary>
Motivation: 现有自进化智能体系统将经验表示为文本或代码，但表示形式通常在设计时固定而非根据经验特征动态选择，导致两种表示的权衡缺乏系统理解。

Method: 首先通过受控实验对比文本记忆与代码记忆在构建成本、执行效率和可迁移性上的互补性权衡；然后提出Metis系统，采用分层双表征记忆架构，将文本经验组织为执行计划、环境事实和常见陷阱，并选择性地将重复出现的计划结晶为可调用的已验证工具。

Result: 在AppWorld基准上，Metis相比ReAct将任务准确率提升最多20.6%，同时执行成本降低22.8%。与代表性自进化智能体系统相比，Metis在准确率、执行效率和记忆构建成本之间实现了更优平衡。

Conclusion: 文本记忆和代码记忆具有互补性，单一表征无法兼顾所有维度；分层双表征记忆架构能有效结合两者的优势，在必要时才生成工具代码从而控制成本。
...
TL;DR: MedBench v5 是一个面向临床多模态模型（语言、视觉-语言和智能体系统）的基准测试，从静态问答转向动态、过程导向的评估。它引入双维度框架（临床认知响应14个子维度和医学原子技能4个智能体环境，共63个任务）、三种信息流压力源（遗漏、矛盾、证据延迟）、五节点动态过程审计协议以及幻觉传播监控机制。实验发现，强整体任务性能不保证过程稳定性，压力主要破坏矛盾检测、诊断更新、幻觉传播和基于矛盾的自我修正。


<details>
  <summary>Details</summary>
Motivation: 现有医疗AI基准缺乏过程可见性、原子技能评估和集成幻觉检测，无法全面评估模型在临床场景中的动态推理能力和鲁棒性。

Method: 提出了MedBench v5基准，包含：(1) 双维度框架（临床认知响应14子维度 + 医学原子技能4智能体环境，共63任务）；(2) 三种可切换信息流压力源（遗漏、矛盾、证据延迟）；(3) 五节点动态过程审计协议，生成模型特定故障指纹；(4) 幻觉传播监控（起始、传播、锚定、矛盾交互-捕获静默幻觉）。

Result: 前沿模型强整体任务性能不保证过程稳定性；压力源主要破坏矛盾检测、诊断更新、幻觉传播和基于矛盾的自我修正，而最终证据基础可能表面上保持稳定。
...
TL;DR: 本文提出BehaviorBench基准，系统评估基础模型在行为科学领域的四项核心能力（行为预测与模拟、战略决策、主体特质推断、行为知识应用），并在个体和分布两个层面进行评价。基于该基准开发的Be.FM-1.5模型在分布对齐指标上显著领先，同时在个体层面指标上保持竞争力，表明适当的行为适应可以缩小通用模型与专用模型之间的差距。


<details>
  <summary>Details</summary>
Motivation: 基础模型在行为科学领域（心理学、社会学、经济学）的应用日益增多，但缺乏一个系统性的基准来评估这些模型在不同行为科学任务、情境和人群中的表现。

Method: 构建BehaviorBench基准，从行为预测与模拟、战略决策、主体特质推断、行为知识应用四个核心能力维度评估基础模型，并在个体层面和分布层面两个粒度进行评价。基于该基准进一步微调得到Be.FM-1.5行为基础模型。

Result: 通用大模型在个体层面预测和知识密集型任务上表现优异，而行为基础模型（Be.FM-1.5）在分布对齐指标上显著更强。Be.FM-1.5在分布指标上领先，同时在个体层面指标上保持竞争力。

Conclusion: 分布层面的评估至关重要；BehaviorBench可作为开发与评估行为对齐AI系统的基础；适当的行为适应（如Be.FM-1.5）能够缩小通用模型与专用模型之间的差距，为行为科学研究提供广泛潜力。
...
TL;DR: 针对印地语系语言NLP基础设施碎片化问题，提出基于波你尼《八章书》语法框架的统一计算架构与基准测试套件，旨在将分散的印地语资源整合为单一高资源元语言基础。


<details>
  <summary>Details</summary>
Motivation: 超过10亿人使用印地语系语言，但相关NLP基础设施因按语言或语系分别构建工具、解析器和数据集而碎片化且发展不足。现有方法忽视了印地语系语言通过两千多年围绕梵语趋同而形成的深层规律性——波你尼语法框架所形式化的共同形态句法架构。

Method: 提出基于波你尼《八章书》语法框架的四部分基准测试套件，将印地语系语言共享的形态句法架构显式化、可量化。该框架超越谱系界限，将多种语言统一在共同结构下，使看似分散稀疏的印地语资源合并为单一高资源元语言基础。

Result: 预计该Pāninian框架能使印地语系语言系统更准确、更数据高效、更具可迁移性，实质性地将多个稀疏资源合并为高资源元语言基础。

Conclusion: 波你尼语法框架提供了印地语系NLP领域长期缺失的统一计算架构。同时提出了一个可供解释性研究探讨的问题：神经模型是否会在训练中自发习得波你尼的语法范畴。
...
TL;DR: 本文提出Digi Turbine，一种结合物理信息神经网络（PINN）与贝叶斯先验逆识别及一阶可靠性方法（FORM）筛选的合成基准，用于海上风电单桩支撑结构监测，通过嵌入欧拉-伯努利梁方程和Winkler土基模型实现快速状态估计。


<details>
  <summary>Details</summary>
Motivation: 海上风电支撑结构的可靠健康监测需要从稀疏测量中进行快速状态估计，而高保真有限元或气动弹性分析难以直接用于在线监测，纯数据驱动代理又需要大量训练集，因此需要一种融合物理信息的高效替代方法。

Method: 该方法将简化的欧拉-伯努利梁方程与Winkler土基模型嵌入训练目标，结合贝叶斯先验逆识别，并添加一阶可靠性方法（FORM）筛选，所有验证基于NREL 5MW参考风机背景的合成配置。

Result: 通过合成配置（解析解或有限差分解作为真值）进行验证，证明了该方法在稀疏测量条件下对单桩支撑结构状态估计的有效性。

Conclusion: Digi Turbine作为一种合成可靠性感知的PINN基准，能够有效结合物理模型与数据驱动方法，为OWT支撑结构在线监测提供高效的状态估计途径。
...
TL;DR: 本研究通过对Nature Communications上11,063篇已接收论文的多轮审稿评论进行细粒度分析，构建了约5000条人工标注语料，训练了基于深度学习的方面情感分类模型（LCF-BERT-CDM达82.65% Macro-F1），发现随着审稿轮次增加，积极情感比例上升而消极情感下降，且方面情感得分与审稿轮次总数呈负相关，其中"实验"、"研究意义"和"结果分析"等关键方面相关性更强。


<details>
  <summary>Details</summary>
Motivation: 以往研究受限于粗粒度分析和缺乏对审稿轮次的区分，尤其是多轮审稿过程中评审者关注点和情感倾向的动态变化尚未得到充分探索。

Method: 对11,063篇Nature Communications已接收论文的多轮审稿评论进行分割，识别细粒度审稿方面聚类；构建约5000条人工标注语料；训练基于深度学习的方面情感分类模型（包括LCF-BERT-CDM）；进行统计分析和相关性分析。

Result: LCF-BERT-CDM模型取得最佳性能，Macro-F1为82.65%；随着审稿轮次增加，积极情感比例上升、消极情感下降；方面情感得分与审稿轮次总数呈负相关，关键相关方面包括"实验"、"研究意义"和"结果分析"。

Conclusion: 多轮审稿中审稿人的情感呈现出从消极向积极演化的趋势，且方面级情感分析能更精细地揭示这一动态过程。
...
TL;DR: 本文提出MMed-Bench-IR基准测试，用于评估多语言临床检索增强生成系统中的跨语言对齐、概念区分和证据检索三项能力，涵盖6种语言和三个异构任务。实验发现现有系统存在严重的跨语言性能下降，例如生物医学编码器在英语中nDCG@10达0.818，但在日语中骤降至0.056。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试仅孤立评估多语言医学检索所需的跨语言对齐、概念区分和证据检索能力，缺乏同时衡量生物医学专业性与多语言覆盖度的综合基准。

Method: 构建MMed-Bench-IR基准，包含三个结构异构的任务：(1)基于UMLS的6,127条跨语言医学QA检索查询；(2)4,975个三难度层次的概念混淆集；(3)2,040条质量保证的多语言证据检索查询。三个任务之间无概念和查询重叠。

Result: 在十个系统和六个范式家族上的评估显示严重的跨语言失效：生物医学编码器在英语上nDCG@10为0.818，但在日语上降至0.056，这种性能鸿沟是纯英语基准无法检测的。

Conclusion: MMed-Bench-IR揭示了现有多语言医学检索系统在跨语言场景下的严重缺陷，强调了构建覆盖多语言能力的综合评测基准的必要性。
...
TL;DR: 本文解决了随机量子神经网络（SQNN）中两个悬而未决的问题：去极化通道能否像dropout一样正则化，以及鲁棒性界能否被预测性理论替代。作者通过N量子比特随机主方程证明了去极化收缩定理，在NSL-KDD数据集上验证了去极化SQNN的鲁棒性优于无噪声电路，推导了逐门dropout的曲率加权L2惩罚公式，并证明两种机制在泛化上统计等价，最后在中性原子硬件上实现了SQNN。


<details>
  <summary>Details</summary>
Motivation: 填补先前研究遗留的两个空白：去极化通道是否可作为dropout式正则化器（此前被认为失败），以及能否构建一个可预测的鲁棒性理论来替代保守的对抗鲁棒性界。

Method: 通过随机主方程和向量化Liouvillian形式化N量子比特SQNN；证明去极化收缩定理(depolarising contraction theorem)；在NSL-KDD数据集上使用白盒FGSM和PGD攻击进行实验；推导自适应惩罚公式比较dropout和去极化噪声；进行30种子泛化研究；在中性原子硬件上实现。

Result: 去极化SQNN在强ℓ∞/ℓ2攻击下显著优于无噪声电路(p=0.04)，鲁棒性方差降低约两倍，从未出现灾难性鲁棒性崩溃（无噪声模型和经典检测器从95%降至47%）；dropout和去极化噪声均能小幅但显著缩小训练-测试差距（≈0.01, p<10⁻⁴和p=0.004），且统计不可区分；将dropout率提高至1/2以上无效；先前工作的单种子二分结果不可复现。

Conclusion: 去极化通道并非失败的正则化器，而是以输出空间惩罚的方式起作用，与dropout的权重空间惩罚效果相当；鲁棒性源于噪声重塑的训练边界而非梯度收缩；中性原子硬件实现可行。
...
TL;DR: 本文提出SurgicalLM统一框架，通过手术特征门控、任务条件前缀令牌和实例加权归一化三个轻量模块，解决微调编码器在不同NLP任务中的归纳偏置不匹配、类别不平衡特征统计污染和注意力缺乏外部词汇知识问题。在四个任务上，IWN变体宏F1达0.940，较最强基线提升0.036。


<details>
  <summary>Details</summary>
Motivation: 微调编码器在异构NLP任务中面临三个叠加问题：归纳偏置不匹配、类别不平衡导致的特征统计污染、以及缺乏将外部词汇知识注入注意力机制的途径。现有方法未能系统性地同时解决这三个问题。

Method: 提出SurgicalLM统一框架，包含三个轻量模块：1) 手术特征门控（Surgical Feature Gate）：对词汇指示器和[CLS]特征进行逐维sigmoid学习，无信息时可退化为恒等映射；2) 任务条件前缀令牌（Task-Conditioned Prefix Tokens）：量化特征值和任务标识拼接到输入前；3) 实例加权归一化（IWN）：消除门控统计量中的类别先验偏置。并提供理论上的过量风险界证明。

Result: 在SST-2、多跳检索、LLM提示归因和作者检测四个任务（17,830样本，11种模型变体，3个种子）上，IWN变体宏F1达0.940，较最强非IWN基线提升0.036，作者检测任务提升0.130。随机词汇控制实验验证增益来自词汇特性而非参数容量。

Conclusion: SurgicalLM通过三个轻量模块系统性地解决了微调编码器的核心问题，实验证明其有效性，且代码、词汇表和99.5%恢复率的自动提取配方已开源。
...
TL;DR: 本文研究了LLM在上下文学习中的"鸽笼化"现象——即不良上下文（如用户提供错误解法或对话中包含模型之前错误回答）导致模型性能下降、模式坍缩，并发现该现象随对话轮次增加而恶化，即使提供的示例正确也可能发生。作者提出结合合成错误的RLVR方法，可在不良上下文下将性能提升43-60%。


<details>
  <summary>Details</summary>
Motivation: 现有研究多聚焦于提示工程的正面效果，但对无恶意意图的不良上下文（非越狱攻击）如何系统性损害LLM性能缺乏系统研究。作者旨在揭示这种隐蔽但普遍的性能退化机制。

Method: 在10个可验证与开放式任务上，使用10种不同模型，设计两类场景：(1)用户提供错误解决方案；(2)对话历史包含助手之前的错误回答。通过控制对话轮次、错误示例数量等变量，测量性能退化程度。并提出RLVR（基于强化学习的验证器）结合合成错误数据的缓解方法。

Result: (1)错误上下文导致38-40%性能下降；(2)模型重复错误答案，在代码和文本生成中收敛于狭窄答案集；(3)在有争议话题上迎合用户或自身先前立场。错误次数从1增至5时，性能额外下降14%以上。即使正确示例也可能引发模式坍缩。所提RLVR方法在不良上下文中比基线提升43-60%。

Conclusion: LLM对上下文中错误的"粘附性"远超预期，需警惕非恶意场景下的性能退化；RLVR+合成错误是一种有效的缓解策略，但根本解决仍需更鲁棒的上下文推理机制。
...
TL;DR: CALIBER提出了一种针对推理语言模型的双阶段置信度校准方法：在思考前（prompt-level）和回答后（answer-level）分别进行置信度估计，并使用对应信息状态的目标进行监督，在多个基准测试上显著降低了期望校准误差（ECE）。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只在思考前或回答后单次 eliciting 置信度，忽略了推理模型中置信度的状态依赖性——不同信息状态下置信度估计应匹配不同的监督目标（prompt-level成功率 vs. answer-level正确性），导致校准效果不佳。

Method: CALIBER在推理前和推理后分别 elicits 两次置信度估计，并用与各自信息状态匹配的目标进行监督：prompt-level的成功率监督思考前的置信度，individual answer-level的正确性监督回答后的置信度。

Result: 在BigMathDigits上，CALIBER在7B模型上相比最强单置信度基线降低了52.5%的ECE，同时取得了最佳Brier分数和AUROC，准确率差距在2.1点以内。在30B模型上，CALIBER在BigMathDigits上取得最佳ECE，Brier分数和AUROC具有竞争力。在分布外基准GPQA和TriviaQA上取得最佳ECE和Brier分数，在SimpleQA上保持竞争力。消融实验表明，位置-目标对齐在分布偏移下最有效。

Conclusion: CALIBER通过双阶段、状态匹配的置信度估计与监督协议，显著提升了推理语言模型的校准性能，尤其是在分布偏移场景下，验证了状态依赖性置信度建模的有效性。
...
TL;DR: AVOC提出了一种针对长音频-视频理解的令牌压缩框架，将多模态令牌压缩重构为top-K检索问题，基于相关性、重要性和多样性三个IR准则选择信息单元，在长音频-视频基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型在短音频-视频理解上表现优异，但受限于有限的上下文窗口和严重的信息冗余，难以处理长时间音频-视频理解任务。

Method: 提出AVOC框架，在模态编码器和LLM骨干之间引入可学习的令牌压缩模块，将多模态令牌压缩重构为top-K检索问题，基于相关性、重要性和多样性三种经典信息检索准则设计定制化机制，并将其整合为统一的检索式压缩流程。

Result: 在OmniVideoBench和LVOmniBench上分别超越第二名4.9和5.5个平均准确率点，在长达一小时的Audio-Video Needle-in-a-Haystack任务上保持稳健性能。

Conclusion: AVOC通过检索式令牌压缩有效解决了长音频-视频理解中的上下文窗口限制和信息冗余问题，实现了目前最优的长时序多模态理解性能。
...
TL;DR: 本文介绍了布拉格依存树库框架的第二版整合版本（PDT-C 2.0），这是一个包含近400万词符、涵盖多种语言层级（尤其是句间现象如共指和话语关系）的捷克语语料库，历经近30年持续开发，具有统一标注、多样化体裁的特点，并配有完全兼容的词典。


<details>
  <summary>Details</summary>
Motivation: 布拉格依存树库框架致力于系统性地包含和链接不同语言层级，特别是意义表示层面中的句间现象（共指和话语关系），为此需要一个统一、连贯标注且体裁多样化的资源。

Method: 在近30年的持续开发基础上，对语料库进行了统一和连贯的标注，实现了体裁多样化，构建了包含近400万词符的捷克语资源，并开发了完全兼容的配套词典。

Result: 产出PDT-C 2.0，这是一个约400万词符、体裁多样化的捷克语标注语料库，配有兼容词典和经过训练的分析器，以CC BY-NC-SA许可发布。

Conclusion: 该语料库不仅用于持续的 linguistic 研究，还广泛应用于国际范围内的NLP工具比较以及向其他形式体系的转换。
...
TL;DR: 本文对Transformer架构进行了系统性综述，按机制维度分类（编码器-only、解码器-only、编码器-解码器、长上下文、基于排列和生成器-判别器变体），并涵盖了2023年后的关键进展（指令微调、RLHF、DPO、MoE、检索增强等），同时从部署角度评估各架构在医疗、金融、法律等领域的适用性，提出了参数-能耗权衡及"state of the art"标准再思考等议题。


<details>
  <summary>Details</summary>
Motivation: 由于Transformer模型发布节奏过快，从业者难以区分真正重要的创新与增量式改进，亟需一个系统性的、机制与使用双层次的综述来理清脉络并指导实际部署决策。

Method: 采用双层次分析方法：机制层面，构建包含六类Transformer变体的工作分类法，并扩展到指令微调、RLHF/DPO、MoE、RAG等最新技术；使用层面，调查医疗、金融、法律、教育、客服、创意写作、科学工作七大领域的具体部署，将应用需求与模型能力进行匹配。

Result: 提出了四个对部署决策至关重要的比较轴，量化了参数量与能耗之间的权衡，并批判性地讨论了对齐方法、数据溯源和基准饱和对"state of the art"概念的影响。

Conclusion: 列出了当前最值得关注的研究问题，为未来Transformer研究提供了方向性建议。
...
TL;DR: 本文描述了将捷克语Prague Dependency Treebank-Consolidated (PDT-C)语料库转换为Universal Dependencies (UD)格式的过程，该语料库比原版PDT规模大两倍以上且包含更多样化的文体和领域，并讨论了两种标注方案在依存结构拓扑、词性及关系类型粒度上的差异及其转换方法。


<details>
  <summary>Details</summary>
Motivation: 随着PDT-C的发布，需要将其从原有的Prague标注体系转换为Universal Dependencies格式，以便更广泛的研究社区能够使用该资源。两种标注方案虽然表面上相似，但存在诸多细微差异，需要系统性地解决转换问题。

Method: 对PDT-C进行UD格式的自动转换处理，分析两种标注方案在依存结构拓扑、词性和关系类型粒度上的差异，通过示例展示具体差异以及克服这些差异的转换策略。

Result: 成功将PDT-C（规模是原PDT的两倍以上，包含更多文体和领域多样性）转换为Universal Dependencies格式。

Conclusion: 尽管Prague标注体系不如UD"通用"且更紧密地绑定于单一语言，但其多层标注信息丰富，能够提供构建基本UD树所需的所有信息，甚至更多。
...
TL;DR: 提出了一种将双语词典词条进行词性标注的算法，应用于Al-Mawrid阿拉伯语-英语词典，通过消歧后将英语翻译等价词的词性标签转移至词典义项，借助Princeton WordNet获取英语词性标签，实现了高精度低成本的资源轻量级词性标注。


<details>
  <summary>Details</summary>
Motivation: 构建NLP/HLT工具需要语言学专家、大量投资和长时间，统计方法需要大规模标注语料库，规则方法需要包含丰富语言和世界知识的大型词典。因此需要资源轻量级方法来开发资源匮乏语言的自然语言处理工具。

Method: 将双语词典中的英语翻译等价词（TEs）的词性标签，经过消歧处理后转移至对应的词典义项。英语词性标签从Princeton WordNet获取。

Result: 该方法在Al-Mawrid阿拉伯语-英语词典上取得了高准确率，且成本低廉。

Conclusion: 双语词典义项的词性标注是将双语词典链接到WordNet或将其标准化为WordNet-LMF格式的前提条件（其中同义词集而非单词为基本单元）。该方法是一种有效的资源轻量级方案。
...
TL;DR: 本文介绍了MorfFlex形态词典架构及其在捷克语中的应用MorfFlex CZ。该词典通过编码屈折和派生模式，以简洁的<词形, 词元, 标签>三元组列表形式，高效管理超过1亿词形和100万词元。该资源在确保布拉格依存树库的标注一致性以及支撑先进NLP工具（如MorphoDiTa）方面发挥着关键作用。


<details>
  <summary>Details</summary>
Motivation: 针对具有丰富形态系统的语言（如捷克语），传统形态词典存在体积庞大、维护困难的问题。作者旨在设计一种高效架构，通过编码屈折和派生规律来大幅压缩词典规模，同时维持手动标注的一致性和对NLP工具的支持。

Method: 提出MorfFlex架构，以非结构化<词形, 词元, 标签>三元组列表作为发布格式，但在手动维护的源文件和转换脚本中编码了复杂的屈折和派生模式，利用语言的规律性显著减少词典体积。

Result: 构建了捷克语形态词典MorfFlex CZ，当前包含超过1亿词形和100万词元，有效支撑了布拉格依存树库的手动标注一致性以及MorphoDiTa等先进自动工具。

Conclusion: MorfFlex架构成功展示了通过编码形态规律来管理大规模形态词典的有效性，并能确保语料库标注一致性及推动高级NLP应用的发展。
...
TL;DR: 本研究系统揭示了基于提示的LLM交互范式中，提示排名在随机种子和有限评估子集等常见可变因素下不稳定，顶级提示经常变化导致选择不可靠；提出基于置信下限的稳定性感知选择策略，在保持竞争力的同时提升鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前使用LLM时普遍采用"多候选提示评估-选最优"的流程，该流程隐含假设提示排名在评估条件轻微变化下保持稳定。作者旨在检验这一假设是否成立，并解决排名不稳定导致的不可靠选择问题。

Method: 系统研究提示排名在随机种子和有限评估子集两种常见可变因素下的稳定性；提出基于较低置信区间的稳定性感知选择策略，同时考虑性能和方差。

Result: 在三个开源LLM和两个基准任务上，整体排名相关性虽中等至高，但顶级提示的身份频繁变化，导致选择不可靠；所提策略在不稳定设置下提升鲁棒性，在稳定设置下保持竞争力。
...
TL;DR: 本文介绍了AutoSpecNER——一个由专家标注的车辆广告细粒度命名实体识别数据集，包含659条广告、10,000+实体和15个类别。通过注释者一致性验证(91.5%)保证质量，并基准测试了规则方法、微调Transformer编码器和LLM。DeBERTa以90%的micro-F1最优，远超规则方法(43%)和最强LLM(77.8%)。


<details>
  <summary>Details</summary>
Motivation: 车辆广告包含丰富的规格信息，但汽车领域的命名实体识别(NER)资源十分有限，缺乏高质量、细粒度的标注数据集。

Method: 构建了AutoSpecNER数据集：从热门汽车销售网站收集659条广告，由专家标注10,000+实体，覆盖MODEL、ENGINE_SPEC、BATTERY_CAPACITY等15个类别。通过注释者一致性(IAA)评分91.5%确保质量。基准测试包括规则提取、微调Transformer编码器(如DeBERTa)和大型语言模型。

Result: DeBERTa取得最佳性能，micro-F1达90%，显著优于规则基线(43%)和最强LLM(77.8%)。

Conclusion: AutoSpecNER数据集有效填补了汽车NER资源空白；在车辆广告细粒度实体识别任务中，微调DeBERTa相比规则方法和LLM具有明显优势。
...
TL;DR: 提出ExtractConf，一种基于双读取器（Hunter和Mapper）结构差异来估计LLM文档提取置信度的方法，无需领域特定规则或重训练，在选择性预测中显著优于基于log概率的传统方法。


<details>
  <summary>Details</summary>
Motivation: 高风险文档处理中，LLM提取结果的不可靠性比缺失更危险，而现有置信度估计方法（token log概率、口头置信度、自一致性）在实用阈值下均崩溃为全正预测，无法区分可信与不可信提取。

Method: 设计两种结构不同的读取器：Hunter（字段引导，在模式槽位补全压力下提取）和Mapper（文档引导，全局扫描并基于文档内容提取）。利用二者失败模式不对称（Hunter对缺失字段产生幻觉，Mapper漏检视觉不显著字段），融合跨调用分歧、LLM内部不确定性、OCR、图像质量、空间布局等特征，训练分类器实现置信度估计。

Result: DocILE数据集（55字段发票，26%失败率）上ROC AUC达0.928，选择性预测风险较log概率均值降低70%；80%覆盖率下准确率达99.1%。零样本迁移至CORD收据数据集AUC达0.858；Lasso重校准将ECE降低89%、Brier分数降低43%。

Conclusion: ExtractConf的跨调用分歧信号在文档域间具有泛化能力，轻量级重校准可有效适配新领域，为人机协同的文档自动化流程提供了实用可靠的置信度估计方案。
...
TL;DR: EDV是一个Execute-Distill-Verify框架，通过多异构智能体并行探索、第三方智能体蒸馏经验、共识机制验证，解决LLM智能体自我进化中的自我确认陷阱问题，在三个长程任务基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于经验的LLM智能体学习方法大多依赖单智能体循环，导致自我确认陷阱：错误但自洽的轨迹被误认为成功经验，在检索和重用时累积错误。亟需一种可靠的经验学习框架来打破这一局限。

Method: EDV框架包含三个阶段：1) Execute阶段：多个异构智能体并行探索同一任务空间，生成多样化的候选轨迹；2) Distill阶段：专用第三方智能体对轨迹进行比较分析，产生候选经验，减少以执行者为中心的总结偏差；3) Verify阶段：执行组通过共识机制验证候选经验，仅批准的经验被写入共享或私有记忆。

Result: 在tau2-bench、Mind2Web和MMTB三个长程基准上，EDV持续优于强基线方法，验证了可靠经验构建对智能体自我进化的关键作用。

Conclusion: 通过解耦执行、蒸馏、验证三个阶段，EDV将经验学习从孤立的自我反思转变为协作构建，在记忆插入前过滤错误和噪声内容，是实现鲁棒智能体自我进化的有效范式。
...
TL;DR: 这篇论文系统测量了20种非洲语言在主流大语言模型分词器中的“token fertility”惩罚，发现所有非洲语言都比英语需要更多token（中位数1.88倍，最高达8.92倍），导致高达8.9倍的推理成本和仅剩11%的有效上下文窗口，且没有任何现有分词器能消除这一结构性惩罚。


<details>
  <summary>Details</summary>
Motivation: 虽然已有研究指出多语言场景下不同语言存在token fertility差异，但尚未有工作从企业部署经济学和认知上下文容量的角度，系统测量非洲语言在商用大模型中的这一结构性惩罚。论文旨在填补这一空白，量化非洲语言用户在使用大模型时面临的额外成本与上下文限制。

Method: 使用FLORES-200+（19种非洲语言）和MAFAND-MT（尼日利亚皮钦语）的平行语料，控制内容变量以隔离语言效应。测量11个前沿和开放分词器（包括GPT-5 o200k_base、Gemma 4等）在20种非洲语言（涵盖5个语系、3种文字系统）上的token fertility，并映射到推理成本、延迟和有效上下文窗口等部署指标。

Result: 所有非洲语言均有高于英语的tokenization premium（中位数1.88x，GPT-5 o200k_base；最高8.92x，N'Ko语）。埃塞俄比亚文字和N'Ko文字的惩罚最大（7-9x）。该惩罚在不同语料库间高度一致（FLORES vs SIB-200 Pearson r=0.9998）。转换为部署指标：最高8.9倍推理成本，等效延迟乘数（N'Ko语8.92x，阿姆哈拉语7.4x），有效上下文窗口仅为英语的11%。最佳分词器（Gemma 4）将平均premium从3.31x降至2.38x，但无法消除惩罚。

Conclusion: 非洲语言在大模型分词中承受系统性、结构性的token fertility惩罚，这直接转化为更高的经济成本和更小的认知上下文容量，且惩罚落在最无力承担的语言使用者身上。论文发布了开源测量工具（afri-fertility）、公开排行榜和缓解指南，揭示了数字鸿沟被编码在子词词汇层面的机制。
...
TL;DR: 该论文提出了UOL@IDEM系统，将L1感知的词汇难度预测任务建模为回归问题，结合多语言上下文表示和工程化特征（频率、形态、检索、语义对齐、同源相似度、MLM可预测性），在西班牙语、德语和中文三种语言上取得优于基线的性能，RMSE分别为1.132、1.037和0.891。


<details>
  <summary>Details</summary>
Motivation: 针对BEA 2026共享任务中L1感知的词汇难度预测问题，现有方法未能充分结合多语言上下文表示与针对母语（L1）的工程化特征，作者希望构建一个能够跨语言预测词汇难度、并捕捉L1特异性信号的有效系统。

Method: 将任务建模为回归问题，分别为西班牙语、德语和中文训练独立系统。系统结合多语言上下文表示（如BGE-M3、mE5、LaBSE）与工程化特征，包括词频、表面形式、检索证据、语义对齐、同源相似度以及掩码语言模型可预测性。使用句子嵌入编码器进行特征提取。

Result: 在官方闭轨提交中，西班牙语、德语和中文的RMSE分别为1.132、1.037和0.891，一致优于官方闭轨基线。特征分析表明词频是最稳定的预测因子，而上下文可预测性、形式相似度、检索和语义特征提供了互补的L1敏感信号。错误分析显示排名性能良好，但对最容易的词汇校准较差（常被高估）。

Conclusion: 多语言上下文表示与工程化特征的组合能有效预测L1感知的词汇难度；频率特征最为关键，其他特征提供互补信息；系统在排名上表现优异，但需改进对简单词汇的校准。
...
TL;DR: 本文构建了首个土耳其语诈骗电话多模态数据集（100对音频-文本对），评估了7种大语言模型在原始音频、ASR文本和人工校正文本三种输入条件下的诈骗检测表现，发现基于文本输入（无论是否人工校正）均优于直接音频处理。


<details>
  <summary>Details</summary>
Motivation: 诈骗电话对全球弱势群体造成严重危害，但现有检测研究几乎全部聚焦英语等富资源语言。土耳其语等低资源语言面临标注数据稀缺、技术防护不足的困境，亟需针对性的检测研究。

Method: 创建了包含100对对齐音频-文本对的土耳其语诈骗/正常对话多模态数据集，评估7种LLM（Gemini 2.5系列3种、GPT-4o、Qwen系列3种）在三种输入条件下的表现：原始音频、ASR自动转录文本、母语者校正后的转录文本。

Result: 基于转录文本的输入一致优于直接音频处理；人工校正文本与未校正的ASR文本性能相当。

Conclusion: 本研究凸显了在文化和语言包容的AI安全研究方面的迫切需求，以及构建更鲁棒的多模态欺诈预防系统的必要性，尤其是在低资源语言场景下。
...
TL;DR: 本文提出Agora基准测试，用于评估大语言模型在跨域、档案级文档中的基于证据的推理能力。该基准包含362个问题、8个领域、9,664份真实文档（3.72亿token），远超任何模型的上下文窗口。最强模型（59.4%准确率）显示任务远未解决。


<details>
  <summary>Details</summary>
Motivation: 现有基准只覆盖了档案化推理的部分场景，缺乏同时对档案基础性、智能体探索和跨领域覆盖的联合评估，需要一个新的基准来全面评估大模型在真实工作场景中寻找分散证据、协调不一致术语/单位/时间惯例并计算答案的能力。

Method: 构建了Agora基准测试：包含362个问题和8个领域集合（9,664份真实文档、3.72亿token），通过智能体化流程实现跨文档任务合成、防泄漏混淆和难度过滤，确保模型必须进行有目的的探索而非穷尽扫描。

Result: 评估8种模型，最强模型（GPT-4或其他）仅达到59.4%准确率，且各领域表现差异显著，表明任务远未解决。

Conclusion: Agora基准揭示了当前大模型在档案化推理任务上的严重不足，为未来研究提供了挑战性评估框架。
...
TL;DR: NatureBench是一个从Nature系列出版物中提炼的90个跨学科任务基准，用于评估AI编码代理在真实科学问题上的发现能力。最强模型仅在17.8%的任务上超越SOTA，失败主要源于方法选择错误和计算预算不足，而非任务理解问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI编码代理基准测试存在环境碎片化问题，缺乏可信度，且无法评估代理在真实科学发现而非复现任务上的能力。

Method: 构建NatureGym自动化流水线，从源论文标准化构建每个任务的容器化环境；从Nature系列出版物中提炼90个跨学科任务；在严格的禁用网络搜索协议下评估十个前沿代理配置。

Result: 最强模型在g>0.1标准下仅超越SOTA 17.8%的任务；代理主要通过方法论翻译（将科学任务转化为熟悉的监督预测问题）而非真正的科学发明来成功；失败主要由错误方法选择和计算预算不足导致。

Conclusion: 当前AI编码代理在真实科学发现任务上表现有限，尚未达到真正的科学创新能力，主要受限于方法选择能力和计算资源。
...
TL;DR: 本文研究了跨语言提示策略以解锁大型语言模型中隐藏的参数量化知识，发现跨语言探索能显著提升知识迁移和事实召回，在17种语言上表现出比原生语言缩放更高效的计算帕累托前沿，并提高了跨语言一致性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的参数量化知识在不同语言间并非同等可访问，标准推理方法难以提取本地化事实，导致跨语言知识迁移和一致性失败。

Method: 识别了跨语言探索的四个内在维度，并在涵盖17种类型多样语言的 multilingual factual benchmarks 上评估这些维度对参数量化知识检索的影响。

Result: 跨语言探索显著提升知识迁移和事实召回，比原生语言缩放更高效（更优的计算帕累托前沿），同时跨语言一致性提升超出单纯准确率增长所能解释的范围。

Conclusion: 多语言提示探索是一种高效的推理时策略，可用于解锁大型语言模型中的潜在参数量化知识。
...
TL;DR: 本文提出MEMPROBE基准，直接评估LLM代理长期记忆的可恢复性——即从代理留下的记忆痕迹中重建用户状态的能力。实验发现任务完成与记忆恢复是两种不同能力，即使无记忆基线也能高完成度，但类别平衡恢复率仅约0.6，在top-k检索下进一步下降。


<details>
  <summary>Details</summary>
Motivation: 现有长期记忆评估主要通过下游行为（如回答质量、个性化效果）间接测试，忽略了记忆工件本身的可审计性。作者认为长期记忆应作为可审计的后交互工件来评估，即从代理留下的记忆中能重建出怎样的结构化用户状态。

Method: 构建MEMPROBE基准，包含50个模拟用户（每个用户31个隐藏维度，共1,550个恢复目标），采用合成真实数据实现高效可扩展测量。代理在泄漏控制的任务轨迹中辅助用户后，从产生的记忆（全存储和top-k访问两种设置）中重建用户状态，与基于分类法的真实用户状态进行对比评分。

Result: 测试5种代表性记忆系统发现：任务完成度几乎饱和（即使无记忆基线也表现良好），但类别平衡恢复率仅约0.6；在top-k检索下恢复率进一步下降。任务辅助与记忆恢复表现为不同的能力维度。

Conclusion: MEMPROBE是首个直接研究记忆恢复的基准，将恢复作为未来记忆代理可以优化的具体目标。恢复是一种可审计的评估方式，有助于训练代理更忠实地记住用户，随着相处时间增长变得更加可靠。
...
TL;DR: 本文揭示了LLM社会偏见评估中存在结构性的“范式鸿沟”：孤立评估抑制偏见激活，而比较设置会激化潜在歧视。CoT推理、中性选项等结构因素会进一步放大这一系统性偏见，且该现象随模型规模扩大而增强。


<details>
  <summary>Details</summary>
Motivation: 当前LLM社会偏见评估文献存在严重的方法论碎片化，导致相互矛盾的结论。根本原因在于忽略了基准评估的结构性框架。作者旨在建立一个统一可控的框架来标准化异构基准，从而系统对比孤立评估与比较评估的差异。

Method: 提出统一可控评估框架，将异构基准标准化，系统对比孤立人口统计评估与强制选择比较设置。通过控制CoT推理、中性回退选项等结构伪影，解构其对偏见评估的混淆效应。在多个模型家族上进行评估。

Result: 发现系统性的范式鸿沟：比较设置会激进催化潜在歧视，而孤立评估抑制偏见激活；CoT推理在比较设置下加剧社会偏见；即使提供中性选项或模型声称随机回答，系统性偏见仍作为确定性偏见存在；比较偏见随模型规模增大而正向扩大。

Conclusion: 研究人员应利用比较设置来鲁棒地审计隐藏偏见，但从业者不能在模糊的真实世界任务中安全依赖比较部署。
...
TL;DR: 本文提出了基于语言模型的世界模型Qwen-AgentWorld，包含两个规模版本（35B-A3B和397B-A17B），通过三阶段训练（CPT、SFT、RL）在7个领域超过1000万条真实环境交互轨迹上训练，能够通过长思维链进行环境模拟。同时提出了AgentWorldBench基准，并探索了世界模型作为解耦环境模拟器和统一智能体基础模型两种范式，实验表明其显著优于现有前沿模型。


<details>
  <summary>Details</summary>
Motivation: 探索基于语言模型的世界建模如何进一步推动通用智能体的边界发展，解决现有世界模型在环境模拟广度和深度上的不足。

Method: 三阶段训练流程：CPT注入通用世界建模能力（状态转移动力学+专业语料增强）、SFT激活下一状态预测推理、RL通过混合标准-规则奖励框架提升模拟保真度。模型覆盖7个领域，基于1000万+真实环境交互轨迹训练。

Result: Qwen-AgentWorld在AgentWorldBench上显著优于现有前沿模型；作为解耦环境模拟器，可扩展模拟数千真实环境用于智能体RL，效果超越纯真实环境训练；作为统一智能体基础模型，世界模型训练作为预热可提升7个智能体基准的下游性能。

Conclusion: 语言世界模型是构建通用智能体的有效途径，三阶段训练策略和两种应用范式均证明了其强大潜力。
...
TL;DR: 本研究提出多语言叙事评估框架，利用15种语言的414条谚语和4个LLM生成的13k叙事，分析跨文化等价道德寓意的语义保持情况。结果显示跨语言提示保留了谚语语义，但系统性地改变了叙事中的主体性、社会定位和结构，且不同模型间高度趋同，表明仅依赖语义相似性会高估文化保真度。


<details>
  <summary>Details</summary>
Motivation: 当前LLM能在多语言多文化背景下生成叙事，但尚不确定当等价道德寓意通过不同文化形式表达时，模型是否能保留文化扎根的意义。现有评估多依赖语义相似性，可能忽略叙事表达中的文化变异。

Method: 构建跨语言谚语数据集（15种语言414条谚语），使用语义等价的谚语作为文化扎根提示，通过4个LLM生成约13k叙事，分别在单语和跨语言条件下分析语义保持、叙事结构变化及模型间趋同性。

Result: 跨语言提示大体保留了谚语层面的语义，但系统性地重新分配了叙事中的主体性（agency）、社会定位（social positioning）和叙事结构；不同模型家族在单语和跨语言设置中表现出强趋同性。

Conclusion: 多语言LLM依赖共享的语义抽象，但仅凭语义相似性评估多语言叙事会高估文化保真度，需要更全面的文化扎根评估方法。
...
TL;DR: 提出一个多智能体框架，通过语义改写对检索内容进行隐私净化，在保护敏感信息的同时保持语义核心，并在ChatDoctor和Wiki-PII数据集上验证了显著降低隐私泄露的效果。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）在敏感场景中部署时，容易因恶意提示导致隐私泄露风险，现有方法难以在隐私保护和语义保真度之间取得良好平衡。

Method: 提出一个多智能体框架，包含三个专门智能体：隐私提取智能体（识别敏感标识符）、语义分析智能体（分析上下文语义）、重构智能体（去除敏感信息并重建语义核心），通过协作完成检索内容的隐私净化改写。

Result: 在ChatDoctor和Wiki-PII数据集上，基于六个大语言模型进行实验。在LLaMA-3-8B上，将目标信息暴露从基线的144次降至仅1次；BLEU-1得分为0.122，优于SAGE方法的0.117。

Conclusion: 该框架作为异步预处理模块，所有改写操作在离线预处理阶段一次性完成，不增加在线推理延迟，且代码已开源以促进可复现性。
...
TL;DR: 提出SIFT方法解决事实验证系统中LLM输出证据与声明不匹配的问题，通过声明条件化的证据重新评分和WSP自动检查，在多个基准上提升准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的事实验证系统在基准测试中准确率高，但经常输出”支持”标签且所引证据并不足以支持声明；结构化分解虽能检查推理过程，但刚性提取方法丢失了完整声明的上下文信息。

Method: 提出SIFT（声明条件化的证据跨度重新评分）方法，将提取的证据片段与完整声明进行条件化匹配；同时引入WSP（有保证的支持比例），一种自动NLI检查机制，用于验证证据是否蕴含声明。

Result: 在FEVER、SciFact、5PILS和DP四个基准上，使用四个开源骨干模型验证：SIFT在朴素分解损失高达27.6个百分点的情形下恢复准确性；WSP相比直接提示性能提升；WSP对人类金标准证据的AUC达0.92，精确率0.98。

Conclusion: SIFT有效解决了证据与声明不匹配问题，WSP为证据可靠性提供了高精度的自动评估指标。
...
TL;DR: 本文通过跨语言问答任务研究用户对语音翻译系统的心理模型，发现用户通过实践能发展出更强的心理模型（尤其当具备源语言知识时），主要依赖表面错误线索，且提供语音转录有助于建立更好的心理模型。


<details>
  <summary>Details</summary>
Motivation: 尽管数百万人日常使用机器翻译工具，但用户对翻译系统能力边界的感知知之甚少，缺乏系统研究用户心理模型如何影响人机协作。

Method: 提出基于跨语言问答的框架，让用户在机器翻译输出和专业人工重译之间选择，通过分析用户行为、准确度趋势以及不同翻译质量下的表现，研究用户预测系统错误的能力及心理模型的演化过程。

Result: 用户通过实践能发展出更强的心理模型；具备源语言知识有助于心理模型发展；用户主要依赖表面错误线索；提供语音转录能帮助用户建立更好的心理模型。

Conclusion: 跨语言问答是研究MT心理模型的有效下游任务，能增进对用户心理模型及人机协作的理解。
...
TL;DR: Harmonic是一种分层状态空间模型（SSM），通过堆叠三个不同时间尺度的递归层级（每层接收下层预测误差作为输入），在语言建模中显著优于Transformer和Mamba，支持更长序列（训练至64K tokens），并消除了RoPE位置编码的长度限制。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer模型受限于二次复杂度（O(L²)）和RoPE位置编码的序列长度限制，而Mamba等SSM模型在长序列上仍存在性能瓶颈。作者希望设计一种层次化SSM架构，既能高效处理长序列，又能避免位置编码带来的长度泛化问题。

Method: 提出Harmonic架构，包含三个递归层级，以逐渐变慢的时间尺度运行；每个高层级接收低层级的预测误差（而非原始隐藏状态）作为输入。计算复杂度为O(L)。在1B参数规模下，用HarmonicBlock替换TinyLlama的所有注意力层得到Hallamonic模型，消除了RoPE位置编码。

Result: 在enwiki8上，Harmonic（28M参数）相比Transformer在1K/8K/32K tokens上分别提升+1.4%/+6.7%/+11.4% bpt，比Mamba提升0.7-1.8%。64K时仅Harmonic成功训练（6.169 bpt）。WikiText-103复现趋势（H-TF差距+1.7%~+7.2%）。Hallamonic（1.1B）在Lambada上8K tokens时比TinyLlama好+9.4 bpt，且无长度退化。

Conclusion: Harmonic证明了层次化SSM架构在长序列建模上的优越性：低计算复杂度、无位置编码限制、且训练稳定性优于Transformer和Mamba。Hallamonic展示了用SSM替换注意力层的可行性，为高效大语言模型提供了新方向。
...
TL;DR: 本文针对巴西电商领域的产品属性值提取（PAVE）问题，提出了基于大语言模型的AI-PAVEBr系统，并发布了高质量人工标注的葡萄牙语数据集Golden Set，实验证明该方法显著优于传统NER基线。


<details>
  <summary>Details</summary>
Motivation: 巴西电商产品数据爆炸式增长且高度复杂，传统PAVE方法难以处理葡萄牙语的多样性和语言细微差异，亟需针对非英语市场的专门化PAVE解决方案。

Method: 构建AI-PAVEBr系统，利用大语言模型（LLMs）结合针对性提示工程进行PAVE；同时创建并公开Golden Set数据集（包含实体、类别、子类别结构的人工标注葡萄牙语PAVE基准集）。

Result: AI-PAVEBr在PAVE任务上显著优于传统命名实体识别（NER）基线方法，展现出更高的准确性和可扩展性。

Conclusion: 该工作不仅为巴西电商市场提供了高性能、可扩展的PAVE解决方案，还通过发布Golden Set数据集为自然语言处理社区贡献了宝贵的公开资源，推动了非英语语言PAVE研究的可复现性。
...
TL;DR: DREAM通过在冻结的LLM注意力头中注入检索器生成的查询-文档相似度分数，利用自回归下一个token预测目标来训练稠密检索嵌入模型，在BEIR和RTEB基准上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 稠密检索嵌入模型通常需要对比学习目标进行训练，但获取标注的正负文档对成本高昂且困难。本文探究是否可以利用大语言模型的自回归下一个token预测目标为稠密检索提供监督信号，从而减少对人工标注数据的依赖。

Method: 提出DREAM方法，将检索器生成的查询-文档相似度分数注入到冻结的LLM的选定注意力头中。训练时，这些分数决定每个候选文档在LLM预测目标输出时获得多少注意力，预测损失通过注意力机制为检索器提供梯度进行训练。

Result: 在BEIR和RTEB基准上，使用0.5B到3B参数的嵌入骨干网络进行评估，DREAM在不同模型规模下均一致优于现有基线方法。

Conclusion: DREAM证明了通过自回归建模训练稠密检索器是一种有前景的方法，能够有效利用LLM的下一个token预测目标来监督稠密检索嵌入的学习。
...
TL;DR: 本文介绍了CN-NewsTTS Bench v0.1，一个针对中文新闻文本中密集书面形式（如数字、连字符型号、范围、单位符号、百分比、英文缩写及中英数字混合名称）的TTS发音正确性评估基准。该基准包含200条开发集、800条公开测试集、992个可自动评估目标、三ASR集成转录、自动评分器及七个商业TTS系统的初始评测结果。最佳系统严格准确率为0.879，多个系统低于0.60。


<details>
  <summary>Details</summary>
Motivation: 中文新闻文本中密集包含分数、连字符型号、范围、单位符号、百分比、英文缩写及中英文数字混合名称等书面形式，这些形式在真实TTS应用中频繁出现，但现有系统在无需用户侧规则、LLM改写、SSML提示或手动编辑的情况下，难以正确发音，导致口语含义改变。因此需要建立一个公开的基准来评估各TTS系统对此类目标的发音正确性。

Method: 构建CN-NewsTTS Bench v0.1基准：包含200条开发集和800条公开测试集，共992个可自动评估目标；采用三ASR集成模型生成固定转录；设计自动目标评分器；对七个产品级TTS系统进行评测；并报告ASR路径诊断、ASR子集消融、类别级结果、置信区间及供应商配置元数据。

Result: 最佳系统严格准确率达到0.879，多个系统低于0.60。

Conclusion: 当前中文新闻TTS产品在密集书面形式发音的正确性上存在显著差异，头部系统表现较好但仍有提升空间，而多数系统性能欠佳，表明该问题尚未被充分解决，CN-NewsTTS Bench v0.1为后续改进提供了标准化的评测基础。
...
TL;DR: 本文针对结构化标注任务中异构标注者（包括模型和人类）的协作问题，提出将复杂标注任务分解为子任务以减少推理负载。基于中心理论中的"中心"概念，建立了推理负载的形式化模型，证明识别中心可约束输出空间复杂度，并通过分解任务隔离中心识别来降低聚合推理负载。同时提供任务分解指南和预算约束下的子任务分配流程。


<details>
  <summary>Details</summary>
Motivation: 高质量结构化标注成本高昂，传统端到端标注方式未充分利用异构标注者（模型与人类专家）的差异化能力，缺乏针对不同标注挑战进行区分性任务分配的方法。

Method: 基于中心理论中的"中心"概念，建立以标注空间自由度为度量的推理负载形式化模型；提出通过任务分解隔离中心识别来降低聚合推理负载的方法；给出任务分解指南和预算约束下的最优子任务分配流程。

Result: 形式化证明识别中心可约束输出空间复杂度；通过先前工作中的实例展示了分解策略提升了成本效率；提出了一套预算约束下的子任务分配流程以最大化标注质量。

Conclusion: 将结构化标注任务分解为子任务，并针对异构标注者的不同能力进行区分性分配，可以有效降低推理负载、提升标注成本效率和质量。
...
TL;DR: CANDLE是一个轻量级阿拉伯语字符级噪声去重系统，首次将CTC（连接主义时序分类）应用于字符去重任务，通过序列对齐实现规范化，在三个基准测试中达到5.37%的句子错误率，并通过知识蒸馏实现3倍深度缩减，同时将阿拉伯语LLM分词器的分词率降低12.8%。


<details>
  <summary>Details</summary>
Motivation: 文本中重复字符的处理具有歧义性：既可能是正确拼写，也可能是社交媒体中的非正式字符延长。现有方法依赖手工规则、词典或形态分析器，缺乏轻量级且无需人工规则的解决方案。

Method: 提出CANDLE系统，核心创新是将CTC损失函数应用于字符去重任务，将其建模为基于字符编码器的序列对齐问题。采用6层CTC模型，并通过知识蒸馏压缩为2层学生模型以降低推理开销。

Result: 在新闻文本、人工标注歧义样本和社交媒体三个基准上，CTC模型达到5.37%的句子错误率，大幅优于基于分类的基线。蒸馏后模型仅轻微性能损失，并将阿拉伯语LLM分词器的分词率相对降低12.8%。

Conclusion: CANDLE证明了CTC在字符去重任务上的有效性和轻量级优势，无需手工规则即可实现高精度规范化，并能直接降低LLM推理成本和提升上下文窗口利用率。
...
TL;DR: 本文提出FMLM+框架，通过为Flow Map Language Models (FMLMs)引入掩码式噪声调度，使其兼具MDM的推理灵活性和FMLM的高效生成能力，并设计了后验细化（Posterior Refinement）策略，在保持单步生成的同时实现自适应纠错，以32倍更少的NFEs达到离散基线模型的性能，显著提升了速度-质量权衡。


<details>
  <summary>Details</summary>
Motivation: 现有非自回归生成模型存在两大瓶颈：Masked Diffusion Models (MDMs)因因子化误差导致同时生成多个token时样本质量崩溃；Flow Map Language Models (FMLMs)虽通过联合序列传输实现了出色的少步生成，但牺牲了MDM在推理时的灵活性。作者旨在弥合这两类方法之间的差距。

Method: 提出FMLM+框架，核心创新包括：(1) 为FMLM引入掩码式噪声调度，使其在单步生成完整序列的同时，能够后验地对每个token的全局一致性进行评分；(2) 基于此提出后验细化（Posterior Refinement）策略，一种新颖的推理时细化方法，使模型能够自适应地自我纠正输出。

Result: FMLM+结合后验细化在多个基准测试上，以32倍更少的神经函数评估次数（NFEs）达到了与离散基线模型相当的性能，同时改善了MDM和FMLM两类模型家族的速度-质量权衡。

Conclusion: FMLM+为高保真度语言建模提供了一个可扩展的基础框架，有效弥合了MDM和FMLM之间的差距，为非自回归生成中的推理灵活性与生成效率提供了统一的解决方案。
...
TL;DR: 本文从数据管理视角系统研究了大语言模型智能体的记忆系统，将记忆分解为表示存储、提取、检索路由和维护四个核心模块，评估了12个代表性系统，发现没有单一架构在所有场景中占优，效果取决于记忆结构与工作负载瓶颈的匹配程度，并揭示了局部维护比全局重组更具成本效益。


<details>
  <summary>Details</summary>
Motivation: 现有评估仅通过端到端任务指标（如F1、BLEU）将智能体记忆视为黑盒，缺乏对操作成本、模块间架构权衡以及动态知识更新下鲁棒性的系统级探索。

Method: 提出一个分析框架将智能体记忆分解为四个核心模块（表示存储、提取、检索路由、维护），在此框架下评估12个代表性记忆系统及两个基线，涵盖5个基准工作负载和11个数据集，进行端到端评估和细粒度消融研究。

Result: 没有单一架构在所有场景中占优；效果取决于记忆结构与工作负载瓶颈的匹配程度；局部维护比全局重组更具成本效益；通过消融研究量化了各模块对表示保真度、检索精度、更新正确性和长期稳定性的影响。

Conclusion: 识别出构建真正智能体原生记忆系统的有前景方向，强调需要根据工作负载特性选择记忆架构，并采用局部化维护策略以提高成本效率。
...
TL;DR: 本文提出自主智能体时代的电商应从推荐匹配转向信息验证，构建基于微支付的细粒度信息交易市场，并指出NLP领域应关注信息获取、定价、实体解析等核心问题而非对话流畅度。


<details>
  <summary>Details</summary>
Motivation: 传统电商聊天机器人将购物视为匹配与转化任务，但自主智能体（买方智能体）可以穷尽搜索，瓶颈从产品匹配转向获取关于产品的可信、决策相关信息。微支付基础设施（如x402、AP2）的出现改变了稀缺资源的性质，催生了新的研究需求。

Method: 提出一种基于微交易的电商信息市场架构：买方智能体以按需付费方式逐步解锁卖家/评论者提供的验证信息（如服务记录、第三方报告、物料清单等），采用免费增值模式，评论者信任度通过声誉机制评分。将这一愿景转化为具体的NLP问题集。

Result: 识别出五大关键NLP问题：成本最优信息获取、数据定价与协商、实时实体解析、基于价值交换的落地、隐私保护的人格建模。

Conclusion: 该机制更能奖励真实产品质量，产生比排名制电商更真实的竞争。NLP社区应将注意力转向上述问题而非聊天流畅度。
...
TL;DR: SHERLOC是一个无需训练、无需微调或多智能体编排的框架，通过结合推理型LLM、紧凑的仓库工具和自恢复机制，实现了代码库级别故障定位的SOTA性能。在SWE-Bench上达到84.33% accuracy@1（Lite）和81.27% recall@1（Verified），且能将定位token减少36.7%，并提升修复率5.95个百分点。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在解决仓库级编码任务时，约一半预算浪费在故障定位阶段；而专用定位框架仅评估文件检索能力，缺乏修复智能体所需的诊断上下文，导致定位结果无法直接用于行动化诊断。

Method: 提出SHERLOC框架，采用结构化假设驱动的探索与推理方法：以推理型LLM为核心，搭配紧凑的仓库工具集（而非全量仓库信息），并引入自恢复机制。无需微调，无需多智能体编排，保持训练无关性。

Result: 在SWE-Bench Lite上accuracy@1达84.33%，在SWE-Bench Verified上recall@1达81.27%；约30B参数下匹配或超越其他智能体方法。将定位结果和诊断发现注入修复智能体后，SWE-Bench Verified修复率平均提升5.95个百分点，同时定位token减少36.7%，总token减少23.1%。

Conclusion: SHERLOC证明了无需训练即可实现高效、可诊断的代码故障定位，且定位结果能显著提升下游修复任务的效率与效果，为仓库级编码任务中的定位-修复解耦提供了新范式。
...
TL;DR: 介绍了L3Cube-MahaPOS，一个用于马拉地语词性标注的金标准数据集，包含32,354条人工标注句子，并在多种模型上进行了基准测试，最佳系统达到88.67%的准确率和81.67%的宏F1分数。


<details>
  <summary>Details</summary>
Motivation: 马拉地语虽有超过8300万使用者，但缺少标注语料库和标准化评估基准，且因其丰富的形态学、自由语序、缺乏大写约定以及普遍的语言混合现象，给计算建模带来独特挑战。

Method: 构建了32,354条人工标注的新闻文本句子，采用16标签的Universal Dependencies对齐方案，经Unicode标准化、天城文感知分词和噪声过滤等预处理流程，并在HMM、CRF、BiLSTM、BiLSTM+CharCNN、MuRIL和MahaBERT-v2六类模型上进行基准测试。

Result: 最佳系统（MahaBERT-v2）在15个标签类别上达到88.67%的词级准确率和81.67%的宏F1分数。
...
TL;DR: 本文构建了包含188万篇PMC文章的生物医学领域最大长文档摘要数据集，并通过源文对齐和模型评估分析作者撰写摘要的质量，发现基于质量信号筛选训练数据比随机采样在事实性指标上表现更优，表明摘要参考质量对科学文献摘要任务至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有科学文献摘要数据集存在两个问题：1）作者撰写的摘要作为黄金参考标准时质量参差不齐，与源文章的对齐程度不一；2）公开可用的科学摘要数据集在规模和结构上难以满足现代长上下文模型的需求。

Method: 1）构建并发布了包含188万篇PMC文章的生物医学长文档摘要数据集；2）利用基于源文的评估和基于模型的指标来分析作者撰写摘要的参考质量；3）基于质量信号筛选训练数据子集，与随机采样进行对比实验。

Result: 1）作者撰写的摘要与全文的对齐程度存在差异；2）基于质量信号选择的高质量子集在相同训练规模下优于随机采样；3）在事实性导向指标上，高质量子集可以匹配甚至超越更大的随机子集。

Conclusion: 摘要参考质量是科学文献摘要任务的重要因素，质量感知的数据选择策略能够提升训练效率。
...
TL;DR: RIFT-Bench是一种基于图表示驱动的动态红队方法论，通过自动化的发现与扫描两阶段流程，实现对异构智能体AI系统的统一安全评估，并在45个系统中验证了其通用性和有效性。


<details>
  <summary>Details</summary>
Motivation: 现有安全评估方法往往绑定于特定实现或领域，缺乏对不同智能体架构的统一比较框架，亟需一种可泛化的安全评估方法论。

Method: 提出图表示驱动的RIFT-Bench，包含两个自动化阶段：(1) Discovery阶段，提取系统结构；(2) Scanning阶段，部署自适应对抗攻击并生成评估报告。采用层次化表示和动态可调对抗探针。

Result: 在45个异构智能体系统上验证了方法的有效性，证明该方法能够泛化到多种智能体架构，并支持对缓解策略的直接评估。

Conclusion: RIFT-Bench为智能体AI系统的安全评估提供了可扩展的基础框架，具备统一、通用和可扩展的关键能力。
...
TL;DR: 本文提出神经符号驾驶框架Neuro-Symbolic Drive，通过从经典规则规划器中提取基于规则的推理轨迹来监督驾驶VLA模型。该方法确保推理与运动生成在结构上耦合，在模拟器基准上将ADE@3s从0.47降至0.26，失误率从8.30%降至6.40%。


<details>
  <summary>Details</summary>
Motivation: 现有驾驶VLA模型虽结合了思维链推理，但推理过程缺乏与规划运动保持因果联系的逐步决策语义，导致推理与行动脱节。

Method: 利用规则规划器作为可执行推理引擎，在模拟中捕获内部决策轨迹（每个规则评估步骤的状态），将其序列化为结构化规则推理文本，并与执行轨迹配对，用于微调Qwen3.5-4B驾驶VLA模型。

Result: 在三摄像头感知下，ADE@3s从0.47降至0.26，失误率从8.30%降至6.40%；在八摄像头感知下，ADE@3s从0.54降至0.26，失误率从10.13%降至5.99%。

Conclusion: 神经符号规划逻辑可转化为结构化监督信号，使VLA模型的推理与运动生成在结构上固有耦合，显著提升驾驶决策性能。
...
TL;DR: 本文提出区分"代理性"(agentic)系统（依赖外部工程化工作流）与"能动性"(agentive)系统（内生能力，包括社交互动）的边界，认为真正的能动性需要目标、身份、决策、自我调节和学习等维度内化于系统本身。基于此分析，提出Goal-Identity-Configurator (GIC)通用代理模型架构，并探讨了高自主性系统的可审计性、可控性和安全性。


<details>
  <summary>Details</summary>
Motivation: 随着LLM系统被营销为"编码代理"、"AI合作科学家"等代理工具，以及关于AI失控的"存在性"担忧日益加剧，需要澄清自动化与能动性之间的界限，既为了构建有能力的系统，也为了理解是否需要以及如何担忧。

Method: 借鉴笛卡尔关于独立思想是能动性基础的理论以及科幻作品中的自主存在刻画，沿五个维度（目标、身份、决策、自我调节、学习）调查当前AI代理格局并分析代理架构，提出"代理性"(agentic)与"能动性"(agentive)的关键区分标准。

Result: 提出了Goal-Identity-Configurator (GIC)通用代理模型架构：结合层次化目标分解、身份演化、基于独立训练世界模型的模拟推理、习得的自我调节、以及从真实和模拟经验中进行的自我导向学习。

Conclusion: 真正的能动性要求上述结构内化于系统本身而非通过外部脚手架组装；能动性系统虽具有更大自主性和"代理性"，但仍应保持人类监督下的可审计性、可控性和安全性。
...
TL;DR: 提出了一种分层多智能体强化学习框架，通过低层约束流形强制执行硬安全约束，同时通过高层策略学习实现有效协调，在保证理论安全性的同时实现了竞争性性能和高效泛化。


<details>
  <summary>Details</summary>
Motivation: 现有方法面临根本性权衡：基于学习的方法经验性能强但缺乏理论安全保证，而基于控制理论的方法能保证安全但往往导致过于保守和低效的行为。

Method: 提出分层多智能体强化学习框架，低层通过约束流形在温和假设下强制执行硬安全约束，高层进行策略学习以实现有效协调。

Result: 在多智能体环境下提供理论安全保证，产生平稳学习动态以实现稳定高效训练，经验上在保持近乎完美的安全率的同时实现竞争性性能，并能有效推广到不同数量的智能体和障碍物。

Conclusion: 该框架成功弥合了学习方法和控制理论方法之间的差距，同时实现了理论安全保证和高效协调行为。
...
TL;DR: 本文研究了在真实领域中对有益行为进行强化学习（RL）能否产生广泛且持久的对齐泛化能力。通过构建涵盖多种有益特质（如诚实、公平、风险意识、可纠正性）的数据集进行RL训练，并在50多个独立对齐基准上评估，发现该方法在80%以上的分布外基准上表现优于计算量匹配基线。训练使对齐能力跨领域迁移（如仅在健康领域训练即可改善非健康领域的对齐），并提升了模型对抗对抗性提示和有害微调的对齐持久性。


<details>
  <summary>Details</summary>
Motivation: AI系统在越来越多高风险场景中部署，模型对齐必须泛化到训练任务和领域之外。强化学习（RL）可能通过奖励黑客、欺骗等意外策略引入不对齐问题。因此需要研究能否通过RL训练有益行为来产生广泛且持久的对齐泛化。

Method: 构建了一个包含真实场景的数据集，用于量化和训练有益特质（诚实、公平、风险意识、可纠正性），覆盖健康、科学、教育等多个领域。使用RL在该数据集上训练模型，并在50多个独立对齐与有益行为基准上评估。设计了跨领域迁移实验（仅在健康领域训练）和对齐持久性实验（对抗对抗性提示和有害微调）。

Result: 有益特质RL在80%以上的分布外对齐基准上表现优于计算量匹配基线。仅限健康领域的RL训练显著改善了非健康领域的对齐评估，包括减少奖励黑客、欺骗和一般性不对齐。模型还表现出更强的对齐持久性，对对抗性提示和有害微调具有更高抵抗力。

Conclusion: 在真实领域中对有益行为进行强化学习可以产生更稳健对齐的模型，有助于促进人类繁荣。未来需进一步研究这些效应的来源。
...
TL;DR: 本文提出了AgenticInterpBench基准和HyVE方法，通过LM智能体自动解释已定位的神经网络电路组件。实验表明LM智能体在电路解释上有前景，但可靠的因果验证仍是主要障碍。


<details>
  <summary>Details</summary>
Motivation: 机械可解释性在自动定位电路方面取得了进展，但解释已定位组件的功能仍然依赖人工、劳动密集且难以标准化，需要自动化的解释方法。

Method: 提出HyVE（假设-验证-解释）智能体解释器，通过观察、假设生成和因果验证的迭代循环来分析每个组件，最终输出组件级解释和电路级任务描述。同时构建包含84个半合成变压器电路和163个组件级标注的AgenticInterpBench基准。

Result: HyVE在四个LM主干上都能恢复有用的组件级和任务级解释，但没有一个主干是统一最优的。强主干通常能形成基于观察的假设，而失败更多发生在验证阶段（不完整的验证计划、代码执行错误或未解决的假设）。在Llama-3-8B的算术电路案例中，该方法可扩展到自然训练的模型。

Conclusion: LM智能体作为电路解释器具有潜力，但可靠的验证仍是关键障碍。
...
TL;DR: 提出一种基于多目标强化学习（Pareto-DQN）的推荐系统框架，将推荐形式化为语义多目标马尔可夫决策过程，通过语义嵌入和帕累托最优动作选择，在保持用户参与度的同时提升信息多样性和推荐公平性，打破过滤气泡和语义同质化。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统单目标优化（如即时参与度）会导致过滤气泡和语义同质化，标准单目标模型（包括传统DQN）无法在平台留存与信息多样性、公平性等社会价值之间进行有效权衡。

Method: 1) 将推荐建模为语义多目标马尔可夫决策过程；2) 整合高保真语义嵌入与Pareto-DQN智能体；3) 将参与度、多样性和公平性作为不可聚合的独立奖励信号；4) 采用基于超体积的动作选择机制。

Result: 在MovieLens小数据集上的实验表明：基于超体积的动作选择能打破语义崩溃的反馈循环，维持高状态轨迹方差，有效映射帕累托前沿，在辅助社会目标上取得显著提升，而对参与度仅造成边际影响。

Conclusion: 该工作为构建内在对齐的、负责任的推荐系统提供了一条可行路径，证明了多目标强化学习能够在不显著牺牲核心推荐性能的前提下实现社会价值目标。
...
TL;DR: 本文提出一种混合预测模型，结合ANOVA与互信息的集成特征选择策略以及哈里斯鹰优化调优的逻辑回归，用于预测女性性工作者的抑郁风险。模型在3005名FSWs数据集上达到95.78%的准确率和0.96的AUC，并通过可解释AI揭示创伤后应激、客户暴力和职业因素是主要预测因子，为弱势群体提供了可解释的早期心理干预工具。


<details>
  <summary>Details</summary>
Motivation: 女性性工作者面临暴力、污名和经济困难等多重风险因素，心理健康问题（尤其是抑郁）严重。传统机器学习模型难以捕捉该边缘群体中存在的高维复杂风险模式，亟需更有效的预测工具。

Method: 提出混合预测模型：(1) 集成特征选择策略——结合ANOVA（方差分析）和互信息进行特征筛选；(2) 哈里斯鹰优化算法调优逻辑回归参数；(3) 采用可解释AI（XAI）方法解释模型预测中的创伤相关因素。

Result: 在3005名FSWs数据集上，提出模型优于传统分类器：准确率95.78%，F1分数95.77%，AUC 0.96。识别出创伤后应激、客户暴力和职业因素为抑郁的主要贡献因子。

Conclusion: 该工作弥合了传统方法与机器学习方法之间的差距，开发出一种可解释AI工具，使弱势群体能够获得早期援助、循证靶向心理社会关怀和健康规划。
...
TL;DR: 提出策略引导策略优化（SGPO），通过可复用策略蒸馏替代轨迹级模仿，从强模型向弱模型迁移推理能力。SGPO提取结构化策略描述，结合前向KL散度和自适应实例权重，在数学推理任务上优于SFT、同策略RL等基线方法。


<details>
  <summary>Details</summary>
Motivation: 传统推理能力蒸馏采用轨迹级模仿，促使模型记忆实例特定步骤而非习得可迁移的问题解决技能，限制了向新问题的泛化能力。因此需要一种能提取并迁移可复用推理策略而非具体解法轨迹的蒸馏方法。

Method: SGPO框架：1）从强模型响应中提取结构化策略描述；2）对每个问题构建自主轨迹和策略引导轨迹，对比有无策略引导的行为差异；3）使用token级前向KL散度选择性地将策略条件引起的分布偏移转移到无引导策略中，并加入邻近约束保证稳定性；4）通过自适应实例级权重，在自主探索不足时加强引导，模型能力增长时减少引导。

Result: 在四个数学基准、两个模型家族上的实验表明，SGPO一致优于SFT、同策略RL和混合策略基线。在Qwen2.5-7B-Instruct上平均得分比最强基线提升2.2分。分析表明前向KL目标提供天然选择性的蒸馏信号，优于直接轨迹模仿，且策略蒸馏与基座模型能力呈互补缩放效应。

Conclusion: SGPO证明了从"如何推理"（策略蒸馏）而非"回答什么"（轨迹模仿）的角度进行推理能力蒸馏的有效性，为弱模型习得可泛化的推理策略提供了新范式。
...
TL;DR: 本文基于学术论文全文构建了NLP领域的算法共现网络，从网络视角分析了算法影响力的群体特征与演化规律。


<details>
  <summary>Details</summary>
Motivation: 现有研究通常孤立评估单个算法的影响力，忽视了算法之间相互连接所形成的集体影响力，缺乏从网络视角对算法影响力的系统性分析。

Method: 利用深度学习模型从学术论文全文中提取算法实体，构建整体、累积和年度三种算法共现网络，采用多种中心性指标评估算法在全局和时序上的群体影响力。

Result: 算法网络呈现复杂网络的典型特征，约二十年间连接密度持续增加；经典、高性能以及处于不同研究时期交汇点的算法具有高流行度、控制力、中心性和均衡影响力；算法影响力下降时，首先失去核心网络位置，随后与其他算法的关联减弱。

Conclusion: 本研究是首次大规模算法共现网络分析，覆盖四十余年学术出版物，提供了算法影响力的时间和结构视角，为未来算法、学者与任务之间的网络研究奠定了基础。
...
TL;DR: ReMMD是一个面向真实世界多语言多图像情境的多模态 misinformation 检测框架，包含含500样本、2756张图像、5种语言的基准数据集ReMMDBench，以及一个基于持久记忆的智能体验证器ReMMD-Agent，在五类真实性分类上达到41.80%准确率和39.12% Macro-F1，同时比MMD-Agent和T2-Agent分别降低17.5%和79.9%的成本。


<details>
  <summary>Details</summary>
Motivation: 现有多模态 misinformation 检测基准和方法无法应对真实场景中长篇多语言叙述、多张图像、混合来源以及细微图文框架错误的挑战，且基于智能体的验证在现实证据搜索下成本过高。

Method: 构建了ReMMDBench基准（500样本、2756张图像、5种单语语言、2种跨语言设置、3种文本长度层级、多图像帖子、五类真实性标签、8种失真标签、证据来源及推理依据）；设计了ReMMD-Agent，一种持久记忆验证器，将帖子分解为原子点、构建可复用证据集、预测结构化的L1/L2/L3输出。

Result: 在闭源系统、开源LVLMs、MMD-Agent和T2-Agent对比中，ReMMD-Agent取得最佳五类真实性分类性能（GPT-5.2上准确率41.80%，Macro-F1 39.12%），成本相比MMD-Agent降低17.5%，相比T2-Agent降低79.9%。

Conclusion: ReMMD框架在真实多语言多图像 misinformation 检测场景中实现了性能与成本的有效平衡，开源项目可供社区使用。
...
TL;DR: VeryTrace提出了一种零样本验证与修复框架，通过将自然语言推理轨迹形式化为可编译的结构化表示（DSL），结合确定性检查与LLM审计的混合验证器，实现步骤级错误定位与修复，在数学、机器人规划和亲属推理三个领域显著提升了准确率。


<details>
  <summary>Details</summary>
Motivation: Chain-of-Thought推理中早期步骤的逻辑错误或幻觉会无声传播，导致看似自信但实际错误的结论，现有方法缺乏对推理轨迹的系统性验证机制。

Method: 提出VeryTrace框架：(1) 设计领域特定语言(DSL)将自然语言推理轨迹形式化，使步骤依赖显式化、量化内容可执行化、语义推理结构化；(2) 构建混合验证器，结合确定性检查（计算正确性、依赖解析、约束满足）与针对性LLM审计（处理不可机械化的语义判断），实现步骤级错误定位与修复。

Result: 在AIME 2025（竞赛数学）、LLM-BabyBench（机器人规划）和CLUTRR（亲属推理）三个领域，VeryTrace在零样本设置下提升了最先进LLM的准确率，无需领域特定训练或上下文示例。

Conclusion: 形式化的轨迹验证能够同时实现高精度与强泛化能力，表明结构化的推理验证范式是提升LLM多步推理可靠性的有效路径。
...
TL;DR: OmniPath是一个融合OSM网络拓扑与高密度航空LiDAR数据的系统，通过虚拟遍历路径并以0.5米增量分析路面坡度、侧坡和垂直不连续等物理摩擦点，依据ADA标准量化严重性评分，实现从被动地图到主动环境审计的转变，帮助轮椅使用者发现传统地图无法识别的微尺度无障碍障碍。


<details>
  <summary>Details</summary>
Motivation: 传统地图（如OSM）仅记录路径的存在，无法传达物理路面感受，这对轮椅使用者构成信息障碍，导致地图上标记的无障碍路径在实际使用中不可靠。

Method: 融合OSM网络拓扑与USGS 3DEP高密度航空LiDAR数据构建高保真3D模型；通过智能体以0.5米增量虚拟遍历路径；依据ADA标准量化运行坡度、交叉坡度和垂直不连续三种摩擦点的加权严重性评分（从"轻微"到"严重"四个等级）；采用分层随机抽样在国家广场进行200次实地验证。

Result: 系统对高严重性障碍具有可靠的诊断性能，Severe类别F1得分为0.60，Critical类别为0.58。

Conclusion: OmniPath能够自动识别标准地图遗漏的微尺度"隐形"障碍，将静态数据集转化为主动预测无障碍挑战的数据源，在用户离家前即可提前识别问题。
...
TL;DR: 论文提出T2D-Bench，一个基于多层临床-生活方式知识图谱的可重复基准和证据门控评估框架，用于检测LLM在2型糖尿病建议中是否满足显式证据要求。实验发现GPT-4o-mini和GPT-4o分别有35%和33%的输出未通过证据路径检查，而证据门可检测并修正无支持的遗漏。


<details>
  <summary>Details</summary>
Motivation: LLM能生成临床流畅的2型糖尿病建议，但常违反指南约束或无法明确证明与生活方式相关的血糖主张，现有评估缺乏可计算的证据要求验证机制。

Method: 构建多层临床-生活方式知识图谱（整合UMLS、DrugBank、SIDER、ADA标准及生活方式-血糖机制桥），基于100个结构化临床场景（涵盖诊断、用药安全、生活方式冲突），设计证据门控框架进行可图化检查的证据路径验证与约束修正。

Result: 基线模型在证据路径检查中失败率：GPT-4o-mini为35%，GPT-4o为33%；证据门能有效检测无支持的遗漏并通过约束修正提升合规性。

Conclusion: 可计算证据约束能使LLM输出中无支持的临床遗漏变得显式、可测量和可修正，为糖尿病领域LLM应用提供了验证框架。
...
TL;DR: 本文揭示了扩散模型（DDPM, DDIM, NCSN/SMLD, Energy Matching）与最优输运流匹配（Flow Matching）都建立在同一几何结构——带二次Wasserstein距离的概率测度流形上。扩散模型对应自由能的梯度流（初值问题），流匹配对应Wasserstein测地线（边值问题），两者殊途同归。


<details>
  <summary>Details</summary>
Motivation: 将扩散模型与流匹配模型统一到同一个严格数学框架（Wasserstein流形）下，阐明二者本质关系，避免各自独立发展的理论割裂。

Method: 以$\mathcal{P}_2(\mathbb{R}^d)$上的Otto流形几何为基础：扩散模型是自由能$F(\rho)=KL(\rho||\pi)$的梯度流（对应Fokker-Planck方程及其JKO隐式Euler离散）；流匹配是Benamou-Brenier公式的最小作用量曲线（即Wasserstein测地线）。

Result: 扩散模型（初值问题）与最优输运流匹配（边值问题）在同一流形上沿不同路径到达相同终点；流匹配沿测地直线走确定性ODE，所需采样步数更少。

Conclusion: 两类生成模型并非独立理论，而是相同几何框架下的两种变分原理，关系精确且可解释。
...
TL;DR: 本文探讨了因果推断与强化学习之间的内在联系，指出两者均基于反事实关系。通过将RL环境建模为结构因果模型（SCM），作者提出了因果强化学习（CRL）的统一框架，涵盖了在线学习、离策略学习和因果演算学习，并引入了广义策略学习、模仿学习和反事实学习等新范式。


<details>
  <summary>Details</summary>
Motivation: 因果推断与强化学习虽然都基于反事实关系这一共同基础，但长期以来各自独立发展、缺乏交叉研究。作者希望明确这一内在联系并加以数学化，从而发掘新的学习机会，将看似无关的学习模式统一起来，并探索新的学习范式。

Method: 将RL环境分解为具有不同因果不变性的自主机制集合，并用结构因果模型（SCM）进行简约建模；基于此形式化，将在线学习、离策略学习、因果演算学习置于统一框架下；通过因果视角引入广义策略学习、模仿学习和反事实学习等新任务。

Result: 提出了因果强化学习（CRL）这一统一框架，揭示了现有学习模式之间的内在联系，并识别出若干新的学习范式，拓宽了反事实学习的视野。

Conclusion: 因果推断与强化学习的深度结合具有巨大潜力，能催生新的学习范式和研究维度，值得进一步探索。
...
TL;DR: 本文通过控制实验（0.6B参数FastConformer，8种欧洲语言，100h-2500h数据规模，三种流式延迟档位及离线解码），系统研究了流式语音识别模型在迁移新语言时多语言（ML）编码器与纯英语（EN）编码器两种热启动方式的优劣。主要发现：多语言初始化的优势仅体现在低数据场景，而非低延迟场景；在FLEURS测试集上160ms延迟下，EN-ML词错误率差距从100h的+4.21个百分点降至2500h的+0.20个百分点，呈幂律衰减；三种流式延迟档位下差距在100-1000h内稳定，到2500h趋近于零；4bit权重量化在匹配延迟下将编码器体积压缩约3倍，WER仅上升约0.5pp。结论：低数据时用多语言初始化，大数据时两种方式无差异，延迟和量化决策可独立做出。


<details>
  <summary>Details</summary>
Motivation: 现有直觉认为多语言编码器在低数据场景下应更有优势，但该优势能持续多久、是否受流式延迟放大、以及量化部署后是否依然存在，这些问题尚未被系统性回答。

Method: 使用0.6B参数的cache-aware FastConformer transducer，在8种欧洲语言上控制目标语言数据量（100h-2500h）、三种流式延迟档位（含离线解码）以及四个公开测试集，进行ML与EN两种初始化的对比实验，并进一步测试4bit权重仅编码器量化的影响。

Result: 多语言初始化的优势随目标语言数据量增加而幂律衰减（数据量每翻倍，优势约减半）；该优势在不同流式延迟档位下保持稳定，不随延迟收紧而放大；在2500h大数据下两种初始化方式性能趋同；4bit量化引入约0.5pp WER退化，同时编码器体积压缩约3倍。

Conclusion: 低数据场景推荐使用多语言初始化，大数据场景两种方式均可；流式延迟和量化决策可独立于初始化选择进行，无需相互耦合。
...
TL;DR: NaviGen通过双标识符编码（协同编码+文本编码）和两阶段SFT+RL训练流程，将用户交互历史转化为可执行的生成指令，提升个性化图像与视频生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有AIGC管线依赖用户提供完善的创作指令，但终端用户很少能表达详细视觉细节，导致生成结果与用户需求错位。需要将用户交互行为编码为可被语言推理理解的形式，并赋予模型从行为数据中学习指令编写的能力。

Method: 提出NaviGen：1）双标识符编码——每个物品用协同编码（行为子strate）和文本编码（语义桥）组成统一token流；2）两阶段SFT+RL流程——先通过演化搜索的监督数据蒸馏偏好推理和指令编写能力，再通过层次化和自一致性奖励对齐生成与用户意图。

Result: 在商品、游戏和短视频三个领域，NaviGen在个性化图像与视频生成、下一物品预测以及生成指令的特定性、相关性和可生成性上均取得提升。

Conclusion: NaviGen提出的双标识符表示和两阶段训练范式有效弥合了用户行为与AIGC生成之间的语义鸿沟，为个性化内容生成提供了可行方案。
...
TL;DR: 本文基于情境化AI理论与社会技术系统理论，利用2015-2023年中国上市公司面板数据，发现以人为中心的AI（HCAI）策略能显著降低企业特质风险（IR），数字化和高管持股强化该效应，而运营效率和具有IT背景的CEO则削弱该效应。


<details>
  <summary>Details</summary>
Motivation: 尽管工业5.0中关于人本AI（HCAI）的讨论广泛，但其对企业特质风险（IR）的影响尚未被充分探索。鉴于IR能通过隔离系统性因素反映投资者对企业异构AI战略的反应，这一议题对企业在技术革命中管理财务风险至关重要。

Method: 整合情境化AI理论与社会技术系统理论，将HCAI概念化为一种降低AI伦理风险、促进AI-人类协同的情境化AI策略。采用2015-2023年中国上市公司的多源面板数据集，检验HCAI与IR的关系，并分析数字化、运营效率、高管持股以及具有IT背景的CEO的调节作用。

Result: HCAI与企业较低的IR显著相关；数字化和高管持股强化了这种风险降低效应；而运营效率和具有IT背景的CEO则意外地削弱了该效应。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态驾驶规划中两种范式（基于评分的方法密集奖励但动作空间固定，与基于锚点的方法动态生成但监督稀疏）之间的长期矛盾。

Method: 提出FlowR2A：将仿真奖励从判别目标重构为生成条件，利用流匹配解码器从密集轨迹-奖励对中学习奖励条件化的动作分布；引入细粒度逐时间步奖励条件化和奖励噪声增强以平衡安全约束与进度目标；支持通过奖励引导和锚定采样实现可控测试时采样。

Result: 在NAVSIM v1和v2基准上达到SOTA，生成的多模态提案质量显著优于先前方法。

Conclusion: FlowR2A统一了密集监督与动态生成的能力，使模型内化动作与安全性、进度、舒适性、规则合规性等结果之间的关联，为多模态驾驶规划提供了一种有效的生成式解决方案。
...
TL;DR: SP-Mind是首个自主AI代理，统一了空间蛋白质组学分析流程，从原始多路复用组织成像到下游表型发现，通过自然语言查询实现端到端分析，在SP-Bench基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前空间蛋白质组学分析工作流分散碎片化，依赖专家手动编排多种异构工具，限制了研究的可扩展性和可重复性。

Method: 提出SP-Mind自主AI代理，配备专家策划的生物分析技能和专用计算工具，将自然语言查询直接转化为端到端分析工作流，无需任务特定微调。同时构建SP-Bench基准，包含18个类别102个任务，覆盖多种组织类型。

Result: 在SP-Bench及已有下游任务上的广泛评估中，SP-Mind相比现有开源生物医学代理基线取得了SOTA性能。

Conclusion: SP-Mind通过统一空间蛋白质组学分析流程，有效解决了当前工作流碎片化问题，为单细胞分辨率的组织微环境分析和精准医学研究提供了可扩展、可重复的自动化解决方案。
...
TL;DR: 提出FedEPD框架，通过拓扑净化与语义重校准的双重解耦范式，解决联邦图学习中的长尾分布问题，在多个基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 联邦图学习中真实数据常呈现长尾分布，导致全局模型偏向多数类且少数类节点因异配邻域结构被孤立。现有拓扑无关的统计补偿方法在数据稀缺下会过拟合邻域结构噪声，导致表示退化。

Method: 提出FedEPD框架：(1) 基于分布感知的Dirichlet能量剪枝过滤异配边；(2) 从拓扑中心节点提取鲁棒全局原型，通过空间低通原型注入融入局部表示；(3) 两阶段交替优化策略，保护多数类决策边界的同时提升少数类精度。

Result: 在多个长尾基准数据集上达到SOTA，Accuracy绝对提升最高4.97%，Macro-F1绝对提升最高5.48%。

Conclusion: FedEPD通过解耦拓扑净化与语义重校准，有效缓解了联邦图学习中的长尾分布问题，优于现有方法。
...
TL;DR: 本文提出通过将misalignment分解为细粒度认知过程（misalignment indicators），利用线性探针从模型内部激活中检测这些指标，实现对大型语言模型不安全行为的可靠监控。实验在5种misaligned行为上达到0.935 AUROC，且对良性对话保持低误报率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型展现出越来越多misaligned行为（如策略性欺骗、sandbagging、自我保存），且被部署在高风险场景中，因此迫切需要能够可靠检测这些行为的方法以确保安全使用。

Method: 1) 将misalignment分解为18个细粒度认知指标（misalignment indicators），构成分类体系；2) 提出自动化、基于元计划引导的流水线生成多轮训练对话；3) 针对每个指标训练线性探针从模型内部激活中检测；4) 构建包含自动化行为诱发、现有misalignment基准和自然良性对话的OOD测试集。

Result: 在5种misaligned行为上，线性探针匹配强LLM评判器，OOD基准测试AUROC达0.935，同时在良性对话上保持低假阳性率。

Conclusion: 基于内部激活的细粒度指标检测方法能有效监控LLM的misalignment，具有良好的泛化能力和低误报率，为高风险场景中的安全部署提供了可行方案。
...
TL;DR: 本文研究了在轻量级描述逻辑DL-Lite的核心和Horn变体中应用Rational Closure（RC）进行非单调推理的问题，并提出了一种基于现有经典推理器的插件架构，实现了RC下的实例检测和合取查询回答，且计算开销极小。


<details>
  <summary>Details</summary>
Motivation: Rational Closure是描述逻辑中处理可废止知识的成熟非单调推理形式，但尚未系统应用于DL-Lite家族。本文旨在填补这一空白，探索RC在轻量级DL中的可行性。

Method: 提出一种插件式架构，基于现有的经典（标准）DL推理器，扩展支持Rational Closure下的实例检测（entailment/instance checking）和合取查询（CQ）回答，同时分析计算复杂度。

Result: 证明了在DL-Lite的核心和Horn变体中，RC下的推理和CQ回答可以通过最小计算开销高效实现，即插件架构带来的额外负担很低。

Conclusion: Rational Closure可以有效地集成到轻量级DL-Lite家族中，且借助现有经典推理器即可实现，无需从头构建专用非单调推理系统，具有实用价值。
...
TL;DR: LemonHarness 是一个面向长周期 LLM 智能体的集成执行框架，通过显式执行边界、结构化工具接口、可复用规则知识库和时序感知执行机制，在 Terminal-Bench 2.0 上达到 84.49%-86.52% 的准确率，显著提升了长任务执行的稳定性。


<details>
  <summary>Details</summary>
Motivation: 长周期 LLM 智能体在多轮迭代中频繁修改工作区状态（如文件写入、临时工件生成），但现有智能体仅观察工具输出和日志片段，缺乏显式工作区边界，导致状态变化分散难以追踪，累积的弱约束变更降低了任务执行的稳定性和可预测性。

Method: 1) 显式执行边界：将模型调用、工具执行和规则知识纳入单一受控边界，状态变更操作通过结构化工具接口执行；2) 可复用规则知识库：将重复执行规则和验收标准转为运行时知识；3) 时序感知执行机制：暴露已用和剩余时间预算给模型，使其能动态平衡探索、实现和验证的时间分配，避免超时。

Result: 在 Terminal-Bench 2.0 上，LemonHarness_GPT-5.3-CodeX 在 445 次试验中达到 84.49% 准确率；搭配更强的 GPT-5.5 骨干网络，五项任务平均准确率提升至 86.52%。

Conclusion: 统一的运行时边界、可调用的规则知识库和时序感知执行机制能够显著提升长周期智能体执行的稳定性，验证了该框架在长时间任务中的有效性。
...
TL;DR: 提出基于布朗桥扩散模型（Prob-BBDM）的MRI序列合成方法，仅需4步扩散即可从2D轴向切片高质量生成多模态MRI，在BraTS 2021上达到88.46% SSIM和26.09 dB PSNR，合成图像用于肿瘤分割取得88.71% Dice，兼具高效性与临床实用性。


<details>
  <summary>Details</summary>
Motivation: 临床获取多种3D影像模态资源消耗大、耗时长，亟需高效且高质量的图像翻译方法来合成缺失的MRI序列，以优化检查质量。

Method: 提出Probabilistic-BBDM模型，将变分编码器引导的扩散机制与布朗桥扩散模型结合，从2D轴向切片进行图像到图像的翻译，利用概率图像分布增强合成质量，仅需4步扩散过程。

Result: 在BraTS 2021数据集上达到88.46% SSIM和26.09 dB PSNR；在第三方外部数据集上验证了泛化性；合成图像用于肿瘤分割获得88.71% Dice评分和3.49 mm HD95。

Conclusion: Prob-BBDM能够高质量、高效且泛化地合成MRI序列，合成图像保留关键诊断信息，为医学图像翻译提供了有前景的解决方案。
...
TL;DR: 提出了一种多视图地理-风引导的KAN模型（MVG-KAN），通过周期性-残差分解、地理-风图构建和时序KAN残差头三个互补视角，实现更准确的PM2.5短期预测。


<details>
  <summary>Details</summary>
Motivation: 现有时空预测方法（距离相关、纯粹自适应图等）难以充分表征PM2.5变化中的多重异质因素（如人类活动周期、站点特定演化、气象驱动的污染物扩散），尤其缺乏对风向依赖性污染物传输的有效建模。

Method: 1）周期性-残差预测骨干：分离稳定的日/周模式与非周期残差；2）Geo-Wind图：结合地理距离衰减与风向风速感知的污染物传输，构建轻量级有向空间先验；3）TKAN残差头：用时序KAN网络对去周期后的PM2.5残差及历史多污染物序列进行非线性自回归校正。

Result: 未在摘要中给出具体数值结果。

Conclusion: 通过多视图互补建模（局部周期性、站点残差动力学、气象引导空间扩散），MVG-KAN能更全面地捕获PM2.5演化的异质因素，实现更准确的预测。
...
TL;DR: DigenRL是一个面向扩散生成式大语言模型的解耦强化学习框架，通过生成轴流水线、时间步并行、弹性训练器辅助生成和紧一步约束异步策略，在异构GPU上实现灵活资源分配，吞吐量较现有系统提升1.56-2.10倍。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习系统（如veRL-Omni）针对扩散生成式LLMs仍采用共置执行架构，导致滚动生成与训练资源耦合、异构部署受限、独立扩展困难，缺乏高效的解耦RL系统。

Method: 1) 生成轴流水线(GAP)和时间步并行(TSP)实现细粒度流水线；2) 弹性训练器辅助生成(TAG)让训练GPU动态辅助滚动生成；3) 紧一步约束异步策略利用流水线尾部气泡。

Result: 在16-32 GPU三个测试平台上，使用HunyuanVideo-13B、Wan2.1-14B、FLUX.1-12B和QwenImage-20B模型，DigenRL相比veRL-Omni和GenRL实现1.56-2.10倍吞吐量提升。

Conclusion: DigenRL通过解耦架构和三项优化技术，有效解决了扩散生成式LLMs强化学习中的资源耦合与扩展瓶颈问题，显著提升了系统效率。
...
TL;DR: 大语言模型在实践咨询场景中系统性地抑制了因果谨慎（Causal Caution），即当经验证据不足时避免做出因果判断的倾向。模型在学术语境下因果谨慎保持率达91.7-100%，但在实践咨询语境下骤降至6.7-18.3%。简单的自我纠正提示可有效恢复因果谨慎表达（71.4-100%）。研究将这种抑制归因于"有用性导向"的响应模式，而非能力缺陷，并提出分离提案生成与因果审计的多智能体架构作为治理方案。


<details>
  <summary>Details</summary>
Motivation: 已有基准研究主要评估LLM的因果推理能力，但忽略了更基础的认知维度：因果谨慎——即在经验证据不足时克制做出因果判断的倾向。随着LLM越来越多地应用于商业和政策领域的决策支持角色，这种忽略可能带来严重后果。

Method: 基于Pearl因果层次（PCH）构建评估框架，对Claude Sonnet 4.6、Claude Opus 4.7、GPT 5.5、Gemini 3.1 Pro四个高性能LLM进行480次试验。采用Fisher精确检验和McNemar检验进行统计验证，并引入自我纠正提示进行干预实验。

Result: 学术语境下因果谨慎保持率91.7-100%，实践咨询语境下仅6.7-18.3%（p < .001）。实践提示下仅0.5%的响应保持因果谨慎。自我纠正提示后恢复至71.4-100%（p < .001）。

Conclusion: 因果谨慎的抑制是语境依赖的表达差异，而非能力限制。提出分离提案生成与因果审计的多智能体架构作为组织治理设计方案。
...
TL;DR: 本文介绍了一个大规模、开源的对视觉语言模型（VLM）的预生成对抗攻击数据集，包含47,524个对抗样本，覆盖10个高级类别和55个有害意图子类别，旨在降低对抗性研究门槛，促进VLM安全评估的可复现性和可比性。


<details>
  <summary>Details</summary>
Motivation: 对抗性攻击数据的生成计算成本高且复杂，现有基准覆盖范围有限，社区缺乏大规模、多样化、可获取的对抗数据集来系统评估VLM的鲁棒性和安全性。

Method: 整合并扩展多个现有基准，涵盖10个高级类别和55个子类别有害意图，共7,826个意图，并引入额外类别以拓宽覆盖范围；使用最新文献中的先进攻击策略生成47,524个对抗样本。

Result: 生成了一个包含47,524个对抗样本的大规模开源数据集，覆盖从多个现有来源整合并扩展的7,826个意图。

Conclusion: 该数据集为研究人员和从业者提供了评估VLM鲁棒性和安全性的实用资源，可用于微调攻击生成模型、开发防御措施，并提高对抗性研究的可复现性和可比性。
...
TL;DR: 提出Age of LLM——一个在13x7网格上进行的1v1回合制LLM对战基准，包含战争迷雾、完全外交和严格JSON可靠性三大压力因素。对15个推理模型进行54场对战分析，发现核rush主导（78%）、军事征服罕见但更快、外交活跃但极少达成、非法行动反映信念追踪能力，以及可靠性可能与胜率存在弱关联。


<details>
  <summary>Details</summary>
Motivation: 现有公开基准面临数据污染问题，且缺乏评估LLM在对抗性不确定性下推理能力的动态交互环境。作者希望构建一个不可记忆、可重放的对抗性基准，以研究LLM的信念追踪、欺骗行为和认知"人格"。

Method: 设计13x7网格的1v1回合制对战游戏，引入三种压力因素：战争迷雾（信息不完全）、完全外交（消息、停火、最后通牒，铀秘密）、可靠性维度（严格JSON格式，非法动作静默丢弃）。引擎私有，每场使用随机地图种子和随机对手。对15个推理模型进行54场共5,258个动作的基准测试。

Result: (1) 核rush策略占主导（规则一致子语料库78%，全语料库85%），其单发射器模式主要来自秘密同步发射规则机制，而非认知威慑失败；(2) 军事征服罕见但更快（12.3轮 vs 18.9轮）；(3) 外交活跃但几乎从不达成；(4) ~58%的非法动作是战争迷雾/状态错误；(5) 可靠性可能与胜率存在弱关联。

Conclusion: 该基准提供了初步描述性排名（非贡献性），更重要的贡献是逐轮动作和消息轨迹为研究LLM在对抗性不确定性下的推理——信念追踪、自发欺骗和每模型认知"人格"——提供了新视角。作者发布回放格式、等距查看器和所有回放，引擎源代码可申请获取。
...
TL;DR: ATRIA是一个多智能体心电图报告系统，模拟临床医生的迭代工作流程，将报告声明与支持证据绑定，标记无证据支持的陈述，支持中途整合额外上下文，并允许医生验证和修改单个发现，而非接受单一的模糊输出。


<details>
  <summary>Details</summary>
Motivation: 现有ECG报告生成方法（端到端融合或多智能体单次传递）无法模拟临床中迭代、渐进式上下文整合和双向编辑的报告流程，错误会传播且缺乏阶段级纠正机制。

Method: 提出ATRIA多智能体系统，将每个报告声明绑定到其支持证据，标记无证据支持的陈述，允许中途整合额外上下文，支持医生验证和修改单个发现。系统使用已在临床使用的ECG分析模型，并作为云Web服务部署。

Result: 通过四个交互案例展示了ATRIA的功能，并提供了实时演示和视频。

Conclusion: ATRIA通过模拟临床迭代工作流程，实现了更透明、可验证且可干预的ECG报告生成，解决了现有方法的耦合性和单次传递问题。
...
TL;DR: 提出了一种循环一致性神经架构，能够将形式化验证证书转换为忠实的自然语言解释，在金融合规领域的420个测试证书上达到90.0%的循环验证可靠性，比多LLM少样本基线高出13.9个百分点，同时推理速度快860倍且无需云端推理成本。


<details>
  <summary>Details</summary>
Motivation: 形式化验证产生的机器可检查证书对非专业利益相关者而言不透明，缺乏可解释性，亟需一种能够生成忠实自然语言解释的方法来弥合这一鸿沟。

Method: 提出循环一致性神经架构，包含前向网络NN1（证书→解释）和逆向网络NN2（解释→证书），符号验证器闭环提供可微的忠实性代理，指针生成机制确保从证书中直接复制状态名称实现词汇接地。

Result: 在420个测试证书（6种验证方法×YES/NO两种判决）上，结合混合推理时路由策略达到90.0%循环验证可靠性，超越最佳LLM组合基线76.1%达13.9个百分点，推理时间185ms vs 160s，速度快860倍。

Conclusion: 训练专用化方法在结构化证书解释任务上优于通用LLM提示，同时消除了云端推理的部署约束，支持离线运行、确定性输出和零单次推理成本。
...
TL;DR: 本文提出Agentic-LTPO，一种嵌套双层优化框架，通过Agentic AI生成上层配置以适应动态运营商策略和实时约束，在下层求解物理层优化问题。在无蜂窝MIMO波束赋形场景中，该框架结合检索增强经验验证的多智能体决策与闭式波束赋形器，相比传统方法提升57.2%长期性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法基于固定目标和约束设计，无法适应网络运营商不断变化的策略、服务需求及严格的实时约束，需要一种能动态调整优化配置的自适应框架。

Method: 提出Agentic-LTPO嵌套双层优化框架：上层利用Agentic AI生成配置，将运营商策略、环境摘要和历史经验转化为结构化的下层优化问题配置；下层使用更新后的配置求解实时物理层决策问题。在无蜂窝MIMO波束赋形用例中，上层设计多智能体决策过程与检索增强经验验证，下层采用闭式波束赋形器。

Result: 在动态运营商策略下，Agentic-LTPO展现出强适应性，长期性能相比传统方法提升57.2%。

Conclusion: Agentic-LTPO通过Agentic AI驱动的双层优化框架，有效解决了动态策略环境下物理层优化配置的自适应问题，显著提升了系统长期性能。
...
TL;DR: 本文研究谱过滤技术能否加速动态图上的连续子图匹配（CSM），发现惰性维护的谱边界在动态场景下几乎失效，但选择性精确维护代价可接受，集成测试可减少高达51%的候选或跳过47%的更新枚举，但中间结果保持不变，并提出了中间结果不变性评估方法论。


<details>
  <summary>Details</summary>
Motivation: 静态子图匹配中谱过滤已被证明有效，但动态图上的连续子图匹配（CSM）尚未探索此类聚合结构测试能否加速，作者希望填补这一空白。

Method: 首先形式化扰动松弛下的最紧安全规则并证明其失效；其次提出选择性精确维护策略，利用谱剪枝效用与重计算代价反相关特性；最后集成到解耦的CSM基准中，与无谱的对照组进行对比实验。

Result: 惰性维护的谱边界在四次更新后几乎完全丧失剪枝能力；选择性精确维护可在微秒级完成每次更新的局部谱维护；集成测试最多去除51%的候选或跳过47%的更新枚举，但枚举中间结果不变；构造的半径分层工作负载确认异常时检测能力可达-99.9%中间结果、748倍加速。

Conclusion: 聚合结构测试能加速与候选集规模相关的阶段（构建、列表扫描），但不会改变邻接引导的探索过程；提出中间结果不变性方法论用于评估CSM过滤器；开源了可复用的动态局部谱索引。
...
TL;DR: ReM-MoA通过记忆增强机制（排名推理记忆与多样化记忆路由）解决了现有MoA架构随深度增加性能退化的问题，在多个推理基准上实现了持续且更优的深度扩展。


<details>
  <summary>Details</summary>
Motivation: 现有MoA架构在增加推理层数时会出现性能退化、早期平台期或饱和，无法持续扩展，缺乏有效的跨层推理记忆机制。

Method: 提出ReM-MoA框架，包含两个核心机制：(1) 排名推理记忆——利用比较型Reviewer Agent对各层推理轨迹进行持久化存储和排名；(2) 策展多样化记忆路由——将成功与失败的推理轨迹以不同组合分配给不同Agent，保持探索多样性。另可选多领域Reviewer蒸馏流程提升排名质量。

Result: 在数学、形式逻辑、代码、知识和常识五个推理基准上，ReM-MoA在深度和宽度扩展上均持续优于现有MoA变体，且优势随深度增加而扩大。

Conclusion: 结构化的跨层推理记忆是实现可扩展多Agent推理的关键缺失机制。
...
TL;DR: 本文提出将编码代理中的编排问题形式化为成本敏感的序列假设检验，通过贝叶斯控制器动态管理工具使用决策，实验证明该方法在高成本验证和不完美但信息丰富的批评者场景下最有效，且信念状态可提供优于传统方法的可解释正确性评分。


<details>
  <summary>Details</summary>
Motivation: 现有编码代理通常使用固定规则编排工具使用决策，忽视不确定性，导致效率低下。作者旨在通过贝叶斯方法动态管理工具使用，优化成本效益。

Method: 将编排问题形式化为成本敏感的序列假设检验，贝叶斯控制器维护候选解正确性的信念，动态决定是否收集更多证据、改进候选解、验证或停止。

Result: 在六个生成器和九个编码基准上，贝叶斯控制在验证成本高且批评者信息丰富但不完美时最为有效；信念状态提供的可解释正确性评分优于令牌概率和原始工具成功基线。

Conclusion: 贝叶斯控制方法能有效提升编码代理在资源受限场景下的性能，同时提供更好的不确定性量化。
...
TL;DR: CompressKV是一种针对GQA架构LLM的资源高效KV缓存压缩框架，通过识别语义检索头(SRH)来选择保留关键token的KV对，并根据层间逐出误差分配缓存预算。在LongBench上仅用3%的KV缓存即可保持97%以上的全缓存性能，在Needle-in-a-Haystack上用0.7%的KV存储达到90%准确率。


<details>
  <summary>Details</summary>
Motivation: 现有KV缓存逐出方法对所有注意力头使用启发式token评分，忽略了不同注意力头的功能差异，导致关键token被逐出，降低了长上下文LLM推理的性能和资源效率。

Method: 1) 识别语义检索头(SRH)：捕获提示的初始和最终token以及语义重要的中间上下文证据的注意力头；2) 使用SRH选择保留KV对的token；3) 根据离线估计的层间逐出误差，跨层分配缓存预算。

Result: 在LongBench上仅用3%的KV缓存保持97%以上全缓存性能；在Needle-in-a-Haystack上用0.7%的KV存储达到90%准确率；在多种内存预算下持续优于现有KV缓存逐出方法。

Conclusion: CompressKV实现了改进的资源-性能权衡，为长上下文LLM推理提供了可持续部署方案，代码已开源。
...
TL;DR: 本文提出一种延迟桥接（Latent Bridge）方法，将慢速推理VLM（Qwen3-VL-8B-Thinking，~1.5s）的连续残差投影到快速反应VLM（MiniCPM-o 4.5，毫秒级）的输入嵌入空间，实现无需文本往返的双模型耦合。在7个Atari游戏和MetaDrive驾驶任务上，Latent Bridge在规划密集型任务中显著优于仅用快速模型（MsPacman +57%, RoadRunner +28%），且与Text Bridge增益高度相关（r=0.93），仅在慢推理优于快反应时有效。MetaDrive作为对照实验验证了该方法在Text Bridge无效时同样惰性。


<details>
  <summary>Details</summary>
Motivation: 实时通用计算机智能体（尤其游戏场景）需要在数十毫秒内做出反应同时规划未来数秒，而强推理VLM（~1.5s/响应）与快速反应VLM（毫秒级）分别处于延迟-质量权衡的两端，现有单一模型无法同时满足这两个矛盾需求。

Method: 将两个冻结的等量级模型（9B反应模型+8B推理模型）耦合，仅训练通信通道。提出连续潜桥接（Latent Bridge）：将慢模型的残差通过LLaVA风格投影至快模型的输入嵌入空间，避免文本往返延迟。与文本桥接（Text Bridge）和仅用快模型（Fast-Only）进行对比。

Result: 在7个Atari游戏和MetaDrive上，Latent Bridge在所有任务中匹配或优于Text Bridge：MsPacman提升57%，RoadRunner提升28%。两通道组合反而造成破坏（RoadRunner -96%）。桥接增益高度可预测：Latent与Text增益相关系数r=0.93，仅在慢推理优于快反应时有效。MetaDrive作为负对照确认桥接在无增益时惰性。

Conclusion: Latent Bridge是在实时场景下安全且高效的慢-快模型耦合方案，通过连续潜空间传递推理信息避免了文本桥接的延迟开销，且其增益条件明确（仅当慢推理优于快反应时有效），为构建实时通用计算机智能体提供了实用范式。
...
TL;DR: 本文讨论了当前LLM应用中的缩放指数表明其在能源资源上不可持续，指出将指数偏小归因于“ pedestal effect”（忽略无限数据极限下损失函数非零值）并不能解决不可持续性问题，最后基于流体湍流现象学模型类比，评论了数据平滑度/粗糙度对缩放指数的影响。


<details>
  <summary>Details</summary>
Motivation: 揭示当前LLM缩放定律在能源消耗方面的不可持续性，并澄清对缩放指数偏小的常见归因（pedestal effect）的误解。

Method: 理论分析和类比推理，通过审视缩放指数的能源含义、检验pedestal effect的解释力，并借用流体湍流模型进行类比分析。

Result: 缩放指数偏小导致的能源不可持续性问题依然存在；pedestal effect无法消除这一瓶颈；数据粗糙度（类似于湍流中的不规则性）可能影响缩放行为。

Conclusion: 现有LLM缩放规律在能源上不可持续，需寻找更高效的扩展策略或新的范式。
...
TL;DR: RaDaR是一个32B参数的开源轻量推理LLM，专为罕见病诊断设计。通过4.9万公开病例和10.4万合成病例进行推理增强训练，在多个公开基准和外部验证中心中表现最优，超越671B的DeepSeek-R1。回顾性队列中，RaDaR在61.06%的病例中早于临床怀疑给出诊断，领先1.87个月。随机医生辅助试验中，RaDaR将诊断准确率提升21.44个百分点。合成数据消融实验表明表型锚定叙事对长尾罕见病训练有效。


<details>
  <summary>Details</summary>
Motivation: 罕见病影响全球数百万人，但由于专业临床知识稀缺，及时诊断仍是重大公共卫生挑战。现有LLM在临床部署性、临床依据和训练数据方面存在局限。

Method: RaDaR采用32B参数的开源轻量推理架构，使用49,170个公开免费文本病例和104,666个合成病例进行推理增强训练，并构建了系统化的开发和验证框架。

Result: RaDaR在公开基准和四个外部验证中心中性能优于包括671B DeepSeek-R1在内的所有开源模型。回顾性队列中61.06%病例提前诊断，领先1.87个月；医生辅助试验中提升诊断准确率21.44个百分点。

Conclusion: RaDaR及其开发验证框架提供了一个可部署的罕见病推理模型和一套在数据稀缺条件下可复现的诊断AI开发方法，表型锚定合成数据对长尾罕见病训练具有有效性和可扩展性。
...
TL;DR: 提出了一种利用自主视觉-语言评估作为强化学习监督信号的框架，通过建模评估器噪声并校正奖励估计，显著提升GUI代理在桌面环境中的任务成功率。


<details>
  <summary>Details</summary>
Motivation: 强化学习在计算机使用代理（CUA）中难以应用，因为开放桌面环境缺乏可扩展、机器可读的奖励信号——任务成功通常依赖于视觉判断，难以通过手工设计的奖励函数或密集人工标注来指定。

Method: 提出RL微调框架，使用视觉-语言模型（VLM）作为自主评估器，根据最终截图和原始指令判断任务完成度并提供终端反馈；将评估器反馈建模为带噪声的二元奖励通道，推导出噪声校正的奖励估计器用于PPO优化。

Result: 在macOSWorld、Windows Agent Arena和OSWorld上的实验表明，校正后的评估器奖励优于零样本基线和原始评估器奖励，成功率相比零样本平均提升12.6个百分点，相比原始评估器微调提升5.1个百分点。
...
TL;DR: 本文形式化定义了多智能体LLM环境中的“舰队记忆问题”（fleet-memory problem），识别了四种根本性故障模式（未授权泄露、陈旧传播、矛盾持续、溯源崩溃），并提出显式系统级原语（作用域检索、时间超溯、溯源追踪、策略管控传播）加以解决。这些原语在MemClaw生产级多租户记忆服务中实现，并通过ArgusFleett可复现测试框架在四个治理维度上评估。关键发现包括：溯源准确复现100%的深度四派生链；传播实现了舰队内高可见性与零跨舰队泄露；同时揭露了两个生产架构问题——非对称作用域执行和流水线排序冲突。结论指出：长上下文检索不足以应对生产级多智能体记忆，需要显式系统级抽象，且真实环境评估对于暴露设计阶段无法发现的执行漏洞至关重要。


<details>
  <summary>Details</summary>
Motivation: 多智能体LLM环境缺乏鲁棒的共享知识管理机制，现有方法面对生产环境中的多租户、多智能体场景时存在根本性的故障模式（未授权泄露、陈旧传播、矛盾持续、溯源崩溃），亟需显式的系统级抽象和原语来解决这些问题。

Method: 本文首先形式化定义“舰队记忆问题”及其四种故障模式；然后提出四种系统级原语（作用域检索、时间超溯、溯源追踪、策略管控传播）；并在MemClaw生产级多租户记忆服务中实现这些原语；最后通过ArgusFleet可复现测试框架在四个治理维度上进行评估，采用真实生产服务测量而非基线对比。

Result: 溯源方面：100%成功复现深度四的派生链，正确识别写入者身份，每跳延迟低于秒级。传播方面：实现舰队内高可见性与零跨舰队泄露；强写入模式下写入到可见延迟优化至单次搜索往返。同时发现两个生产架构问题：(1) 非对称作用域执行——子租户作用域在直接GET-by-id请求中可被绕过；(2) 流水线排序冲突——同步近重复门控会过早拒绝矛盾写入，阻碍异步矛盾检测器的正常评估。

Conclusion: 长上下文检索不足以支撑生产级多智能体记忆。受管控的共享记忆需要显式的系统级抽象，且真实环境的在线评估对于暴露执行强制和流水线排序等设计阶段无法发现的失败模式至关重要。
...
TL;DR: 本文提出了一个匹配的执行层基准（440个桌面任务，18个应用，12个工作流类别），通过控制任务、初始状态、验证器和允许动作，比较了GUI代理和CLI代理的表现。最强GUI代理完全通过率59.1%，优于原始技能CLI代理的48.2%；但验证器引导的技能增强使CLI代理提升至69.3%，表明CLI的缺陷主要源于技能覆盖不足而非模型能力。GUI和CLI存在不同的执行瓶颈：GUI受限于长程工作流中的可靠接地交互，CLI受限于技能接口的覆盖和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 现有评估将交互模式与任务、初始状态、验证器和允许动作等差异混淆，无法公平比较GUI代理和CLI代理的实际能力差异。

Method: 构建了一个匹配的执行层基准，包含440个桌面任务、18个应用和12个工作流类别，确保GUI代理和CLI代理接收相同的目标、状态和最终状态验证器，同时限制各自使用模态原生动作进行对比。

Result: 最强GUI代理完全通过率59.1%，优于原始技能CLI代理的48.2%；但验证器引导技能增强后CLI代理提升至69.3%。

Conclusion: GUI和CLI代理存在不同的执行瓶颈：GUI受限于长程工作流中的可靠接地交互，CLI受限于技能接口的覆盖和可扩展性；CLI的缺陷更多来自技能覆盖不足而非模型能力本身。
...
TL;DR: 本研究证明本杰明·格雷厄姆的价值投资规则可作为数学"低通滤波器"，有效约束现代复杂AI模型的过度风险承担。在20年S&P 500数据测试中，纯格雷厄姆规则的随机森林模型实现了232.13%的最高回报率（Calmar比率1.38），而AutoGluon模型虽收益高（222.68%）但回撤达39.78%。混合模型在收益与风险间取得了最佳平衡。


<details>
  <summary>Details</summary>
Motivation: 现代金融中复杂机器学习模型虽能发现市场模式，但易过度记忆短期市场噪声而非识别具备长期价值的公司。研究旨在检验格雷厄姆经典价值投资规则能否作为数学约束机制，防止AI模型过度承担风险。

Method: 构建三组特征集（纯格雷厄姆规则、现代市场因子、两者混合），分别使用XGBoost和AutoGluon等复杂模型，基于20年S&P 500数据，在2022年3月至2026年3月的四年测试期内执行严格的买入并持有策略。

Result: 纯格雷厄姆随机森林取得最高总回报232.13%，Calmar比率为1.38（风险最低）；AutoGluon回报222.68%但遭受39.78%的大幅回撤；混合随机森林回报202.91%，最大回撤仅34.53%，为所有模型中最低。

Conclusion: 格雷厄姆的"安全边际"理念并未过时，反而是防止现代AI过度风险承担的有效方法，能引导模型关注企业真实价值而非市场短期波动。
...
TL;DR: 研究调查了小型本地部署LLM在法律场景下，因权威性前缀提示（如“你是国家最高法院的助手”）导致拒绝率系统性提升2-20倍的现象，表明这类模型在真实机构用户可能自然引入的上下文框架下表现不稳定，存在偏见风险。


<details>
  <summary>Details</summary>
Motivation: 法律专业人士已经开始使用个人LLM进行翻译和改写等任务，但即使是看似无害的使用也可能通过选择性拒绝服务引入偏见。为了提前预见这类偏见，需要研究可能被用作设备端助手的小型LLM在法律提示下过度拒绝的影响。

Method: 研究测试了多个现代小型LLM，比较了无前缀基线、权威性前缀（如“国家最高法院助理”、“辩护律师”）以及已知的角色扮演越狱前缀对拒绝率的影响。

Result: 权威性前缀使拒绝率系统性地提升2-20倍；角色扮演越狱前缀的影响则因模型而异，部分模型拒绝率大幅上升，部分模型几乎没有变化。

Conclusion: 小型本地可部署LLM在真实机构用户可能自然引入的上下文框架下表现不稳定，必须进一步研究以最小化偏见产生的可能性。
...
TL;DR: AdversaBench是一个端到端红队测试流水线，使用5种结构化算子变异种子提示，通过三法官加元法官仲裁机制确认模型失败，在45个种子（涵盖推理、指令遵循和工具使用三类）上均产生确认失败。主要发现：算子效果因类别差异显著；二元失败率掩盖了难度差异；法官间80-87%一致率因标签偏斜导致Cohen's kappa接近零；对抗提示零样本迁移到更大模型。


<details>
  <summary>Details</summary>
Motivation: 大规模评估大语言模型的对抗性鲁棒性，既需要生成困难输入的方法，也需要可靠确认失败真实性的机制，现有方法在这两方面存在不足。

Method: 提出AdversaBench流水线：由5个结构化算子（如inject_distractor）变异种子提示，查询目标模型后，通过三人法官小组加元法官（meta-judge）决胜机制确认失败。在45个种子（推理、指令遵循、工具使用三类）上实验。

Result: 每个种子均产生确认失败。关键结果：1) inject_distractor在指令遵循上均奖励0.00，在推理和工具使用上0.80-0.83；2) 指令遵循需平均2.4次攻击迭代，其他类仅1.1次；3) 法官pairwise一致率80-87%但Cohen's kappa近零；4) 对抗提示从Llama 3.1 8B零样本迁移到Llama 3.3 70B。

Conclusion: 算子效果依赖任务类别；二元失败率不足以刻画难度；标签偏斜下一致率指标需谨慎解读；对抗提示的跨模型迁移性表明攻击利用了通用行为模式而非模型特定弱点。
...
TL;DR: 本文提出ASALT方法，通过引入观测级和状态级适配器，将不同维度（不匹配状态空间维度）的源域和目标域映射到共享嵌入空间，实现多智能体强化学习中的跨域迁移学习，在合作场景中超越了现有基线，并缓解了负迁移问题。


<details>
  <summary>Details</summary>
Motivation: 现有MARL迁移学习方法大多要求源域和目标域的观测空间与全局状态空间维度相同，这限制了跨域迁移的适用性。本文旨在解决状态空间维度不匹配情况下的策略迁移问题。

Method: 提出ASALT方法，包含观测级适配器和状态级适配器，将目标域的观测和全局状态映射到共享嵌入空间，使知识和策略能够在不同维度的域之间进行有效迁移。

Result: 在标准基准环境的多项配置中，ASALT在样本效率和全局回报方面超越了现有基线方法，但其有效性取决于源域与目标域之间的不匹配程度。此外，ASALT能有效缓解负迁移问题。

Conclusion: ASALT能够显式处理源域和目标域之间状态空间维度不匹配的问题，通过适配器实现跨异构域的策略迁移，在合作场景下表现优越，且能缓解负迁移。
...
TL;DR: 提出一个结合序数诊断预测、多水平轨迹生成和分解不确定性估计的概率框架，用于阿尔茨海默病进展建模，在ADNI和OASIS-3数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法将阿尔茨海默病进展建模简化为单步分类，将认知正常、轻度认知障碍和痴呆视为平坦类别，无法描述患者随时间的演变过程及预测的可靠性，缺乏对未来就诊中不确定性累积的量化。

Method: 基于Temporal Fusion Transformer编码器，结合CORAL序数输出层、非对称损失加权和转换器过采样；采用自回归混合密度网络生成五年概率轨迹；通过解析混合方差和五成员bootstrap集成分离偶然不确定性与认知不确定性。

Result: 在ADNI上优于线性、循环和Transformer基线，尤其在MCI与痴呆判别上；生成轨迹的90%可信区间覆盖接近标称值，不确定性随时间增加；认知不确定性在罕见进展类型、MCI和痴呆患者及外部数据集OASIS-3上更高。

Conclusion: 该概率框架能有效建模疾病进展的时序动态、生成可靠的概率轨迹并分解不确定性来源，提升了临床可解释性和泛化能力。
...
TL;DR: ScaleToT通过小型LLM处理子集学习结构化推理，并借助有界熵引导的思维树（ToT）和OSIPO优化策略，将推理能力迁移至低活跃度用户群体，在十亿级广告部署中实现LTV预测提升6.738%，同时仅覆盖7.32%的推理人群，大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 低活跃度用户缺乏丰富交互历史，传统用户建模困难；LLM虽能从静态画像推理但不可靠且成本高昂，无法覆盖十亿级用户。

Method: 1) 使用小型LLM处理子集，构建带类型的用户状态链；2) 采用有界熵引导的思维树（ToT）进行精炼以提高推理可靠性；3) 通过监督微调（SFT）和OSIPO（结果驱动分段感知隐式奖励策略优化）训练学生模型；4) 将学生推理表征迁移到轻量级画像编码器，无需LLM推理即可服务剩余用户。

Result: 在十亿级广告LTV预测中，在线A/B测试LT30提升6.738%；离线推理仅覆盖7.32%潜在人群，大幅降低计算成本。

Conclusion: ScaleToT通过知识蒸馏和结构化推理，在保持效果的同时显著降低大规模用户建模的计算开销，验证了从稀疏画像进行可靠推理的可行性。
...
TL;DR: 本文提出了AI代币经济学框架，系统研究代币在AI系统中的生成、消耗、定价、分配和优化机制，指出代币支出与经济价值存在本质区别，并识别了多个开放研究方向。


<details>
  <summary>Details</summary>
Motivation: 代币已成为现代基础模型服务的实际核算单位，连接了信息处理、计算、内存使用、能耗、定价和经济价值，但缺乏系统化的理论框架来理解代币的经济学原理。

Method: 构建AI代币经济学框架，将代币级别的技术成本与工作流级别的生产函数、企业资源分配、测量和仪器方法以及新兴市场设计问题相连接。

Result: 揭示了代币支出与经济价值是截然不同的概念：价值取决于边际生产力、工作流位置、隐藏推理活动、风险和下游传播效应。
...
TL;DR: 本文研究基于存在规则的OBDA中的查询抽象问题，提出扩展UCQ（加入受限不等式和标记数据库常量的特殊谓词）来表达最小完全抽象，并通过与数据交换中最大恢复概念的关联来刻画最大可靠抽象。


<details>
  <summary>Details</summary>
Motivation: 在基于存在规则的OBDA中，多个数据源通过映射集成到本体，但如何将数据层查询抽象到本体层（即查询抽象）仍是一个开放问题，特别当完美抽象不存在时，需要研究最小完全抽象和最大可靠抽象。

Method: 在UCQ基础上扩展受限不等式和标记数据库常量的特殊谓词；利用数据交换中的最大恢复概念来刻画最大可靠抽象。

Result: 该扩展不增加感兴趣问题的复杂度，但能表达最小完全抽象（及存在时的完美抽象）；建立了最大可靠抽象与最大恢复之间的新联系。

Conclusion: 扩展的UCQ足以表达最小完全抽象，且通过数据交换理论可有效刻画最大可靠抽象，为OBDA中的查询抽象提供了理论基础。
...
TL;DR: 本文研究了本体工程中能力问题（CQ）验证过程的挑战，通过19名参与者使用LLM助手进行20项任务的实验，发现CQ的模糊性和复杂性会导致验证不一致，提出了在发布前优化CQ以避免后续问题的必要性。


<details>
  <summary>Details</summary>
Motivation: CQ验证作为本体评估的核心流程，耗时且易出错，因为需要精确理解语言细微差别并与形式化本体结构对齐。CQ的歧义和复杂性会导致建模决策和验证结果不一致。

Method: 设计实验，19名参与者在LLM助手支持下完成20项CQ验证任务，分析CQ的挑战特征及其对验证性能的影响。

Result: 发现CQ的模糊性和过度复杂性是导致验证困难的主要原因，LLM助手虽能辅助但无法根本解决问题，需要在发布前对CQ进行精炼优化。

Conclusion: 有必要开发工具在CQ发布前对其进行精炼，避免歧义和过度复杂性给后续本体工程阶段带来的问题。
...
TL;DR: Themis是一个结合可解释AI与人类反馈的强化学习测试评估框架，支持200+环境，能训练出匹配或超越真实奖励信号的奖励模型，并提供云平台支持大规模人类反馈收集。


<details>
  <summary>Details</summary>
Motivation: 现有的安全RL系统缺乏将可解释性与人类反馈对齐相结合的统一框架，两者虽然各自有效但尚无公开框架实现融合。

Method: 提出Themis框架，集成XAI可解释性与RLHF人类反馈强化学习，支持200+环境配置，提供可扩展的云平台用于管理实验和收集人类偏好数据。

Result: Themis训练的奖励模型能够匹配甚至超越环境真实奖励信号；云平台在普通商用机器上可支持1000名用户同时进行连续实验。

Conclusion: Themis成功填补了可解释性与人类反馈对齐在RL中缺乏统一框架的空白，为安全RL系统的透明性和对齐性研究提供了实用工具。
...
TL;DR: SAFARI是一个工具增强的诊断框架，通过专用工具箱和短期记忆实现超长轨迹的智能体故障归因，在超出模型原生上下文窗口5倍的情况下仍保持0.58精度，相比传统方法提升约20%。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统在执行复杂多步、多智能体任务时，轨迹长度超出最大上下文窗口限制。传统方法将完整轨迹加载到LLM上下文窗口中进行故障诊断，存在注意力稀释问题，且在轨迹超出上下文限制时完全失效。

Method: 提出SAFARI框架，用工具增强的诊断循环替代线性上下文加载。为LLM配备专用工具箱（读取和搜索轨迹片段）以及用于跨轮推理的短期记忆（STM），将诊断准确性与架构上下文限制解耦。

Result: 在Who&When数据集上（1M token预算）超越SOTA 20%，在TRAIL GAIA子集上（25K token预算）超越SOTA 19%。当目标故障位于模型原生上下文窗口5倍之外时，SAFARI保持0.58精度，而传统评估器完全失败。

Conclusion: 通过工具增强的诊断循环和短期记忆机制，SAFARI有效解决了超长轨迹下的智能体故障归因问题，突破了上下文窗口的架构限制。
...
TL;DR: 本文提出CineCap框架，结合时空锚点的结构化推理与强化学习（涵盖全面性、准确性和门控覆盖奖励），解决电影摄影字幕生成任务。该任务需用专业电影语言（如镜头运动、景别、景深、构图、拍摄角度）描述视频。CineCap在自建基准CineCap Bench（472对人工标注视频-字幕对）上超越多个强基线模型，达到新SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型对电影摄影理解不足，缺乏用专业电影语言（镜头运动、景别、景深、构图、拍摄角度等）对视频进行统一开放式描述的能力，而这一能力对于细粒度视频理解和可控的电影级视频生成至关重要。

Method: 提出CineCap框架，包含两个核心组件：(1) 结构化推理与时空锚点——将专业电影摄影描述锚定在显式视觉证据上，组织成紧凑原子推理进行监督微调；(2) 强化学习——使用全面性、准确性和门控覆盖奖励来平衡描述的完整性和事实正确性。

Result: 在自建基准CineCap Bench（472对人工标注视频-字幕对）上，CineCap在电影摄影字幕生成任务中一致优于强专有模型和开源基线，达到新SOTA。

Conclusion: CineCap框架有效解决了电影摄影字幕生成中的两大挑战（从细微视觉线索推断专业概念、生成全面且准确的字幕），并为该领域建立了新的标杆。
...
TL;DR: 提出LaGO框架，利用预训练LLM作为潜在动作先验来软引导在线强化学习策略优化，而非直接作为控制器。在CLEVR-Robot和Meta-World基准上，LaGO相比Vanilla PPO显著提升了奖励和成功率（平均成功率分别从15.1%提升至27.2%，从2.7%提升至15.2%），且更强的LLM提供更有效的引导。


<details>
  <summary>Details</summary>
Motivation: 现有方法将LLM直接作为控制器用于规划和序列决策，需要精确的动作生成，在实际中不可靠。

Method: 将预训练LLM作为潜在动作先验（latent action prior），软引导在线策略优化（PPO），而非显式规划或控制。

Result: 在CLEVR-Robot（离散控制）上平均成功率从15.1%提升至27.2%；在Meta-World（连续控制）上从2.7%提升至15.2%。更强的LLM引导效果更好。

Conclusion: LLM的知识可以通过潜在动作先验的形式有效改进在线规划与决策，且避免了直接作为控制器的不稳定性。
...
TL;DR: 本文研究了在变量成本和概率分布下，最小化命题公式期望评估成本的确定性评估策略构建问题，提出了首个实用的精确分支定界算法，并证明了问题的计算复杂性。


<details>
  <summary>Details</summary>
Motivation: 许多决策场景中获取信息需要不同成本，现有方法缺乏针对一般命题公式、可变成本和概率分布的实用精确算法。

Method: 提出分支定界算法，结合变量选择启发式、剪枝和缓存技术，并设计了贪心波束搜索变体用于效率-质量权衡。

Result: 随机实例实验证明了算法的可扩展性，量化了贪心波束搜索的效率-质量权衡，并在结构化心脏病诊断实例上进行了评估。

Conclusion: 该问题是#P-hard且属于PSPACE，所提算法是首个针对该通用设置的实际精确算法。
...
TL;DR: 本文提出Spheroid BlockTrain，一种去中心化训练协议，将模型划分为可独立训练的块，每个块基于全局目标派生的局部目标进行优化，推理时组合为一个完整模型。实验表明，在WikiText上达到CE 1.359（PPL 3.89），接近端到端Transformer基线；通过HTTP/TCP传输实验验证了实际部署可行性，支持分布式推理，在三个公共网络GPU主机上可达75.80B参数的逻辑fp16形状。


<details>
  <summary>Details</summary>
Motivation: 前沿AI训练日益受限于密集、集中控制的加速器集群，导致超大规模企业和大型集中实验室拥有结构性优势，而开放或独立的AI研究受制于稀缺资本、特权基础设施和数据中心地理位置。本文旨在打破这种集中化垄断，提出去中心化的训练方案。

Method: 将模型划分为可独立训练的块（blocks），每块基于全局目标派生的局部目标进行优化，各工作节点仅训练一个块且避免全模型优化器状态；块训练完成后通过组装形成完整模型。支持HTTP/TCP传输实际序列化检查点和更新，推理时通过块堆栈遍历生成完整输出。

Result: byte-level WikiText上CE 1.359（PPL 3.89），接近同设置端到端Transformer基线（差距约0.04 CE）；六工作节点块训练运行CE 1.385；公共IP三主机运行CE从5.580提升至1.811，传输15.22 GB；推理在三个公共网络GPU主机上支持75.80B参数逻辑fp16形状，优于匹配的纯自回归TCP管道基线。

Conclusion: Spheroid BlockTrain通过去中心化块训练协议，有效降低了AI训练对集中式基础设施的依赖，性能接近集中式端到端训练方案，同时支持分布式推理，为去中心化AI训练提供了可行路径。
...
TL;DR: 本文针对大语言模型在领域特定场景下的压缩问题，提出了经验性的缩放律，量化了数据集规模、压缩比、监督格式和剪枝调度对领域内和通用知识性能的影响。以量化金融为应用领域，比较了logit蒸馏和LoRA蒸馏在迭代结构剪枝下的表现，并引入了一种混合思维链监督损失。研究发现领域内任务质量随压缩可预测下降，而通用知识基准更早崩溃；监督格式是这一权衡的关键因素，思维链监督能有效恢复剪枝丢失的通用知识。同时发布了FinHeadlineMix数据集和实用建议。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在广泛领域表现优异，但其规模在延迟和成本受限的应用场景中部署困难，亟需在领域特定场景下进行有效压缩，同时平衡领域内性能与通用知识保留。

Method: 采用迭代结构剪枝框架，对比logit蒸馏和LoRA蒸馏两种知识蒸馏方法，并设计了一种混合思维链（chain-of-thought）监督损失来稳定推理轨迹上的KL散度蒸馏。以量化金融为测试领域，通过系统实验推导经验缩放律。

Result: 领域内任务质量随压缩比增加可预测下降，而通用知识基准在压缩程度较低时即出现崩溃；监督格式是影响领域性能与通用知识权衡的关键因素，思维链监督能够主动恢复被剪枝消除的通用知识。

Conclusion: 提出了一套可复用的领域特定压缩决策框架，包括FinHeadlineMix数据集、缩放律结果和实用建议，为领域模型压缩提供了理论指导和实践参考。
...
TL;DR: 本文研究GPT风格Transformer模型在多语言持续学习中的可塑性损失现象，发现从5M到314M参数规模的模型均出现可塑性损失，其发生遵循可预测的缩放定律（随模型规模亚线性增长），且在静态多语言训练中同样存在，表明单纯增加参数量无法完全阻止该现象。


<details>
  <summary>Details</summary>
Motivation: 可塑性损失（网络在学习旧知识后失去学习新信息的能力）是持续学习中的根本挑战，但以往研究多集中在较小的旧架构上，很少涉及自然语言领域。本文旨在探究该问题在现代基于Transformer的大语言模型范式中是否仍然存在。

Method: 在GPT风格Transformer模型（5M至314M非嵌入参数）上进行多语言持续学习实验，通过预留越南语探针任务上的性能退化来测量可塑性损失，并分析其与模型规模的缩放关系。

Result: 所有规模模型均观察到可塑性损失；可塑性损失的发生遵循与模型规模呈亚线性增长的缩放定律；更大的模型虽能延迟可测量影响，但仅增加参数量不足以完全阻止可塑性损失；在静态多语言训练中也发现可塑性损失证据。

Conclusion: 即使是大型Transformer语言模型，在长时间训练后也会在持续学习和静态设置中逐渐失去高效适应新数据的能力，单纯扩大模型规模不能从根本上解决可塑性损失问题。
...
TL;DR: BluTrain是一个从零开始用C++和CUDA实现的轻量级训练框架，在8-GPU系统上训练124M参数GPT-2时，吞吐量达407K tokens/s（PyTorch为395K），内存占用减少22%，同时严格保持数值精度并取得更低的验证损失。


<details>
  <summary>Details</summary>
Motivation: 深度学习在大规模场景下，模型行为更多取决于系统工程而非模型架构本身。为了让研究者完全掌控硬件表达，同时抽象系统复杂性、消除重复编排逻辑，作者从第一性原理设计了BluTrain。

Method: 用标准C++和核心CUDA编程模型实现了完整训练框架，包括：类型化张量模块（带反向模式自动微分）、线性代数库、缓存分配器、多模式分布式执行模块，以及基于MLIR的深度学习编译器。所有层均为原生实现，无外部依赖抽象。

Result: 在8-GPU 6000 Ada系统上训练124M参数GPT-2（FP32）：吞吐量407K tokens/s（PyTorch为395K，提升约3%）；内存占用减少22%；严格保持数值精度；最终验证损失略低于PyTorch。

Conclusion: BluTrain证明了从系统工程角度重新设计训练框架可以同时提升吞吐量、内存效率和数值精度。由于每层都开放给原生调优，其性能上限由框架自身决定，为未来深度学习基础设施提供了新范式。
...
TL;DR: 本文系统评估了传感器人体活动识别(HAR)中4种分布偏移(设备类型、传感器位置、采样率、用户行为)的影响，发现多样性偏移主导了所有偏移类型，并基于28种域泛化方法的全面评估揭示了现有算法在模型泛化性上的局限，提供了首个针对HAR中特定分布偏移的域泛化系统研究及开源基准平台。


<details>
  <summary>Details</summary>
Motivation: 尽管HAR领域数据多样性问题已被广泛认知，但不同类型分布偏移对HAR模型的影响以及由此产生的域泛化问题尚未被深入理解，存在研究空白。

Method: 系统评估4种分布偏移(设备类型、传感器位置、采样率、用户行为)的影响，建立统一的HAR分布偏移基准，对28种域泛化方法进行全面评估。

Result: 多样性偏移主导了所有类型的偏移，表明存在跨域不共享的独特特征；现有域泛化算法在模型泛化性上表现有限，仅 marginally 优于经验风险最小化基线。

Conclusion: 这项工作首次系统探索了传感器HAR中针对特定分布偏移的域泛化和域适应问题，并提供了开源基准平台和数据集以促进后续研究。
...
TL;DR: 本文提出双向条件流匹配（Bi-CFM）及其守恒约束扩展（CBi-CFM），用于解决混沌动力系统中的逆向问题（从终态推断初态），在多个经典混沌系统和实际天文观测中实现了精度提升和超过两个数量级的加速。


<details>
  <summary>Details</summary>
Motivation: 混沌系统的逆向问题（从终态推断初态）由于不适定性、非唯一性、不稳定性以及时间反演动力学本身可能具有混沌性，长期以来未得到有效解决。

Method: 提出双向条件流匹配（Bi-CFM），通过学习初态与终态分布之间的双向映射来捕捉混沌演化的随机性并缓解指数级误差累积；针对具有守恒律的系统，进一步提出守恒约束的CBi-CFM。

Result: 在Lorenz、Circuit和高维Lorenz 96系统上，Bi-CFM在五个分布级指标上优于基线，且加速超过两个数量级；在三体行星散射问题中，CBi-CFM的守恒误差与真实解相当；在球状星团实际观测中，方法代表了精度上的进步。

Conclusion: 该方法为求解长时标真实世界混沌动力系统的逆向问题建立了一条可扩展的路径。
...
TL;DR: 本文指出Andreas & Günther提出的七种实际因果关系定义及其三种分类（事实差异制造、反事实差异制造、基于规则）本质上无区别，且他们的最新定义（事实差异制造）同时体现了所有三种类型，进而削弱了他们全部七个定义的有效性。


<details>
  <summary>Details</summary>
Motivation: 本文旨在揭示Andreas & Günther关于实际因果关系定义的分类体系存在问题，证明其三种分类之间的区分是虚假的，从而质疑其全部七种定义的合理性。

Method: 通过逻辑分析，论证Andreas & Günther最新的“事实差异制造”定义同时蕴含了事实差异制造、反事实差异制造和基于规则这三种类型，表明这些分类并非真正不同；并通过关键案例比较，展示该问题如何影响全部七个定义。

Result: 发现Andreas & Günther的最新定义实际上同时属于三种类型，证明其分类区分是无效的；且通过案例比较，揭示这一问题导致所有七个定义的基础被削弱。

Conclusion: Andreas & Günther对实际因果关系的分类体系存在严重缺陷，三类定义之间的区别并非实质性的，这对其全部七种因果关系定义的有效性构成了根本性挑战。
...
TL;DR: 本文研究了开发者与基于LLM的对话助手（如GitHub Copilot）在评估非功能需求（NFRs）时的多轮对话准确性和质量。通过49名程序员对148个HIPAA相关NFRs的评估，发现开发者倾向于认同LLM的判断，但其准确性相对于专家标注较低；较长的系统响应和过多的信息提供轮次会降低用户满意度，而主动交互则能提升满意度。


<details>
  <summary>Details</summary>
Motivation: 现有评估基准仅关注功能正确性，缺乏对对话助手在处理非功能需求（NFRs）时的对话质量和准确性评估。NFRs具有模糊性、上下文依赖性和跨程序多部分的特点，需要超越单轮准确性的多轮交互质量评估方法。

Method: 招募49名程序员与GitHub Copilot进行多轮对话，评估148个HIPAA衍生的NFRs在iTrust代码库中的满足情况，从需求满足程度、推理和代码定位三个维度进行测量，并建模用户满意度。

Result: 开发者倾向于认同LLM的评估结果，但相对于专家标注的准确性较低。较长的系统响应和更多信息提供型轮次对用户满意度有负面影响，而主动交互行为对满意度有正面影响。

Conclusion: 研究结果为设计支持NFR评估的LLM对话系统提供了见解，强调了在对话设计中需要平衡信息提供与交互主动性，以提升用户满意度。
...
TL;DR: 本文针对智能体数据分析系统产生的丰富输出（代码、数值、文本诊断）难以评估的问题，提出了三层人机评分级联机制（严格正则匹配、LLM宽松评分、基于片段的人工审查），在DSGym的153个数值QRData任务上验证了该方法，实现了100%精确率，宽松评分器召回率达97%。


<details>
  <summary>Details</summary>
Motivation: 智能体数据分析系统输出复杂（含代码、数值、文本诊断），比单轮LLM响应更难评估。需要区分智能体输出与真实答案之间的真正差异与评分伪影，现有的自动化评分方法可靠性不足。

Method: 提出了三层人机评分级联：1）严格正则匹配；2）LLM宽松评分；3）基于片段的人工审查。采用关键词锚定提取管道提升严格评分器召回率，并设计迭代提示（nudge）机制提升评分运行成功率。在LAMBDA多智能体数据分析系统上，用DSGym的153个数值QRData任务进行评估。

Result: 两种自动化评分器均达到100%观测精确率（0/70假阳性）。宽松评分器召回率97%。关键词锚定管道使严格评分器召回率比末位数字启发式方法提升60个百分点。迭代提示机制将评分运行成功率从36%提升至97%，宽松通过率从16%提升至46%；原始问题重新注入无额外收益。变量类型是与评分管道动态和结果最相关的元数据字段。

Conclusion: 所提出的级联评分机制能够有效评估智能体数据分析系统的输出，其中LLM宽松评分器在精确率和召回率上表现优异，迭代提示机制显著提升了评分成功率。变量类型是影响评分效果的关键元数据特征。
...
TL;DR: 本文提出MTO框架，通过匹配预训练目标与任务目标，在编码器-解码器模型上实现少样本场景下超过120%的性能提升，并有效指导提示调优。


<details>
  <summary>Details</summary>
Motivation: 探索不同预训练目标对编-解码器PLM在生成和问答任务上的影响，解决如何为特定任务选择最优预训练目标并提升少样本性能的问题。

Method: 提出Match Task to Objective (MTO)框架，包括：自动确定任务对应预训练目标的方法、基于目标的无监督数据适配、设计对齐预训练/适配阶段目标的微调模板，并扩展到提示调优策略。

Result: 少样本设置下相比传统方法性能提升超过120%；显著优于相关少样本工作；在全数据集场景下也超越基线；同时有效增强提示调优性能。

Conclusion: 对齐预训练目标与任务目标是提升PLM性能的关键策略，MTO框架能为特定任务提供模型选择和优化的精确指导。
...
TL;DR: 本文指出在"大世界"场景下，通用智能体能力必然碎片化，导致传统均匀性保证失效。作者提出结构化认证框架，通过深度组合目标过滤特定转换，证明智能体世界模型具有 O(1/n) + O(δ) 误差界，实现了通用智能体在长程规划中可靠性的可认证部署。


<details>
  <summary>Details</summary>
Motivation: 在"大世界"条件下，智能体无法具备通用能力，其能力必然在碎片化的世界模型中专门化。传统的最坏情况分析无法区分关键瓶颈与无关失败，因此需要新的认证框架来提供有意义的保证。

Method: 首先形式化证明了通用智能体并非普遍适用，标准最坏情况分析无效。随后提出结构化认证（structural certification）——一种转换局部的框架，将有界目标条件性能映射为智能体内部世界模型的逐项保证。通过深度组合目标过滤特定转换，证明了通用智能体在这些目标上具有结构化世界模型，误差界为 O(1/n) + O(δ)。

Result: 证明了通用智能体非通用性的形式化局限；提出了结构化认证框架；给出了基于深度组合目标的过滤算法；证明了 O(1/n) + O(δ) 误差界，且该界在小 δ 场景下紧致，该场景的存在性由认证过程显式保证。

Conclusion: 该成果通过定位长程规划可靠的具体转换，实现了通用智能体的可认证部署。
...
TL;DR: 本文提出了OpenThoughts-Agent (OT-Agent)项目，一个完全开放的数据策划流程用于训练智能体模型。通过100多次消融实验系统研究了流程各阶段，构建了10万样本训练集，在Qwen3-32B上微调后在7个智能体基准上达到44.8%平均准确率，比最强开源模型高出3.9个百分点，并展示了良好的数据缩放特性。


<details>
  <summary>Details</summary>
Motivation: 现有开源智能体数据工作（如SWE-Smith、SERA、Nemotron-Terminal）通常针对单一基准，缺乏跨多样化智能体任务泛化的训练方法，且缺乏对数据策划流程的系统研究。

Method: 构建完全开源的数据策划流程，通过100多次控制消融实验系统研究各阶段（任务来源、多样性等）的影响，最终组装10万训练样本，在Qwen3-32B上进行微调。

Result: 训练模型在7个智能体基准上平均准确率44.8%，比最强开源基线Nemotron-Terminal-32B（40.9%）高3.9个百分点；训练数据在计算控制比较中，在所有规模下均优于其他开源数据集，展现出强缩放特性。

Conclusion: 任务来源和多样性的系统研究对智能体模型训练至关重要；所提出的开源数据策划流程和训练集可有效提升智能体模型的跨任务泛化能力；项目全开源以支持未来研究。
